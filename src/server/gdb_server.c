		...
		char *buffer, int len)
{
	...
	while (1) {
		...
		if ((size_t)len + 4 <= sizeof(local_buffer)) {
			...
			len += snprintf(&local_buffer[len], sizeof(local_buffer), "#%02x", my_checksum);
			...
			snprintf(local_buffer + 1, sizeof(local_buffer), "#%02x", my_checksum);
		...
		char *tstr, struct reg *reg)
{
	...
	for (i = 0; i < buf_len; i++) {
		...
		tstr += snprintf(tstr, 3, "%02x", buf[j]);
	}
}
