	...
	enum breakpoint_type type)
{
	...
	while (breakpoint) {
		...
		if (breakpoint->address == address) {
			 ...
			 * succeeding.
			 */
			LOG_DEBUG("Duplicate Breakpoint address: 0x%08" PRIx32 " (BP %" PRIx32 ")",
...
fail:
	...
	LOG_DEBUG("added %s breakpoint at 0x%8.8" PRIx32 " of length 0x%8.8x, (BPID: %" PRIx32 ")",
	...
	enum breakpoint_type type)
{
	...
	while (breakpoint) {
		...
		if ((breakpoint->asid == asid) && (breakpoint->address == address)) {
			...
			LOG_DEBUG("Duplicate Breakpoint IVA: 0x%08" PRIx32 " (BP %" PRIx32 ")",
				address, breakpoint->unique_id);
			return -1;
	...
	LOG_DEBUG(
		"added %s Hybrid breakpoint at address 0x%8.8" PRIx32 " of length 0x%8.8x, (BPID: %" PRIx32 ")",
		breakpoint_type_strings[(*breakpoint_p)->type],
