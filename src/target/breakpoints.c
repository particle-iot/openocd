	...
	enum breakpoint_type type)
{
	...
	while (breakpoint) {
		...
		if (breakpoint->address == address) {
			 ...
			 * succeeding.
			 */
			LOG_DEBUG("Duplicate Breakpoint address: 0x%08" PRIu32 " (BP %" PRIx32 ")",
...
fail:
	...
	LOG_DEBUG("added %s breakpoint at 0x%8.8" PRIu32 " of length 0x%8.8x, (BPID: %" PRIx32 ")",
	...
	enum breakpoint_type type)
{
	...
	while (breakpoint) {
		...
		if (breakpoint->asid == asid) {
			 ...
			 * succeeding.
			 */
			LOG_DEBUG("Duplicate Breakpoint asid: 0x%08" PRIx32 " (BP %" PRIx32 ")",
	...
	LOG_DEBUG("added %s Context breakpoint at 0x%8.8" PRIx32 " of length 0x%8.8x, (BPID: %" PRIx32 ")",
		breakpoint_type_strings[(*breakpoint_p)->type],
	...
	enum breakpoint_type type)
{
	...
	while (breakpoint) {
		...
		if ((breakpoint->asid == asid) && (breakpoint->address == address)) {
			 ...
			 * succeeding.
			 */
			LOG_DEBUG("Duplicate Hybrid Breakpoint asid: 0x%08" PRIx32 " (BP %" PRIx32 ")",
				asid, breakpoint->unique_id);
			return -1;
		} else if ((breakpoint->address == address) && (breakpoint->asid == 0)) {
			LOG_DEBUG("Duplicate Breakpoint IVA: 0x%08" PRIu32 " (BP %" PRIx32 ")",
				address, breakpoint->unique_id);
			return -1;
	...
	LOG_DEBUG(
		"added %s Hybrid breakpoint at address 0x%8.8" PRIu32 " of length 0x%8.8x, (BPID: %" PRIx32 ")",
		breakpoint_type_strings[(*breakpoint_p)->type],
...
static void breakpoint_free(struct target *target, struct breakpoint *breakpoint_to_remove)
{
	...
	LOG_DEBUG("free BPID: %" PRIx32 " --> %d", breakpoint->unique_id, retval);
	(*breakpoint_p) = breakpoint->next;
...
int breakpoint_remove_internal(struct target *target, uint32_t address)
{
	...
	if (breakpoint) {
		...
		if (!target->smp)
			LOG_ERROR("no breakpoint at address 0x%8.8" PRIu32 " found", address);
		return 0;
	}
}
	...
	while (watchpoint) {
		if (watchpoint->address == address) {
			if (watchpoint->length != length
				...
				|| watchpoint->rw != rw) {
				LOG_ERROR("address 0x%8.8" PRIu32
...
bye:
			LOG_ERROR("can't add %s watchpoint at 0x%8.8" PRIu32 ", %s",
	...
	LOG_DEBUG("added %s watchpoint at 0x%8.8" PRIu32
...
void watchpoint_remove(struct target *target, uint32_t address)
{
	...
	if (watchpoint)
		...
		LOG_ERROR("no watchpoint at address 0x%8.8" PRIu32 " found", address);
}
...
int watchpoint_hit(struct target *target, enum watchpoint_rw *rw, uint32_t *address)
{
	...
	*address = hit_watchpoint->address;
	...
	LOG_DEBUG("Found hit watchpoint at 0x%8.8" PRIu32 " (WPID: %d)",
