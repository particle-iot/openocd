	...
	enum breakpoint_type type)
{
	...
	while (breakpoint) {
		...
		if (breakpoint->address == address) {
			 ...
			 * succeeding.
			 */
			LOG_DEBUG("Duplicate Breakpoint address: 0x%16" PRIx64 " (BP %" PRIu32 ")",
	...
	enum breakpoint_type type)
{
	...
	LOG_DEBUG("added %s Context breakpoint at 0x%16.16" PRIx64 " of length 0x%8.8x, (BPID: %" PRIu32 ")",
		breakpoint_type_strings[(*breakpoint_p)->type],
...
int breakpoint_remove_internal(struct target *target, uint64_t address)
{
	...
	if (breakpoint) {
		...
		if (!target->smp)
			LOG_ERROR("no breakpoint at address 0x%16.16" PRIx64 " found", address);
		return 0;
	}
}
...
int watchpoint_add(struct target *target, uint64_t address, uint32_t length,
	...
	while (watchpoint) {
		if (watchpoint->address == address) {
			if (watchpoint->length != length
				...
				|| watchpoint->rw != rw) {
				LOG_ERROR("address 0x%16.16" PRIx64
...
bye:
			LOG_ERROR("can't add %s watchpoint at 0x%16.16" PRIx64 ", %s",
	...
	LOG_DEBUG("added %s watchpoint at 0x%16.16" PRIx64
...
void watchpoint_remove(struct target *target, uint64_t address)
{
	struct watchpoint *watchpoint = target->watchpoints;
	...
	if (watchpoint)
		...
		LOG_ERROR("no watchpoint at address 0x%16.16" PRIx64 " found", address);
}
