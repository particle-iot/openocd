	...
	enum breakpoint_type type)
{
	...
	while (breakpoint) {
		...
		if (breakpoint->address == address) {
			 ...
			 * succeeding.
			 */
			LOG_DEBUG("Duplicate Breakpoint address: 0x%" PRIX " (BP %d)",
...
fail:
	...
	LOG_DEBUG("added %s breakpoint at 0x%" PRIX " of length 0x%8.8x, (BPID: %d)",
	...
	enum breakpoint_type type)
{
	...
	while (breakpoint) {
		...
		if ((breakpoint->asid == asid) && (breakpoint->address == address)) {
			...
			LOG_DEBUG("Duplicate Breakpoint IVA: 0x%" PRIX " (BP %d)",
				address, breakpoint->unique_id);
			return -1;
	...
	LOG_DEBUG(
		"added %s Hybrid breakpoint at address 0x%" PRIX " of length 0x%8.8x, (BPID: %d)",
		breakpoint_type_strings[(*breakpoint_p)->type],
...
int breakpoint_remove_internal(struct target *target, target_ulong address)
{
	...
	if (breakpoint) {
		...
		if (!target->smp)
			LOG_ERROR("no breakpoint at address 0x%" PRIX " found", address);
		return 0;
	}
}
...
int watchpoint_add(struct target *target, target_ulong address, uint32_t length,
	enum watchpoint_rw rw, target_ulong value, target_ulong mask)
{
...
bye:
			LOG_ERROR("can't add %s watchpoint at 0x%" PRIX ", %s",
	...
	LOG_DEBUG("added %s watchpoint at 0x%" PRIX
...
void watchpoint_remove(struct target *target, target_ulong address)
{
	...
	if (watchpoint)
		...
		LOG_ERROR("no watchpoint at address 0x%" PRIX " found", address);
}
