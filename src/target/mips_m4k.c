		...
		target_addr_t address, int handle_breakpoints, int debug_execution)
{
	...
	if (!current) {
		mips_m4k_isa_filter(mips32->isa_imp, &address, debug_execution);
		...
		target_addr_t address, int handle_breakpoints)
{
	...
	/* current = 1: continue on current pc, otherwise continue at <address> */
	if (!current) {
		mips_m4k_isa_filter(mips32->isa_imp, &address, 0);
		buf_set_u32(mips32->core_cache->reg_list[MIPS32_PC].value, 0, 32, address);
		mips32->core_cache->reg_list[MIPS32_PC].dirty = 1;
		mips32->core_cache->reg_list[MIPS32_PC].valid = 1;
	}
...
static int mips_m4k_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	if ((breakpoint->length > 5 || breakpoint->length < 2) ||		/* out of range */
		...
		(mips32->isa_imp == MIPS32_ONLY && breakpoint->length != 4) ||		/* mips32 specific */
		(mips32->isa_imp == MMIPS32_ONLY && !(breakpoint->length & 1)) ||	/* micro mips32 specific */
		(mips32->isa_imp == MIPS32_MIPS16 && (breakpoint->length & 1)))		/* not compatible */
		return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
