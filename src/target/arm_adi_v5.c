...
/* JEP106 is encoded in the ARM representation:
 * bits 11:8 : JEP106 Bank (0 = Bank 1, 1 = Bank 2, 2 = Bank 3, etc.)
 * bit 7     : JEDEC Code Is Used (must be set)
...
static const struct {
	...
	{ 0x955, 0x4BB, "CoreSight Component",	"(unidentified Cortex-A5 component)", },
				...
				struct adiv5_dap *dap, int ap, uint32_t dbgbase, int depth)
{
	...
	for (entry_offset = 0; ; entry_offset += 4) {
		...
		if (romentry & 0x01) {
			...
			LOG_DEBUG("Unrecognized Part number 0x%" PRIx32 " JEP106 0x%" PRIx32, part_num, JEP106_id);
