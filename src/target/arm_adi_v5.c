...
{
	bool ti_be_32 = dap->ti_be_32_quirks;
	...
	if (size == 4) {
		...
		csw_size = CSW_16BIT;
		addr_xor = ti_be_32 ? 2 : 0;
	} else if (size == 1) {
		csw_size = CSW_8BIT;
		addr_xor = ti_be_32 ? 3 : 0;
	} else {
		return ERROR_TARGET_UNALIGNED_ACCESS;
	}

	/* Unaligned reads on TI BE-32 processors seem to cause a sticky error. */
	if (ti_be_32 && (address % size != 0))
		return ERROR_TARGET_UNALIGNED_ACCESS;
		...
		uint32_t adr, bool addrinc)
{
	...
	/* Unaligned reads on TI BE-32 processors seem to return garbage in some bytes. */
	if (ti_be_32 && (adr % size != 0))
		return ERROR_TARGET_UNALIGNED_ACCESS;
	...
	while (nbytes > 0) {
		...
		/* Select packed transfer if possible */
		if (!ti_be_32 && addrinc && dap->packed_transfers && nbytes >= 4
				&& max_tar_block_size(dap->tar_autoincr_block, address) >= 4) {
			this_size = 4;
			retval = dap_setup_accessport_csw(dap, csw_size | CSW_ADDRINC_PACKED);
		} else {
