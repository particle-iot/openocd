		...
		uint32_t address, bool addrinc)
{
	...
	if (retval != ERROR_OK) {
		...
		if (mem_ap_read_tar(ap, &tar) == ERROR_OK)
			LOG_ERROR("Failed to write memory at 0x%08"PRIx32, tar - mem_ap_get_tar_increment(ap));
		else
		...
		uint32_t adr, bool addrinc)
{
	...
	if (retval != ERROR_OK) {
		uint32_t tar;
		if (mem_ap_read_tar(ap, &tar) == ERROR_OK) {
			/* TAR is incremented after failed transfer, roll back one xfer */
			tar -= mem_ap_get_tar_increment(ap);
