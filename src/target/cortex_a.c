	...
	uint32_t count, uint8_t *buffer)
{
	...
	LOG_DEBUG("Reading APB-AP memory address 0x%x size %d count %d", address, size, count);
	if (target->state != TARGET_HALTED) {
		LOG_WARNING("target not halted");
		return ERROR_TARGET_NOT_HALTED;
	}
	 ...
	 */
	retval = mem_ap_sel_read_buf_noincr(swjdp, armv7a->debug_ap, u8buf_ptr, 4, read_byte_cnt,
