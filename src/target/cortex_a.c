	...
	uint32_t size, uint32_t count, const uint8_t *buffer, uint32_t *dscr)
{
	...
	retval = cortex_a_set_dcc_mode(target, DSCR_EXT_DCC_NONBLOCKING_MODE, dscr);
	if (retval != ERROR_OK)
		return retval;
	...
	while (count) {
		...
		if (dscr & DSCR_STICKY_ABORT_PRECISE)
			return ERROR_OK; /* Not really, but we will check for faults properly later. */
		...
		while (dscr & DSCR_DTRRX_FULL_LATCHED) {
			retval = mem_ap_sel_read_atomic_u32(swjdp, armv7a->debug_ap,
					armv7a->debug_base + CPUDBG_DSCR, &dscr);
			if (retval != ERROR_OK)
				return retval;
	...
	uint32_t size, uint32_t count, uint8_t *buffer, uint32_t *dscr)
{
	...
	while (count) {
		...
		while (!(dscr & DSCR_DTRTX_FULL_LATCHED)) {
			retval = mem_ap_sel_read_atomic_u32(swjdp, armv7a->debug_ap,
					armv7a->debug_base + CPUDBG_DSCR, &dscr);
			if (retval != ERROR_OK)
				return retval;
