	...
	uint32_t size, uint32_t count, const uint8_t *buffer, uint32_t *dscr)
{
	...
	retval = cortex_a_set_dcc_mode(target, DSCR_EXT_DCC_STALL_MODE, dscr);
	if (retval != ERROR_OK)
		return retval;
...
out:
	...
	/* Check for sticky abort flags in the DSCR, if we care (we only care if
	 * the operation otherwise succeeded). */
	if (final_retval == ERROR_OK && (dscr & (DSCR_STICKY_ABORT_PRECISE |
					DSCR_STICKY_ABORT_IMPRECISE))) {
		/* Abort occurred - clear it. */
		LOG_ERROR("abort occurred - dscr = 0x%08" PRIx32, dscr);
		mem_ap_sel_write_atomic_u32(swjdp, armv7a->debug_ap,
				armv7a->debug_base + CPUDBG_DRCR, 1<<2);
		final_retval = ERROR_FAIL;
	}
