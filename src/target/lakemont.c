			...
			uint8_t *in, uint8_t ir_len)
{
	...
	LOG_DEBUG("irlen=%d, out=%d, in=%d", ir_len, *out, *in);
	if (NULL == t->tap) {
		retval = ERROR_FAIL;
...
static int drscan(struct target *t, uint8_t *out, uint8_t *in, uint8_t len)
{
	...
	struct x86_32_common *x86_32 = target_to_x86_32(t);
	if (len == PIR_SIZE) {
		LOG_DEBUG("out=0x%08x%08x, in=0x%X, len=%d",
				target_buffer_get_u32(t, out+4),
				target_buffer_get_u32(t, out), *in, len);
	} else if (len == PDR_SIZE)
		LOG_DEBUG("out=0x%08x, in=0x%X, len=%d",
				target_buffer_get_u32(t, out), *in, len);
	else
		LOG_DEBUG("out=0x%X, in=0x%X, len=%d", *out, *in, len);
...
static int lakemont_get_core_reg(struct reg *reg)
{
	...
	LOG_DEBUG("reg=%s, value=%08x", reg->name,
...
static int lakemont_set_core_reg(struct reg *reg, uint8_t *buf)
{
	...
	LOG_DEBUG("reg=%s, newval=%08x", reg->name, value);
	if (check_not_halted(t))
		return ERROR_TARGET_NOT_HALTED;
...
static uint32_t get_tapstatus(struct target *t)
{
	...
	if (drscan(t, NULL, scan.out, TS_SIZE) != ERROR_OK)
		return 0;
	return target_buffer_get_u32(t, scan.out);
}
...
static int enter_probemode(struct target *t)
{
	...
	LOG_DEBUG("TS before PM enter = %08x", tapstatus);
	...
	if ((tapstatus & TS_PM_BIT) && (!(tapstatus & TS_EN_PM_BIT)))
		...
		LOG_ERROR("%s PM enter error, tapstatus = %08x"
				, __func__, tapstatus);
		return ERROR_FAIL;
	}
}
...
static int exit_probemode(struct target *t)
{
	...
	LOG_DEBUG("TS before PM exit = %08x", tapstatus);
...
static int halt_prep(struct target *t)
{
	...
	if (write_hw_reg(t, DSB, PM_DSB, 0) != ERROR_OK)
		return ERROR_FAIL;
	LOG_DEBUG("write %s %08x", regs[DSB].name, PM_DSB);
	if (write_hw_reg(t, DSL, PM_DSL, 0) != ERROR_OK)
		return ERROR_FAIL;
	LOG_DEBUG("write %s %08x", regs[DSL].name, PM_DSL);
...
static int read_hw_reg(struct target *t, int reg, uint32_t *regval, uint8_t cache)
{
	...
	*regval = target_buffer_get_u32(t, scan.out);
	if (cache) {
	...
	LOG_DEBUG("reg=%s, op=0x%016lX, val=%08x",
...
static int write_hw_reg(struct target *t, int reg, uint32_t regval, uint8_t cache)
{
	...
	uint32_t valout = target_buffer_get_u32(t, (uint8_t *)&regval);
	p_val = (uint8_t *)&valout;
...
static int transaction_status(struct target *t)
{
	...
	if ((TS_EN_PM_BIT | TS_PRDY_BIT) & tapstatus) {
		LOG_ERROR("%s transaction error tapstatus = %08x"
				, __func__, tapstatus);
		return ERROR_FAIL;
	} else {
		return ERROR_OK;
...
static int submit_reg_pir(struct target *t, int num)
{
	...
	uint64_t reg_op = target_buffer_get_u64(t, (uint8_t *)&arch_info->op);
	uint8_t *p_reg = (uint8_t *)&reg_op;

	LOG_DEBUG("reg %s op=0x%016lX", regs[num].name,
			(long unsigned int)regs[num].op);
...
static int submit_instruction_pir(struct target *t, int num)
{
	uint64_t instr_op = target_buffer_get_u64(t, (uint8_t *)&instructions[num].op);
	uint8_t *p_instr = (uint8_t *)&instr_op;

	LOG_DEBUG("%s op=0x%016lX", instructions[num].name,
	...
	if (t->state == TARGET_HALTED) {
		...
		if (bp != NULL /*&& bp->type == BKPT_SOFT*/) {
			...
			if (lakemont_step(t, 0, 0, 1) != ERROR_OK) {
				LOG_ERROR("%s stepping over a software breakpoint at 0x%08x "
