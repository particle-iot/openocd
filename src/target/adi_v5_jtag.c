			...
			list_for_each_entry_from(tmp, &dap->cmd_journal, lh) {
				if (tmp->ack == JTAG_ACK_OK_FAULT) {
					...
					if (el->invalue != el->invalue_buf)
						*((uint32_t *)el->invalue) = le_to_h_u32(tmp->invalue_buf);
					prev = NULL;
					break;
				}
			}
			...
			if (prev != NULL) {
				...
				*/
				...
				do {
					...
					if (tmp->ack == JTAG_ACK_OK_FAULT) {
						log_dap_cmd("FND", tmp);
						if (tmp->invalue != tmp->invalue_buf)
							*((uint32_t *)tmp->invalue) = le_to_h_u32(tmp->invalue_buf);
						break;
					}
	...
	if (found_wait) {
		...
		list_for_each_entry_safe(el, tmp, &replay_list, lh) {
			...
			do {
				...
				if (el->ack == JTAG_ACK_OK_FAULT) {
					if (el->invalue != el->invalue_buf)
						*((uint32_t *)el->invalue) = le_to_h_u32(el->invalue_buf);
					break;
				}
