		...
		struct breakpoint *breakpoint, uint8_t matchmode)
{
	...
	while ((brp_list[brp_i].used || (brp_list[brp_i].type != BRP_CONTEXT)) &&
		(brp_i < cortex_r4->brp_num))
			brp_i++ ;
...
static int cortex_r4_set_hybrid_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	/*check available context BRPs*/
	while ((brp_list[brp_1].used || (brp_list[brp_1].type != BRP_CONTEXT)) &&
		(brp_1 < cortex_r4->brp_num))
		brp_1++;
	...
	while ((brp_list[brp_2].used || (brp_list[brp_2].type != BRP_NORMAL)) &&
		(brp_2 < cortex_r4->brp_num))
		brp_2++;
