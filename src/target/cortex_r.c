...
static int cortex_r4_restore_cp15_control_reg(struct target *target)
{
	...
	if (cortex_r4->cp15_control_reg != cortex_r4->cp15_control_reg_curr) {
		...
		retval = armv7r->armv4_5_common.mcr(target, 15,
				0, 0,   /* op1, op2 */
				1, 0,   /* CRn, CRm */
...
/*  check address before cortex_r4_apb read write access with MPU on
...
static int cortex_r4_check_address(struct target *target, uint32_t address)
{
	...
	TODO: use PMU to see if we are in a protected region
	*/
...
{
	struct armv7r_common *armv7r = target_to_armv7r(target);
	struct adiv5_dap *swjdp = armv7r->armv4_5_common.dap;
	int retval;
	uint32_t dummy;
...
/* Write to memory mapped registers directly with no cache */
static int cortex_r4_dap_write_memap_register_u32(struct target *target, uint32_t address, uint32_t value)
{
	int retval;
	struct armv7r_common *armv7r = target_to_armv7r(target);
	struct adiv5_dap *swjdp = armv7r->armv4_5_common.dap;
...
static int cortex_r4_write_dcc(struct cortex_r4_common *r4, uint32_t data)
{
	...
	return mem_ap_sel_write_u32(r4->armv7r_common.armv4_5_common.dap,
...
static int cortex_r4_read_dcc(struct cortex_r4_common *r4, uint32_t *data,
		uint32_t *dscr_p)
{
	struct adiv5_dap *swjdp = r4->armv7r_common.armv4_5_common.dap;
...
static int cortex_r4_dpm_prepare(struct arm_dpm *dpm)
{
	...
	if (dscr & DSCR_DTR_RX_FULL) {
		...
		retval = cortex_r4_exec_opcode(
				r4->armv7r_common.armv4_5_common.target,
...
static int cortex_r4_dpm_setup(struct cortex_r4_common *r4, uint32_t didr)
{
	...
	dpm->arm = &r4->armv7r_common.armv4_5_common;
	dpm->didr = didr;
...
static int cortex_r4_post_debug_entry(struct target *target)
{
	...
	cortex_r4->curr_mode = armv7r->armv4_5_common.core_mode;
		...
		struct breakpoint *breakpoint, uint8_t matchmode)
{
	...
	while ((brp_list[brp_i].used || (brp_list[brp_i].type != BRP_CONTEXT)) && (brp_i < cortex_r4->brp_num))
			brp_i++ ;
...
static int cortex_r4_set_hybrid_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	/*check available context BRPs*/
	while ((brp_list[brp_1].used || (brp_list[brp_1].type != BRP_CONTEXT)) && (brp_1 < cortex_r4->brp_num))
		brp_1++;
	...
	while ((brp_list[brp_2].used || (brp_list[brp_2].type != BRP_NORMAL)) && (brp_2 < cortex_r4->brp_num))
		brp_2++;
...
static int cortex_r4_assert_reset(struct target *target)
{
	...
	/* registers are now invalid */
	register_cache_invalidate(armv7r->armv4_5_common.core_cache);
...
static int cortex_r4_examine_first(struct target *target)
{
	...
	if ((retval = mem_ap_sel_read_atomic_u32(swjdp, swjdp_debugap,
			armv7r->debug_base + CPUDBG_CPUID, &cpuid)) != ERROR_OK) {
		LOG_DEBUG("Examine %s failed", "CPUID");
		return retval;
	}

	if ((retval = mem_ap_sel_read_atomic_u32(swjdp, swjdp_debugap,
			armv7r->debug_base + CPUDBG_CTYPR, &ctypr)) != ERROR_OK) {
		LOG_DEBUG("Examine %s failed", "CTYPR");
		return retval;
	}

	if ((retval = mem_ap_sel_read_atomic_u32(swjdp, swjdp_debugap,
			armv7r->debug_base + CPUDBG_TTYPR, &ttypr)) != ERROR_OK) {
		LOG_DEBUG("Examine %s failed", "TTYPR");
		return retval;
	}

	if ((retval = mem_ap_sel_read_atomic_u32(swjdp, swjdp_debugap,
			armv7r->debug_base + CPUDBG_DIDR, &didr)) != ERROR_OK) {
	...
	armv7r->armv4_5_common.core_type = ARM_MODE_MON;
		...
		struct cortex_r4_common *cortex_r4, struct jtag_tap *tap)
{
	...
	if (!tap->dap) {
		...
		armv7r->armv4_5_common.dap = tap->dap;
	}
