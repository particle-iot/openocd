...
int mips_ejtag_config_step(struct mips_ejtag *ejtag_info, int enable_step)
{
	...
	pracc_add(&ctx, 0, MIPS32_MFC0(isa, 8, 23, 0));			/* move COP0 Debug to $8 */
	pracc_add(&ctx, 0, MIPS32_ORI(isa, 8, 8, 0x0100));			/* set SSt bit in debug reg */
	if (!enable_step)
		pracc_add(&ctx, 0, MIPS32_XORI(isa, 8, 8, 0x0100));		/* clear SSt bit in debug reg */

	pracc_add(&ctx, 0, MIPS32_MTC0(isa, 8, 23, 0));			/* move $8 to COP0 Debug */
