...
static void print_opcode(struct command_context *context, const cs_insn *insn)
{
	uint32_t opcode = 0;
...
int a64_disassemble(struct command_context *context, struct target *target, target_addr_t address, size_t count)
{
	int ret;
	int csret;
	csh handle;
...
int a64_disassemble(struct command_context *context, struct target *target, target_addr_t address, size_t count)
{
	command_print(context, "capstone disassembly framework required");
