	...
	if (breakpoint->type == BKPT_HARD) {
		...
		if (breakpoint->length == 4) {
			...
			if (verify != arm7_9->arm_bkpt) {
				LOG_ERROR("Unable to set 32 bit software breakpoint at address %08" PRIX
			...
			if (verify != arm7_9->thumb_bkpt) {
				LOG_ERROR("Unable to set thumb software breakpoint at address %08" PRIX
						" - check that memory is read/writable", breakpoint->address);
				return ERROR_OK;
			}
		}
	...
	uint8_t *buffer)
{
	...
	LOG_DEBUG("address: 0x%" PRIXX ", size: 0x%" PRIXX ", count: 0x%" PRIXX "",
		address, size, count);
	...
	if (((cpsr & 0x1f) == ARM_MODE_ABT) && (arm->core_mode != ARM_MODE_ABT)) {
		LOG_WARNING(
			"memory read caused data abort (address: 0x%" PRIXX ", size: 0x%" PRIX ", count: 0x%" PRIX ")",
			address,
			size,
			count);
	...
	const uint8_t *buffer)
{
	...
	if (((cpsr & 0x1f) == ARM_MODE_ABT) && (arm->core_mode != ARM_MODE_ABT)) {
		LOG_WARNING(
			"memory write caused data abort (address: 0x%8.8" PRIX ", size: 0x%" PRIX ", count: 0x%" PRIX ")",
			address,
			size,
			count);
	...
	const uint8_t *buffer)
{
	...
	if (retval == ERROR_OK) {
		...
		if (endaddress != (address + count*4)) {
			LOG_ERROR(
				"DCC write failed, expected end address 0x%08" PRIX " got 0x%0" PRIx32 "",
				(address + count*4),
				endaddress);
			retval = ERROR_FAIL;
		}
