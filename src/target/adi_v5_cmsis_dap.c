...
static int cmsis_dap_run(struct adiv5_dap *dap)
{
	...
	/*
	  ...
	  This is not guaranteed to be reported via status byte of the
	  return USB packet from CMSIS-DAP, so we need to check
	  CTRL/STAT and if that happens to clear it.
	*/
