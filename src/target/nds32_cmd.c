...
COMMAND_HANDLER(handle_nds32_memory_access_command)
{
	...
	command_print(CMD_CTX, "memory access channel: %s", NDS_MEMORY_ACCESS_NAME[nds32->memory.access_channel]);
...
COMMAND_HANDLER(handle_nds32_memory_mode_command)
{
	...
	command_print(CMD_CTX, "memory mode: %s", NDS_MEMORY_SELECT_NAME[nds32->memory.mode]);
