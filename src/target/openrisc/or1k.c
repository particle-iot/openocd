				  ...
				  struct breakpoint *breakpoint)
{
	...
	memcpy(&ins, breakpoint->orig_instr, 4);
		...
		uint32_t count, const uint8_t *buffer)
{
	...
	uint32_t *t = NULL;
	t = malloc(count * sizeof(uint8_t));
	if (t == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}

	uint32_t *block_count_buffer = t;

	memcpy(t, buffer, count * sizeof(uint8_t));
	...
	while (block_count_left) {
		...
		block_count_buffer += blocks_per_round;
	}
		...
		uint32_t count, const uint8_t *buffer)
{
	...
	memcpy(t, buffer, count * sizeof(uint32_t));
		...
		uint32_t size, uint32_t count, const uint8_t *buffer)
{
	...
	memcpy(t, buffer, count * size * sizeof(uint8_t));

	int retval = ERROR_OK;

	switch (size) {
	case 4:
		retval = du_core->or1k_jtag_write_memory32(&or1k->jtag, address, count,
							 t);
		break;
	case 2:
		retval = du_core->or1k_jtag_write_memory16(&or1k->jtag, address, count,
							 (uint16_t *)t);
		break;
	case 1:
		retval = du_core->or1k_jtag_write_memory8(&or1k->jtag, address, count,
							buffer);
		break;
	default:
		break;
