...
struct insn_regs i_regs[] = {{IDCODE,    32},
			     {TGT_PINS,  164},
			     {CMD,       12},
			     {DATA,      100},
			     {BKPT,      69},
...
static int jem_resume(struct target *target,
					  int current,
					  uint32_t address,
					  int handle_breakpoints,
					  int debug_execution);
static int jem_add_breakpoint(struct target *target, struct breakpoint *breakpoint);
static int jem_step(struct target *target,
					int current,
					uint32_t address,
					int handle_breakpoints);
static int jem_read_memory(struct target *target,
						   uint32_t address,
						   uint32_t size,
						   uint32_t count,
						   uint8_t *buffer);
static int jem_write_memory(struct target *target,
							uint32_t address,
							uint32_t size,
							uint32_t count,
...
static int jem_get_dr32(struct target *target,
						enum IR_NAMES ir,
						uint32_t start,
						uint32_t length,
						uint32_t *value);
static int jem_set_dr32(struct target *target,
						enum IR_NAMES ir,
						uint32_t start,
						uint32_t length,
						uint32_t value);
static int jem_set_dr_bit(struct target *target,
						  enum IR_NAMES ir,
						  uint32_t bit,
						  uint32_t value);
...
/* JEM Helpers */
static int jem_check_status(struct target *target,
							uint32_t bit,
							int expected,
							enum IR_NAMES return_state,
							uint32_t retries);
static int jem_reset_jtag(void);

static int create_scan_fields(unsigned char *src,
							  int bit_length,
							  struct scan_field **return_fields,
							  int *num_fields);
...
static int byte_idx(int bit, enum IR_NAMES ins)
{
	uint32_t dr_len;

	dr_len = get_dr_length(ins);

	return bit / 8;
}

static int shift_idx(int bit, enum IR_NAMES ins)
{
	uint32_t dr_len;
...
static int jem_check_reg(struct target *target, enum IR_NAMES ir, uint32_t bit, int expected, uint32_t ms_timeout)
{
	...
	if (jem_change_ir(target, ir) == ERROR_OK) {
		...
		do {
			...
			if (jem_get_dr_bit(target, ir, get_dr_length(ir)))
				;
	...
	if (retval == ERROR_OK)
		LOG_DEBUG("!!!!!!!!!!!!!!!!!CHECK WORKED!!!!!!!!!!!!!!!!!!!");
	else
		LOG_DEBUG("vvvvvvvvvvvvvvvvvvvvvvvvCHECK FAILEDvvvvvvvvvvvvvvvvvvvvvvvvv");
	return retval;
}
							...
							uint32_t retries)
{
	...
	do {
		if (jem_check_reg(target, STATUS, bit, expected, default_timeout_ms) == ERROR_OK) {
			retval = ERROR_OK;
			break;
		}
	} while (--retries > 0);
...
static NOT_USED int jem_addr_sel(struct target *target, int val1, int val2)
{
...
static NOT_USED int jem_arch_read(struct target *target)
{
	LOG_DEBUG("%s(%d)", __FILE__, __LINE__);
	return ERROR_OK;
}
		...
		if (jem_get_drscan_buffer(target, STATUS) == ERROR_OK) {
			...
			if (jem_change_ir(target, STATUS) == ERROR_OK) {
				if (jem_get_drscan_buffer(target, STATUS) == ERROR_OK) {
					...
					if (jem_change_ir(target, CMD) == ERROR_OK) {
						if (jem_get_drscan_buffer(target, CMD) == ERROR_OK) {
							if (status != 1 && jem_get_dr_bit(target, CMD, 7) == 0)
								running = 1;
							else {
								jem_set_dr_bit(target, CMD, 7, 1);
								...
								if (jem_set_drscan_buffer(target, CMD, 1) == ERROR_OK) {
									...
									running = (jem_set_drscan_buffer(target, CMD, 1) == ERROR_OK) ? 1 : 0;
								}
							}
						}
					}
...
static int jem_halt(struct target *target)
{
	...
	do {
		if (jem_change_ir(target, CMD) == ERROR_OK) {
			...
			jem_set_dr_bit(target, CMD, 7, 1);
			if (jem_set_drscan_buffer(target, CMD, 0) == ERROR_OK) {
				if (jem_check_status(target, 12, 1, CMD, 1) == ERROR_OK) {
...
static int jem_soft_reset_halt(struct target *target)
{
	...
	if (jem_change_ir(target, CMD) == ERROR_OK) {
		if (jem_get_drscan_buffer(target, CMD) == ERROR_OK) {
			...
			jem_set_dr_bit(target, CMD,  8, 1);
			if (jem_set_drscan_buffer(target, CMD, 0) == ERROR_OK) {
				if (jem_check_reg(target, TGT_PINS, 128, 0, default_timeout_ms*4) == ERROR_OK) {
					if (jem_change_ir(target, CMD) == ERROR_OK) {
						if (jem_get_drscan_buffer(target, CMD) == ERROR_OK) {
							jem_set_dr_bit(target, CMD, 11, 0);
							jem_set_dr_bit(target, CMD,  9, 1);
							if (jem_set_drscan_buffer(target, CMD, 0) == ERROR_OK) {
								...
								if (jem_check_reg(target, TGT_PINS, 128, 1, default_timeout_ms*4) == ERROR_OK) {
									if (jem_change_ir(target, CMD) == ERROR_OK) {
										if (jem_get_drscan_buffer(target, CMD) == ERROR_OK) {
	...
	if (target->state == TARGET_HALTED) {
		if (jem_change_ir(target, DATA) == ERROR_OK) {
			do {
				...
				jem_set_dr32(target, DATA, 64, 32, address+(offset++));
				jem_set_dr32(target, DATA, 32, 32, 0x0);
				jem_set_dr32(target, DATA, 0, 32, local);
				err = (jem_set_drscan_buffer(target, DATA, 1) == ERROR_OK) ? ERROR_OK : ERROR_FAIL;
...
static int jem_read_memory(struct target *target, uint32_t addr, uint32_t size, uint32_t count, uint8_t *buffer)
{
	...
	if (target->state == TARGET_HALTED) {
		if (jem_change_ir(target, DATA) == ERROR_OK) {
			do {
				LOG_DEBUG("read addr=0x%x", addr+offset);
...
/*	*value = buf_get_u32(&JTAG(target).dr[byte*4],first, length); */
	bit_copy((uint8_t *)value, 0, JTAG(target).dr, start, length);
...
static int jem_set_dr32(struct target *target, enum IR_NAMES ir, uint32_t start, uint32_t length, uint32_t value)
{
	...
	bit_copy(JTAG(target).dr, start, (const uint8_t *)&value, 0, length);
...
static int jem_set_dr_bit(struct target *target, enum IR_NAMES ir, uint32_t bit, uint32_t value)
{
	...
	byte = byte_idx(bit, ir);
	shift = shift_idx(bit, ir);

	if (v) {
		LOG_DEBUG("********* ENTER to SET BIT %d/%d *************", bit, dr_len);
		LOG_DEBUG("cur bits: %s", buf_to_str(JTAG(target).dr, dr_len, 16));
		JTAG(target).dr[byte] |= v << shift;
	} else {
		LOG_DEBUG("********* ENTER to CLR BIT %d/%d *************", bit, dr_len);
		LOG_DEBUG("cur bits: %s", buf_to_str(JTAG(target).dr, dr_len, 16));
		JTAG(target).dr[byte] &= ~(1 << shift);
	}
...
static int jem_get_dr_bit(struct target *target, enum IR_NAMES ir, uint32_t bit)
{
	...
	get = (JTAG(target).dr[byte_idx(bit, ir)] & (1 << shift_idx(bit, ir))) > 0 ? 1 : 0;

	LOG_DEBUG("**************** BIT %d = %02x,%02x,%02x *************************", bit,
																					(unsigned char)get,
																					JTAG(target).dr[byte_idx(bit, ir)],
																					(1 << shift_idx(bit, ir)));
...
static int jem_irscan_buffer(struct target *target, enum IR_NAMES ir)
{
	NOT_USED uint32_t ms_timeout = 200;
	...
	buf_set_u32((uint8_t *)&ir_register, 0, ir_field.num_bits, ir);
	jtag_add_ir_scan(target->tap, &ir_field, TAP_IDLE);
/*	jtag_add_runtest((jtag_get_speed_khz()*ms_timeout)/1000, TAP_IDLE); */
	if (jtag_execute_queue() != ERROR_OK)
		return ERROR_FAIL;
...
static int jem_get_drscan_buffer(struct target *target, enum IR_NAMES ir)
{
	...
	if (create_scan_fields(JTAG(target).dr, dr_len, &dr_fields, &dr_field_count)) {
		...
		for (i = 0; i < dr_field_count; i++) {
			/* bit_copy(JTAG(target).dr, bit, dr_fields[i].in_value, 0, dr_fields[i].num_bits); */
			buf_set_buf(dr_fields[i].in_value, 0, JTAG(target).dr, bit, dr_fields[i].num_bits);
			bit += dr_fields[i].num_bits;
		}
		LOG_DEBUG("************captured 1: %s", buf_to_str(JTAG(target).dr, dr_len, 16));
	}
...
{
	unsigned int i;
	for (i = 0; i < sizeof(i_regs); i++)
		if (i_regs[i].id == ir_val)
			return i_regs[i].size;
	return -1;
}
...
static int create_scan_fields(uint8_t *src, int bit_length, struct scan_field **created_fields, int *num_fields)
{
	...
	if (created_fields == NULL || src == NULL)
		return bit_length;
	...
	do {
		...
		/*	LOG_DEBUG("Creating 32 bit scan field...");
			...
			t = calloc(1, sizeof(uint32_t));
			r = calloc(1, sizeof(uint32_t));
			field_bits = 32;
...
static uint64_t orwl_timestart = (uint64_t)0;
