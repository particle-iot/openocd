...
/**
 * Sets target state and clears register cache
 * during reset process
 */
int cortex_m_reset_clear_internal_state(struct target *target)
{
	struct cortex_m_common *cortex_m = target_to_cm(target);
	struct armv7m_common *armv7m = &cortex_m->armv7m;
...
int cortex_m_reset_prepare_trigger(struct target *target, bool halt, bool trigger)
{
	...
	cortex_m_reset_clear_internal_state(target);
...
struct target_type cortexm_target = {
	...
	.reset_clear_internal_state = cortex_m_reset_clear_internal_state,
