			      ...
			      uint32_t max_num_samples, uint32_t *num_samples, uint32_t seconds)
{
	...
	if (reg_value != 0) {
		use_pcsr = 1;
		LOG_INFO("Starting cortex-m profiling. Sampling pcsr as fast as we can...");
	} else {
	...
	for (;;) {
		if (use_pcsr) {
			...
			if (read_count > 1024)
				read_count = 1024;
			retval = mem_ap_read_buf_noincr(armv7m->debug_ap, (void *)&samples[sample_count], 4, read_count, DWT_PCSR);
