...
{
	if (breakpoint->set) {
		int res = cortex_m_unset_breakpoint(target, breakpoint);

		/* REVISIT: We might need to call poll now.
		 * If the target hits the bp just before we unset it, give poll the chance
		 * to find bp in the list before the list item is removed.
		 * Ignore error from poll as it is not important in bp removing
		 *
		(void)target_poll(target);
		 */
