			...
			if ((pc_value & 0x02) && breakpoint_find(target, pc_value & ~0x03)) {
				...
				if (breakpoint)
					...
					enum breakpoint_type type =
						(cortex_m->isrmasking_mode == CORTEX_M_ISRMASK_AUTO
							&& !target_addr_in_flash(target, pc_value))
						? BKPT_SOFT : BKPT_HARD;
...
COMMAND_HANDLER(handle_cortex_m_mask_interrupts_command)
{
	...
	static const Jim_Nvp nvp_maskisr_modes[] = {
		...
		{ .name = "auto_hard_bp", .value = CORTEX_M_ISRMASK_AUTO_HARD_BP },
		{ .name = NULL, .value = -1 },
	};
	const Jim_Nvp *n;
...
static const struct command_registration cortex_m_exec_command_handlers[] = {
	{
		...
		.usage = "['auto'|'on'|'off'|'auto_hard_bp']",
	},
	{
		.name = "vector_catch",
