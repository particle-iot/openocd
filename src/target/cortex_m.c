...
COMMAND_HANDLER(cortex_m_handle_smp_off_command)
{
	struct target *target = get_current_target(CMD_CTX);
...
COMMAND_HANDLER(cortex_m_handle_smp_on_command)
{
	struct target *target = get_current_target(CMD_CTX);
	struct target_list *head;
	struct target *curr;
...
static const struct command_registration cortex_m_exec_command_handlers[] = {
	{   .name = "smp_off",
		.handler = cortex_m_handle_smp_off_command,
		.mode = COMMAND_EXEC,
		.help = "Stop smp handling",
		.usage = "",},
	{
		.name = "smp_on",
		.handler = cortex_m_handle_smp_on_command,
		.mode = COMMAND_EXEC,
