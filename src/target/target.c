...
int target_reset_prepare(struct target *target, enum target_reset_mode reset_mode)
{
	...
	return target->type->reset_prepare(target, reset_mode);
}

int target_reset_trigger(struct target *target, enum target_reset_mode reset_mode)
{
	...
	return target->type->reset_trigger(target, reset_mode);
}
...
static int jim_target_reset(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	...
	case NVP_PREPARE:
		if (target->type->reset_prepare) {
			e = target->type->reset_prepare(target, target->reset_halt ? RESET_HALT : RESET_RUN);
	...
	case NVP_TRIGGER:
		if (target->type->reset_trigger) {
			e = target->type->reset_trigger(target, target->reset_halt ? RESET_HALT : RESET_RUN);
		} else {
			Jim_SetResultFormatted(interp,
