		...
		uint32_t entry_point, uint32_t exit_point, void *arch_info)
{
	...
	while (count > 0) {
		...
		if (thisrun_bytes == 0) {
			...
			if (timeout++ >= 500) {
				LOG_ERROR("timeout waiting for algorithm; try resetting the target before performing this operation, check power supply rails, !RESET, BOOTx lines and external clocks (if needed)");
