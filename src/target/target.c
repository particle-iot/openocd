...
static int jim_target_md(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	...
	*response = 0;
	...
	for (x = 0; x < count; x++) {
		...
		snprintf(element, 10, format, (unsigned int)val);
	...
	/* Print the hexump-like characters even if the data is not divisable by 16 */
	if ((count * dwidth) % 16 != 0) {
		int i = (count * dwidth) - (count*dwidth) % 16;
		for (; i < count*dwidth; i++) {
			if ((target_buf[i] >= 0x20) && (target_buf[i] <= 0x7e)) {
				/* good */
				command_print_sameline(NULL, "%c", target_buf[i]);
			} else {
