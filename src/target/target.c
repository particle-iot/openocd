...
static struct target_reset_callback *target_reset_callbacks;
...
{
	struct target_reset_callback **callbacks_p = &target_reset_callbacks;
	...
	(*callbacks_p) = malloc(sizeof(struct target_reset_callback));
	(*callbacks_p)->callback = callback;
	(*callbacks_p)->priv = priv;
	(*callbacks_p)->next = NULL;
...
int target_unregister_reset_callback(int (*callback)(struct target *target,
		enum target_reset_mode reset_mode, void *priv), void *priv)
{
	struct target_reset_callback **p = &target_reset_callbacks;
	struct target_reset_callback *c = target_reset_callbacks;

	if (callback == NULL)
		return ERROR_COMMAND_SYNTAX_ERROR;

	while (c) {
		struct target_reset_callback *next = c->next;
		if ((c->callback == callback) && (c->priv == priv)) {
			*p = next;
			free(c);
			return ERROR_OK;
...
{
	struct target_reset_callback *callback = target_reset_callbacks;
	struct target_reset_callback *next_callback;
