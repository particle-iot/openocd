			...
			for (int dst_offset = 0; dst_offset <= 1; dst_offset++) {
				for (int read_size = 1; read_size <= 4; read_size *= 2) {
					for (int write_size = 1; write_size <= 4; write_size *= 2) {
						...
						static const uint8_t data[] = "This is really constant data and must not be touched!";

						retval = target_write_memory(target, wa->address + offset, write_size, MAX(size, (int)sizeof(data)) / write_size, data);
						if (retval != ERROR_OK) {
							LOG_ERROR("Const data write failed");
							fail = true;
						}
