 ...
 *                                                                         *
 *   Copyright (C) 2011 by Konstantin Kostyukhin, Nikolay Shmyrev          *
 *                                                                         *
...
static int target_read_buffer_default(struct target *target, uint64_t address,
		uint32_t count, uint8_t *buffer);
static int target_write_buffer_default(struct target *target, uint64_t address,
...
static int err_read_phys_memory(struct target *target, uint64_t address,
...
static int err_write_phys_memory(struct target *target, uint64_t address,
 ...
 * possible
 */
int target_write_buffer(struct target *target, uint64_t address, uint32_t size, const uint8_t *buffer)
{
	LOG_DEBUG("writing buffer of %i byte at 0x%16.16llx",
	...
	if ((address + size - 1) < address) {
		...
		LOG_ERROR("address + size wrapped(0x%16llx, 0x%08x)",
				  address,
				  (unsigned)size);
		return ERROR_FAIL;
	}
...
static int target_write_buffer_default(struct target *target, uint64_t address, uint32_t count, const uint8_t *buffer)
{
	uint32_t size;
	uint32_t max_size = 4;
...
{
	LOG_DEBUG("reading buffer of %i byte at 0x%16.16llx",
		(int)size, address);
	...
	if ((address + size - 1) < address) {
		/* GDB can request this when e.g. PC is 0xfffffffc*/
		LOG_ERROR("address + size wrapped(0x%16" PRIx64 ", 0x%08" PRIx32 ")",
				  address,
				  size);
		return ERROR_FAIL;
	}
...
static int target_read_buffer_default(struct target *target, uint64_t address, uint32_t count, uint8_t *buffer)
{
	uint32_t size;
	uint32_t max_size = 4;
...
int target_checksum_memory(struct target *target, uint64_t address, uint32_t size, uint32_t* crc)
{
	uint8_t *buffer;
	int retval;
	uint32_t i;
...
int target_blank_check_memory(struct target *target, uint64_t address, uint32_t size, uint32_t* blank)
{
	int retval;
	if (!target_was_examined(target)) {
		LOG_ERROR("Target not examined yet");
...
int target_read_u32(struct target *target, uint64_t address, uint32_t *value)
{
	uint8_t value_buf[4];
	if (!target_was_examined(target)) {
		LOG_ERROR("Target not examined yet");
	...
	if (retval == ERROR_OK) {
		*value = target_buffer_get_u32(target, value_buf);
		LOG_DEBUG("address: 0x%16.16" PRIx64 ", value: 0x%8.8" PRIx32 "",
				  address,
				  *value);
	} else {
		*value = 0x0;
		LOG_DEBUG("address: 0x%16.16" PRIx64 " failed",
				  address);
	}
...
int target_read_u16(struct target *target, uint64_t address, uint16_t *value)
{
	uint8_t value_buf[2];
	if (!target_was_examined(target)) {
		LOG_ERROR("Target not examined yet");
	...
	if (retval == ERROR_OK) {
		*value = target_buffer_get_u16(target, value_buf);
		LOG_DEBUG("address: 0x%16.16" PRIx64 ", value: 0x%4.4x",
				  address,
				  *value);
	} else {
		*value = 0x0;
...
int target_read_u8(struct target *target, uint64_t address, uint8_t *value)
{
	if (!target_was_examined(target)) {
		LOG_ERROR("Target not examined yet");
		return ERROR_FAIL;
	...
	if (retval == ERROR_OK) {
		LOG_DEBUG("address: 0x%16.16" PRIx64 ", value: 0x%2.2x",
				  address,
				  *value);
	} else {
		*value = 0x0;
...
int target_write_u32(struct target *target, uint64_t address, uint32_t value)
{
	int retval;
	uint8_t value_buf[4];
	if (!target_was_examined(target)) {
...
int target_write_u16(struct target *target, uint64_t address, uint16_t value)
{
	int retval;
	uint8_t value_buf[2];
	if (!target_was_examined(target)) {
	...
	LOG_DEBUG("address: 0x%16.16" PRIx64 ", value: 0x%8.8x",
			  address,
			  value);
...
int target_write_u8(struct target *target, uint64_t address, uint8_t value)
{
	int retval;
	if (!target_was_examined(target)) {
		LOG_ERROR("Target not examined yet");
...
static void handle_md_output(struct command_context *cmd_ctx,
		struct target *target, uint64_t address, unsigned size,
...
COMMAND_HANDLER(handle_mw_command)
{
	...
	uint64_t value;
	COMMAND_PARSE_NUMBER(ullong, CMD_ARGV[1], value);
		...
		uint64_t *min_address, uint64_t *max_address)
{
	...
	if (CMD_ARGC >= 4)
		COMMAND_PARSE_NUMBER(u64, CMD_ARGV[3], *min_address);
	if (CMD_ARGC == 5) {
		COMMAND_PARSE_NUMBER(u64, CMD_ARGV[4], *max_address);
...
COMMAND_HANDLER(handle_load_image_command)
{
	...
	for (i = 0; i < image.num_sections; i++) {
		...
		if ((image.sections[i].base_address + buf_cnt >= min_address) &&
			...
			image_size += length;
			command_print(CMD_CTX, "%u bytes written at address 0x%16.16" PRIx64 "",
					(unsigned int)length,
...
static COMMAND_HELPER(handle_verify_image_command_internal, int verify)
{
	...
	for (i = 0; i < image.num_sections; i++) {
		...
		if (verify) {
			...
			if (checksum != mem_checksum) {
				...
				if (retval == ERROR_OK) {
					...
					for (t = 0; t < buf_cnt; t++) {
						if (data[t] != buffer[t]) {
							command_print(CMD_CTX,
										  "diff %d address 0x%16llx. Was 0x%02x instead of 0x%02x",
										  diffs,
			...
			command_print(CMD_CTX, "address 0x%16" PRIx64 " length 0x%08zx",
...
static int handle_bp_command_list(struct command_context *cmd_ctx)
{
	...
	while (breakpoint) {
		if (breakpoint->type == BKPT_SOFT) {
			...
			command_print(cmd_ctx, "IVA breakpoint: 0x%16.16" PRIx64 ", 0x%x, %i, 0x%s",
					breakpoint->address,
					breakpoint->length,
					breakpoint->set, buf);
			free(buf);
			...
			if ((breakpoint->address == 0) && (breakpoint->asid != 0))
				command_print(cmd_ctx, "Context breakpoint: 0x%16.16" PRIx64 ", 0x%x, %i",
							breakpoint->asid,
							breakpoint->length, breakpoint->set);
			else if ((breakpoint->address != 0) && (breakpoint->asid != 0)) {
				command_print(cmd_ctx, "Hybrid breakpoint(IVA): 0x%16.16" PRIx64 ", 0x%x, %i",
							breakpoint->address,
							breakpoint->length, breakpoint->set);
				command_print(cmd_ctx, "\t|--->linked with ContextID: 0x%16.16" PRIx64,
							breakpoint->asid);
			} else
				command_print(cmd_ctx, "Breakpoint(IVA): 0x%16.16" PRIx64 ", 0x%x, %i",
							breakpoint->address,
							breakpoint->length, breakpoint->set);
		}
		...
		uint64_t addr, uint32_t asid, uint32_t length, int hw)
{
	...
	if (asid == 0) {
		...
		if (ERROR_OK == retval)
			command_print(cmd_ctx, "breakpoint set at 0x%16.16" PRIx64 "", addr);
		else {
		...
		if (ERROR_OK == retval)
			command_print(cmd_ctx, "Context breakpoint set at 0x%16.16" PRIx64 "", asid);
		else {
		...
		if (ERROR_OK == retval)
			command_print(cmd_ctx, "Hybrid breakpoint set at 0x%16.16" PRIx64 "", asid);
		else {
...
COMMAND_HANDLER(handle_wp_command)
{
	...
	if (CMD_ARGC == 0) {
		...
		while (watchpoint) {
			command_print(CMD_CTX, "address: 0x%16.16" PRIx64
					", len: 0x%8.8" PRIx32
					", r/w/a: %i, value: 0x%16.16" PRIx64
					", mask: 0x%16.16" PRIx64,
					watchpoint->address,
					watchpoint->length,
					(int)watchpoint->rw,
					watchpoint->value,
...
COMMAND_HANDLER(handle_virt2phys_command)
{
	...
	uint64_t va;
	COMMAND_PARSE_NUMBER(u64, CMD_ARGV[0], va);
	uint64_t pa;

	struct target *target = get_current_target(CMD_CTX);
	int retval = target->type->virt2phys(target, va, &pa);
	if (retval == ERROR_OK)
		command_print(CMD_CTX, "Physical address 0x%16" PRIx64 "", pa);
...
static void write_gmon(uint64_t *samples, uint32_t sampleNum, const char *filename,
		bool with_range, uint64_t start_address, uint64_t end_address)
{
	...
	uint64_t addressSpace = max - min;
	assert(addressSpace >= 2);

	/* FIXME: What is the reasonable number of buckets?
	 * The profiling result will be more accurate if there are enough buckets. */
	static const uint64_t maxBuckets = 128 * 1024; /* maximum buckets. */
	uint64_t numBuckets = addressSpace / sizeof(UNIT);
	if (numBuckets > maxBuckets)
		numBuckets = maxBuckets;
	uint64_t *buckets = malloc(sizeof(uint64_t) * numBuckets);
	if (buckets == NULL) {
		fclose(f);
		return;
	}
	memset(buckets, 0, sizeof(uint64_t) * numBuckets);
	for (i = 0; i < sampleNum; i++) {
...
COMMAND_HANDLER(handle_profile_command)
{
	...
	uint64_t *samples = malloc(sizeof(uint64_t) * MAX_PROFILE_SAMPLE_NUM);
	if (samples == NULL) {
...
static int target_mem2array(Jim_Interp *interp, struct target *target, int argc, Jim_Obj *const *argv)
{
	...
	if ((width == 1) ||
		...
		char buf[100];
		Jim_SetResult(interp, Jim_NewEmptyStringObj(interp));
		sprintf(buf, "mem2array address: 0x%16" PRIx64 " is not aligned for %" PRId32 " byte reads",
				addr,
				(unsigned)width);
	...
	while (len) {
		...
		if (retval != ERROR_OK) {
			/* BOO !*/
			LOG_ERROR("mem2array: Read @ 0x%16llx, w=%d, cnt=%d, failed",
					  addr,
					  (int)width,
					  (int)count);
			Jim_SetResult(interp, Jim_NewEmptyStringObj(interp));
		...
		int argc, Jim_Obj *const *argv)
{
	...
	if ((width == 1) ||
		...
		char buf[100];
		Jim_SetResult(interp, Jim_NewEmptyStringObj(interp));
		sprintf(buf, "array2mem address: 0x%16llx is not aligned for %d byte reads",
				addr,
				(int)width);
		Jim_AppendStrings(interp, Jim_GetResult(interp), buf , NULL);
		return JIM_ERR;
	...
	while (len) {
		...
		if (retval != ERROR_OK) {
			/* BOO !*/
			LOG_ERROR("array2mem: Write @ 0x%16llx, w=%d, cnt=%d, failed",
					  addr,
					  (int)width,
					  (int)count);
			Jim_SetResult(interp, Jim_NewEmptyStringObj(interp));
	...
	while (bytes > 0) {
		...
		case 8:
			for (x = 0 ; (x < 16) && (x < y) ; x += 8) {
				...
				command_print_sameline(NULL, "%16llx ", (z));
			}
			for (; (x < 16) ; x += 8)
...
static const struct command_registration target_instance_command_handlers[] = {
	...
	},
	{
		...
		.help = "Displaytargetmemoryas64-bitwords",
		.usage = "address[count]",
	},
	{
		.name = "mdw",
...
COMMAND_HANDLER(handle_fast_load_command)
{
	...
	for (i = 0; i < fastload_num; i++) {
		struct target *target = get_current_target(CMD_CTX);
		command_print(CMD_CTX, "Write to 0x%16llx, length 0x%08x",
					  fastload[i].address,
...
static const struct command_registration target_exec_command_handlers[] = {
	...
	},
	{
		...
		.mode = COMMAND_EXEC,
		.help = "writememorydoubleword",
		.usage = "['phys']addressvalue[count]",
	},
	{
		.name = "mww",
		.handler = handle_mw_command,
