...
static int mem_ap_examine(struct target *target)
{
	...
	if (!target_was_examined(target)) {
		int retval = dap_dp_init(mem_ap->dap);
		if (retval != ERROR_OK)
			return retval;
			       ...
			       uint32_t size, uint32_t count, uint8_t *buffer)
{
	...
	retval = mem_ap_read_buf(mem_ap->ap, buffer, size, count, address);
	if (retval == ERROR_OK)
				...
				const uint8_t *buffer)
{
	...
	retval = mem_ap_write_buf(mem_ap->ap, buffer, size, count, address);
	if (retval == ERROR_OK)
		return jtag_execute_queue();
