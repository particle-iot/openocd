...
static int dsp5680xx_read(struct target *t, target_ulong a, target_ulong size,
			  target_ulong count, uint8_t *buf)
{
	struct target *target = t;
 ...
 * @return
 */
static int dsp5680xx_write(struct target *t, target_ulong a, target_ulong s, target_ulong c,
			   const uint8_t *b)
{
...
static int dsp5680xx_bulk_write_memory(struct target *t, target_ulong a,
				       target_ulong al, const uint8_t *b)
{
	LOG_ERROR("Not implemented yet.");
	return ERROR_FAIL;
}

static int dsp5680xx_write_buffer(struct target *t, target_ulong a, target_ulong size,
				  const uint8_t *b)
{
 ...
 * @return
 */
static int dsp5680xx_read_buffer(struct target *t, target_ulong a, target_ulong size,
				 uint8_t *buf)
{
	check_halt_and_debug(t);
 ...
 * @return
 */
static int dsp5680xx_checksum_memory(struct target *t, target_ulong a, target_ulong s,
				     uint32_t *checksum)
{
	return ERROR_FAIL;
}
