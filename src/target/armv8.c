...
static int armv8_read_reg(struct armv8_common *armv8, int regnum, uint64_t *regval)
{
	...
	case ARMV8_V0 ... ARMV8_V31:
		retval = dpm->instr_read_data_r0_64(dpm,
				ARMV8_MOV_GPR_VFP(0, (regnum - ARMV8_V0), 1), &value_64);
		...
		retval = dpm->instr_read_data_r0_64(dpm,
				ARMV8_MOV_GPR_VFP(0, (regnum - ARMV8_V0), 0), &value_64);
		break;
	case ARMV8_FPSR:
		retval = dpm->instr_read_data_r0(dpm,
...
static int armv8_write_reg(struct armv8_common *armv8, int regnum, uint64_t *value_64)
{
	...
	case ARMV8_V0 ... ARMV8_V31:
		retval = dpm->instr_write_data_r0_64(dpm,
				ARMV8_MOV_VFP_GPR((regnum - ARMV8_V0), 0, 1), value_64[1]);
		...
		retval = dpm->instr_write_data_r0_64(dpm,
				ARMV8_MOV_VFP_GPR((regnum - ARMV8_V0), 0, 0), value_64[0]);
		break;
	case ARMV8_FPSR:
		value = *value_64;
		retval = dpm->instr_write_data_r0(dpm,
...
static int armv8_write_reg32(struct armv8_common *armv8, int regnum, uint64_t *value_64)
{
	struct arm_dpm *dpm = &armv8->dpm;
	int retval;
	uint32_t value = *value_64;
...
static int armv8_set_core_reg(struct reg *reg, uint8_t *buf)
{
	...
	if (reg->size <= 64) {
		...
		value[0] = buf_get_u64(buf, 0, 64);
		value[1] = buf_get_u64(buf, 64, reg->size - 64);

		buf_set_u64(reg->value, 0, 64, value[0]);
		buf_set_u64(reg->value, 64, reg->size - 64, value[1]);
	}
