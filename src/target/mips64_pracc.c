...
static int mips64_pracc_exec_read(struct mips64_pracc_context *ctx, uint32_t address)
{
	...
	/* Send the data out */
	mips64_ejtag_set_instr(ctx->ejtag_info, EJTAG_INST_DATA);
	mips_ejtag_drscan_64_out(ctx->ejtag_info, data);

	/* Clear the access pending bit (let the processor eat!) */
	ejtag_ctrl = ejtag_info->ejtag_ctrl & ~EJTAG_CTRL_PRACC;
	mips64_ejtag_set_instr(ctx->ejtag_info, EJTAG_INST_CONTROL);
		...
		int num_param_in, uint64_t *param_in, int num_param_out, uint64_t *param_out, int cycle)
{
	...
	while (1) {
		...
		address = 0;
		mips64_ejtag_set_instr(ejtag_info, EJTAG_INST_ADDRESS);
...
static int mips64_pracc_read_u64(struct mips_ejtag *ejtag_info, uint64_t addr, uint64_t *buf)
{
	uint32_t code[] = {
		...
		MIPS64_LUI(15, MIPS64_PRACC_UPPER_BASE_ADDR),		/* $15 = MIPS64_PRACC_BASE_ADDR */
		MIPS64_SD(8, MIPS64_PRACC_STACK_OFFSET, 15),		/* sw $8,PRACC_STACK_OFFSET($15) */

		MIPS64_LUI(8, UPPER64_16((addr + 0x8000))),             /* load  $8 with modified upper address */
		MIPS64_LD(8, LOWER64_16(addr), 8),			/* ld $8, LOWER64_16(addr)($8) */
		MIPS64_SD(8, MIPS64_PRACC_OUT_OFFSET, 15),		/* sd $8,PRACC_OUT_OFFSET($15) */

		MIPS64_LD(8, MIPS64_PRACC_STACK_OFFSET, 15),		/* ld $8,PRACC_STACK_OFFSET($15) */
		MIPS64_B(NEG16(8)),					/* b start */
		MIPS64_DMFC0(15, 31, 0),				/* move COP0 DeSave to $15 */
	};

	return mips64_pracc_exec(ejtag_info, ARRAY_SIZE(code), code, 0, NULL, 1, buf, 1);
}
...
static int mips64_pracc_read_u32(struct mips_ejtag *ejtag_info, uint64_t addr, uint32_t *buf)
{
	uint32_t code[] = {
		...
		MIPS64_DMTC0(15, 31, 0),				/* move $15 to COP0 DeSave */
		MIPS64_LUI(15, MIPS64_PRACC_UPPER_BASE_ADDR),		/* $15 = MIPS64_PRACC_BASE_ADDR */
		MIPS64_SD(8, MIPS64_PRACC_STACK_OFFSET, 15),		/* sd $8,PRACC_STACK_OFFSET($15) */

		MIPS64_LUI(8, UPPER64_16((addr + 0x8000))),		/* load  $8 with modified upper address */
		MIPS64_LD(8, LOWER64_16(addr), 8),			/* ld $8, LOWER64_16(addr)($8) */
		MIPS64_SD(8, MIPS64_PRACC_OUT_OFFSET, 15),		/* sd $8,PRACC_OUT_OFFSET($15) */

		MIPS64_LW(8, MIPS64_PRACC_STACK_OFFSET, 15),		/* lw $8,PRACC_STACK_OFFSET($15) */
		MIPS64_B(NEG16(8)),					/* b start */
		MIPS64_DMFC0(15, 31, 0),				/* move COP0 DeSave to $15 */
	};

	return mips64_pracc_exec(ejtag_info, ARRAY_SIZE(code), code, 0, NULL, 1, (uint64_t *)buf, 1);
}

int mips64_pracc_read_mem(struct mips_ejtag *ejtag_info, target_ulong addr,
	target_ulong size, target_ulong count, void *buf)
{
	if (count == 1 && size == 8)
		return mips64_pracc_read_u64(ejtag_info, addr, (uint64_t *)buf);

	if (count == 1 && size == 4)
		return mips64_pracc_read_u32(ejtag_info, addr, (uint32_t *)buf);
	...
	while (count) {
		...
		last_upper_base_addr = UPPER64_16((addr + 0x8000));
		uint32_t *code_p = code;

		*code_p++ = MIPS64_DMTC0(15, 31, 0);				/* save $15 in DeSave */
		*code_p++ = MIPS64_LUI(15, MIPS64_PRACC_UPPER_BASE_ADDR);	/* $15 = MIPS64_PRACC_BASE_ADDR */
		*code_p++ = MIPS64_SD(8, MIPS64_PRACC_STACK_OFFSET, 15);	/* save $8 and $9 to pracc stack */
		*code_p++ = MIPS64_SD(9, MIPS64_PRACC_STACK_OFFSET, 15);
		*code_p++ = MIPS64_LUI(9, last_upper_base_addr);		/* load the upper memory address in $9*/
		code_len = 5;

		for (i = 0; i != this_round_count; i++) {			/* Main code loop */
			upper_base_addr = UPPER64_16((addr + 0x8000));
			if (last_upper_base_addr != upper_base_addr) {
				*code_p++ = MIPS64_LUI(9, upper_base_addr);	/* if needed, change upper address in $9*/
				code_len++;
				last_upper_base_addr = upper_base_addr;
			}

			if (size == 4)
				*code_p++ = MIPS64_LW(8, LOWER64_16(addr), 9);	/* load from memory to $8 */
			else if (size == 2)
				*code_p++ = MIPS64_LHU(8, LOWER64_16(addr), 9);
			else
				*code_p++ = MIPS64_LBU(8, LOWER64_16(addr), 9);

			*code_p++ = MIPS64_SD(8, MIPS64_PRACC_OUT_OFFSET + i * 8, 15);	/* store $8 at param out */

			code_len += 2;
			addr += size;
		}

		*code_p++ = MIPS64_LD(9, MIPS64_PRACC_STACK_OFFSET, 15);	/* restore $8 and $9 from pracc stack */
		*code_p++ = MIPS64_LD(8, MIPS64_PRACC_STACK_OFFSET, 15);

		code_len += 4;
		*code_p++ = MIPS64_B(NEG16(code_len - 1));			/* jump to start */
		*code_p = MIPS64_DMFC0(15, 31, 0);				/* restore $15 from DeSave */

		if (size == 4) {
			retval = mips64_pracc_exec(ejtag_info, code_len,
				code, 0, NULL, this_round_count, (uint64_t *)buf32, 1);
			if (retval != ERROR_OK)
				goto exit;
			buf32 += this_round_count;
		} else {
			retval = mips64_pracc_exec(ejtag_info, code_len,
			code, 0, NULL, this_round_count, (uint64_t *)data, 1);
			if (retval != ERROR_OK)
				goto exit;
...
int mips64_cp0_read(struct mips_ejtag *ejtag_info, uint32_t *val, uint32_t cp0_reg, uint32_t cp0_sel)
{
	/**
	...
	uint32_t code[] = {
		...
		/* 3 */ MIPS64_DMFC0(8, 0, 0),				/* move COP0 [cp0_reg select] to $8 */
		MIPS64_SD(8, MIPS64_PRACC_OUT_OFFSET, 15),		/* sd $8,PRACC_OUT_OFFSET($15) */

		MIPS64_LD(8, MIPS64_PRACC_STACK_OFFSET, 15),		/* ld $8,PRACC_STACK_OFFSET($15) */
		MIPS64_B(NEG16(7)),					/* b start */
		MIPS64_DMFC0(15, 31, 0),				/* move COP0 DeSave to $15 */
	};
	 ...
	 *
	 * These are not fix, but can be different for each mips64_cp0_read() function call,
	 * and that is why we must insert them directly into opcode,
	 * i.e. we can not pass it on EJTAG microprogram stack (via param_in),
	 * and put them into the gprs later from MIPS64_PRACC_STACK
	 * because mfc0 do not use gpr as a parameter for the cp0_reg and select part,
	 * but plain (immediate) number.
	 *
	 * MIPS64_MTC0 is implemented via MIPS64_R_INST macro.
	 * In order to insert our parameters, we must change rd and funct fields.
	 */
	code[3] |= (cp0_reg << 11) | cp0_sel;  /* change rd and funct of MIPS64_R_INST macro */

	return mips64_pracc_exec(ejtag_info, ARRAY_SIZE(code),
		code, 0, NULL, 1, (uint64_t *)val, 1);
}
...
int mips64_cp0_write(struct mips_ejtag *ejtag_info, uint32_t val, uint32_t cp0_reg, uint32_t cp0_sel)
{
	uint32_t code[] = {
									/* start: */
		MIPS64_DMTC0(15, 31, 0),				/* move $15 to COP0 DeSave */
		MIPS64_LUI(15, UPPER64_16(val)),			/* Load val to $15 */
		MIPS64_ORI(15, 15, LOWER64_16(val)),

		/* 3 */ MIPS64_DMTC0(15, 0, 0),				/* move $15 to COP0 [cp0_reg select] */

		MIPS64_B(NEG16(5)),					/* b start */
		MIPS64_DMFC0(15, 31, 0),				/* move COP0 DeSave to $15 */
	};

	/**
	 * Note that MIPS64_DMTC0 macro is implemented via MIPS64_R_INST macro.
	 * In order to insert our parameters, we must change rd and funct fields.
	 */
	code[3] |= (cp0_reg << 11) | cp0_sel;  /* change rd and funct fields of MIPS64_R_INST macro */

	return mips64_pracc_exec(ejtag_info, ARRAY_SIZE(code), code, 0, NULL, 0, NULL, 1);
}
...
static int mips64_pracc_write_mem_generic(struct mips_ejtag *ejtag_info, uint64_t addr, int size, int count, void *buf)
{
	...
	while (count) {
		...
		*code_p++ = MIPS64_DMTC0(15, 31, 0);					/* save $15 in DeSave */
		*code_p++ = MIPS64_LUI(15, MIPS64_PRACC_UPPER_BASE_ADDR);		/* $15 = MIPS64_PRACC_BASE_ADDR */
		*code_p++ = MIPS64_SD(8, MIPS64_PRACC_STACK_OFFSET, 15);		/* save $8 to pracc stack */
		*code_p++ = MIPS64_LUI(15, last_upper_base_addr);			/* reuse $15 as memory base address */
		code_len = 4;

		for (i = 0; i != this_round_count; i++) {
			upper_base_addr = UPPER64_16((addr + 0x8000));
			if (last_upper_base_addr != upper_base_addr) {
				*code_p++ = MIPS64_LUI(15, upper_base_addr);		/* if needed, change upper address in $15*/
				code_len++;
				last_upper_base_addr = upper_base_addr;
			}

			if (size == 4) {	/* for double word write check if one half word is 0 and load it accordingly */
				if (LOWER64_16(*buf32) == 0) {
					*code_p++ = MIPS64_LUI(8, UPPER64_16(*buf32));		/* load only upper value */
					code_len++;
				} else if (UPPER64_16(*buf32) == 0) {
					*code_p++ = MIPS64_ORI(8, 0, LOWER64_16(*buf32));	/* load only lower value */
					code_len++;
				} else {
					*code_p++ = MIPS64_LUI(8, UPPER64_16(*buf32));		/* load upper and lower */
					*code_p++ = MIPS64_ORI(8, 8, LOWER64_16(*buf32));
					code_len += 2;
				}
				*code_p++ = MIPS64_SW(8, LOWER64_16(addr), 15);		/* store word to memory */
				code_len++;
				buf32++;

			} else if (size == 2) {
				*code_p++ = MIPS64_ORI(8, 0, *buf16);			/* load lower value */
				*code_p++ = MIPS64_SH(8, LOWER64_16(addr), 15);		/* store half word to memory */
				code_len += 2;
				buf16++;

			} else {
				*code_p++ = MIPS64_ORI(8, 0, *buf8);			/* load lower value */
				*code_p++ = MIPS64_SB(8, LOWER64_16(addr), 15);		/* store byte to memory */
				code_len += 2;
				buf8++;
			}
		...
		*code_p++ = MIPS64_LUI(15, MIPS64_PRACC_UPPER_BASE_ADDR);		/* $15 = MIPS64_PRACC_BASE_ADDR */
		*code_p++ = MIPS64_LW(8, MIPS64_PRACC_STACK_OFFSET, 15);		/* restore $8 from pracc stack */

		code_len += 4;
		*code_p++ = MIPS64_B(NEG16(code_len - 1));				/* jump to start */
		*code_p = MIPS64_MFC0(15, 31, 0);					/* restore $15 from DeSave */

		retval = mips64_pracc_exec(ejtag_info, code_len, code, 0, NULL, 0, NULL, 1);
		if (retval != ERROR_OK)
			goto exit;
...
int mips64_pracc_write_mem(struct mips_ejtag *ejtag_info,
	target_ulong addr, target_ulong size, target_long count, void *buf)
{
...
int mips64_pracc_write_regs(struct mips_ejtag *ejtag_info, uint64_t *regs)
{
	...
	uint32_t *code;
	code = malloc((37 * 2 + 6 + 1) * sizeof(uint64_t));	/* alloc memory for the worst case */
	if (code == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
	...
	for (int i = 2; i < 32; i++) {
		if (LOWER64_16((regs[i])) == 0) {
			*code_p++ = MIPS64_LUI(i, UPPER64_16((regs[i])));	/* if lower half word is 0, lui instruction only */
			code_len++;
		} else if (UPPER64_16((regs[i])) == 0) {
			*code_p++ = MIPS64_ORI(i, 0, LOWER64_16((regs[i])));	/* if upper half word is 0, ori with $0 only*/
			code_len++;
		} else {
			*code_p++ = MIPS64_LUI(i, UPPER64_16((regs[i])));	/* default, load with lui and ori instructions */
			*code_p++ = MIPS64_ORI(i, i, LOWER64_16((regs[i])));
			code_len += 2;
		}
	}

	for (int i = 0; i != 6; i++) {
		*code_p++ = MIPS64_LUI(1, UPPER64_16((regs[i + 32])));		/* load CPO value in $1, with lui and ori */
		*code_p++ = MIPS64_ORI(1, 1, LOWER64_16((regs[i + 32])));
		*code_p++ = cp0_write_code[i];					/* write value from $1 to CPO register */
		code_len += 3;
	}

	*code_p++ = MIPS64_LUI(1, UPPER64_16((regs[1])));			/* load upper half word in $1 */
	code_len += 3;
	*code_p++ = MIPS64_B(NEG16(code_len - 1)),				/* b start */
	*code_p = MIPS64_ORI(1, 1, LOWER64_16((regs[1])));			/* load lower half word in $1 */

	int retval = mips64_pracc_exec(ejtag_info, code_len, code, 0, NULL, 0, NULL, 1);
	free(code);
	return retval;
}
...
int mips64_pracc_read_regs(struct mips_ejtag *ejtag_info, uint64_t *regs)
{
	...
	*code_p++ = MIPS64_DMTC0(1, 31, 0),						/* move $1 to COP0 DeSave */
	*code_p++ = MIPS64_LUI(1, MIPS64_PRACC_UPPER_BASE_ADDR);				/* $1 = MIP64_PRACC_BASE_ADDR */

	for (int i = 2; i != 32; i++)
		*code_p++ = MIPS64_SD(i, MIPS64_PRACC_OUT_OFFSET + (i * 8), 1);		/* store GPR's 2 to 31 */

	for (int i = 0; i != 6; i++) {
		*code_p++ = cp0_read_code[i];						/* load COP0 needed registers to $2 */
		*code_p++ = MIPS64_SD(2, MIPS64_PRACC_OUT_OFFSET + (i + 32) * 8, 1);	/* store COP0 registers
											from $2 to param out */
	}

	*code_p++ = MIPS64_DMFC0(2, 31, 0),						/* move DeSave to $2, reg1 value */
	*code_p++ = MIPS64_SD(2, MIPS64_PRACC_OUT_OFFSET + 8, 1);			/* store reg1 value from $2 to param out */

	*code_p++ = MIPS64_LD(2, MIPS64_PRACC_OUT_OFFSET + 16, 1);	/* restore $2 from param out (singularity) */
	*code_p++ = MIPS64_B(NEG16(48));						/* b start */
	*code_p = MIPS64_DMFC0(1, 31, 0);						/* move COP0 DeSave to $1 */


	int retval = mips64_pracc_exec(ejtag_info, 49, code, 0, NULL, MIPS64NUMCOREREGS, regs, 1);
		...
		int write_t, target_ulong addr, int count, uint32_t *buf)
{
	...
	for (i = 0; i < (int) ARRAY_SIZE(jmp_code); i++) {
		...
		mips64_ejtag_set_instr(ejtag_info, EJTAG_INST_DATA);
	...
	val = addr;
	mips64_ejtag_set_instr(ejtag_info, EJTAG_INST_FASTDATA);
