...
static int mips64_pracc_exec_read(mips64_pracc_context *ctx, uint64_t address)
{
	...
	if ((address >= MIPS64_PRACC_PARAM_IN)
		...
		if (offset >= MIPS64_PRACC_PARAM_IN_SIZE) {
			printf("Error: iparam size exceeds MIPS64_PRACC_PARAM_IN_SIZE");
			return ERROR_JTAG_DEVICE_ERROR;
		}

		if (ctx->local_iparam == NULL) {
			printf("Error: unexpected reading of input parameter\n");
			return ERROR_JTAG_DEVICE_ERROR;
		}
		data = ctx->local_iparam[offset];
		LOG_DEBUG("Reading %llx at %llx\n", (unsigned long long) data, (unsigned long long) address);
		...
		if (ctx->local_oparam == NULL) {
			printf("Error: unexpected reading of output parameter\n");
			return ERROR_JTAG_DEVICE_ERROR;
		}
		...
		if ((address != MIPS64_PRACC_TEXT) &&
			...
			LOG_DEBUG("Warning: probably reading unexpected text address or there was jump due to branch from %llx\n",
				(unsigned long long) prev_text_address);
		}

		offset = (address - MIPS64_PRACC_TEXT) / MIPS64_PRACC_ADDR_STEP;

		data = (uint64_t)ctx->code[offset] << (8 * (4 - (address & 7)));

		prev_text_address = address;
		LOG_DEBUG("Running commands %llx at address %llx\n",
			(unsigned long long) data, (unsigned long long) address);
	} else if ((address & ~7llu) == MIPS64_PRACC_STACK) {
		/* load from our debug stack */
		if (ctx->stack_offset == 0) {
			printf("Error reading from stack: stack is empty\n");
			return ERROR_JTAG_DEVICE_ERROR;
		}
		...
		data = 0;
		printf("Error reading unexpected address %llx\n", (unsigned long long) address);
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
static int mips64_pracc_exec_write(mips64_pracc_context *ctx, uint64_t address)
{
	...
	LOG_DEBUG("Writing %llx at %llx\n", (unsigned long long) data, (unsigned long long) address);

	if ((address >= MIPS64_PRACC_PARAM_IN)
		&& (address <= MIPS64_PRACC_PARAM_IN + ctx->num_iparam * MIPS64_PRACC_DATA_STEP)) {
		offset = (address - MIPS64_PRACC_PARAM_IN) / MIPS64_PRACC_DATA_STEP;
		if (ctx->local_iparam == NULL) {
			printf("Error: unexpected writing of input parameter\n");
			return ERROR_JTAG_DEVICE_ERROR;
		}
		...
		offset = (address - MIPS64_PRACC_PARAM_OUT) / MIPS64_PRACC_DATA_STEP;
		if (ctx->local_oparam == NULL) {
			printf("Error: unexpected writing of output parameter\n");
			return ERROR_JTAG_DEVICE_ERROR;
		}
	...
	int num_param_in, uint64_t *param_in, int num_param_out, uint64_t *param_out, int cycle)
{
	...
	while (1) {
		...
		if (retval != ERROR_OK) {
			printf("ERROR wait_for_pracc_rw\n");
			return retval;
		}
		address = data = 0;
		...
		address |= 0xffffffffff200000ull;
		LOG_DEBUG("-> %016llx", address);

		if (first_time_call && address != MIPS64_PRACC_TEXT) {
			printf("Error reading address %llx (%x expected)\n",
				(unsigned long long) address, MIPS64_PRACC_TEXT);
			return ERROR_JTAG_DEVICE_ERROR;
		}
		...
		if (ejtag_ctrl & EJTAG_CTRL_PRNW) {
			...
			if ((address == MIPS64_PRACC_TEXT) && (pass++)) {
				LOG_DEBUG("@MIPS64_PRACC_TEXT");
				break;
				if (ejtag_info->after_reset)
					ejtag_info->after_reset = 0;
				else
					break;
			}
...
int mips64_pracc_read_regs(struct mips_ejtag *ejtag_info, uint64_t *regs)
{
	...
	uint32_t code[] = {
		...
		MIPS64_SD(2, 36*8, 1),
		MIPS64_DMFC0(2, 24, 0),  /* (37)cpu.cpu_core.cop0.DEPC */
		MIPS64_SD(2, 37*8, 1),
		MIPS64_LD(2, 0, 15),
