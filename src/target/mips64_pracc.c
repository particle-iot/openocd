...
int mips64_pracc_queue_exec(struct mips_ejtag *ejtag_info, struct mips64_pracc_queue_info *ctx, uint64_t *buf)
{
	...
	for (int i = 0; i != 2 * ctx->code_count; i++) {				/* verify every pracc access */
		...
		if (store_addr != 0) {
			...
			if (addr != store_addr) {
				LOG_ERROR("Store address mismatch, read: %lx expected: %lx count: %d",
			...
			if (addr != fetch_addr) {
				LOG_ERROR("Fetch addr mismatch, read: %lx expected: %lx count: %d", addr, fetch_addr, scan_count);
				retval = ERROR_FAIL;
				goto exit;
			}
			fetch_addr += 8;
	...
	uint32_t size, uint32_t count, void *buf)
{
	...
	while (count) {
		...
		mips64_pracc_add(&ctx, 0, MIPS64_LUI(15, MIPS64_PRACC_UPPER_BASE_ADDR));			/* $15 = MIPS32_PRACC_BASE_ADDR */
		mips64_pracc_add(&ctx, 0, MIPS64_LUI(9, last_upper_base_addr));		/* load the upper memory address in $9 */
		...
		for (int i = 0; i != this_round_count; i++) {			/* Main code loop */
			...
			if (size == 8)
				mips64_pracc_add(&ctx, 0, MIPS64_LD(8, LOWER64_16(addr), 9));		/* load from memory to $8 */
			...
			mips64_pracc_add(&ctx, MIPS64_PRACC_PARAM_OUT + i * 8,
					  MIPS64_SD(8, MIPS64_PRACC_OUT_OFFSET + i * 8, 15));	/* store $8 at param out */
			addr += size;
...
int mips64_pracc_write_regs(struct mips_ejtag *ejtag_info, uint64_t *regs)
{
	...
	mips64_pracc_add(&ctx, 0, MIPS64_LUI(1, UPPER64_16((regs[1]))));	/* load upper half word in $1 */
	mips64_pracc_add(&ctx, 0, MIPS64_B(NEG16(ctx.code_count + 1)));		/* jump to start */
	mips64_pracc_add(&ctx, 0, MIPS64_ORI(1, 1, LOWER64_16((regs[1]))));	/* load lower half word in $1 */
