...
static int mips64_pracc_write_mem_generic(struct mips_ejtag *ejtag_info, uint64_t addr, int size, int count, void *buf)
{
	...
	while (count) {
		...
		mips64_pracc_add(&ctx, 0, MIPS64_LUI(8, UPPER64_16(ejtag_info->reg8l)));		/* restore upper 16 bits of reg 8 */
		mips64_pracc_add(&ctx, 0, MIPS64_ORI(8, 8, LOWER64_16(ejtag_info->reg8l)));	/* restore lower 16 bits of reg 8 */
