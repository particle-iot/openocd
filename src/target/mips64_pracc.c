	...
	uint32_t size, uint32_t count, void *buf)
{
	...
	if (size != 8) {
		data = malloc(256 * sizeof(uint64_t));
		if (data == NULL) {
			LOG_ERROR("Out of memory");
			goto exit;
		}
	...
	while (count) {
		...
		for (int i = 0; i != this_round_count; i++) {		/* Main code loop */
			...
			mips64_pracc_add(&ctx, 0, MIPS64_LUI(8, UPPER64_16(ejtag_info->reg8l)));
			/* restore lower 16 bits of reg 8 */
			mips64_pracc_add(&ctx, 0, MIPS64_ORI(8, 8, LOWER64_16(ejtag_info->reg8l)));
			/* restore upper 16 bits of reg 9 */
			mips64_pracc_add(&ctx, 0, MIPS64_LUI(9, UPPER64_16(ejtag_info->reg9l)));
			/* restore lower 16 bits of reg 9 */
			mips64_pracc_add(&ctx, 0, MIPS64_ORI(9, 9, LOWER64_16(ejtag_info->reg9l)));
			...
			if (size == 8) {
				ctx.retval = mips64_pracc_queue_exec(ejtag_info, &ctx, buf64);
				if (ctx.retval != ERROR_OK)
					goto exit;
				buf64 += this_round_count;
			} else {
...
int mips64_cp0_read(struct mips_ejtag *ejtag_info,
		uint64_t *val, uint32_t cp0_reg, uint32_t cp0_sel)
{
	...
	ctx.retval = mips64_pracc_queue_exec(ejtag_info, &ctx, val);
exit:
	mips64_pracc_queue_free(&ctx);
	return ctx.retval;
...
int mips64_cp0_write(struct mips_ejtag *ejtag_info, uint64_t val, uint32_t cp0_reg, uint32_t cp0_sel)
{
...
static int mips64_pracc_write_mem_generic(struct mips_ejtag *ejtag_info, uint64_t addr, int size, int count, void *buf)
{
	...
	while (count) {
		...
		mips64_pracc_add(&ctx, 0,
				 MIPS64_LUI(8, UPPER64_16(ejtag_info->reg8l)));		/* restore upper 16 bits of reg 8 */
		mips64_pracc_add(&ctx, 0,
				 MIPS64_ORI(8, 8, LOWER64_16(ejtag_info->reg8l)));	/* restore lower 16 bits of reg 8 */
...
int mips64_pracc_write_mem(struct mips_ejtag *ejtag_info,
	uint64_t addr, uint32_t size, uint32_t count, void *buf)
{
...
int mips64_pracc_write_regs(struct mips_ejtag *ejtag_info, uint64_t *regs)
{
	...
	ejtag_info->reg8l = regs[8];
	ejtag_info->reg9l = regs[9];
exit:
	mips64_pracc_queue_free(&ctx);
	return ctx.retval;
}
...
int mips64_pracc_read_regs(struct mips_ejtag *ejtag_info, uint64_t *regs)
{
	...
	ejtag_info->reg8l = regs[8];	/* reg8l is saved but not restored, next called function should restore it */
	ejtag_info->reg9l = regs[9];
exit:
	mips64_pracc_queue_free(&ctx);
	return ctx.retval;
...
int mips64_pracc_fastdata_xfer(struct mips_ejtag *ejtag_info, struct working_area *source,
		int write_t, uint64_t addr, int count, uint32_t *buf)
{
	...
	LOG_DEBUG("%s using 0x%.16" PRIx32 " for write handler", __func__, source->address);
