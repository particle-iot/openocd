...
int nds32_probe_tlb(struct nds32 *nds32, const uint32_t virtual_address, uint32_t *physical_address)
{
...
struct page_table_walker_info_s page_table_info[PAGE_SIZE_NUM] = {
	{0xFFC00000, 20, 0x003FF000, 10, 0x00000FFF, 0xFFFFF000, 0xFFFFF000, 0xFFFFF000},  /* 4K page */
	{0xFF000000, 22, 0x00FFE000, 11, 0x00001FFF, 0xFFFFF000, 0xFFFFE000, 0xFFFFE000},  /* 8K page */
};

int nds32_walk_page_table(struct nds32 *nds32, const uint32_t virtual_address, uint32_t *physical_address)
{
	struct target *target = nds32->target;
	uint32_t value_mr1;
	...
	struct page_table_walker_info_s *page_table_info_p = &(page_table_info[page_size_index]);

	/* Read L1 Physical Page Table */
	nds32_get_mapped_reg(nds32, MR1, &value_mr1);
	load_address = (value_mr1 & page_table_info_p->L1_base_mask) |
		((virtual_address & page_table_info_p->L1_offset_mask) >> page_table_info_p->L1_offset_shift);
	...
	load_address = (L1_page_table_entry & page_table_info_p->L2_base_mask) |
		((virtual_address & page_table_info_p->L2_offset_mask) >> page_table_info_p->L2_offset_shift);
	/* load_address is physical address */
