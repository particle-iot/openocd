 ...
 *                                                                         *
 *   revised:  2/14/17 by brent@mbari.org [non-blocking DCC support]       *
 *                                                                         *
...
/*
  max size of a DCC request
*/
#define MaxDCCreqSize  ((1<<15)-1)
...
static inline unsigned dccQlen(struct dccQ *q)
{
	unsigned len = q->tail - q->head;
	if (len >= q->size)
		len -= q->size;
	return len;
}
...
static uint32_t *dccQrealloc(struct dccQ *q, unsigned newSize)
/*
...
static int hl_dcc_read(struct hl_interface_s *hl_if, uint8_t *value)
{
	...
	int retval = hl_if->layout->api->read_mem(hl_if->handle,
			DCB_DCRDR, sizeof(dcrdr), 1, (uint8_t *)&dcrdr);
	if (retval == ERROR_OK) {
		if (!(dcrdr & 1))
			return NOT_READY;
		*value = (uint8_t)(dcrdr >> 8);

		/* clear the data ready bit in the dcc control register */
		static const uint32_t zero;
		hl_if->layout->api->write_mem(hl_if->handle,
			DCB_DCRDR, sizeof(zero), 1, (uint8_t *)&zero);
	...
	uint32_t size, uint8_t *buffer)
{
	...
	int retval = ERROR_OK;
	unsigned len = dccQlen(&dcc->q);

	if (size > len) {
		unsigned overage = size - len;
...
LOG_DEBUG("DCC request 0x%x; expecting %d more words", dcc->request, reqLen);
						...
						dccQput(&dcc->q, word);
						if (dcc->q.tail >= (unsigned)dcc->requestLen) {
processRequest:				target_request(target, dcc->request);  /*got all*/
							if (!dccQempty(&dcc->q))
								LOG_WARNING(
...
void adapter_deinit_target(struct target *target)
{
	struct cortex_m_common *cortex_m = target_to_cm(target);

	free(&cortex_m->armv7m.private);
	cortex_m_deinit_target(target);
}
...
struct target_type hla_target = {
	...
	.deinit_target = adapter_deinit_target,
