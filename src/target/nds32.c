		...
		struct nds32 *nds32)
{
	...
	for (i = 0; i < TOTAL_REG_NUM; i++) {
		...
		if (FD0 <= reg_arch_info[i].num && reg_arch_info[i].num <= FD31) {
			...
			reg_list[i].group = REG_GROUP_FLOAT;
		} else {
...
/** get core register list */
static int nds32_get_core_reg_list(struct nds32 *nds32,
		struct reg **reg_list[], int *reg_list_size)
{
	struct reg *reg_current;
	int i;
	...
	/** freed in gdb_server.c */
	*reg_list = malloc(sizeof(struct reg *) * (SECUR0 - CR0 + 1));
	current_idx = 0;

	for (i = CR0; i < SECUR0 + 1; i++) {
		reg_current = nds32_reg_current(nds32, i);
		if (((struct nds32_reg *)reg_current->arch_info)->enable) {
			(*reg_list)[current_idx] = reg_current;
			current_idx++;
...
/** get audio register list */
static int nds32_get_audio_reg_list(struct nds32 *nds32,
		struct reg **reg_list[], int *reg_list_size)
{
	struct reg *reg_current;
	int i;
	...
	/** freed in gdb_server.c */
	*reg_list = malloc(sizeof(struct reg *) * (CBE3 - D0L24 + 1));
	current_idx = 0;

	for (i = D0L24; i < CBE3 + 1; i++) {
		reg_current = nds32_reg_current(nds32, i);
		if (((struct nds32_reg *)reg_current->arch_info)->enable) {
			(*reg_list)[current_idx] = reg_current;
			current_idx++;
...
/** get fpu register list */
static int nds32_get_fpu_reg_list(struct nds32 *nds32,
		struct reg **reg_list[], int *reg_list_size)
{
	struct reg *reg_current;
	int i;
	...
	/** freed in gdb_server.c */
	*reg_list = malloc(sizeof(struct reg *) * (FD31 - FPCSR + 1));
	current_idx = 0;

	for (i = FPCSR; i < FD31 + 1; i++) {
		reg_current = nds32_reg_current(nds32, i);
		if (((struct nds32_reg *)reg_current->arch_info)->enable) {
			(*reg_list)[current_idx] = reg_current;
			current_idx++;
		...
		enum target_register_class reg_class)
{
	...
	switch (reg_class) {
		...
		case REG_CLASS_CORE:
			return nds32_get_core_reg_list(nds32, reg_list, reg_list_size);
		case REG_CLASS_FPU:
			return nds32_get_fpu_reg_list(nds32, reg_list, reg_list_size);
		case REG_CLASS_ARCH_EXTENSION_1: /* AUDIO */
			return nds32_get_audio_reg_list(nds32, reg_list, reg_list_size);
		case REG_CLASS_ARCH_EXTENSION_2:
		case REG_CLASS_ARCH_EXTENSION_3:
		case REG_CLASS_ARCH_EXTENSION_4:
		case REG_CLASS_ARCH_EXTENSION_5:
		case REG_CLASS_ARCH_EXTENSION_6:
			*reg_list_size = 0;
			return ERROR_OK;
		default:
			return ERROR_FAIL;
