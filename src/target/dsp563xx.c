...
{
  int err = ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
	return err;
}
...
{
  int err = ERROR_OK;
  err = dsp563xx_add_custom_watchpoint(target, breakpoint->address, MEM_P, WPT_READ, EQUAL);
...
{
  int err = ERROR_OK;
  err = dsp563xx_remove_custom_watchpoint(target);
