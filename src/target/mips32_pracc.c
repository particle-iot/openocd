...
int mips32_pracc_exec_queue(struct mips_ejtag *ejtag_info, struct pracc_access *pqueue, int num_access)
{
	...
	for (int i = 0; i != num_access; i++) {			/* queue every scan */
	...
	for (int i = 0; i != num_access; i++) {				/* verify every pracc access */
		...
		uint32_t addr = buf_get_u32(pqueue[i].in_scan + 8, 0, 32);
		if (pqueue[i].access_type == PRACC_FETCH) {		/* fetch access */
			...
			*pqueue[i].data = buf_get_u32(pqueue[i].in_scan + 4, 0, 32);
		}
	}
	return ERROR_OK;
}
...
{
	int code_len = 9;
	struct pracc_access *pqueue = malloc(sizeof(struct pracc_access) * (code_len + 1));	/* one store processor access */
	if (pqueue == NULL) {
	...
	int retval = mips32_pracc_exec_queue(ejtag_info, pqueue, code_len + 1);
	free(pqueue);
	return retval;
}
...
int mips32_pracc_read_mem(struct mips_ejtag *ejtag_info, uint32_t addr, int size, int count, void *buf)
{
	...
	struct pracc_access *pqueue = malloc((256 * 3 + 10) * sizeof(struct pracc_access));
	if (pqueue == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
	...
	while (count) {
		...
		uint32_t *data_p = (size != 4) ? data : buf32;

		struct pracc_access *p = pqueue;
		pracc_queue_fetch(p++, MIPS32_MTC0(15, 31, 0));					/* save $15 in DeSave */
		pracc_queue_fetch(p++, MIPS32_LUI(15, PRACC_UPPER_BASE_ADDR));			/* $15 = MIPS32_PRACC_BASE_ADDR */
		pracc_queue_fetch(p++, MIPS32_LUI(9, last_upper_base_addr));		/* load the upper memory address in $9 */
		int code_len = 3;
		...
		for (int i = 0; i != this_round_count; i++) {		/* Main code loop */
			...
			if (last_upper_base_addr != upper_base_addr) {			/* if needed, change upper address in $9 */
				pracc_queue_fetch(p++, MIPS32_LUI(9, upper_base_addr));
				code_len++;
				changed_upper = 1;
				last_upper_base_addr = upper_base_addr;
			}

			if (i > 0 && changed_upper)			/* store access from previous iteration */
				pracc_queue_store(p++, data_p++, MIPS32_PRACC_PARAM_OUT + (i - 1) * 4);

			if (size == 4)
				pracc_queue_fetch(p++, MIPS32_LW(8, LOWER16(addr), 9));		/* load from memory to $8 */
			else if (size == 2)
				pracc_queue_fetch(p++, MIPS32_LHU(8, LOWER16(addr), 9));
			else
				pracc_queue_fetch(p++, MIPS32_LBU(8, LOWER16(addr), 9));

			if (i > 0 && !changed_upper)			/* store access from previous iteration */
				pracc_queue_store(p++, data_p++, MIPS32_PRACC_PARAM_OUT + (i - 1) * 4);

			pracc_queue_fetch(p++, MIPS32_SW(8, PRACC_OUT_OFFSET + i * 4, 15));		/* store $8 at param out */

			code_len += 2;
			addr += size;
		}
		pracc_queue_fetch(p++, MIPS32_LUI(8, UPPER16(ejtag_info->reg8)));	/* restore upper 16 bits of reg 8 */

		/* store from last iteration */
		pracc_queue_store(p++, data_p, MIPS32_PRACC_PARAM_OUT + ((this_round_count - 1) * 4));

		pracc_queue_fetch(p++, MIPS32_ORI(8, 8, LOWER16(ejtag_info->reg8)));	/* restore lower 16 bits of reg 8 */
		pracc_queue_fetch(p++, MIPS32_LUI(9, UPPER16(ejtag_info->reg8)));	/* restore upper 16 bits of reg 9 */
		pracc_queue_fetch(p++, MIPS32_ORI(9, 9, LOWER16(ejtag_info->reg8)));	/* restore lower 16 bits of reg 9 */

		code_len += 6;
		pracc_queue_fetch(p++, MIPS32_B(NEG16(code_len - 1)));					/* jump to start */
		pracc_queue_fetch(p, MIPS32_MFC0(15, 31, 0));					/* restore $15 from DeSave */

		retval = mips32_pracc_exec_queue(ejtag_info, pqueue, code_len + this_round_count);
		if (retval != ERROR_OK)
			goto exit;
...
int mips32_cp0_read(struct mips_ejtag *ejtag_info, uint32_t *val, uint32_t cp0_reg, uint32_t cp0_sel)
{
	...
	pracc_queue_fetch(p++, MIPS32_MTC0(15, 31, 0));					/* move $15 to COP0 DeSave */
	pracc_queue_fetch(p++, MIPS32_LUI(15, PRACC_UPPER_BASE_ADDR));			/* $15 = MIPS32_PRACC_BASE_ADDR */
	pracc_queue_fetch(p++, MIPS32_MFC0(8, 0, 0) | (cp0_reg << 11) | cp0_sel);	/* move COP0 [cp0_reg select] to $8 */
	pracc_queue_fetch(p++, MIPS32_SW(8, PRACC_OUT_OFFSET, 15));			/* store $8 to pracc_out */
	pracc_queue_fetch(p++, MIPS32_MFC0(15, 31, 0));					/* move COP0 DeSave to $15 */

	pracc_queue_store(p++, val, MIPS32_PRACC_PARAM_OUT);			/* store access from $8 */

	pracc_queue_fetch(p++, MIPS32_LUI(8, UPPER16(ejtag_info->reg8)));		/* restore upper 16 bits  of $8 */
	pracc_queue_fetch(p++, MIPS32_B(NEG16(code_len - 1)));					/* jump to start */
	pracc_queue_fetch(p, MIPS32_ORI(8, 8, LOWER16(ejtag_info->reg8)));		/* restore lower 16 bits of $8 */
...
int mips32_cp0_write(struct mips_ejtag *ejtag_info, uint32_t val, uint32_t cp0_reg, uint32_t cp0_sel)
{
	...
	pqueue = malloc(sizeof(struct pracc_access) * 6);
	if (pqueue == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
	...
	pracc_queue_fetch(p++, MIPS32_MTC0(15, 31, 0));					/* move $15 to COP0 DeSave */
	pracc_queue_fetch(p++, MIPS32_LUI(15, UPPER16(val)));				/* Load val to $15 */
	pracc_queue_fetch(p++, MIPS32_ORI(15, 15, LOWER16(val)));

	pracc_queue_fetch(p++, MIPS32_MTC0(15, 0, 0) | (cp0_reg << 11) | cp0_sel);	/* write cp0 reg / sel */

	pracc_queue_fetch(p++, MIPS32_B(NEG16(5)));					/* jump to start */
	pracc_queue_fetch(p, MIPS32_MFC0(15, 31, 0));					/* move COP0 DeSave to $15 */

	int retval = mips32_pracc_exec_queue(ejtag_info, pqueue, 6);
	free(pqueue);
	return retval;
...
static int mips32_pracc_write_mem_generic(struct mips_ejtag *ejtag_info, uint32_t addr, int size, int count, void *buf)
{
	...
	pqueue = malloc((128 * 3 + 6) * sizeof(struct pracc_access));	/* alloc memory for the worst case */
	if (pqueue == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
	...
	while (count) {
		...
		pracc_queue_fetch(p++, MIPS32_MTC0(15, 31, 0));				/* save $15 in DeSave */
		pracc_queue_fetch(p++, MIPS32_LUI(15, last_upper_base_addr));		/* load $15 with memory base address */
		int code_len = 2;
		int i;
		for (i = 0; i != this_round_count; i++) {
			upper_base_addr = UPPER16((addr + 0x8000));
			if (last_upper_base_addr != upper_base_addr) {
				pracc_queue_fetch(p++, MIPS32_LUI(15, upper_base_addr));/* if needed, change upper address in $15*/
				code_len++;
				last_upper_base_addr = upper_base_addr;
			}
			...
			if (size == 4) {	/* for word write check if one half word is 0 and load it accordingly */
				if (LOWER16(*buf32) == 0) {
					pracc_queue_fetch(p++, MIPS32_LUI(8, UPPER16(*buf32)));		/* load only upper value */
					code_len++;
				} else if (UPPER16(*buf32) == 0) {
					pracc_queue_fetch(p++, MIPS32_ORI(8, 0, LOWER16(*buf32)));	/* load only lower value */
					code_len++;
				} else {
					pracc_queue_fetch(p++, MIPS32_LUI(8, UPPER16(*buf32)));		/* load upper and lower */
					pracc_queue_fetch(p++, MIPS32_ORI(8, 8, LOWER16(*buf32)));
					code_len += 2;
				}
				pracc_queue_fetch(p++, MIPS32_SW(8, LOWER16(addr), 15));		/* store word to memory */
				code_len++;
				buf32++;

			} else if (size == 2) {
				pracc_queue_fetch(p++, MIPS32_ORI(8, 0, *buf16));		/* load lower value */
				pracc_queue_fetch(p++, MIPS32_SH(8, LOWER16(addr), 15));	/* store half word to memory */
				code_len += 2;
				buf16++;

			} else {
				pracc_queue_fetch(p++, MIPS32_ORI(8, 0, *buf8));		/* load lower value */
				pracc_queue_fetch(p++, MIPS32_SB(8, LOWER16(addr), 15));	/* store byte to memory */
				code_len += 2;
				buf8++;
			}
			addr += size;
		...
		retval = mips32_pracc_exec_queue(ejtag_info, pqueue, code_len);
		if (retval != ERROR_OK)
			goto exit;
...
int mips32_pracc_write_regs(struct mips_ejtag *ejtag_info, uint32_t *regs)
{
	...
	struct pracc_access *pqueue;
	pqueue = malloc((37 * 2 + 6 + 1) * sizeof(struct pracc_access));	/* alloc memory for the worst case */
	if (pqueue == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
	...
	for (int i = 2; i < 32; i++) {
		if (LOWER16((regs[i])) == 0) {					/* if lower half word is 0, lui instruction only */
			pracc_queue_fetch(p++, MIPS32_LUI(i, UPPER16((regs[i]))));
			code_len++;
		} else if (UPPER16((regs[i])) == 0) {				/* if upper half word is 0, ori with $0 only*/
			pracc_queue_fetch(p++, MIPS32_ORI(i, 0, LOWER16((regs[i]))));
			code_len++;
		} else {							/* default, load with lui and ori instructions */
			pracc_queue_fetch(p++, MIPS32_LUI(i, UPPER16((regs[i]))));
			pracc_queue_fetch(p++, MIPS32_ORI(i, i, LOWER16((regs[i]))));
			code_len += 2;
		}
	}

	for (int i = 0; i != 6; i++) {
		pracc_queue_fetch(p++, MIPS32_LUI(1, UPPER16((regs[i + 32]))));		/* load CPO value in $1, with lui and ori */
		pracc_queue_fetch(p++, MIPS32_ORI(1, 1, LOWER16((regs[i + 32]))));
		pracc_queue_fetch(p++, cp0_write_code[i]);				/* write value from $1 to CPO register */
		code_len += 3;
	}

	pracc_queue_fetch(p++, MIPS32_LUI(1, UPPER16((regs[1]))));		/* load upper half word in $1 */
	code_len += 3;
	pracc_queue_fetch(p++, MIPS32_B(NEG16(code_len - 1)));					/* b start */
	pracc_queue_fetch(p, MIPS32_ORI(1, 1, LOWER16((regs[1]))));		/* load lower half word in $1 */
...
int mips32_pracc_read_regs(struct mips_ejtag *ejtag_info, uint32_t *regs)
{
	...
	int code_len = 48;
	/* 37 store processor accesses, 31 gpr + 6 cp0 */
	struct pracc_access *pqueue = malloc((code_len + 37) * sizeof(struct pracc_access));
	if (pqueue == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
	...
	pracc_queue_fetch(p++, MIPS32_LUI(1, PRACC_UPPER_BASE_ADDR));				/* $1 = MIP32_PRACC_BASE_ADDR */

	for (int i = 2; i != 32; i++) {
		pracc_queue_fetch(p++, MIPS32_SW(i, PRACC_OUT_OFFSET + (i * 4), 1));		/* store GPR's 2 to 31 */
		if (i > 2)								/* store from previous iteration */
			pracc_queue_store(p++, &regs[i - 1], MIPS32_PRACC_PARAM_OUT + (i - 1) * 4);
	}
	/* load COP0 needed registers to $2  (first) */
	pracc_queue_fetch(p++, cp0_read_code[0]);

	pracc_queue_store(p++, &regs[31], MIPS32_PRACC_PARAM_OUT + 31 * 4);	/* last store from previous loop */

	/* store COP0 registers from $2 to param out (first) */
	pracc_queue_fetch(p++, MIPS32_SW(2, PRACC_OUT_OFFSET + 32 * 4, 1));

	for (int i = 1; i != 6; i++) {
		pracc_queue_fetch(p++, cp0_read_code[i]);			/* load COP0 needed registers to $2 */

		/* store access from $2, COP0 values. Note first store instruction is outside this loop */
		pracc_queue_store(p++, &regs[i - 1 + 32], MIPS32_PRACC_PARAM_OUT + (i - 1 + 32) * 4);

		/* store COP0 registers from $2 to param out */
		pracc_queue_fetch(p++, MIPS32_SW(2, PRACC_OUT_OFFSET + (i + 32) * 4, 1));
	}
	pracc_queue_fetch(p++, MIPS32_MFC0(2, 31, 0));					/* move DeSave to $2, reg1 value */

	pracc_queue_store(p++, &regs[37], MIPS32_PRACC_PARAM_OUT + 37 * 4);	/* last store from previous loop */

	pracc_queue_fetch(p++, MIPS32_SW(2, PRACC_OUT_OFFSET + 4, 1));			/* store reg1 value from $2 to param out */
	pracc_queue_fetch(p++, MIPS32_B(NEG16(code_len - 1)));					/* jump to start */

	pracc_queue_store(p++, &regs[1], MIPS32_PRACC_PARAM_OUT + 4);			/* store acces for reg1 */

	pracc_queue_fetch(p, MIPS32_MFC0(1, 31, 0));					/* move COP0 DeSave to $1, restore reg1 */

	int retval = mips32_pracc_exec_queue(ejtag_info, pqueue, code_len + 37);
	if (retval != ERROR_OK)
		goto exit;

	code_len = 3;
	p = pqueue;
	pracc_queue_fetch(p++, MIPS32_LUI(2, UPPER16(regs[2])));		/* Restore register 2 */
	pracc_queue_fetch(p++, MIPS32_B(NEG16(code_len - 1)));
	pracc_queue_fetch(p, MIPS32_ORI(2, 2, LOWER16(regs[2])));
	retval = mips32_pracc_exec_queue(ejtag_info, pqueue, code_len);
