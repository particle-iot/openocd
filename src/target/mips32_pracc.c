...
int mips32_pracc_read_regs(struct mips_ejtag *ejtag_info, uint32_t *regs)
{
	...
	/**
	 * The branch target buffer in the JZ4730 also remembers branches executed
	 * in dmseg. It will repeat unconditional branches previously done at an
	 * address regardless of the actual instruction transmitted. We therefore
	 * must disable it immediately when entering debug mode.
	 */
	if (ejtag_info->has_xburst_btb) {
		pracc_add(&ctx, 0, MIPS32_MFC0(ctx.isa, 8, 16, 7));		/* move config7 to $8 */
