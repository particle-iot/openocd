...
int mips32_pracc_read_regs(struct mips_ejtag *ejtag_info, uint32_t *regs)
{
	...
	pracc_queue_fetch(p++, cp0_read_code[0]);				/* load COP0 needed registers to $2 (first) */

	pracc_queue_store(p++, &regs[31], MIPS32_PRACC_PARAM_OUT + 31 * 4);	/* last store from previous loop */

	pracc_queue_fetch(p++, MIPS32_SW(2, PRACC_OUT_OFFSET + 32 * 4, 1)); /* store COP0 registers from $2 to param out (first) */
