...
int mips32_pracc_read_mem(struct mips_ejtag *ejtag_info, uint32_t addr, int size, int count, void *buf)
{
	...
	while (count) {
		...
		*code_p++ = MIPS32_LUI(9, UPPER16(ejtag_info->reg9));			/* restore upper 16 bits of reg 9 */
		*code_p++ = MIPS32_ORI(9, 9, LOWER16(ejtag_info->reg9));		/* restore lower 16 bits of reg 9 */
