...
{
	int code_len = 6;
	struct pracc_access *pqueue = malloc(sizeof(struct pracc_access) * code_len);
	if (pqueue == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
...
static int mips32_pracc_write_mem_generic(struct mips_ejtag *ejtag_info, uint32_t addr, int size, int count, void *buf)
{
	...
	struct pracc_access *pqueue = malloc((128 * 3 + 6) * sizeof(struct pracc_access));
	if (pqueue == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
...
int mips32_pracc_write_regs(struct mips_ejtag *ejtag_info, uint32_t *regs)
{
	...
	/* alloc memory for the worst case */
	struct pracc_access *pqueue = malloc((37 * 2 + 6 + 1) * sizeof(struct pracc_access));
	if (pqueue == NULL) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
