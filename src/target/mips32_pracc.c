...
int mips32_pracc_read_mem(struct mips_ejtag *ejtag_info, uint32_t addr, int size, int count, void *buf)
{
	...
	while (count) {
		...
		*code_p++ = MIPS32_LW(8, PRACC_STACK_OFFSET, 15);
		...
		code_len += 3;
		*code_p++ = MIPS32_B(NEG16(code_len));					/* jump to start */
		*code_p = MIPS32_MFC0(15, 31, 0);					/* restore $15 from DeSave */

		retval = mips32_pracc_exec(ejtag_info, code_len + 1, code, 0, NULL, this_round_count, data, 1);
		if (retval != ERROR_OK)
			goto exit;
...
static int mips32_pracc_write_mem_generic(struct mips_ejtag *ejtag_info, uint32_t addr, int size, int count, void *buf)
{
	...
	while (count) {
		...
		*code_p = MIPS32_MFC0(15, 31, 0);					/* restore $15 from DeSave */
		...
		retval = mips32_pracc_exec(ejtag_info, code_len + 1, code, 0, NULL, 0, NULL, 1);
		if (retval != ERROR_OK)
			goto exit;
...
int mips32_pracc_write_regs(struct mips_ejtag *ejtag_info, uint32_t *regs)
{
	...
	*code_p++ = MIPS32_LUI(1, UPPER16((regs[1])));				/* load upper half word in $1 */
	code_len += 2;
	*code_p++ = MIPS32_B(NEG16(code_len)),							/* b start */
	*code_p = MIPS32_ORI(1, 1, LOWER16((regs[1])));				/* load lower half word in $1 */

	int retval = mips32_pracc_exec(ejtag_info, code_len + 1, code, 0, NULL, 0, NULL, 1);
	free(code);
	return retval;
}
