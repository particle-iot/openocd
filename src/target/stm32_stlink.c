...
static struct stm32_stlink_common *stm32_stlink = NULL;
...
static int stm32_stlink_assert_reset(struct target *target)
{
	...
	if ((res = stlink_if->layout->api->reset(stlink_if->fd)) != ERROR_OK)
		return res;
...
static int stm32_stlink_deassert_reset(struct target *target)
{
	...
	if (!target->reset_halt) {
		if ((res = target_resume(target, 1, 0, 0, 0)) != ERROR_OK)
			return res;
	}
...
static int stm32_stlink_halt(struct target *target)
{
	...
	if ((res = stlink_if->layout->api->halt(stlink_if->fd)) != ERROR_OK)
		return res;
			       ...
			       int debug_execution)
{
	...
	LOG_DEBUG("%s %d %x %d %d", __FUNCTION__, current, address,
	...
	if ((res = stlink_if->layout->api->run(stlink_if->fd)) != ERROR_OK)
		return res;
			     ...
			     uint32_t address, int handle_breakpoints)
{
	...
	if ((res = stlink_if->layout->api->step(stlink_if->fd)) != ERROR_OK)
		return res;
				    ...
				    uint8_t * buffer)
{
	...
	if (!count || !buffer)
		return ERROR_INVALID_ARGUMENTS;
	if (size != 4) {
		LOG_DEBUG("%s %x %d %d", __FUNCTION__, address, size, count);
		return ERROR_INVALID_ARGUMENTS;
	}
