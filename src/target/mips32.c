		...
		uint32_t count, uint32_t *checksum)
{
	...
	/* convert crc code into a buffer in target endianness */
	target_buffer_set_u32_array(target, (uint8_t *)mips_crc_code, ARRAY_SIZE(mips_crc_code), mips_crc_code);

	target_write_buffer(target, crc_algorithm->address, sizeof(mips_crc_code), (uint8_t *)mips_crc_code);
		...
		uint32_t address, uint32_t count, uint32_t *blank)
{
	...
	target_buffer_set_u32_array(target, (uint8_t *)erase_check_code, ARRAY_SIZE(erase_check_code), erase_check_code);

	target_write_buffer(target, erase_check_algorithm->address, sizeof(erase_check_code), (uint8_t *)erase_check_code);
