...
static const struct {
	...
	{ 38, "pc", REG_TYPE_INT, NULL, "org.gnu.gdb.mips.cpu", 0 },
...
{
	int retval;
	mips64_core_reg_t *mips64_reg = reg->arch_info;
	struct target *target = mips64_reg->target;
	mips64_common_t *mips64_target = target->arch_info;
...
struct reg_cache *mips64_build_reg_cache(struct target *target)
{
	...
	mips64_core_reg_t *arch_info = malloc(sizeof(mips64_core_reg_t) * num_regs);
...
int mips64_init_arch_info(struct target *target, mips64_common_t *mips64, struct jtag_tap *tap)
{
...
int mips64_run_algorithm(struct target *target, int num_mem_params,
	...
	target_ulong entry_point, target_ulong exit_point, int timeout_ms, void *arch_info)
{
	/*FIXME*/
	return ERROR_OK;
}
...
int mips64_configure_break_unit(struct target *target)
{
	...
	if (dcr & (1 << 16)) {
		...
		mips64->inst_break_list = calloc(mips64->num_inst_bpoints, sizeof(mips64_comparator_t));
	...
	if (dcr & (1 << 17)) {
		...
		mips64->data_break_list = calloc(mips64->num_data_bpoints, sizeof(mips64_comparator_t));
