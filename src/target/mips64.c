...
int mips64_get_gdb_reg_list(struct target *target, struct reg **reg_list[], int *reg_list_size)
{
	/* get pointers to arch-specific information */
	struct mips64_common *mips64 = target_to_mips64(target);
	int i;
...
int mips64_arch_state(struct target *target)
{
	...
	LOG_USER("target halted in %s mode due to %s, pc: 0x%8.8" PRIx64 "",
...
static int mips64_run_and_wait(struct target *target, uint32_t entry_point,
		int timeout_ms, uint32_t exit_point, struct mips64_common *mips64)
{
	uint64_t pc;
	int retval;
...
int mips64_configure_break_unit(struct target *target)
{
	...
	if (dcr & EJTAG_DCR_IB) {
		...
		for (i = 0; i < mips64->num_inst_bpoints; i++)
			mips64->inst_break_list[i].reg_address = EJTAG_IBA1 + (0x100 * i);
	...
	if (dcr & EJTAG_DCR_DB) {
		...
		for (i = 0; i < mips64->num_data_bpoints; i++)
			mips64->data_break_list[i].reg_address = EJTAG_DBA1 + (0x100 * i);
		...
		target_ulong count, uint32_t *checksum)
{
	...
	*checksum = buf_get_u32(reg_params[0].value, 0, 64);
