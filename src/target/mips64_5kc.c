...
static int mips_5kc_examine_debug_reason(struct target *target)
{
	...
	if ((target->debug_reason != DBG_REASON_DBGRQ)
		...
		if (break_status & 0x1f) {
			...
			retval = target_write_u64(target, EJTAG_IBS, 0);
			if (retval != ERROR_OK)
				return retval;
		...
		if (break_status & 0x1f) {
			/* we have halted on a  breakpoint */
			retval = target_write_u64(target, EJTAG_DBS, 0);
			if (retval != ERROR_OK)
				return retval;
...
static int mips_5kc_assert_reset(struct target *target)
{
	...
	if (target->reset_halt) {
		...
		mips64_ejtag_set_instr(ejtag_info, EJTAG_INST_EJTAGBOOT);
	} else
		mips64_ejtag_set_instr(ejtag_info, EJTAG_INST_NORMALBOOT);
		...
		uint64_t address, int handle_breakpoints, int debug_execution)
{
	...
	if (handle_breakpoints) {
		...
		if (breakpoint) {
			LOG_DEBUG("unset breakpoint at 0x%" PRIX "", breakpoint->address);
		...
		target_ulong size, target_ulong count, uint8_t *buffer)
{
	...
	LOG_DEBUG("address: 0x%" PRIX ", size: 0x%" PRIX ", count: 0x%" PRIX "",
			address, size, count);
		...
		target_ulong count, const uint8_t *buffer)
{
	...
	LOG_DEBUG("address: 0x%" PRIX ", count: 0x%" PRIX "", address, count);
...
struct target_type mips64_5kc_target = {
	...
	.bulk_write_memory = mips_5kc_bulk_write_memory,
