...
static void print_opcode(struct command_context *context, const cs_insn *insn)
{
	...
	if (insn->size == 4) {
		...
		command_print(context, "0x%08" PRIx64"  %04x %04x\t%s\t%s",
			insn->address, opcode, opcode_high, insn->mnemonic, insn->op_str);
	} else {
		command_print(context, "0x%08" PRIx64"  %04x\t%s\t%s",
			insn->address, opcode, insn->mnemonic, insn->op_str);
	}
}

int arm_disassemble(struct command_context *context, struct target *target,
