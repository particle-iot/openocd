...
**************************************************************************/
static int aarch64_read_regs_through_mem(struct target *target, uint32_t address,
	uint64_t *regfile)
{
	/* TODO */
	return ERROR_OK;
...
{
	int i;
	uint64_t regfile[ARMV8_LAST_REG], cpsr;
	uint32_t dscr;
	int retval = ERROR_OK;
	...
	uint32_t count, uint8_t *buffer)
{
	...
	retval = aarch64_instr_read_data_dcc_64(arm->dpm, 0xd5130400, (uint64_t *)buffer);
	if (retval != ERROR_OK)
		goto error_unset_dtr_r;
	retval = mem_ap_sel_read_atomic_u32(swjdp, armv8->debug_ap,
			armv8->debug_base + CPUDBG_DSCR, &dscr);
