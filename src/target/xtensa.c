 ...
 *   gus@projectgus.com                                                    *
 *                                                                         *
 *   Copyright (C) 2019 by Valery Balagurov                                *
 *   balagurov@gmail.com                                                   *
 *                                                                         *
...
static bool s_DisableInterruptsForStepping;
static bool s_FeedWatchdogDuringStops;
...
static int xtensa_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	if (breakpoint->type == BKPT_SOFT) {
		...
		if (breakpoint->length == sizeof(s_3ByteBreakpoint))
			...
			if (breakpoint->length == sizeof(s_2ByteBreakpoint))
...
static int xtensa_remove_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	if (breakpoint->type == BKPT_SOFT) {
		...
		if (res == ERROR_OK) {
			xtensa->free_brps++;
			assert(xtensa->free_brps <= xtensa->num_brps
