...
static void xtensa_feed_esp8266_watchdog(struct target *target)
{
	uint64_t wdtval = 0;
	uint32_t wdtctl = 0;
	uint8_t wdtval_buff[8] = {0};
	uint8_t wdtctl_buff[4] = {0};

	int r;
	r = xtensa_read_memory(target, 0x3ff21048, 4, 2, wdtval_buff);
	if (r != ERROR_OK) {
		LOG_ERROR("failed to read wdtval: error %d", r);
		return;
	}

	wdtval = target_buffer_get_u64(target, wdtval_buff);
	wdtval += 1600000;
	target_buffer_set_u64(target, wdtval_buff, wdtval);

	r = xtensa_write_memory(target, 0x3ff210cc, 4, 2, wdtval_buff);
	if (r != ERROR_OK) {
		LOG_ERROR("failed to write wdtovf: error %d", r);
		return;
	}

	r = xtensa_read_memory(target, 0x3ff210c8, 4, 1, wdtctl_buff);
	if (r != ERROR_OK) {
		LOG_ERROR("failed to read wdtctl: error %d", r);
		return;
	}

	wdtctl = target_buffer_get_u32(target, wdtctl_buff);
	wdtctl |= (1 << 31);
	target_buffer_set_u32(target, wdtctl_buff, wdtctl);

	r = xtensa_write_memory(target, 0x3ff210c8, 4, 1, wdtctl_buff);
	if (r != ERROR_OK) {
