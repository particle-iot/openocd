...
static bool disable_interrupts_for_stepping;
static bool feed_watchdog_during_stops;
...
#define XT_INS_FORMAT_RSR(OPCODE, SR, T) (OPCODE			\
					 | ((SR & 0xFF) << 8)	\
					 | ((T & 0x0F) << 4))

#define XT_INS_FORMAT_RRI8(OPCODE, R, S, T, IMM8) (OPCODE			\
						| ((IMM8 & 0xFF) << 16) \
						| ((R & 0x0F) << 12)	\
						| ((S & 0x0F) << 8)	\
						| ((T & 0x0F) << 4))
...
#define XT_INS_L32I(S, T, IMM8)  XT_INS_FORMAT_RRI8(0x002002, 0, S, T, IMM8)
/* Load 16-bit Unsigned from A(S)+2*IMM8 to A(T) */
#define XT_INS_L16UI(S, T, IMM8) XT_INS_FORMAT_RRI8(0x001002, 0, S, T, IMM8)
/* Load 8-bit Unsigned from A(S)+IMM8 to A(T) */
#define XT_INS_L8UI(S, T, IMM8)  XT_INS_FORMAT_RRI8(0x000002, 0, S, T, IMM8)

/* Store 32-bit Indirect to A(S)+4*IMM8 from A(T) */
#define XT_INS_S32I(S, T, IMM8) XT_INS_FORMAT_RRI8(0x006002, 0, S, T, IMM8)
/* Store 16-bit to A(S)+2*IMM8 from A(T) */
#define XT_INS_S16I(S, T, IMM8) XT_INS_FORMAT_RRI8(0x005002, 0, S, T, IMM8)
/* Store 8-bit to A(S)+IMM8 from A(T) */
#define XT_INS_S8I(S, T, IMM8)  XT_INS_FORMAT_RRI8(0x004002, 0, S, T, IMM8)

/* Read Special Register */
#define XT_INS_RSR(SR, T) XT_INS_FORMAT_RSR(0x030000, SR, T)
/* Write Special Register */
#define XT_INS_WSR(SR, T) XT_INS_FORMAT_RSR(0x130000, SR, T)
/* Swap Special Register */
#define XT_INS_XSR(SR, T) XT_INS_FORMAT_RSR(0x610000, SR, T)

/* WDT peripheral addresses */
#define WDTVAL_ADDRESS 0x3ff21048
#define WDTOVF_ADDRESS 0x3ff210cc
#define WDTCTL_ADDRESS 0x3ff210c8
...
static int xtensa_init_target(struct command_context *cmd_ctx, struct target *target)
{
	...
	for (unsigned int i = 0; i < XTENSA_NUM_TAP_INS; i++)
...
static void xtensa_deinit(struct target *target)
{
	struct xtensa_common *xtensa = target_to_xtensa(target);
	...
	if (dosr & (DOSR_IN_OCD_MODE)) {
		if (target->state != TARGET_HALTED) {
			...
			if (state == TARGET_DEBUG_RUNNING) {
				...
				/* target->debug_reason is checked in gdb_last_signal() that is invoked as a result
				   of calling target_call_event_callbacks() below.
				   Unless we specify it, GDB will get confused and report the stop to the user
				   on its internal breakpoints. */
...
static int xtensa_examine(struct target *target)
{
	...
	if (!target_was_examined(target)) {
		...
		if (target->state == TARGET_HALTED) {
			...
			for (size_t i = 0; i < XT_NUM_WATCHPOINTS; i++) {
	...
	int handle_breakpoints)
{
	...
	if (res == ERROR_OK) {
		int ps_value = buf_get_u32(xtensa->core_cache->reg_list[XT_REG_IDX_PS].value, 0, 8);
		int int_level = ps_value & 0x0f;
		if ((ps_value & 0x10) || (int_level == 1)) {
			/* We are executing code in the exception mode.
			   Setting ICOUNTLEVEL to 1 would step into the first instruction
			   that gets executed after the exception handler is done.
			   What we actually want is to step into the next instruction of
			   the code we are debugging (i.e. an exception handler).
			   Hence we ned to set ICOUNTLEVEL to 2 in order to count exception mode instructions as well. */
			icount_level = 2;
		}
	}

	if (disable_interrupts_for_stepping) {
		...
		if (res != ERROR_OK)
			...
			original_intenable = buf_get_u32(xtensa->core_cache->reg_list[XT_REG_IDX_INTENABLE].value, 0, 32);
			if (original_intenable) {
				buf_set_u32(xtensa->core_cache->reg_list[XT_REG_IDX_INTENABLE].value, 0, 32, 0);
				xtensa->core_cache->reg_list[XT_REG_IDX_INTENABLE].dirty = true;
			}
		}
	}

	res = xtensa_tap_queue_write_sr(target, XT_REG_IDX_ICOUNTLEVEL, icount_level);
	if (res != ERROR_OK)
		return res;
	...
	if (original_intenable) {
		buf_set_u32(xtensa->core_cache->reg_list[XT_REG_IDX_INTENABLE].value, 0, 32, original_intenable);
		xtensa->core_cache->reg_list[XT_REG_IDX_INTENABLE].dirty = true;
	}
...
static int xtensa_deassert_reset(struct target *target)
{
	...
	if (target->reset_halt) {
		...
		if (res != ERROR_OK) {
			LOG_ERROR("%s: failed to halt after reset", __func__);
			return res;
		}
				    ...
				    uint8_t *buffer)
{
	...
	for (uint8_t imm8 = 0; imm8 < count; imm8++) {
			       ...
			       const uint8_t *buffer)
{
	...
	*/
	while (count > 255) {
		LOG_DEBUG("%s: splitting write at address " TARGET_ADDR_FMT " size %d count 255", __func__,
			  address, size);
			      ...
			      uint8_t *buffer)
{
	...
	LOG_DEBUG("%s: aligned_address=" TARGET_ADDR_FMT " aligned_count=0x%"
...
static int xtensa_set_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	reg_list[XT_REG_IDX_IBREAKENABLE].valid = false;
	reg_list[bp_reg_idx].valid = false;

	return ERROR_OK;
}

static const uint8_t three_byte_breakpoint[] = { 0x00, 0x40, 0x00 };
static const uint8_t two_byte_breakpoint[] = { 0x2d, 0xf0 };
...
static int xtensa_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	if (breakpoint->type == BKPT_SOFT) {
		const uint8_t *breakpoint_insn = NULL;
		int res = xtensa_read_buffer(target, breakpoint->address, breakpoint->length, breakpoint->orig_instr);
		if (res != ERROR_OK)
			return res;

		if (breakpoint->length == sizeof(three_byte_breakpoint))
			breakpoint_insn = three_byte_breakpoint;
		else
			if (breakpoint->length == sizeof(two_byte_breakpoint))
				breakpoint_insn = two_byte_breakpoint;
			else {
				LOG_ERROR("Unexpected SW breakpoint size: %d", breakpoint->length);
				return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
			}

		res = xtensa_write_buffer(target, breakpoint->address, breakpoint->length, breakpoint_insn);
		if (res != ERROR_OK)
			return res;

		uint8_t tmp_buff[16];
		res = xtensa_read_buffer(target, breakpoint->address, breakpoint->length, tmp_buff);
		if (res == ERROR_OK && !memcmp(tmp_buff, breakpoint_insn, breakpoint->length))
			return res;

		breakpoint->type = BKPT_HARD;
		LOG_WARNING("Cannot set a software breakpoint at " TARGET_ADDR_FMT
			". Trying a hardware one instead...", breakpoint->address);
	}
...
static int xtensa_unset_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	if (slot >= xtensa->num_brps) {
		LOG_ERROR("Breakpoint slot not found");
		return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
	}
...
static void xtensa_build_reg_cache(struct target *target)
{
	...
	for (uint8_t i = 0; i < XT_NUM_REGS; i++) {
		...
		reg_list[i].number = arch_info[i].reg_num;
...
static const char *xtensa_get_gdb_arch(struct target *target)
{
	return "xtensa";
}
...
static void xtensa_feed_esp8266_watchdog(struct target *target)
{
	...
	int r;
	r = target_read_u64_bf(target, WDTVAL_ADDRESS, &wdtval);
	if (r != ERROR_OK) {
	...
	r = target_write_u64_bf(target, WDTOVF_ADDRESS, wdtval);
	if (r != ERROR_OK) {
		LOG_ERROR("failed to write wdtovf: error %d", r);
		return;
	}

	r = target_read_u32(target, WDTCTL_ADDRESS, &wdtctl);
	if (r != ERROR_OK) {
	...
	r = target_write_u32(target, WDTCTL_ADDRESS, wdtctl);
	if (r != ERROR_OK) {
		LOG_ERROR("failed to write wdtctl: error %d", r);
		return;
	}
}
...
COMMAND_HANDLER(xtensa_no_interrupts_during_steps)
{
	if (CMD_ARGC > 1)
		return ERROR_COMMAND_SYNTAX_ERROR;
	if (CMD_ARGC == 1)
		COMMAND_PARSE_ENABLE(CMD_ARGV[0], disable_interrupts_for_stepping);

	command_print(CMD_CTX, "Interrupt suppression during single-stepping is %s%s", (CMD_ARGC == 1) ? "now " : "",
		disable_interrupts_for_stepping ? "enabled" : "disabled");
...
COMMAND_HANDLER(esp8266_autofeed_watchdog)
{
	if (CMD_ARGC > 1)
		return ERROR_COMMAND_SYNTAX_ERROR;
	if (CMD_ARGC == 1)
		COMMAND_PARSE_ENABLE(CMD_ARGV[0], feed_watchdog_during_stops);

	command_print(CMD_CTX, "Watchdog feeding during stops is %s%s", (CMD_ARGC == 1) ? "now " : "",
		feed_watchdog_during_stops ? "enabled" : "disabled");
...
struct command_registration xtensa_commands[] = {
	{
		...
		.help = "Specifies whether the INTENABLE register will be set to 0 during single-stepping, "
