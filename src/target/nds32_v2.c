...
static int nds32_v2_get_debug_reason(struct nds32 *nds32, uint32_t *reason)
{
	...
	aice->port->api->read_reg(IR6, &val_itype);
...
static int nds32_v2_check_interrupt_stack(struct nds32_v2_common *nds32_v2)
{
	...
	aice->port->api->read_reg(IR0, &val_ir0); /* get $IR0 directly */
	...
	if (nds32_reach_max_interrupt_level(nds32)) {
		...
		aice->port->api->write_reg(IR0, modified_psw);
	...
	if ((nds32->max_interrupt_level == 3) && (nds32->current_interrupt_level == 2)) {
		aice->port->api->read_reg(IR2, &val_ir2); /* get $IR2 directly */
		val_ir2 &= ~(0x01 << 11);
		aice->port->api->write_reg(IR2, val_ir2);
	}
	...
	aice->port->api->read_reg(IR1, &val_ir1);
...
static int nds32_v2_restore_interrupt_stack(struct nds32_v2_common *nds32_v2)
{
	...
	/* restore origin $IR0 */
	aice->port->api->write_reg(IR0, nds32_v2->backup_ir0);
...
static int nds32_v2_get_exception_address(struct nds32 *nds32, uint32_t *address, uint32_t reason)
{
	...
	aice->port->api->read_reg(IR4, address); /* read $EVA directly */
...
static int nds32_v2_translate_address(struct target *target, uint32_t *address)
{
	...
	if ((AICE_MEMORY_ACC_BUS == memory->access_channel) ||
			((AICE_MEMORY_ACC_CPU == memory->access_channel) &&
		...
		uint32_t size, uint8_t *buffer)
{
	...
	if ((AICE_MEMORY_ACC_CPU == memory->access_channel) &&
