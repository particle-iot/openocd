	...
	if (!(cr0 & CR0_PG)) {
		...
		*physical = dsb + address;
		...
		if (calcaddr_pyhsfromlin(t, address, physical) != ERROR_OK) {
			LOG_ERROR("%s failed to calculate physical address from 0x%08x",
					__func__, address);
			return ERROR_FAIL;
		}
	}
			...
			uint32_t size, uint32_t count, uint8_t *buffer)
{
	...
	LOG_DEBUG("addr=%08x, size=%d, count=%d, buf=%p",
	...
	if (!count || !buffer || !phys_address) {
		LOG_ERROR("%s invalid params count=%d, buf=%p, addr=%08x",
...
int calcaddr_pyhsfromlin(struct target *t, uint32_t addr, uint32_t *physaddr)
{
	...
	if (isPAE) {
		...
		if (x86_32_common_read_phys_mem(t, pdpt_addr, 4, 2, entry_buffer) != ERROR_OK) {
			LOG_ERROR("%s couldn't read page directory pointer table entry at 0x%08x",
					__func__, pdpt_addr);
			return ERROR_FAIL;
		}
		uint64_t pdpt_entry = target_buffer_get_u64(t, entry_buffer);
		if (!(pdpt_entry & 0x0000000000000001)) {
			LOG_ERROR("%s page directory pointer table entry at 0x%08x is not present",
					__func__, pdpt_addr);
			return ERROR_FAIL;
		}
		...
		if (x86_32_common_read_phys_mem(t, pd_addr, 4, 2, entry_buffer) != ERROR_OK) {
			LOG_ERROR("%s couldn't read page directory entry at 0x%08x",
					__func__, pd_addr);
			return ERROR_FAIL;
		}
		uint64_t pd_entry = target_buffer_get_u64(t, entry_buffer);
		if (!(pd_entry & 0x0000000000000001)) {
			LOG_ERROR("%s page directory entry at 0x%08x is not present",
					__func__, pd_addr);
			return ERROR_FAIL;
		}
		...
		if (pd_entry & 0x0000000000000080) {
			...
			*physaddr = page_base + offset;
			...
			if (x86_32_common_read_phys_mem(t, pt_addr, 4, 2, entry_buffer) != ERROR_OK) {
				LOG_ERROR("%s couldn't read page table entry at 0x%08x", __func__, pt_addr);
				return ERROR_FAIL;
			}
			uint64_t pt_entry = target_buffer_get_u64(t, entry_buffer);
			if (!(pt_entry & 0x0000000000000001)) {
				LOG_ERROR("%s page table entry at 0x%08x is not present", __func__, pt_addr);
				return ERROR_FAIL;
			}
		...
		if (x86_32_common_read_phys_mem(t, pd_addr, 4, 1, entry_buffer) != ERROR_OK) {
			LOG_ERROR("%s couldn't read page directory entry at 0x%08x", __func__, pd_addr);
			return ERROR_FAIL;
		}
		uint32_t pd_entry = target_buffer_get_u32(t, entry_buffer);
		if (!(pd_entry & 0x00000001)) {
			LOG_ERROR("%s page directory entry at 0x%08x is not present", __func__, pd_addr);
			return ERROR_FAIL;
		}
			...
			uint32_t size, uint32_t count, uint8_t *buf)
{
	...
	if (x86_32->is_paging_enabled(t)) {
		...
		if (calcaddr_pyhsfromlin(t, addr, &physaddr) != ERROR_OK) {
			LOG_ERROR("%s failed to calculate physical address from 0x%08x", __func__, addr);
			retval = ERROR_FAIL;
		}
		...
		if (retval == ERROR_OK
			...
			LOG_ERROR("%s failed to read memory from physical address 0x%08x", __func__, physaddr);
			retval = ERROR_FAIL;
		}
		...
		if (x86_32_common_read_phys_mem(t, addr, size, count, buf) != ERROR_OK) {
			LOG_ERROR("%s failed to read memory from address 0%08x", __func__, addr);
			retval = ERROR_FAIL;
		}
	}
			...
			uint32_t size, uint32_t count, const uint8_t *buf)
{
	...
	if (x86_32->is_paging_enabled(t)) {
		...
		if (retval == ERROR_OK
			...
			LOG_ERROR("%s failed to write memory to physical address 0x%08x",
					__func__, physaddr);
			retval = ERROR_FAIL;
		}
		/* restore PG bit if it was cleared prior (regardless of retval) */
		...
		if (x86_32_common_write_phys_mem(t, addr, size, count, buf) != ERROR_OK) {
			LOG_ERROR("%s failed to write memory to address 0x%08x",
					__func__, addr);
			retval = ERROR_FAIL;
		}
	}
			...
			uint32_t size, uint8_t *buf)
{
	...
	LOG_DEBUG("addr=%08x, size=%d, buf=%p", addr, size, buf);
	check_not_halted(t);
	if (!buf || !addr) {
		LOG_ERROR("%s invalid params buf=%p, addr=%08x", __func__, buf, addr);
		return retval;
	}
...
{
	LOG_DEBUG("type=%d, addr=%08x", bp->type, bp->address);
	if (check_not_halted(t))
		return ERROR_TARGET_NOT_HALTED;
...
{
	struct x86_32_common *x86_32 = target_to_x86_32(t);
	LOG_DEBUG("addr=%08x, bp_num=%d, bp_type=%d, pb_length=%d",
	...
	if (DR7_BP_FREE(dr7, bp_num))
		...
		LOG_ERROR("%s dr7 error, already enabled, val=%08x", __func__, dr7);
...
static int unset_debug_regs(struct target *t, uint8_t bp_num)
{
	...
	if (!(DR7_BP_FREE(dr7, bp_num))) {
		...
		LOG_ERROR("%s dr7 error, not enabled, val=%08x", __func__, dr7);
		return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
	}
...
static int set_hwbp(struct target *t, struct breakpoint *bp)
{
	...
	LOG_USER("%s hardware breakpoint %d set at 0x%08x (hwreg=%d)", __func__,
...
static int unset_hwbp(struct target *t, struct breakpoint *bp)
{
	...
	LOG_USER("%s hardware breakpoint %d removed from 0x%08x (hwreg=%d)",
...
static int set_swbp(struct target *t, struct breakpoint *bp)
{
	...
	if (readback != SW_BP_OPCODE) {
		LOG_ERROR("%s software breakpoint error at 0x%08x, check memory",
				__func__, bp->address);
		LOG_ERROR("%s readback=%02x orig=%02x",
	...
	LOG_USER("%s software breakpoint %d set at 0x%08x",
...
static int unset_swbp(struct target *t, struct breakpoint *bp)
{
	...
	if (current_instr == SW_BP_OPCODE) {
		...
		LOG_ERROR("%s software breakpoint remove error at 0x%08x, check memory",
				__func__, bp->address);
		LOG_ERROR("%s current=%02x orig=%02x",
	...
	LOG_USER("%s software breakpoint %d removed from 0x%08x",
			__func__, bp->unique_id, bp->address);
	return ERROR_OK;
}
...
static int set_breakpoint(struct target *t, struct breakpoint *bp)
{
	...
	if (bp->type == BKPT_HARD) {
		error = set_hwbp(t, bp);
		if (error != ERROR_OK) {
			LOG_ERROR("%s error setting hardware breakpoint at 0x%08x",
					__func__, bp->address);
			return error;
		}
	} else {
		if (x86_32->sw_bpts_supported(t)) {
			error = set_swbp(t, bp);
			if (error != ERROR_OK) {
				LOG_ERROR("%s error setting software breakpoint at 0x%08x",
						__func__, bp->address);
				return error;
			}
		} else {
...
static int unset_breakpoint(struct target *t, struct breakpoint *bp)
{
	...
	if (bp->type == BKPT_HARD) {
		if (unset_hwbp(t, bp) != ERROR_OK) {
			LOG_ERROR("%s error removing hardware breakpoint at 0x%08x",
					__func__, bp->address);
			return ERROR_FAIL;
		}
	} else {
		if (unset_swbp(t, bp) != ERROR_OK) {
			LOG_ERROR("%s error removing software breakpoint at 0x%08x",
					__func__, bp->address);
			return ERROR_FAIL;
		}
	}
...
static int set_watchpoint(struct target *t, struct watchpoint *wp)
{
	...
	struct x86_32_dbg_reg *debug_reg_list = x86_32->hw_break_list;
	int wp_num = 0;
	LOG_DEBUG("type=%d, addr=%08x", wp->rw, wp->address);
...
static int write_hw_reg_from_cache(struct target *t, int num)
{
	...
	LOG_DEBUG("reg %s value 0x%08x", x86_32->cache->reg_list[num].name,
...
COMMAND_HANDLER(handle_iod_command)
{
	...
	if (address > 0xffff) {
		LOG_ERROR("%s IA-32 I/O space is 2^16, %08x exceeds max", __func__, address);
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
