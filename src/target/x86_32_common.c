...
{
	uint32_t i = 0;
	uint8_t buf4bytes[4] = { 0x00, 0x00, 0x00, 0x00 };
	struct x86_32_common *x86_32 = target_to_x86_32(t);
	 ...
	 */
	retval = x86_32->read_hw_reg(t, EDX, (uint32_t *)buf4bytes, 0);
			...
			uint32_t size, uint8_t *buf)
{
	...
	/* TODO: BUG: Stack overwrite happens here
	 * read_hw_reg() will write to 4 bytes (uint32_t)
	 * Watch out, the buffer passed into x86_32_common_read_io() might be 1 or 2 bytes.
	 */
	retval = x86_32->read_hw_reg(t, EAX, (uint32_t *)buf, 0);
			...
			uint32_t size, const uint8_t *buf)
{
	...
	retval = x86_32->write_hw_reg(t, EAX, *(uint32_t *)buf, 0);
	if (retval != ERROR_OK) {
