...
#include "mips64.h"
#include "mips_xlp3xx.h"
#include "target_type.h"
#include "register.h"

static void mips_xlp3xx_enable_breakpoints(struct target *target);
static void mips_xlp3xx_enable_watchpoints(struct target *target);
static int mips_xlp3xx_set_breakpoint(struct target *target,
		struct breakpoint *breakpoint);
static int mips_xlp3xx_unset_breakpoint(struct target *target,
		struct breakpoint *breakpoint);

static int mips_xlp3xx_examine_debug_reason(struct target *target)
{
...
static int mips_xlp3xx_debug_entry(struct target *target)
{
	...
	mips_xlp3xx_examine_debug_reason(target);
...
static int mips_xlp3xx_poll(struct target *target)
{
	int retval;
	mips64_common_t *mips64 = target->arch_info;
	struct mips_ejtag *ejtag_info = &mips64->ejtag_info;
	...
	if (ejtag_ctrl & EJTAG_CTRL_BRKST) {
		if ((target->state == TARGET_RUNNING) || (target->state == TARGET_RESET)) {
			...
			retval = mips_xlp3xx_debug_entry(target);
			if (retval != ERROR_OK)
				return retval;
...
static int mips_xlp3xx_halt(struct target *target)
{
	mips64_common_t *mips64 = target->arch_info;
	struct mips_ejtag *ejtag_info = &mips64->ejtag_info;
...
static int mips_xlp3xx_assert_reset(struct target *target)
{
	mips64_common_t *mips64 = target->arch_info;
	struct mips_ejtag *ejtag_info = &mips64->ejtag_info;
	...
	if (strcmp(target->variant, "komdiv_srst") == 0) {
		uint32_t ejtag_ctrl;

		ejtag_ctrl = EJTAG_CTRL_PRRST | EJTAG_CTRL_PERRST;
		mips_ejtag_set_instr(ejtag_info, EJTAG_INST_CONTROL);
		mips_ejtag_drscan_32(ejtag_info, &ejtag_ctrl);

		mips_ejtag_set_instr(ejtag_info, EJTAG_INST_CONTROL);
		ejtag_ctrl = EJTAG_CTRL_ROCC | EJTAG_CTRL_PRACC | EJTAG_CTRL_PROBEN |
			     EJTAG_CTRL_SETDEV | EJTAG_CTRL_JTAGBRK;
		mips_ejtag_drscan_32(ejtag_info, &ejtag_ctrl);
		jtag_add_sleep(5000);
...
static int mips_xlp3xx_deassert_reset(struct target *target)
{
	LOG_DEBUG("target->state: %s",
		target_state_name(target));
...
static int mips_xlp3xx_soft_reset_halt(struct target *target)
{
	/* TODO */
	return ERROR_OK;
}

static int mips_xlp3xx_single_step_core(struct target *target)
{
	mips64_common_t *mips64 = target->arch_info;
	struct mips_ejtag *ejtag_info = &mips64->ejtag_info;
	...
	mips_xlp3xx_debug_entry(target);

	return ERROR_OK;
}

static int mips_xlp3xx_resume(struct target *target, int current, uint64_t address,
	int handle_breakpoints, int debug_execution)
{
	...
	if (!debug_execution) {
		...
		mips_xlp3xx_enable_breakpoints(target);
		mips_xlp3xx_enable_watchpoints(target);
	}
	...
	if (handle_breakpoints) {
		...
		if (breakpoint) {
			...
			mips_xlp3xx_unset_breakpoint(target, breakpoint);
			mips_xlp3xx_single_step_core(target);
			mips_xlp3xx_set_breakpoint(target, breakpoint);
		}
	}
...
static int mips_xlp3xx_step(struct target *target, int current, uint64_t address, int handle_breakpoints)
{
	...
	/* the front-end may request us not to handle breakpoints */
	if (handle_breakpoints) {
		breakpoint = breakpoint_find(target, (uint64_t) buf_get_u64(mips64->core_cache->reg_list[MIPS64_PC].value, 0, 64));
		if (breakpoint)
			mips_xlp3xx_unset_breakpoint(target, breakpoint);
	}
...
static int mips_xlp3xx_set_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	mips64_common_t *mips64 = target->arch_info;
	mips64_comparator_t *comparator_list = mips64->inst_break_list;
	int retval;
	...
	if (breakpoint->type == BKPT_HARD) {
		...
		if (breakpoint->length == 4) {
			...
			if (verify != MIPS64_SDBBP) {
				LOG_ERROR("Unable to set 32bit breakpoint at address %16" PRIx64 " - check that memory is read/writable", breakpoint->address);
				return ERROR_OK;
			}
		} else {
			...
			if (verify != MIPS16_SDBBP) {
				LOG_ERROR("Unable to set 16bit breakpoint at address %16" PRIx64 " - check that memory is read/writable", breakpoint->address);
				return ERROR_OK;
			}
		}
...
static int mips_xlp3xx_unset_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	/* get pointers to arch-specific information */
	mips64_common_t *mips64 = target->arch_info;
	mips64_comparator_t *comparator_list = mips64->inst_break_list;
...
static int mips_xlp3xx_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	mips64_common_t *mips64 = target->arch_info;
...
static int mips_xlp3xx_remove_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	/* get pointers to arch-specific information */
	mips64_common_t *mips64 = target->arch_info;
...
static int mips_xlp3xx_set_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	uint64_t wp_value;
	mips64_common_t *mips64 = target->arch_info;
	mips64_comparator_t *comparator_list = mips64->data_break_list;
	int wp_num = 0;
	/*
...
static int mips_xlp3xx_unset_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	/* get pointers to arch-specific information */
	mips64_common_t *mips64 = target->arch_info;
	mips64_comparator_t *comparator_list = mips64->data_break_list;
...
static int mips_xlp3xx_add_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	mips64_common_t *mips64 = target->arch_info;
	...
	mips_xlp3xx_set_watchpoint(target, watchpoint);
	return ERROR_OK;
}

static int mips_xlp3xx_remove_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	/* get pointers to arch-specific information */
	mips64_common_t *mips64 = target->arch_info;
	...
	if (watchpoint->set)
		mips_xlp3xx_unset_watchpoint(target, watchpoint);
...
static int mips_xlp3xx_read_memory(struct target *target, uint64_t address,
...
static int mips_xlp3xx_write_memory(struct target *target, uint64_t address,
...
static int mips_xlp3xx_init_target(struct command_context *cmd_ctx,
...
static int mips_xlp3xx_init_arch_info(struct target *target, mips_xlp3xx_common_t *mips_xlp3xx, struct jtag_tap *tap)
{
	mips64_common_t *mips64 = &mips_xlp3xx->mips64_common;

	mips_xlp3xx->common_magic = MIPSXLP3xx_COMMON_MAGIC;

	/* initialize mips4k specific info */
	mips64_init_arch_info(target, mips64, tap);
	mips64->arch_info = mips_xlp3xx;

	return ERROR_OK;
}

static int mips_xlp3xx_target_create(struct target *target, Jim_Interp *interp)
{
	mips_xlp3xx_common_t *mips_xlp3xx = calloc(1, sizeof(mips_xlp3xx_common_t));

	mips_xlp3xx_init_arch_info(target, mips_xlp3xx, target->tap);

	return ERROR_OK;
}

static int mips_xlp3xx_examine(struct target *target)
{
	int retval;
	mips64_common_t *mips64 = target->arch_info;
	struct mips_ejtag *ejtag_info = &mips64->ejtag_info;
...
#define TIME_COUNT 1000
	{
		...
		uint32_t ejtag_ctrl = ejtag_info->ejtag_ctrl;
		register int i;
		clock_t cur_ticks, end_ticks;
		struct timespec cur_time, end_time;
		unsigned long cur_time_mks, end_time_mks;
		...
		cur_time_mks = cur_time.tv_sec*1000000 + cur_time.tv_nsec/1000;
		end_time_mks = end_time.tv_sec*1000000 + end_time.tv_nsec/1000;
		printf("\n\nReal time for getting ECR value (mks) = %ld\n\n\n",
			(end_time_mks - cur_time_mks)/TIME_COUNT);
		printf("\n\nProcess time for getting ECR value (mks) = %ld\n\n\n",
			(end_ticks - cur_ticks)/TIME_COUNT);
	}
...
static int mips_xlp3xx_checksum_memory(struct target *target, uint64_t address, uint32_t size, uint32_t *checksum)
{
	return ERROR_FAIL; /* use bulk read method */
}

struct target_type mips_xlp3xx_target = {
	.name = "mips_xlp3xx",

	.poll = mips_xlp3xx_poll,
	.arch_state = mips64_arch_state,

	.target_request_data = NULL,

	.halt = mips_xlp3xx_halt,
	.resume = mips_xlp3xx_resume,
	.step = mips_xlp3xx_step,

	.assert_reset = mips_xlp3xx_assert_reset,
	.deassert_reset = mips_xlp3xx_deassert_reset,
	.soft_reset_halt = mips_xlp3xx_soft_reset_halt,

	.get_gdb_reg_list = mips64_get_gdb_reg_list,

	.read_memory = mips_xlp3xx_read_memory,
	.write_memory = mips_xlp3xx_write_memory,
	.checksum_memory = mips_xlp3xx_checksum_memory,
	.blank_check_memory = NULL,

	.run_algorithm = mips64_run_algorithm,

	.add_breakpoint = mips_xlp3xx_add_breakpoint,
	.remove_breakpoint = mips_xlp3xx_remove_breakpoint,
	.add_watchpoint = mips_xlp3xx_add_watchpoint,
	.remove_watchpoint = mips_xlp3xx_remove_watchpoint,

	.target_create = mips_xlp3xx_target_create,
	.init_target = mips_xlp3xx_init_target,
	.examine = mips_xlp3xx_examine,
};
