...
static int nds32_parse_group_0_insn(struct nds32 *nds32, uint32_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
	...
	switch (opc_6 & 0x7) {
		case 0: /* LBI */
			nds32_parse_type_2(opcode, &(instruction->info.rt), &(instruction->info.ra), &(instruction->info.imm));
			...
			snprintf(instruction->text,
					...
					address,
					opcode, instruction->info.rt, instruction->info.ra, instruction->info.imm);
			break;
		case 1: /* LHI */
			nds32_parse_type_2(opcode, &(instruction->info.rt), &(instruction->info.ra), &(instruction->info.imm));
...
static int nds32_parse_group_1_insn(struct nds32 *nds32, uint32_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
	uint8_t opc_6;
...
static int nds32_parse_group_2_insn(struct nds32 *nds32, uint32_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
	uint8_t opc_6;
	...
	switch (opc_6 & 0x7) {
		...
		case 3: { /* DPREFI */
				uint8_t sub_type;
				nds32_parse_type_2(opcode, &sub_type, &(instruction->info.ra), &(instruction->info.imm));
				instruction->info.sub_opc = sub_type & 0xF;
				instruction->type = NDS32_INSN_MISC;
				if (sub_type & 0x10) { /* DPREFI.d */
					instruction->info.imm = (instruction->info.imm << 17) >> 14; /* sign-extend */
					snprintf(instruction->text,
							128,
							"0x%8.8" PRIx32 "\t0x%8.8" PRIx32 "\tDPREFI.d\t%d,[$r%d+#%d]",
							address,
							opcode, instruction->info.sub_opc, instruction->info.ra, instruction->info.imm);
				} else { /* DPREFI.w */
					instruction->info.imm = (instruction->info.imm << 17) >> 15; /* sign-extend */
					snprintf(instruction->text,
							128,
		...
		case 0:
			switch (sub_opcode & 0x7) {
				case 0: /* LB */
					...
					instruction->access_start = val_ra + (val_rb << ((instruction->info.imm >> 8) & 0x3));
					instruction->access_end = instruction->access_start + 1;
					snprintf(instruction->text,
							128,
							"0x%8.8" PRIx32 "\t0x%8.8" PRIx32 "\tLB\t$r%d,[$r%d+($r%d<<%d)]",
							address,
							opcode, instruction->info.rt, instruction->info.ra, instruction->info.rb, \
		...
		case 2:
			switch (sub_opcode & 0x7) {
				...
				case 1: /* LHS */
					...
					break;
				case 3: /* DPREF */
					nds32_parse_type_3(opcode, &(instruction->info.sub_opc), &(instruction->info.ra), \
							&(instruction->info.rb), &(instruction->info.imm));
					instruction->type = NDS32_INSN_MISC;
					snprintf(instruction->text,
							128,
							...
							address,
							opcode, instruction->info.sub_opc, instruction->info.ra, instruction->info.rb, \
							(instruction->info.imm >> 8) & 0x3);
					break;
				case 4: /* LBS.bi */
					nds32_parse_type_3(opcode, &(instruction->info.rt), &(instruction->info.ra), \
...
static int nds32_calculate_lsmw_access_range(struct nds32 *nds32, struct nds32_instruction *instruction)
{
	...
	if (id) { /* decrease */
		instruction->access_start = instruction->access_end - 4 * (instruction->info.rd - instruction->info.rb + 1);
		instruction->access_start -= (4 * enable4_bits[enable4]);
	} else { /* increase */
		instruction->access_end = instruction->access_start + 4 * (instruction->info.rd - instruction->info.rb + 1);
		...
		struct nds32_instruction *instruction)
{
	if (opcode & 0x20) { /* SMW, SMWA, SMWZB */
		switch (opcode & 0x3) {
			...
			case 0: /* SMW */
				/* use rd as re */
				nds32_parse_type_3(opcode, &(instruction->info.rb), &(instruction->info.ra), \
				...
				snprintf(instruction->text,
						...
						address,
						opcode, instruction->info.rb, instruction->info.ra, instruction->info.rd, \
	...
	switch ((opcode >> 18) & 0x3) {
		...
		case 3:
			...
			if ((opcode >> 17) & 0x1) { /* SWI.gp */
				nds32_parse_type_1(opcode, &(instruction->info.rt), &(instruction->info.imm));
				instruction->info.imm = (instruction->info.imm << 15) >> 13; /* sign-extend */
				nds32_get_mapped_reg(nds32, R29, &(instruction->access_start));
				instruction->access_start += instruction->info.imm;
				instruction->access_end = instruction->access_start + 4;
				snprintf(instruction->text,
		...
		struct nds32_instruction *instruction)
{
	switch (opcode & 0x1F) {
		...
		case 15: { /* ROTR */
			 ...
			 break;
		case 16: { /* SEB */
				 nds32_parse_type_2(opcode, &(instruction->info.rt), &(instruction->info.ra), \
						 &(instruction->info.imm));
				 instruction->type = NDS32_INSN_DATA_PROC;
				 snprintf(instruction->text,
						 128,
		...
		case 21: /* OR_SRLI */
			...
			if (instruction->info.imm)
			 ...
			 break;
		case 22: { /* DIVSR */
				 nds32_parse_type_4(opcode, &(instruction->info.rt), &(instruction->info.ra), \
						 &(instruction->info.rb), &(instruction->info.rd), &(instruction->info.sub_opc));
				 instruction->type = NDS32_INSN_DATA_PROC;
				 snprintf(instruction->text,
						 128,
	...
	switch (opcode & 0x3F) {
		...
		case 32: /* MFUSR */
			 ...
			 snprintf(instruction->text,
					 ...
					 address,
					 opcode, instruction->info.rt, (instruction->info.imm >> 10) & 0x3FF);
			 break;
		case 33: /* MTUSR */
			 nds32_parse_type_1(opcode, &(instruction->info.rt), &(instruction->info.imm));
			 instruction->type = NDS32_INSN_RESOURCE_ACCESS;
		...
		case 40: { /* MULTS64 */
				 ...
				 snprintf(instruction->text,
						 ...
						 address,
						 opcode, (dt_val >> 1) & 0x1, instruction->info.ra, instruction->info.rb);
			 }
			 break;
		case 41: { /* MULT64 */
				 uint8_t dt_val;
...
static int nds32_parse_group_4_insn(struct nds32 *nds32, uint32_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
	uint8_t opc_6;
	...
	switch (opc_6 & 0x7) {
		...
		case 2: /* MOVI */
			nds32_parse_type_1(opcode, &(instruction->info.rt), &(instruction->info.imm));
			instruction->info.imm = (instruction->info.imm << 12) >> 12; /* sign-extend */
			instruction->type = NDS32_INSN_DATA_PROC;
			snprintf(instruction->text,
					128,
		...
		case 4: /* JI */
			...
			instruction->info.imm = (instruction->info.imm << 8) >> 8; /* sign-extend */
				...
				switch (imm & 0x1F) {
					...
					case 3: /* JRALNEZ */
						...
						if (instruction->info.rt == R30)
							...
							snprintf(instruction->text,
									128,
									"0x%8.8" PRIx32 "\t0x%8.8" PRIx32 "\tJRALNEZ\t$r%d,$r%d",
									address,
									opcode, instruction->info.rt, instruction->info.rb);
						break;
				}
		...
		case 6: { /* BR1 */
				...
				if ((imm >> 14) & 0x1) { /* BNE */
					...
					instruction->info.imm = (instruction->info.imm << 18) >> 18; /* sign-extend */
					snprintf(instruction->text,
							128,
...
static int nds32_parse_group_5_insn(struct nds32 *nds32, uint32_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
	uint8_t opc_6;
...
static int nds32_parse_group_6_insn(struct nds32 *nds32, uint32_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
	uint8_t opc_6;
		...
		case 2: { /* MISC */
				...
				switch (sub_opc) {
					...
					case 2: /* MFSR */
						...
						break;
					case 3: /* MTSR */
						nds32_parse_type_1(opcode, &(instruction->info.ra), &(instruction->info.imm));
						instruction->type = NDS32_INSN_RESOURCE_ACCESS;
						snprintf(instruction->text,
								128,
								"0x%8.8" PRIx32 "\t0x%8.8" PRIx32 "\tMTSR\t$r%d,#%d",
								address,
								opcode, instruction->info.ra, (instruction->info.imm >> 10) & 0x3FF);
						break;
					case 4: /* IRET */
						instruction->type = NDS32_INSN_MISC;
						snprintf(instruction->text,
					...
					case 6: /* TEQZ */
						...
						snprintf(instruction->text,
								...
								address,
								opcode, instruction->info.ra, (instruction->info.imm >> 5) & 0x7FFF);
						break;
					case 7: /* TNEZ */
						nds32_parse_type_1(opcode, &(instruction->info.ra), &(instruction->info.imm));
						instruction->type = NDS32_INSN_MISC;
...
static int nds32_parse_group_0_insn_16(struct nds32 *nds32, uint16_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
...
static int nds32_parse_group_1_insn_16(struct nds32 *nds32, uint16_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
	switch ((opcode >> 9) & 0xF) {
		case 0: /* LWI333 */
		...
		case 1: /* LWI333.BI */
			...
			snprintf(instruction->text,
					...
					address,
					opcode, instruction->info.rt, instruction->info.ra, instruction->info.imm << 2);
			break;
		case 2: /* LHI333 */
			instruction->info.rt = nds32_extract_field_8u(opcode, 6, 3);
			instruction->info.ra = nds32_extract_field_8u(opcode, 3, 3);
...
static int nds32_parse_group_2_insn_16(struct nds32 *nds32, uint16_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
...
static int nds32_parse_group_3_insn_16(struct nds32 *nds32, uint16_t opcode, uint32_t address,
		struct nds32_instruction *instruction)
{
	switch ((opcode >> 11) & 0x3) {
		case 0:
			...
			switch ((opcode >> 9) & 0x3) {
				...
				case 2: /* PUSH25, POP25, MOVD44 */
					switch ((opcode >> 7) & 0x3) {
						case 0: /* PUSH25 */
							{
								...
								instruction->type = NDS32_INSN_LOAD_STORE;
								instruction->info.imm = nds32_extract_field_8u(opcode, 0, 5) << 3;
								...
								gpr_count = 3 + (re - 5); /* GPRs list: R6 ~ Re and fp, gp, lp */

								nds32_get_mapped_reg(nds32, R31, &(instruction->access_end));
								instruction->access_start = instruction->access_end - (gpr_count * 4);

								snprintf(instruction->text,
										128,
										"0x%8.8" PRIx32 "\t0x%4.4" PRIx32 "\t\tPUSH25\t$r%d,#%d",
										address,
										opcode, instruction->info.rd, instruction->info.imm);
							}
							break;
						case 1: /* POP25 */
							{
								...
								nds32_get_mapped_reg(nds32, R31, &(instruction->access_start));
								instruction->access_start += instruction->info.imm;
								instruction->access_end = instruction->access_start + (gpr_count * 4);

								snprintf(instruction->text,
										128,
										"0x%8.8" PRIx32 "\t0x%4.4" PRIx32 "\t\tPOP25\t$r%d,#%d",
										address,
										opcode, instruction->info.rd, instruction->info.imm);
							}
							break;
						case 2: /* MOVD44 */
						case 3:
							instruction->info.ra = nds32_extract_field_8u(opcode, 0, 4) * 2;
							instruction->info.rt = nds32_extract_field_8u(opcode, 4, 4) * 2;
							instruction->type = NDS32_INSN_MISC;
							snprintf(instruction->text,
									128,
									"0x%8.8" PRIx32 "\t0x%4.4" PRIx32 "\t\tMOVD44\t$r%d,$r%d",
									address,
									opcode, instruction->info.rt, instruction->info.ra);
							break;
					}
				...
				case 3: /* NEG33, NOT33, MUL33, XOR33, AND33, OR33 */
					...
					switch (opcode & 0x7) {
						case 2: /* NEG33 */
							snprintf(instruction->text,
									128,
									"0x%8.8" PRIx32 "\t0x%4.4" PRIx32 "\t\tNEG33\t$r%d,$r%d",
									address,
									opcode, instruction->info.rt, instruction->info.ra);
							break;
						case 3: /* NOT33 */
							snprintf(instruction->text,
									128,
									"0x%8.8" PRIx32 "\t0x%4.4" PRIx32 "\t\tNOT33\t$r%d,$r%d",
									address,
									opcode, instruction->info.rt, instruction->info.ra);
							break;
						case 4: /* MUL33 */
							snprintf(instruction->text,
									128,
									"0x%8.8" PRIx32 "\t0x%4.4" PRIx32 "\t\tMUL33\t$r%d,$r%d",
									address,
									opcode, instruction->info.rt, instruction->info.ra);
							break;
						case 5: /* XOR33 */
							snprintf(instruction->text,
									128,
									"0x%8.8" PRIx32 "\t0x%4.4" PRIx32 "\t\tXOR33\t$r%d,$r%d",
									address,
									opcode, instruction->info.rt, instruction->info.ra);
							break;
						case 6: /* AND33 */
							snprintf(instruction->text,
									128,
									"0x%8.8" PRIx32 "\t0x%4.4" PRIx32 "\t\tAND33\t$r%d,$r%d",
									address,
									opcode, instruction->info.rt, instruction->info.ra);
							break;
						case 7: /* OR33 */
							snprintf(instruction->text,
									128,
									"0x%8.8" PRIx32 "\t0x%4.4" PRIx32 "\t\tOR33\t$r%d,$r%d",
									address,
									opcode, instruction->info.rt, instruction->info.ra);
							break;
					}
