...
static const struct {
	...
	{  0,  "pc", REG_TYPE_ARCH_DEFINED, NULL, "akre", 0 },
	{  1,  "a", REG_TYPE_INT8, NULL, "akre", 0 },
	{  2,  "x", REG_TYPE_INT16, NULL, "akre", 0 },
	{  3,  "y", REG_TYPE_INT16, NULL, "akre", 0 },
	{  4,  "sp", REG_TYPE_INT16, NULL, "akre", 0 },
	{  5,  "cc", REG_TYPE_INT8, NULL, "akre", 0 },
};
...
enum hw_break_type { exec, rd, wr, acc };
		...
		uint32_t addr, int size, int count, void *buf)
{
	...
	ret =  adapter->layout->api->read_mem(adapter->handle, addr, size, count, buf);
	if (ret != ERROR_OK)
		return ret;
	return ERROR_OK;
}
		...
		uint32_t addr, int size, int count, const void *buf)
{
	...
	ret =  adapter->layout->api->write_mem(adapter->handle, addr, size, count, buf);
	if (ret != ERROR_OK)
		return ret;
	return ERROR_OK;
}
...
/*
	...
	since the interrupt flags are restored on debug_entry. We don't have any way
	for the debugger to exclusively disable the interrupts
*/
...
static int stm8_set_hwbreak(struct target *target, struct stm8_comparator comparator_list[])
{
	...
	if (!comparator_list[0].used) {
		comparator_list[0].type = exec;
		comparator_list[0].bp_value = -1;
	}

	if (!comparator_list[1].used) {
		comparator_list[1].type = exec;
		comparator_list[1].bp_value = -1;
	}

	if ((comparator_list[0].type == exec) && (comparator_list[1].type == exec)) {
	...
	if ((comparator_list[1].type == exec) && (comparator_list[0].type != exec)) {
	...
	for (i = 0 ; i < 2; i++) {
		...
		ret =  stm8_write_u8(target, DM_CR1, (bc << 3) + (bir<<2) + (biw<<1));
		...
		const uint8_t *buffer)
{
	...
	while (bytecnt) {
		if ((bytecnt >= 128) && blocksize_param && ((address & blocksize_param) == 0)) {
...
static int stm8_write_memory(struct target *target, uint32_t address,
		...
		const uint8_t *buffer)
{
...
#ifdef LOG_STM8
	LOG_DEBUG("%s address: 0x%8.8" PRIx32 ", size: 0x%8.8" PRIx32 ", count: 0x%8.8" PRIx32 "",
	...
	if ((address >= stm8->flashstart) && (address <= stm8->flashend))
		retval = stm8_write_flash(target, 1, address, size, count, stm8->blocksize, buffer);
	else if ((address >= stm8->eepromstart) && (address <= stm8->eepromend))
		retval = stm8_write_flash(target, 2, address, size, count, stm8->blocksize, buffer);
	else if ((address >= stm8->optionstart) && (address <= stm8->optionend))
		retval = stm8_write_flash(target, 3, address, size, count, 0, buffer);
	else
		retval = stm8_adapter_write_memory(target, address, size, count, buffer);
...
static int stm8_read_memory(struct target *target, uint32_t address,
		...
		int debug_execution)
{
	...
	if (!current) {
		...
		stm8->core_cache->reg_list[STM8_PC].dirty = 1;
		stm8->core_cache->reg_list[STM8_PC].valid = 1;
	}

	if (!current)
		resume_pc = address;
	else
		resume_pc = buf_get_u32(stm8->core_cache->reg_list[STM8_PC].value, 0, 32);
...
static int stm8_read_core_reg(struct target *target, unsigned int num)
{
	...
	stm8->core_cache->reg_list[num].valid = 1;
	stm8->core_cache->reg_list[num].dirty = 0;
...
static struct reg_cache *stm8_build_reg_cache(struct target *target)
{
	...
	struct stm8_core_reg *arch_info = malloc(sizeof(struct stm8_core_reg) * num_regs);
...
static int stm8_step(struct target *target, int current,
		uint32_t address, int handle_breakpoints)
{
	LOG_DEBUG("%s %" PRIx32 " %" PRIu32 " %" PRIx32, __func__, current, address, handle_breakpoints);
		...
		struct breakpoint *breakpoint)
{
	...
	if (breakpoint->type == BKPT_HARD) {
		...
		comparator_list[bp_num].type = exec;
		...
		if (breakpoint->length == 1) {
			...
			retval = target_read_u8(target, breakpoint->address, breakpoint->orig_instr);
			if (retval != ERROR_OK)
				return retval;
			...
			if (verify != STM8_BREAK) {
				LOG_ERROR("Unable to set breakpoint at address %08" PRIx32
...
/*
	   1                    ;;  as blank.s -o blank.o -al
	   2                    ;;  objcopy -I elf32-stm8 -O binary blank.o blank.bin
	   3                    ;;  xxd -i blank.bin
	   4                     .org 0x0
	   5                    ;; start address
	   6 0000 00             VAR0: .byte 0x00
	   7 0001 0000                 .word 0x0000
	   8                    ;; byte count
	   9 0003 00             VAR1: .byte 0x00
	  10 0004 0000                 .word 0x0000
	  11                    ;
	  12                    ; SP must point to VAR0 on entry
	  13                    ; first relocate VAR0 to the location
	  14                    ; we are running at
	  15 0006 96                    ldw X,SP
	  16 0007 CF0010                ldw .L1+2,X
	  17 000a 1E01                  ldw X,(VAR0+1,SP)       ;start addr
	  18 000c 1604                  ldw Y,(VAR1+1,SP)       ;count
	  19                    ;
	  20                    ; first check if [VAR0] is 0xff
	  21                    .L1:
	  22 000e 92BC0000              ldf A, [VAR0].e
	  23 0012 A1FF                  cp A,#0xff
	  24 0014 2615                  jrne .L4 ; exit if not 0xff
	  25                    ;
	  26                    ; increment VAR0
	  27 0016 5C                    incw X
	  28 0017 1F01                  ldw (VAR0+1,SP),X
	  29 0019 2602                  jrne .L2
	  30 001b 0C00                  inc (VAR0,SP)   ;increment high byte
	  31                    ;
	  32                    ; decrement VAR1
	  33                    .L2:
	  34 001d 905D                  tnzw Y
	  35 001f 2602                  jrne .L3
	  36 0021 0A03                  dec (VAR1,SP)   ;decrement high byte
	  37                    .L3:
	  38 0023 905A                  decw Y
	  39                    ;
	  40                    ; check if VAR1 == 0
	  41 0025 26E7                  jrne .L1;
	  42 0027 0D03                  tnz (VAR1,SP)   ;test if high byte is 0
	  43 0029 26E3                  jrne .L1;
	  44                    .L4:
	  45 002b 1F01                  ldw (VAR0+1,SP),X       ;start addr
	  46 002d 1704                  ldw (VAR1+1,SP),Y       ;count
	  47 002f 8B                    break
*/

	static const uint8_t erase_check_code[] = {
			  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x96, 0xcf, 0x00, 0x10, 0x1e, 0x01,
			  0x16, 0x04, 0x92, 0xbc, 0x00, 0x00, 0xa1, 0xff, 0x26, 0x15, 0x5c, 0x1f,
			  0x01, 0x26, 0x02, 0x0c, 0x00, 0x90, 0x5d, 0x26, 0x02, 0x0a, 0x03, 0x90,
			  0x5a, 0x26, 0xe7, 0x0d, 0x03, 0x26, 0xe3, 0x1f, 0x01, 0x17, 0x04, 0x8b
	};
	...
	target_write_buffer(target, erase_check_algorithm->address, sizeof(erase_check_code), erase_check_code);
	...
	int retval = target_run_algorithm(target, 2, mem_params, 2, reg_params,
				...
				erase_check_algorithm->address + (sizeof(erase_check_code)-1),
				10000, &stm8_info);
...
static int stm8_checksum_memory(struct target *target, uint32_t address,
...
/*
	   1                    ;;  as checksum.s -o checksum.o -al
	   2                    ;;  objcopy -I elf32-stm8 -O binary checksum.o checksum.bin
	   3                    ;;  xxd -i checksum.bin
	   4                     .org 0x0
	   5                    ;; start address
	   6 0000 00             VAR0:  .byte 0x00
	   7 0001 0000                  .word 0x0000
	  ...
	  10 0004 0000                  .word 0x0000
	  11                    ;; crc
	  12 0006 FFFF           CRC:   .word 0xffff
	  13 0008 00             LOOP:  .byte 0
	  14 0009 00             TMP:   .byte 0
	  15                    ;
	  16                    ; SP must point to VAR0 on entry
	  17                    ; first relocate VAR0 to the location
	  18                    ; we are running at
	  19 000a 96                    ldw X,SP
	  20 000b CF0010                ldw .L1+2,X
	  21                    ;
	  22                    .L1:
	  23                    ; load data in A
	  24 000e 92BC0000              ldf A, [VAR0].e
	  25 0012 5F                    clrw X
	  26 0013 97                    ld XL,A ;move data to X
	  27                    ;
	  28                    ; loop 8 bits
	  29 0014 A608                  ld A,#8
	  30 0016 C70008                ld LOOP,A
	  31                    ;
	  32                    ;
	  33                    .L2:
	  34 0019 9F                    ld A,XL ;low byte data in A
	  35 001a 1807                  xor A,(CRC+1,SP)        ;data ^ crc (low bytes)
	  36 001c C70009                ld TMP,A        ;store result in TMP
	  37                    ;
	  38                    ; shift crc>>1
	  39 001f 1606                  ldw Y,(CRC,SP)  ;crc in Y
	  40 0021 9054                  srlw Y
	  41 0023 1706                  ldw (CRC,SP),Y
	  42                    ;
	  43                    ; shift data>>1
	  44 0025 54                    srlw X  ;data in X
	  45                    ;
	  46 0026 72010009              btjf TMP,#0,.L3 ;skip if bit 0 is zero
	  46      0E
	  47 002b 909F                  ld A,YL
	  48 002d A808                  xor A,#0x08     ;xor with 0x8408
	  49 002f 9097                  ld YL,A
	  50 0031 909E                  ld A,YH
	  51 0033 A884                  xor A,#0x84
	  52 0035 9095                  ld YH,A
	  53 0037 1706                  ldw (CRC,SP),Y
	  54                    .L3:
	  55                    ;
	  56                    ; decrement bit counter
	  57 0039 725A0008              dec LOOP
	  58 003d 26DA                  jrne .L2
	  59                    ;
	  60                    ; increment VAR0
	  61 003f 1E01                  ldw X,(VAR0+1,SP)
	  62 0041 5C                    incw X
	  63 0042 1F01                  ldw (VAR0+1,SP),X
	  64 0044 2602                  jrne .L4
	  65 0046 0C00                  inc (VAR0,SP)   ;increment high byte
	  66                    ;
	  67                    ; decrement VAR1
	  68                    .L4:
	  69 0048 1E04                  ldw X,(VAR1+1,SP)
	  70 004a 2602                  jrne .L5
	  71 004c 0A03                  dec (VAR1,SP)   ;decrement high byte
	  72                    .L5:
	  73 004e 5A                    decw X
	  74 004f 1F04                  ldw (VAR1+1,SP),X
	  75                    ;
	  76                    ; check if VAR1 == 0
	  77 0051 26BB                  jrne .L1;
	  78 0053 0D03                  tnz (VAR1,SP)   ;test if high byte is 0
	  79 0055 26B7                  jrne .L1;
	  80                    .L6:
	  81 0057 8B                    break
*/

	static const uint8_t crc_code[] = {
			  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0x96, 0xcf,
			  0x00, 0x10, 0x92, 0xbc, 0x00, 0x00, 0x5f, 0x97, 0xa6, 0x08, 0xc7, 0x00,
			  0x08, 0x9f, 0x18, 0x07, 0xc7, 0x00, 0x09, 0x16, 0x06, 0x90, 0x54, 0x17,
			  0x06, 0x54, 0x72, 0x01, 0x00, 0x09, 0x0e, 0x90, 0x9f, 0xa8, 0x08, 0x90,
			  0x97, 0x90, 0x9e, 0xa8, 0x84, 0x90, 0x95, 0x17, 0x06, 0x72, 0x5a, 0x00,
			  0x08, 0x26, 0xda, 0x1e, 0x01, 0x5c, 0x1f, 0x01, 0x26, 0x02, 0x0c, 0x00,
			  0x1e, 0x04, 0x26, 0x02, 0x0a, 0x03, 0x5a, 0x1f, 0x04, 0x26, 0xbb, 0x0d,
			  0x03, 0x26, 0xb7, 0x8b
	};

	/* make sure we have a working area */
	if (target_alloc_working_area(target, sizeof(crc_code), &crc_algorithm) != ERROR_OK)
		return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;

	target_write_buffer(target, crc_algorithm->address, sizeof(crc_code), crc_code);
	...
	/* crc */
	init_mem_param(&mem_params[2], 0x6, 2, PARAM_IN_OUT);
	buf_set_u32(mem_params[2].value, 0, 16, 0xffff);

	init_reg_param(&reg_params[0], "SP", 32, PARAM_OUT);
	buf_set_u32(reg_params[0].value, 0, 32, crc_algorithm->address);

	int timeout = 20000 * (1 + (count / (1024 * 1024)));

	int retval = target_run_algorithm(target, 3, mem_params, 1, reg_params,
			crc_algorithm->address+0xa, crc_algorithm->address + (sizeof(crc_code) - 1), timeout,
			&stm8_info);

	if (retval == ERROR_OK)
		*checksum = buf_get_u32(mem_params[2].value, 0, 16);
		...
		uint32_t exit_point, int timeout_ms, void *arch_info)
{
	...
	/* NOTE: stm8_run_algorithm requires that each algorithm uses a software breakpoint
	 * at the exit point */
	...
	for (int i = 0; i < num_reg_params; i++) {
		struct reg *reg = register_get_by_name(stm8->core_cache, reg_params[i].reg_name, 0);
	...
	retval = stm8_run_and_wait(target, entry_point, timeout_ms, exit_point, stm8);
	...
	for (unsigned int i = 0; i < STM8_NUM_REGS; i++) {
		...
		if (regvalue != context[i]) {
			...
			buf_set_u32(stm8->core_cache->reg_list[i].value,
					0, 32, context[i]);
			stm8->core_cache->reg_list[i].valid = 1;
			stm8->core_cache->reg_list[i].dirty = 1;
		}
	}
...
int stm8_jim_configure(struct target *target, Jim_GetOptInfo *goi)
{
	...
	if (!strcmp(arg, "-blocksize")) {
		...
		if (goi->argc == 0) {
			Jim_WrongNumArgs(goi->interp, goi->argc, goi->argv, "-blocksize ?bytes? ...");
			return JIM_ERR;
		}
	...
	if (!strcmp(arg, "-flashstart")) {
		...
		if (goi->argc == 0) {
			Jim_WrongNumArgs(goi->interp, goi->argc, goi->argv, "-flashstart ?address? ...");
			return JIM_ERR;
		}
	...
	if (!strcmp(arg, "-flashend")) {
		...
		if (goi->argc == 0) {
			Jim_WrongNumArgs(goi->interp, goi->argc, goi->argv, "-flashend ?address? ...");
			return JIM_ERR;
		}
	...
	if (!strcmp(arg, "-eepromstart")) {
		...
		if (goi->argc == 0) {
			Jim_WrongNumArgs(goi->interp, goi->argc, goi->argv, "-eepromstart ?address? ...");
			return JIM_ERR;
		}
	...
	if (!strcmp(arg, "-eepromend")) {
		...
		if (goi->argc == 0) {
			Jim_WrongNumArgs(goi->interp, goi->argc, goi->argv, "-eepromend ?address? ...");
			return JIM_ERR;
		}
	...
	if (!strcmp(arg, "-optionstart")) {
		...
		if (goi->argc == 0) {
			Jim_WrongNumArgs(goi->interp, goi->argc, goi->argv, "-optionstart ?address? ...");
			return JIM_ERR;
		}
	...
	if (!strcmp(arg, "-optionend")) {
		...
		if (goi->argc == 0) {
			Jim_WrongNumArgs(goi->interp, goi->argc, goi->argv, "-optionend ?address? ...");
			return JIM_ERR;
		}
