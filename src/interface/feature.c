...
int feature_add(struct feature *features, struct feature *new_feature)
{
	struct feature *last_feature;
	...
	if (feature_find(features, new_feature->name) != NULL) {
...
int feature_update(struct feature *features, struct feature *new_feature)
{
	...
	if (feature_find(features, new_feature->name) == NULL) {
		...
		if (feature->next == NULL)
			if (strncmp(new_feature->name, feature->name, FEATURE_NAME_MAXLEN) == 0) {
				feature = new_feature;
				return ERROR_OK;
			}
		while (feature != NULL) {
			if (feature->next == NULL)
				break;
			if (strncmp(new_feature->name, feature->next->name, FEATURE_NAME_MAXLEN) == 0) {
...
int feature_del(struct feature *features, char *name)
{
	...
	if (feature->next == NULL) {
		if (strncmp(name, feature->name, FEATURE_NAME_MAXLEN) == 0) {
			features = NULL;
	...
	while (feature != NULL) {
		if (feature->next == NULL)
			break;
		if (strncmp(name, feature->next->name, FEATURE_NAME_MAXLEN) == 0) {
			if (feature->next->next == NULL)
				feature->next = NULL;
			else
				feature->next = feature->next->next;
			LOG_DEBUG("Removed feature: %s", name);
			return ERROR_OK;
		}
		feature = feature->next;
