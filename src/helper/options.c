...
static void test_one_find_relative_path(const char *from, const char *to, const char *expected)
{
	char *result = find_relative_path(from, to);
	LOG_DEBUG("Testing '%s'->'%s', expected '%s', got '%s'", from, to, expected, result);
	assert(result != NULL);
