...
static int bluenrgx_write_word(struct target *target, uint32_t address_base, uint32_t *values, uint32_t count)
{
	int retval = ERROR_OK;
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		if (read_back != values[i]) {
			LOG_ERROR("Comparison failed: expected: %08x, got %08x", values[i], read_back);
			return ERROR_FAIL;
		}
	}
	return retval;
...
static int bluenrgx_write_bytes(struct target *target, uint32_t address_base, uint8_t *buffer, uint32_t count)
{
	...
	retval = bluenrgx_write_word(target, address_base - pre_bytes, (uint32_t *) buffer, count/4);
...
#include "../../../contrib/loaders/flash/bluenrg-x/bluenrg-x_write.inc"
	...
	if (fast_size) {
		...
		LOG_DEBUG("source->address+ source->size = %08lx", source->address+source->size);
		LOG_DEBUG("write_algorithm_sp->address = %08lx", write_algorithm_sp->address);
