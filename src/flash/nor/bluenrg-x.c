...
static int bluenrgx_write_word(struct target *target, uint32_t address_base, uint8_t *values, uint32_t count)
{
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		retval = target_write_buffer(target, FLASH_REG_DATA, FLASH_WORD_LEN, (uint8_t *) &val);
		if (retval != ERROR_OK) {
			LOG_ERROR("Register write failed, error code: %d", retval);
			return retval;
		}
		...
		retval = target_read_buffer(target, address, FLASH_WORD_LEN, (uint8_t *) &read_back);
		if (retval != ERROR_OK) {
		...
		if (read_back != val) {
			LOG_ERROR("Comparison failed: expected: %08x, got %08x", val, read_back);
			return ERROR_FAIL;
		}
	}
	return retval;
