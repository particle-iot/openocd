...
static int stm32l4_probe(struct flash_bank *bank)
{
	...
	retval = target_read_u32(target, DBGMCU_IDCODE_L4, &device_id);
	...
	case 0x470: /* STM32L4Rx/L4Sx */
		...
		break;
	default:
		LOG_WARNING("Cannot identify target as an STM32L4/G0 family device.");
		return ERROR_FAIL;
	}
	...
	switch (device_id & 0xfff) {
		case 0x470:
			...
			return ERROR_FAIL;
		case 0x460:
			/* G0x0/G0x1 have single bank only.
			 * Page size always 2K. */
			page_size = 2048;
			num_pages = flash_size_in_kb / 2;
			/* check that calculation result makes sense */
			assert(num_pages > 0);
			stm32l4_info->bank2_start = num_pages;
			break;
		default:
...
static int get_stm32l4_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	default:
		snprintf(buf, buf_size, "Cannot identify target as a STM32L4/G0\n");
		return ERROR_FAIL;
	}
