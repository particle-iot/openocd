...
static int stm32l4_protect_check(struct flash_bank *bank)
{
	...
	uint8_t wrp1a_start = wrp1ar & 0xFF;
	uint8_t wrp1a_end = (wrp1ar >> 16) & 0xFF;
	uint8_t wrp1b_start = wrp1br & 0xFF;
	uint8_t wrp1b_end = (wrp1br >> 16) & 0xFF;
	uint8_t wrp2a_start = wrp2ar & 0xFF;
	uint8_t wrp2a_end = (wrp2ar >> 16) & 0xFF;
	uint8_t wrp2b_start = wrp2br & 0xFF;
	uint8_t wrp2b_end = (wrp2br >> 16) & 0xFF;

	for (int i = 0; i < bank->num_sectors; i++) {
		if (i < stm32l4_info->option_bytes.bank2_start) {
			if (((i >= wrp1a_start) &&
				 (i <= wrp1a_end)) ||
				((i >= wrp1b_start) &&
				 (i <= wrp1b_end)))
			...
			uint8_t snb;
			snb = i - stm32l4_info->option_bytes.bank2_start + 256;
			if (((snb >= wrp2a_start) &&
				 (snb <= wrp2a_end)) ||
				((snb >= wrp2b_start) &&
				 (snb <= wrp2b_end)))
...
static int stm32l4_protect(struct flash_bank *bank, int set, int first, int last)
{
	...
	if (last >= stm32l4_info->option_bytes.bank2_start) {
		if (set == 1)
			reg_value = ((last & 0xFF) << 16) | ((first - stm32l4_info->option_bytes.bank2_start) & 0xFF);

		ret = stm32l4_write_option(bank, STM32_FLASH_WRP2AR, reg_value, 0xffffffff);
	}
	/* Bank 1 */
	reg_value = 0xFF; /* Default to bank un-protected */
	if (first < stm32l4_info->option_bytes.bank2_start) {
		if (set == 1)
			reg_value = (((last - stm32l4_info->option_bytes.bank2_start) & 0xFF) << 16) | (first & 0xFF);
...
static int stm32l4_probe(struct flash_bank *bank)
{
	...
	if ((flash_size_in_kb == 1024) || !(options & OPT_DUALBANK))
		stm32l4_info->option_bytes.bank2_start = 256;
	else
		stm32l4_info->option_bytes.bank2_start = flash_size_in_kb << 9;
...
COMMAND_HANDLER(stm32l4_handle_lock_command)
{
	...
	if (stm32l4_write_option(bank, STM32_FLASH_OPTR, RDP, 0x000000FF) != ERROR_OK) {
