 ...
 * option byte is avaiable  to map all sectors to the first bank.
...
static int stm32l4_protect_check(struct flash_bank *bank)
{
	...
	for (int i = 0; i < bank->num_sectors; i++) {
		if (i < stm32l4_info->option_bytes.bank_b_start) {
			...
			if (((i >= stm32l4_info->option_bytes.wpr1b_start) &&
				 (i <= stm32l4_info->option_bytes.wpr1b_end)) ||
				((i >= stm32l4_info->option_bytes.wpr2b_start) &&
				 (i <= stm32l4_info->option_bytes.wpr2b_end)))
				bank->sectors[i].is_protected = 1;
			else
				bank->sectors[i].is_protected = 0;
		}
...
static int stm32l4_erase(struct flash_bank *bank, int first, int last)
{
	...
	for (i = first; i <= last; i++) {
		...
		erase_flags = FLASH_PER | (i << FLASH_PAGE_SHIFT) | FLASH_STRT;

		if  (i >= stm32l4_info->option_bytes.bank_b_start)
			erase_flags |= FLASH_CR_BKER;
		...
		uint32_t offset, uint32_t count)
{
	...
	if (count & 0x7) {
		LOG_WARNING("count 0x%" PRIx32 " breaks required 8-byte write size",
					count);
		return ERROR_FLASH_DST_BREAKS_ALIGNMENT;
	}
	...
	retval = stm32l4_write_block(bank, buffer, offset, halfwords);
	if (retval != ERROR_OK) {
		LOG_WARNING("block write failed");
		return retval;
		}

	LOG_WARNING("block write succeded");
...
static int stm32l4_probe(struct flash_bank *bank)
{
	...
	if ((flash_size_in_kb == 1024) || !(options & OPT_DUALBANK))
		stm32l4_info->option_bytes.bank_b_start = 0x100;
	else
