 ...
 *   2017 - 2018 by Andreas Bolsch <andreas.bolsch@mni.thm.de			   *
...
{																			\
	if ((data & msoftspi_info->pin.mask) != 0)								\
		pdir ^= msoftspi_info->io[index].mask;								\
	(data) &= ~(msoftspi_info->pin.mask);									\
	retval = target_write_u32(target, msoftspi_info->pin.addr, data);		\
	if (retval == ERROR_OK)													\
		retval = target_write_u32(target, msoftspi_info->io[index].addr		\
			+ (int8_t) ((msoftspi_info->dir_off >> (index << 3)) & 0xFF),	\
			pdir);															\
}
...
{																			\
	if ((data & msoftspi_info->pin.mask) == 0)								\
		pdir = (pdir & ~msoftspi_info->io[index].mask) |					\
			(msoftspi_info->io[index].mask & msoftspi_info->io_def[index]);	\
	(data) |= (msoftspi_info->pin.mask);									\
	retval = target_write_u32(target, msoftspi_info->pin.addr, data);		\
	if (retval == ERROR_OK)													\
		retval = target_write_u32(target, msoftspi_info->io[index].addr		\
...
#define SPI_ADDR_BYTES														\
	((log2u(msoftspi_info->dev.size_in_bytes) + 7) >> 3)

/* number of bytes required for address */
#define I2C_ADDR_BYTES														\
	((log2u(msoftspi_info->dev.size_in_bytes >> msoftspi_info->i2c_bits) + 7) >> 3)

#define PARAM(field) h_to_le_32(msoftspi_info->field)

#define FILL_PORT_BUFFER(spi_cmd)											\
	{																		\
		h_to_le_32((msoftspi_info->inp_off << 16) |							\
			(msoftspi_info->dummy << 8) | (spi_cmd)),						\
		PARAM(bit_no), PARAM(dir_off),										\
		PARAM(ncs.addr), PARAM(ncs.mask),									\
		PARAM(clk.addr), PARAM(clk.mask),									\
		PARAM(io[0].addr),													\
		PARAM(mode == MODE_SPI ? msoftspi_info->io0_mask : msoftspi_info->io[0].mask),	\
...
typedef struct {
	...
	uint32_t mask;				/* bit mask for pin ir direction exor mask */
} port_pin;
...
struct msoftspi_flash_bank {
	...
	uint8_t inp_off;			/* offset port output reg. to port input reg. */
	...
	uint32_t dir_off;			/* offsets output reg. to port dir reg. for io3, ..., io0 */
	uint32_t io0_mask;			/* original IO0/MOSI exor mask for regular spi */
...
static int deactivate_io3_io2(struct flash_bank *bank)
{
	...
	if (msoftspi_info->io[3].addr != 0) {
		retval = target_read_u32(target, msoftspi_info->io[3].addr, &port);
		if (retval != ERROR_OK)
			return retval;
		port |= (1UL << ((msoftspi_info->bit_no >> (3 << 3)) & 0xFF));
		retval = target_write_u32(target, msoftspi_info->io[3].addr, port);
		if (retval != ERROR_OK)
			return retval;
	}

	if (msoftspi_info->io[2].addr != 0) {
		retval = target_read_u32(target, msoftspi_info->io[2].addr, &port);
		if (retval != ERROR_OK)
			return retval;
		port |= (1UL << ((msoftspi_info->bit_no >> (2 << 3)) & 0xFF));
		retval = target_write_u32(target, msoftspi_info->io[2].addr, port);
		if (retval != ERROR_OK)
			return retval;
	}
...
static int set_to_output(struct flash_bank *bank, bool out)
{
	...
	switch (msoftspi_info->mode) {
		case MODE_SPI:
			LOG_DEBUG("spi mode, direction: %s", out ? "out" : "in");
			dir_off = (msoftspi_info->dir_off >> (0 << 3)) & 0xFF;
			retval = target_write_u32(target,
				msoftspi_info->io[0].addr + ((int8_t) dir_off),
				msoftspi_info->io_def[0] ^ (out ? msoftspi_info->io0_mask : 0));
			if (retval != ERROR_OK) {
				LOG_ERROR("setting io[0] direction failed");
				return retval;
			}
			break;
		...
		case MODE_DPI:
			...
			for (int k = 0; k < 2; k++) {
				dir_off = (msoftspi_info->dir_off >> (k << 3)) & 0xFF;
				if (dir_off != 0) {
					retval = target_write_u32(target,
						msoftspi_info->io[k].addr + ((int8_t) dir_off),
						msoftspi_info->io_def[k] ^ (out ? msoftspi_info->io[k].mask : 0));
					if (retval != ERROR_OK) {
...
FLASH_BANK_COMMAND_HANDLER(msoftspi_flash_bank_command)
{
	...
	memset(msoftspi_info, 0, sizeof(struct msoftspi_flash_bank));
	bank->driver_priv = msoftspi_info;

	COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], msoftspi_info->inp_off);

	if (index + 8 >= CMD_ARGC) {
		/* I2C mode has only 8 further parameters */
		LOG_INFO("I2C mode selected");
		msoftspi_info->mode = MODE_I2C;

		/* two data pins to follow */
		k = 2;
	} else {
		LOG_INFO("SPI/DPI/QPI mode selected");
		msoftspi_info->mode = MODE_SPI;		/* default mode */

		/* handle NCS and CLK pins */
		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[index++], msoftspi_info->ncs.addr);
		COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], byte);
		if (byte < 32) {
			msoftspi_info->ncs.mask = 1UL << byte;
		} else {
			command_print(CMD_CTX, "msoftspi: NCS bit number must be in 0 ... 31");
			return ERROR_COMMAND_SYNTAX_ERROR;
		}

		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[index++], msoftspi_info->clk.addr);
		COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], byte);
		if (byte < 32) {
			msoftspi_info->clk.mask = 1UL << byte;
		} else {
			command_print(CMD_CTX, "msoftspi: CLK bit number must be in 0 ... 31");
			return ERROR_COMMAND_SYNTAX_ERROR;
		}
	...
	do {
		...
		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[index++], msoftspi_info->io[k].addr);
		COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], byte);
		if (byte < 32) {
			msoftspi_info->bit_no |= (byte << (k << 3));
		} else {
			command_print(CMD_CTX, "msoftspi: IO3/NHOLD, IO2/NWP, IO1/MISO/SDA, "
				"IO0/MOSI/SCL bit numbers must be in 0 ... 31");
			return ERROR_COMMAND_SYNTAX_ERROR;
		}
		COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], byte);
		if (byte == 0) {
			command_print(CMD_CTX, "msoftspi: dir offset must be non-zero");
			return ERROR_COMMAND_SYNTAX_ERROR;
		}
		msoftspi_info->dir_off |= (byte << (k << 3));
		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[index++], msoftspi_info->io[k].mask);
	} while (k-- > 0);

	if (msoftspi_info->mode == MODE_I2C) {
		/* copy IO1 data to NCS and IO0 to CLK */
		msoftspi_info->SPI_SDA.addr = msoftspi_info->io[I2C_SDA].addr;
		msoftspi_info->SPI_SDA.mask = (1UL << ((msoftspi_info->bit_no >> (I2C_SDA << 3)) & 0xFF));
		msoftspi_info->SPI_SCL.addr = msoftspi_info->io[I2C_SCL].addr;
		msoftspi_info->SPI_SCL.mask = (1UL << ((msoftspi_info->bit_no >> (I2C_SCL << 3)) & 0xFF));
		msoftspi_info->same_port = (msoftspi_info->io[I2C_SDA].addr == msoftspi_info->io[I2C_SCL].addr);
		msoftspi_info->same_conf =  (msoftspi_info->same_port &&
			(((msoftspi_info->dir_off ^ (msoftspi_info->dir_off >> 8)) & 0xFF) == 0));

		LOG_DEBUG("SDA (in/out):  0x%08" PRIx32 ", mask 0x%08" PRIx32,
			msoftspi_info->io[I2C_SDA].addr, msoftspi_info->io[I2C_SDA].mask);
		LOG_DEBUG("SCL (in/out):  0x%08" PRIx32 ", mask 0x%08" PRIx32,
			msoftspi_info->io[I2C_SCL].addr, msoftspi_info->io[I2C_SCL].mask);
		if (msoftspi_info->same_port)
			LOG_DEBUG("identical SDA/SCL input/output registers");
		if (msoftspi_info->same_conf)
			LOG_DEBUG("identical SDA/SCL config registers");
	} else {
		/* save original IO0/MOSI exor mask for regular spi mode */
		msoftspi_info->io0_mask = msoftspi_info->io[0].mask;

		/* check for same port addresses and combine info accordingly */
		for (k = 0; k < DATA_PINS; k++) {
			byte = (msoftspi_info->dir_off >> (k << 3)) & 0xFF;
			for (j = k + 1; j < DATA_PINS; j++)
				if (msoftspi_info->io[k].addr == msoftspi_info->io[j].addr) {
					/* same port for pin j and pin k */
					if (((msoftspi_info->dir_off >> (j << 3)) & 0xFF) == byte) {
						/* same port and same dir offset, then absorb dir info of
						* pin j into dir info for pin k */
						msoftspi_info->io[k].mask |= msoftspi_info->io[j].mask;

						/* clear dir info for pin j */
						msoftspi_info->dir_off &= ~(0xFFUL << (j << 3));
						msoftspi_info->io[j].mask = 0x0;
					}
				}
		}
		...
		k = DATA_PINS - 1;
		do {
			LOG_DEBUG("IO%d (in/out): 0x%08" PRIx32 ", exor 0x%08" PRIx32,
				k, msoftspi_info->io[k].addr, msoftspi_info->io[k].mask);
		} while (k-- > 0);
		...
		LOG_DEBUG("CLK (out):     0x%08" PRIx32 ", mask 0x%08" PRIx32,
			msoftspi_info->clk.addr, msoftspi_info->clk.mask);
	}

	LOG_DEBUG("bit_no:        0x%08" PRIx32, msoftspi_info->bit_no);
	LOG_DEBUG("inp_off:       0x%08" PRIx32, (int8_t) msoftspi_info->inp_off);
	LOG_DEBUG("dir_off:       0x%08" PRIx32, msoftspi_info->dir_off);

	return ERROR_OK;
}

/* send start or stop condition */
static int msoftspi_i2c_start_stop(struct flash_bank *bank, bool start)
{
	struct target *target = bank->target;
	struct msoftspi_flash_bank *msoftspi_info = bank->driver_priv;
	...
	retval = target_read_u32(target, msoftspi_info->SPI_SCL.addr, &port_clk);
	if (retval != ERROR_OK)
		return retval;
	retval = target_read_u32(target, msoftspi_info->SPI_SCL.addr
		+ (int8_t) ((msoftspi_info->dir_off >> (I2C_SCL << 3)) & 0xFF), &pdir_clk);
	if (retval != ERROR_OK)
		return retval;
	...
	if (msoftspi_info->same_port)
		...
		retval = target_read_u32(target, msoftspi_info->SPI_SDA.addr, &port_iox);
		if (retval != ERROR_OK)
			return retval;
	}
	if (msoftspi_info->same_conf)
		...
		retval = target_read_u32(target, msoftspi_info->SPI_SDA.addr
			+ (int8_t) ((msoftspi_info->dir_off >> (I2C_SDA << 3)) & 0xFF), &pdir_iox);
		if (retval != ERROR_OK)
			return retval;
	}
...
static int msoftspi_i2c_resync(struct flash_bank *bank)
{
	struct target *target = bank->target;
	struct msoftspi_flash_bank *msoftspi_info = bank->driver_priv;
	uint32_t port_clk, pdir_clk, port_iox, pdir_iox;
	...
	if (retval != ERROR_OK)
		return retval;
	port_clk |= (msoftspi_info->SPI_SCL.mask);
	pdir_clk = msoftspi_info->io_def[I2C_SCL];
	SET_SCL(port_clk, pdir_clk);
	...
	if (retval != ERROR_OK)
		return retval;
	port_iox |= (msoftspi_info->SPI_SDA.mask);
	pdir_iox = msoftspi_info->io_def[I2C_SDA];
	SET_SDA(port_iox, pdir_iox);

	/* generate start condition 9 times */
	for (k = 0; k < 9; k++) {
		retval = msoftspi_i2c_start_stop(bank, true);
		if (retval != ERROR_OK)
			return retval;
	}

	/* and finally one stop condition */
	retval = msoftspi_i2c_start_stop(bank, false);
	return retval;
}
...
static int msoftspi_shift_out(struct flash_bank *bank, uint32_t word)
{
	...
	retval = target_read_u32(target, msoftspi_info->clk.addr, &port_clk);
	if (retval != ERROR_OK)
		return retval;

	switch (msoftspi_info->mode) {
		case MODE_SPI:
			if (msoftspi_info->clk.addr == msoftspi_info->io[0].addr)
				port_iox = port_clk;
			else {
				retval = target_read_u32(target, msoftspi_info->io[0].addr, &port_iox);
				if (retval != ERROR_OK)
					return retval;
			}

			bit_mask = 1UL << ((msoftspi_info->bit_no >> 0) & 0xFF);
			for (k = 0; k < 8; k++)	{
				...
				retval = target_write_u32(target, msoftspi_info->io[0].addr, port_iox);
				if (retval != ERROR_OK)
					return retval;
		...
		case MODE_DPI:
			for (k = 0; k < 8 / 2; k++) {
				...
				for (m = 2 - 1; m >= 0; --m) {
					if (msoftspi_info->clk.addr == msoftspi_info->io[m].addr)
						port_iox = port_clk;
					else {
						retval = target_read_u32(target, msoftspi_info->io[m].addr, &port_iox);
						if (retval != ERROR_OK)
							return retval;
					}

					/* copy current data bit to port data */
					bit_mask =  1UL << ((msoftspi_info->bit_no >> (m << 3)) & 0xFF);
					if (word & (1UL << 7))
						port_iox |= bit_mask;
					else
						port_iox &= ~bit_mask;
					...
					retval = target_write_u32(target, msoftspi_info->io[m].addr, port_iox);
					if (retval != ERROR_OK)
						return retval;
		...
		case MODE_I2C:
			...
			retval = target_read_u32(target,
				msoftspi_info->SPI_SDA.addr + ((int8_t) msoftspi_info->inp_off), &in_iox);
			if (retval != ERROR_OK)
				return retval;
			...
			LOG_DEBUG("I2C %s", in_iox & msoftspi_info->ncs.mask ? "NACK" : "ACK");
			/* return error if NACK */
			if (in_iox & msoftspi_info->SPI_SDA.mask)
...
static int msoftspi_shift_in(struct flash_bank *bank, uint32_t *word)
{
	...
	switch (msoftspi_info->mode) {
		case MODE_SPI:
			...
			bit_no = (msoftspi_info->bit_no >> (1 << 3)) & 0xFF;
			for (k = 0; k < 8; k++)	{
				...
				/* read data bit */
				retval = target_read_u32(target,
					msoftspi_info->io[1].addr + ((int8_t) msoftspi_info->inp_off), &port_iox);
				if (retval != ERROR_OK)
					return retval;
		...
		case MODE_DPI:
			...
			for (k = 0; k < 8 / 2; k++) {
				...
				for (m = 2 - 1; m >= 0; --m) {
					/* shift in data bits, msb first */
					retval = target_read_u32(target,
						msoftspi_info->io[m].addr + ((int8_t) msoftspi_info->inp_off), &port_iox);
					if (retval != ERROR_OK)
						return retval;

					bit_no = (msoftspi_info->bit_no >> (m << 3)) & 0xFF;
					*word <<= 1;
					*word |= ((port_iox >> bit_no) & 0x1);
				}
		...
		case MODE_I2C:
			...
			for (k = 0; k < 8; k++)	{
				...
				/* read SDA */
				retval = target_read_u32(target,
					msoftspi_info->SPI_SDA.addr + ((int8_t) msoftspi_info->inp_off), &port_iox);
				if (retval != ERROR_OK)
					return retval;

				*word = (*word << 1) | ((port_iox & msoftspi_info->SPI_SDA.mask) ? 0x1 : 0x0);
...
err:
	...
	if ((data & SPIFLASH_WE_BIT) == 0) {
		LOG_DEBUG("Sector erase not accepted by flash or already completed. Status=0x%02" PRIx8, data);
		/* return ERROR_FAIL; */
	}
...
int msoftspi_i2c_calib(struct flash_bank *bank, uint32_t count, double *time)
{
	struct target *target = bank->target;
	struct msoftspi_flash_bank *msoftspi_info = bank->driver_priv;
	...
	/* see contrib/loaders/erase_check/msofti2c_calib.S for src */
	static const uint8_t msofti2c_calib_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msofti2c_calib.inc"
	};
	const uint32_t code_size = sizeof(msofti2c_calib_code);
	...
	/* this will overlay the words of msoft*_*_code in target */
	...
	retval = target_write_buffer(target, calib_algorithm->address,
		code_size - sizeof(port_buffer), msofti2c_calib_code);
	if (retval != ERROR_OK)
		goto err;
...
int msoftspi_i2c_scan(struct flash_bank *bank)
{
	struct target *target = bank->target;
	struct msoftspi_flash_bank *msoftspi_info = bank->driver_priv;
	struct armv7m_algorithm armv7m_info;
	...
	/* see contrib/loaders/flash/msoftspi/msoftmsofti2c_scan.S for src */
	static const uint8_t msofti2c_scan_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msofti2c_scan.inc"
	};
	const uint32_t code_size = sizeof(msofti2c_scan_code);
	...
	if (target_alloc_working_area_try(target, code_size + SCAN_SIZE,
			&scan_algorithm) != ERROR_OK) {
		LOG_ERROR("allocating working area failed");
		return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
	};
	...
	retval = target_write_buffer(target, scan_algorithm->address,
		code_size - sizeof(port_buffer), msofti2c_scan_code);
	if (retval != ERROR_OK)
		goto err;
...
static int msoftspi_blank_check(struct flash_bank *bank)
{
	...
	static const uint8_t msoftspi_erase_check_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msoftspi_erase_check.inc"
	};

	/* see contrib/loaders/erase_check/msoftqpi_erase_check.S for src */
	static const uint8_t msoftqpi_erase_check_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msoftqpi_erase_check.inc"
	};

	/* see contrib/loaders/erase_check/msoftdpi_erase_check.S for src */
	static const uint8_t msoftdpi_erase_check_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msoftdpi_erase_check.inc"
	};

	/* see contrib/loaders/erase_check/msofti2c_erase_check.S for src */
	static const uint8_t msofti2c_erase_check_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msofti2c_erase_check.inc"
	};

	switch (msoftspi_info->mode) {
		case MODE_SPI:
			code = msoftspi_erase_check_code;
			code_size = sizeof(msoftspi_erase_check_code);
			cmd = msoftspi_info->dev.read_cmd;
			break;

		case MODE_DPI:
			code = msoftdpi_erase_check_code;
			code_size = sizeof(msoftdpi_erase_check_code);
			cmd = msoftspi_info->dev.qread_cmd;
			break;

		case MODE_QPI:
			code = msoftqpi_erase_check_code;
			code_size = sizeof(msoftqpi_erase_check_code);
			cmd = msoftspi_info->dev.qread_cmd;
			break;

		case MODE_I2C:
			code = msofti2c_erase_check_code;
			code_size = sizeof(msofti2c_erase_check_code);
			cmd = msoftspi_info->i2c_addr;
			break;
	...
	/* this will overlay the last words of msoft*_erase_check_code in target */
	...
	while (sector < bank->num_sectors) {
		...
		buf_set_u32(reg_params[0].value, 0, 32, count);
		buf_set_u32(reg_params[1].value, 0, 32, msoftspi_info->dev.sectorsize);
		buf_set_u32(reg_params[2].value, 0, 32,
			(msoftspi_info->mode == MODE_I2C) ? I2C_ADDR_BYTES : SPI_ADDR_BYTES);
		...
		retval = target_run_algorithm(target,
			...
			4 * count * (bank->sectors[sector].size >>
				((msoftspi_info->mode == MODE_I2C) ? 0 : 6)),
			&armv7m_info);
		if (retval != ERROR_OK)
			break;
...
err:
	if (msoftspi_info->mode == MODE_I2C) {
		/* error recovery */
		msoftspi_i2c_resync(bank);
	} else {
		/* set to input */
		set_to_output(bank, false);

		/* set NCS, ignoring errors */
		(void) target_read_u32(target, msoftspi_info->ncs.addr, &port);
		SET_PORT_BIT(port, ncs);
	}
		...
		uint32_t offset, uint32_t count, bool write)
{
	...
	/* see contrib/loaders/flash/msoftspi/msoftspi_read.S for src */
	static const uint8_t msoftspi_read_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msoftspi_read.inc"
	};

	/* see contrib/loaders/flash/msoftspi/msoftspi_write.S for src */
	static const uint8_t msoftspi_write_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msoftspi_write.inc"
	};

	/* see contrib/loaders/flash/msoftspi/msoftdpi_read.S for src */
	static const uint8_t msoftdpi_read_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msoftdpi_read.inc"
	};

	/* see contrib/loaders/flash/msoftspi/msoftdpi_write.S for src */
	static const uint8_t msoftdpi_write_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msoftdpi_write.inc"
	};

	/* see contrib/loaders/flash/msoftspi/msoftqpi_read.S for src */
	static const uint8_t msoftqpi_read_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msoftqpi_read.inc"
	};

	/* see contrib/loaders/flash/msoftspi/msoftqpi_write.S for src */
	static const uint8_t msoftqpi_write_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msoftqpi_write.inc"
	};

	/* see contrib/loaders/flash/msoftspi/msofti2c_read.S for src */
	static const uint8_t msofti2c_read_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msofti2c_read.inc"
	};

	/* see contrib/loaders/flash/msoftspi/msofti2c_write.S for src */
	static const uint8_t msofti2c_write_code[] = {
		#include "../../../contrib/loaders/flash/msoftspi/msofti2c_write.inc"
	};
	...
	switch (msoftspi_info->mode) {
		case MODE_SPI:
			...
			cmd = write ? msoftspi_info->dev.pprog_cmd : msoftspi_info->dev.read_cmd;
			break;

		case MODE_DPI:
			code = write ? msoftdpi_write_code : msoftdpi_read_code;
			code_size = write ? sizeof(msoftdpi_write_code) : sizeof(msoftdpi_read_code);
			cmd = write ? msoftspi_info->dev.pprog_cmd : msoftspi_info->dev.qread_cmd;
			break;

		case MODE_QPI:
			code = write ? msoftqpi_write_code : msoftqpi_read_code;
			code_size = write ? sizeof(msoftqpi_write_code) : sizeof(msoftqpi_read_code);
			cmd = write ? msoftspi_info->dev.pprog_cmd : msoftspi_info->dev.qread_cmd;
			break;

		case MODE_I2C:
			code = write ? msofti2c_write_code : msofti2c_read_code;
			code_size = write ? sizeof(msofti2c_write_code) : sizeof(msofti2c_read_code);
			cmd = msoftspi_info->i2c_addr;
			break;
	...
	fifosize = msoftspi_info->dev.sectorsize ?
		msoftspi_info->dev.sectorsize : msoftspi_info->dev.size_in_bytes;
	pagesize = msoftspi_info->dev.pagesize;
	if (pagesize == 0)
	...
	buf_set_u32(reg_params[1].value, 0, 32,
		write ? pagesize : msoftspi_info->dev.sectorsize);
	...
	uint32_t offset, uint32_t count)
{
	...
	return msoftspi_read_write_block(bank, (uint8_t *) buffer, offset, count, true);
}
	...
	uint32_t offset, uint32_t count)
{
	...
	return msoftspi_read_write_block(bank, buffer, offset, count, false);
}
...
static int read_flash_id(struct flash_bank *bank, uint32_t *id)
{
	...
	*id = 0;
	...
	if ((msoftspi_info->mode != MODE_SPI) &&
		(msoftspi_info->mode != MODE_DPI) &&
		(msoftspi_info->mode != MODE_QPI))
		return ERROR_FAIL;
	...
	/* clear CLK */
	retval = target_read_u32(target, msoftspi_info->clk.addr, &port);
	if (retval != ERROR_OK)
		goto err;
...
static int msoftspi_get_io_def(struct flash_bank *bank)
{
	struct msoftspi_flash_bank *msoftspi_info = bank->driver_priv;
	struct target *target = bank->target;
	uint32_t dir_reg;
	...
	do {
		...
		if ((msoftspi_info->io[k].addr != 0) && (msoftspi_info->io[k].mask != 0)) {
			dir_reg = msoftspi_info->io[k].addr
				+ ((int8_t) ((msoftspi_info->dir_off >> (k << 3)) & 0xFF));
			retval = target_read_u32(target, dir_reg, &msoftspi_info->io_def[k]);
			if (retval != ERROR_OK) {
				LOG_ERROR("dir register at 0x%08" PRIx32 " not readable", dir_reg);
				return retval;
			} else
				LOG_DEBUG("dir register %d at 0x%08" PRIx32 ": 0x%08" PRIx32,
					k, dir_reg, msoftspi_info->io_def[k]);
		}
	} while (k-- > 0);
...
static int msoftspi_probe(struct flash_bank *bank)
{
	...
	if (msoftspi_info->probed) {
		...
		if (bank->sectors)
			free(bank->sectors);
		bank->sectors = NULL;
		msoftspi_info->probed = false;
	}
	memset(&msoftspi_info->dev, 0, sizeof(msoftspi_info->dev));

	retval = msoftspi_get_io_def(bank);
	if (retval != ERROR_OK)
		return retval;
		...
		case MODE_QPI:
			...
			for (p = flash_devices; id && p->name ; p++) {
				if (p->device_id == id) {
					...
					LOG_INFO("flash \'%s\' id = 0x%06" PRIx32 ", size = %lukbytes",
			...
			if (msoftspi_info->dev.sectorsize == 0)
				msoftspi_info->dev.sectorsize = msoftspi_info->dev.size_in_bytes;
			/* if no pagesize, then use sectorsize instead */
			if (msoftspi_info->dev.pagesize == 0)
				msoftspi_info->dev.pagesize = msoftspi_info->dev.sectorsize;

			bank->num_sectors = msoftspi_info->dev.size_in_bytes / msoftspi_info->dev.sectorsize;
...
static int get_msoftspi_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	snprintf(buf, buf_size, "flash \'%s\', device id = 0x%06" PRIx32
			...
			", mass_erase = 0x%02" PRIx8 ", sector size = %ld%sBytes"
			", sector_erase = 0x%02" PRIx8 ")",
			msoftspi_info->dev.name, msoftspi_info->dev.device_id,
			(bank->size > (1UL << 12)) ? bank->size >> 10 : bank->size,
			(bank->size > (1UL << 12)) ? "k" : "", msoftspi_info->dev.pagesize,
			msoftspi_info->dev.read_cmd, msoftspi_info->dev.qread_cmd,
			msoftspi_info->dev.pprog_cmd, msoftspi_info->dev.chip_erase_cmd,
			(msoftspi_info->dev.sectorsize > (1UL << 12)) ?
				msoftspi_info->dev.sectorsize >> 10 : msoftspi_info->dev.sectorsize,
			(msoftspi_info->dev.sectorsize > (1UL << 12)) ? "k" : "",
			msoftspi_info->dev.erase_cmd);

	return ERROR_OK;
}

COMMAND_HANDLER(msoftspi_handle_mass_erase)
{
...
err:
		...
		count = msoftspi_info->dev.size_in_bytes;
		...
		for (offset = 0; offset < msoftspi_info->dev.size_in_bytes; offset += count) {
			retval = msoftspi_read_write_block(bank, buffer, offset, count, true);
			if (retval != ERROR_OK)
				break;
		}
		free(buffer);
...
COMMAND_HANDLER(msoftspi_handle_set)
{
	...
	if (index + 2 > CMD_ARGC) {
		command_print(CMD_CTX, "msoftspi: not enough arguments");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}

	COMMAND_PARSE_NUMBER(u32, CMD_ARGV[index++], temp);
	msoftspi_info->dev.size_in_bytes = temp;
	if (log2u(msoftspi_info->dev.size_in_bytes) < 7) {
		command_print(CMD_CTX, "msoftspi: device size must be 2^n with n >= 7");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}

	COMMAND_PARSE_NUMBER(u32, CMD_ARGV[index++], msoftspi_info->dev.pagesize);
	if (msoftspi_info->dev.pagesize == 0)
		msoftspi_info->dev.pagesize = 1;
	if ((msoftspi_info->dev.pagesize > msoftspi_info->dev.size_in_bytes) ||
	...
	switch (msoftspi_info->mode) {
		...
		case MODE_QPI:
			...
			COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], msoftspi_info->dev.read_cmd);
			if ((msoftspi_info->dev.read_cmd != 0x03) &&
				(msoftspi_info->dev.read_cmd != 0x13)) {
				command_print(CMD_CTX, "msoftspi: only 0x03/0x13 READ allowed");
				return ERROR_COMMAND_SYNTAX_ERROR;
			}

			COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], msoftspi_info->dev.qread_cmd);
			if ((msoftspi_info->dev.qread_cmd != 0x00) &&
				(msoftspi_info->dev.qread_cmd != 0x0B) &&
				(msoftspi_info->dev.qread_cmd != 0x0C) &&
				(msoftspi_info->dev.qread_cmd != 0x3B) &&
				(msoftspi_info->dev.qread_cmd != 0x3C) &&
				(msoftspi_info->dev.qread_cmd != 0x6B) &&
				(msoftspi_info->dev.qread_cmd != 0x6C) &&
				(msoftspi_info->dev.qread_cmd != 0xBB) &&
				(msoftspi_info->dev.qread_cmd != 0xBC) &&
				(msoftspi_info->dev.qread_cmd != 0xEB) &&
				(msoftspi_info->dev.qread_cmd != 0xEC)) {
				command_print(CMD_CTX, "msoftspi: only 0x0B/0x0C/0x3B/0x3C/"
					"0x6B/0x6C/0xBB/0xBC/0xEB/0xEC QREAD allowed");
				return ERROR_COMMAND_SYNTAX_ERROR;
			}

			COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], msoftspi_info->dev.pprog_cmd);
			if ((msoftspi_info->dev.pprog_cmd != 0x02) &&
				(msoftspi_info->dev.pprog_cmd != 0x12) &&
				(msoftspi_info->dev.pprog_cmd != 0x32)) {
				command_print(CMD_CTX, "msoftspi: only 0x02/0x12/0x32 PPRG allowed");
				return ERROR_COMMAND_SYNTAX_ERROR;
			}

			if (index < CMD_ARGC)
				COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], msoftspi_info->dev.chip_erase_cmd);
			else
			...
			if (index < CMD_ARGC) {
				...
				if ((msoftspi_info->dev.sectorsize > msoftspi_info->dev.size_in_bytes) ||
					(msoftspi_info->dev.sectorsize < msoftspi_info->dev.pagesize) ||
				...
				if (index < CMD_ARGC)
					COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], msoftspi_info->dev.erase_cmd);
				else {
					command_print(CMD_CTX, "msoftspi: erase command missing");
					return ERROR_COMMAND_SYNTAX_ERROR;
				}
			} else {
		...
		case MODE_I2C:
			if (index + 3  > CMD_ARGC) {
				command_print(CMD_CTX, "msoftspi: missing I2C addr, bits or delay ");
				return ERROR_COMMAND_SYNTAX_ERROR;
			}

			COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], msoftspi_info->i2c_addr);
			if ((msoftspi_info->i2c_addr & 0xF0) != I2C_ADDR_EEPROM_NORM)
				LOG_INFO("unusual I2C address - please check");
			if (msoftspi_info->i2c_addr & 0x1) {
				LOG_ERROR("R/W bit must be cleared");
				return ERROR_COMMAND_SYNTAX_ERROR;
			}

			COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], msoftspi_info->i2c_bits);
			if (msoftspi_info->i2c_bits > 3) {
				LOG_ERROR("at most 3 memory address bits in I2C address");
				return ERROR_FAIL;
			}
			if (msoftspi_info->i2c_bits != 0) {
				/* consistency check: if any memory address bit in I2C address */
				if ((log2u(msoftspi_info->dev.size_in_bytes) - msoftspi_info->i2c_bits) & 0x7) {
				...
				if (msoftspi_info->i2c_addr & ((1UL << msoftspi_info->i2c_bits) - 1) << 1) {
			...
			COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], msoftspi_info->dummy);

			/* just in case ... */
			retval = msoftspi_i2c_resync(bank);
			if (retval != ERROR_OK) {
			...
			i2c_ack = msoftspi_shift_out(bank, msoftspi_info->i2c_addr | 0x0);
			msoftspi_i2c_start_stop(bank, false);
			if (retval != ERROR_OK) {
			...
			LOG_INFO("i2c device at 0x%02" PRIx8 " detected", msoftspi_info->i2c_addr);
	...
	if (index < CMD_ARGC) {
		command_print(CMD_CTX, "msoftspi: extra arguments");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
	...
	if (msoftspi_info->dev.size_in_bytes >> 10)
		LOG_INFO("flash \'%s\' id = unknown\nflash size = %lukbytes",
			msoftspi_info->dev.name, msoftspi_info->dev.size_in_bytes >> 10);
	else
		LOG_INFO("flash \'%s\' id = unknown\nflash size = %lubytes",
			msoftspi_info->dev.name, msoftspi_info->dev.size_in_bytes);
	msoftspi_info->probed = true;

	return ERROR_OK;
}

COMMAND_HANDLER(msoftspi_handle_cmd)
{
	struct target *target = NULL;
	struct flash_bank *bank;
	struct msoftspi_flash_bank *msoftspi_info;
	...
	COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], num_bytes);
	...
	switch (msoftspi_info->mode) {
		...
		case MODE_QPI:
			...
			snprintf(output, sizeof(output), "%s: %02" PRIx8 " ",
				(msoftspi_info->mode == MODE_SPI) ? "spi" :
					(msoftspi_info->mode == MODE_DPI) ? "dpi" : "qpi", cmd_byte & 0xFF);
		...
		case MODE_I2C:
			snprintf(output, sizeof(output), "i2c 0x%2" PRIx8 " ",
				msoftspi_info->i2c_addr);
			msoftspi_i2c_resync(bank);
			...
			if (index < CMD_ARGC) {
				/* there are bytes to write to slave, send addr with R/W = 0 */
				retval = msoftspi_shift_out(bank, msoftspi_info->i2c_addr | 0x0);
				if (retval != ERROR_OK) {
					if (retval == ERROR_FLASH_OPERATION_FAILED)
						LOG_ERROR("i2c no ack on address for write");
					goto err;
				}
			...
			if (num_bytes > 0) {
				/* there are bytes to read from slave, send addr with R/W = 1 */
				retval = msoftspi_shift_out(bank, msoftspi_info->i2c_addr | 0x1);
				if (retval != ERROR_OK) {
					if (retval == ERROR_FLASH_OPERATION_FAILED)
						LOG_ERROR("i2c no ack on address for read");
					goto err;
				}
			}
...
COMMAND_HANDLER(msoftspi_handle_spi)
{
	struct flash_bank *bank;
	struct msoftspi_flash_bank *msoftspi_info;
	int retval;
...
COMMAND_HANDLER(msoftspi_handle_dpi)
{
	struct flash_bank *bank;
	struct msoftspi_flash_bank *msoftspi_info;
	int retval;
	...
	if (msoftspi_info->dev.qread_cmd == 0)
		LOG_WARNING("DPI not available for this device");

	/* number of dummy clocks */
	COMMAND_PARSE_NUMBER(u8, CMD_ARGV[1], msoftspi_info->dummy);
...
COMMAND_HANDLER(msoftspi_handle_qpi)
{
	struct flash_bank *bank;
	struct msoftspi_flash_bank *msoftspi_info;
	int retval;
	...
	if ((msoftspi_info->io[3].addr == 0) ||
		(msoftspi_info->io[2].addr == 0)) {
		command_print(CMD_CTX, "msoftspi: no IO2/IO3, QPI mode not possible");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
...
COMMAND_HANDLER(msoftspi_handle_calib)
{
	struct flash_bank *bank;
	struct msoftspi_flash_bank *msoftspi_info;
	...
	dummy = msoftspi_info->dummy;

	/* use this value for measurement */
	COMMAND_PARSE_NUMBER(u8, CMD_ARGV[1], delay);
	msoftspi_info->dummy = delay;

	msoftspi_i2c_calib(bank, 1000, &time1);
	msoftspi_i2c_calib(bank, 10000, &time2);

	/* restore old value */
	msoftspi_info->dummy = dummy;
...
COMMAND_HANDLER(msoftspi_handle_scan)
{
	struct flash_bank *bank;
	struct msoftspi_flash_bank *msoftspi_info;
	int retval;
...
static const struct command_registration msoftspi_exec_command_handlers[] = {
	{
		.name = "mass_erase",
		.handler = msoftspi_handle_mass_erase,
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
...
struct flash_driver msoftspi_flash = {
	...
	.usage = "$_FLASHNAME msoftspi base 0 0 0 $_TARGETNAME inp_off dir_off "
		"ncs_addr ncs_bit clk_addr clk_bit "
		"io3_addr io3_bit io3_exor io2_addr io2_bit io2_exor "
		"io1_addr io1_bit io1_exor io0_addr io0_bit io0_exor",
};
