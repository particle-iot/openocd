...
static int lpcspifi_set_sw_mode(struct flash_bank *bank) {
	...
	//IOCONFIG_WRITE_REG(0x400F6020, 0x00000800);
	//IOCONFIG_WRITE_REG(0x400F6004, 0x00018000);

	//SSP_WRITE_REG()
	LOG_DEBUG("Initializing SSP");
...
static int read_status_reg(struct flash_bank *bank, uint32_t *status)
{
	...
	SSP_WRITE_REG(SSP_DATA, 0x00); //Dummy write to clock in the register
	SSP_POLL_BUSY(SSP_CMD_TIMEOUT);
	SSP_STOP_COMMAND();
...
static int lpcspifi_read_flash_id(struct flash_bank *bank, uint32_t *id)
{
	...
	SSP_WRITE_REG(SSP_DATA, 0x00); //Dummy write to clock in data
	SSP_POLL_BUSY(SSP_CMD_TIMEOUT);
	...
	//*id = SSP_READ_REG(SSP_DATA) & 0x00ffffff;
	return ERROR_OK;
}
