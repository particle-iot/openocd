...
#define FLASH_BOOTCUR   0x40
#define FLASH_BOOTPRG   0x44
...
static int stm32x_read_options(struct flash_bank *bank)
{
	...
	/* read boot addresses */
	retval = target_read_u32(target, FLASH_REG_BASE_B0 + FLASH_BOOTCUR, &stm32x_info->option_bytes.boot_addr);
	if (retval != ERROR_OK)
		return retval;
...
static int stm32x_write_options(struct flash_bank *bank)
{
	...
	retval = target_read_u32(target, FLASH_REG_BASE_B0 + FLASH_OPTCUR, &optiondata);
	if (retval != ERROR_OK)
		return retval;
	...
	retval = target_write_u32(target, FLASH_REG_BASE_B0 + FLASH_BOOTPRG, stm32x_info->option_bytes.boot_addr);
	if (retval != ERROR_OK)
		return retval;
...
COMMAND_HANDLER(stm32x_handle_options_read_command)
{
	...
	if (CMD_ARGC != 1) {
		command_print(CMD_CTX, "stm32h7x options_read <bank>");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
	...
	command_print(CMD_CTX, "stm32h7x user_options 0x%02" PRIX8
			" user2_options 0x%02" PRIX8
			" user3_options 0x%02" PRIX8
			" boot_add0 0x%04" PRIX32
...
COMMAND_HANDLER(stm32x_handle_options_write_command)
{
	...
	if (CMD_ARGC < 1) {
		command_print(CMD_CTX, "stm32h7x options_write <bank> ...");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
	...
	if (CMD_ARGC != 4 && CMD_ARGC != 6) {
		command_print(CMD_CTX,
				"stm32h7x options_write <bank> <user_options> <user2_options> <user3_options> [ <boot_add0> <boot_add1> ]");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
	COMMAND_PARSE_NUMBER(u8, CMD_ARGV[1], info->option_bytes.user_options);
	if (info->option_bytes.user_options & 0x03) {
		command_print(CMD_CTX, "stm32h7x invalid user_options: bits 0 and 1 cannot be set");
	...
	if (info->option_bytes.user3_options & 0x5c) {
		command_print(CMD_CTX, "stm32h7x invalid user3_options: bits 2, 3, 4, and 6 cannot be set");
		return ERROR_COMMAND_ARGUMENT_INVALID;
	}
	if (CMD_ARGC == 6) {
		uint16_t boot_addr0, boot_addr1;
		COMMAND_PARSE_NUMBER(u16, CMD_ARGV[4], boot_addr0);
		COMMAND_PARSE_NUMBER(u16, CMD_ARGV[5], boot_addr1);
		info->option_bytes.boot_addr = boot_addr0 | (((uint32_t) boot_addr1) << 16);
	}
...
static const struct command_registration stm32x_exec_command_handlers[] = {
	...
	},
	{
		...
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
		.help = "Read and display device opton bytes.",
	},
	{
		.name = "options_write",
		.handler = stm32x_handle_options_write_command,
		.mode = COMMAND_EXEC,
		.usage = "bank_id user_options user2_options user3_options [ boot_add0 boot_add1 ]",
