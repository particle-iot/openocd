...
static int stm32x_read_options(struct flash_bank *bank)
{
	...
	stm32x_info->option_bytes.rdp = (option_bytes & (1 << OPT_READOUT)) ? 0xFF : 0xA5;
	...
	retval = target_read_u32(target, STM32_FLASH_WRPR_B0, &option_bytes);
	if (retval != ERROR_OK)
		return retval;

	stm32x_info->option_bytes.protection = option_bytes;
