 ...
 */
const struct psoc4_chip_detials psoc4_devices[] = {
	/* 4200 series */
...
static const struct psoc4_chip_detials * psoc4_details_by_id(uint32_t silicon_id)
{
	const struct psoc4_chip_detials *p = psoc4_devices;
	unsigned int i;
...
static int psoc4_get_silicon_id(struct target *target, uint32_t *silicon_id, uint8_t *protection)
{
	...
	if (silicon_id) *silicon_id= silicon;
...
COMMAND_HANDLER(psoc4_handle_flash_autoerase_command)
{
	...
	if (enable) {
		...
		LOG_INFO("Flash auto-erase enabled, no need to erase before.");
	} else {
...
static int psoc4_probe(struct flash_bank *bank)
{
	...
	const struct psoc4_chip_detials *details = psoc4_details_by_id(silicon_id);
	if (details) {
...
static int get_psoc4_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	const struct psoc4_chip_detials *details= psoc4_details_by_id(psoc4_info->silicon_id);
