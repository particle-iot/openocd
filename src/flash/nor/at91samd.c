...
static int samd_issue_nvmctrl_command(struct target *target, uint16_t cmd)
{
	...
	/* Disable caching */
	uint16_t tmp = 0;
	target_read_u16(target, SAMD_NVMCTRL + SAMD_NVMCTRL_CTRLB, &tmp);
	target_write_u16(target, SAMD_NVMCTRL + SAMD_NVMCTRL_CTRLB, tmp | (1<<18));

	int res = target_write_u16(target, SAMD_NVMCTRL + SAMD_NVMCTRL_CTRLA,
			SAMD_NVM_CMD(cmd));
