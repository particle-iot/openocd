...
COMMAND_HELPER(get_u64_from_hexarg, unsigned int num, uint64_t *value)
{
	if (num >= CMD_ARGC) {
...
static const char str_nvmuserrow_option_all[] = "all";
static const char str_nvmuserrow_option_lock[] = "lock";
COMMAND_HANDLER(samd_handle_nvmuserrow_command)
{
	...
	if (target) {
		...
		if (CMD_ARGC > 0) {
			...
			int index_value = -1;
			int index_mask = -1;
			uint64_t mask_reserved;
			res = samd_get_reservedmask(target, &mask_reserved);
			if (res != ERROR_OK) {
				LOG_ERROR("Couldn't get the mask for reserved bits");
				return ERROR_FAIL;
			}

			uint64_t value;
			const uint64_t mask_orig = mask_reserved & NVMUSERROW_LOCKBIT_MASK;
			uint64_t mask = mask_orig;

			for (i = 0 ; i < CMD_ARGC ; i++) {
				if (strcasecmp(str_nvmuserrow_option_all, CMD_ARGV[i]) == 0) {
					if (mask != mask_orig) { /* mask already changed */
error_multiple_mask:
						command_print(CMD_CTX,
							"Multiple mask options at argument %d.", i);
						return ERROR_COMMAND_SYNTAX_ERROR;
					}
					mask = UINT64_MAX;
				} else if (strcasecmp(str_nvmuserrow_option_lock, CMD_ARGV[i]) == 0) {
					if (mask != mask_orig)  /* mask already changed */
						goto error_multiple_mask;
					...
					command_print(CMD_CTX, "Too much arguments.");
					return ERROR_COMMAND_SYNTAX_ERROR;
				}
			}

			if (index_value == -1) {
				command_print(CMD_CTX, "At least the value is needed.");
				return ERROR_COMMAND_SYNTAX_ERROR;
			}

			res = CALL_COMMAND_HANDLER(get_u64_from_hexarg, index_value, &value);
			if (res != ERROR_OK)
				return res;
			if (index_mask != -1) {
				uint64_t mask_temp;
				res = CALL_COMMAND_HANDLER(get_u64_from_hexarg, index_mask, &mask_temp);
				if (res != ERROR_OK)
					return res;
				mask &= mask_temp;
			}
			res = samd_modify_user_row_masked(target, value, mask);
		} else { /* No Arguments, therefore read register */
			uint64_t value;
...
static const struct command_registration at91samd_exec_command_handlers[] = {
	...
	},
	{
		...
		.help = "Show or set the nvmuserrow register. It is 64 bit wide "
			...
			"in background. Use the argument \"lock\" to change the lockbits "
			"too. Using the argument \"all\" is not recommended as it lets "
			"you even change the reserved calibration bits.",
	},
