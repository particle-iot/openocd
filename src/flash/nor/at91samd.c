...
struct samd_family {
	...
	size_t num_parts;
	uint64_t nvm_userrow_res_mask; /* protect bits which are reserved */
};
		...
		uint64_t value_input, uint64_t value_mask)
{
	...
	assert(page_size > 0 && page_size <= SAMD_PAGE_SIZE_MAX && page_size >= sizeof(value_input));

	/* Make sure we're within the single page that comprises the User Row. */
	if (page_size < sizeof(value_input)) {
	...
	uint64_t value_device = buf_get_u64(buf, 0, 64);
	...
	/* Modify */
	buf_set_u64(buf, 0, 64, value_new);
...
static int samd_read_userrow(struct target *target, uint64_t *value)
{
	int res;
	uint32_t val_lo, val_hi;
	res = target_read_u32(target, SAMD_USER_ROW, &val_lo);
	if (res != ERROR_OK)
		return res;
	res = target_read_u32(target, SAMD_USER_ROW + 4, &val_hi);
	*value = (((uint64_t)val_hi) << 32) | (uint64_t)val_lo;
	return res;
}
...
COMMAND_HANDLER(samd_handle_nvmuserrow_command)
{
	...
	if (target) {
		...
		if (CMD_ARGC >= 1) {
			...
			if (CMD_ARGC >= 2) {
				res = CALL_COMMAND_HANDLER(get_u64_from_hexarg, 1, &mask);
				if (res != ERROR_OK)
					return res;
			} else if (CMD_ARGC > 2) {
				command_print(CMD_CTX, "Invalid amount of Arguments, should be 1 or 2.");
				return ERROR_COMMAND_SYNTAX_ERROR;
			}
			...
			uint64_t value;
			res = samd_read_userrow(target, &value);
			if (res == ERROR_OK)
...
static const struct command_registration at91samd_exec_command_handlers[] = {
	...
	},
	{
		...
		.help = "Show or set the nvmuserrow register. It is 64 bit wide "
			...
			"to prevent change at positions where the of mask bitvalue is zero.",
	},
