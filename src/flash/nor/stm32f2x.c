...
#define OPT_NDBOOT    (1 << 28)	/* not dual boot mode */
#define OPT_NDBANK    (1 << 29)	/* not dual bank mode */
#define OPT_DB1M      (1 << 30)	/* 1 MiB devices dual flash bank option */
...
static int stm32x_probe(struct flash_bank *bank)
{
	...
	if ((device_id & 0xfff) == 0x419 || (device_id & 0xfff) == 0x434) {
		...
		if ((flash_size_in_kb > 1024) || (optiondata & OPT_DB1M)) {
			stm32x_info->has_large_mem = true;
