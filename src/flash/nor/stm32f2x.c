...
static int stm32x_unlock_reg(struct target *target, bool unlock_options)
{
	uint32_t ctrl;
...
static int stm32x_prepare_erase_options(struct flash_bank *bank)
{
	struct stm32x_flash_bank *stm32x_info = NULL;
...
static int stm32x_write_options(struct flash_bank *bank)
{
	...
	int retval = stm32x_unlock_reg(target, true);
	if (retval != ERROR_OK)
		return retval;
	...
	if (stm32x_info->has_large_mem) {
		...
		uint32_t option_save = optiondata;
		optiondata = 0;
		buf_set_u32(&optiondata, 16, 12, stm32x_info->option_bytes.protection >> 12);
		retval = target_write_u32(target, STM32_FLASH_OPTCR1, optiondata);
		if (retval != ERROR_OK)
			return retval;
		optiondata = option_save;
	}
...
static int stm32x_erase(struct flash_bank *bank, int first, int last)
{
	...
	int retval;
	retval = stm32x_unlock_reg(target, false);
	if (retval != ERROR_OK)
		return retval;
...
COMMAND_HANDLER(stm32x_handle_lock_command)
{
	...
	if (stm32x_prepare_erase_options(bank) != ERROR_OK) {
		command_print(CMD_CTX, "%s failed to erase options", bank->driver->name);
		return ERROR_OK;
	}
