...
#define STM32_OTP_SIZE 512
#define STM32_OTP_SECTOR_SIZE 32
#define STM32_OTP_BANK_BASE       0x1FFF7800
#define STM32_OTP_LOCK_BASE       (0x1FFF7800 + (STM32_OTP_SIZE))
...
{
	return bank->base == STM32_OTP_BANK_BASE;
}
...
FLASH_BANK_COMMAND_HANDLER(stm32x_flash_bank_command)
{
	...
	bank->bank_number = bank->base == FLASH_BANK_BASE ? 0 : 1;
...
static int stm32x_otp_read_protect(struct flash_bank *bank)
{
	...
	for (i = 0; i < bank->num_sectors; i++) {
		retval = target_read_u8(target, STM32_OTP_LOCK_BASE + i, &lock);
		if (retval != ERROR_OK)
			return retval;
...
static int stm32x_otp_protect(struct flash_bank *bank, int first, int last)
{
	...
	for (i = first; first <= last; i++) {
		...
		lock = 0xff;
		retval = target_write_u8(target, STM32_OTP_LOCK_BASE + i, lock);
		if (retval != ERROR_OK)
			return retval;
	}
...
static int stm32x_probe(struct flash_bank *bank)
{
	...
	if (stm32x_is_otp(bank)) {
		flash_size_in_b = STM32_OTP_SIZE;
		LOG_INFO("flash size = %d bytes", flash_size_in_b);
		num_sectors = flash_size_in_b / STM32_OTP_SECTOR_SIZE;

		assert(num_sectors > 0);

		bank->num_sectors = num_sectors;
		bank->sectors = calloc(sizeof(struct flash_sector), num_sectors);
		bank->size = STM32_OTP_SIZE;
