				  ...
				  const uint8_t *buffer, uint32_t offset, uint32_t count)
{
	...
	size_t start_extra_length, end_extra_length;

	start_extra_length	= region.start % chip->code_page_size;
	end_extra_length	= region.end  % chip->code_page_size;

	uint32_t page_cnt = count / chip->code_page_size;
	uint8_t buffer_to_flash[(page_cnt+1)*chip->code_page_size];

	if (start_extra_length) {
		uint8_t page[chip->code_page_size];

		res = target_read_memory(bank->target,
					 region.start - start_extra_length,
					 1, start_extra_length, buffer_to_flash);
		if (res != ERROR_OK)
			return res;

		memcpy(buffer_to_flash, page, start_extra_length);

		region.start -= start_extra_length;
	}

	memcpy(buffer_to_flash+start_extra_length, buffer, count);
	...
	if (end_extra_length) {
		...
		res = target_read_memory(bank->target,
					 region.end,
					 1,
					 (chip->code_page_size - end_extra_length),
					 page);
		if (res != ERROR_OK)
			return res;

		memcpy(buffer_to_flash+start_extra_length+count, page, end_extra_length);

		region.end += chip->code_page_size - end_extra_length;
	}

	nrf51_write_pages(bank, region.start, region.end, buffer_to_flash);
