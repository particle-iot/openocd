...
static int nrf51_erase_page(struct nrf51_info *chip, struct flash_sector *sector)
{
	...
	if (sector->offset == NRF51_UICR_BASE) {
		...
		if ((ppfc & 0xFF) == 0xFF) {
			LOG_ERROR("The chip was not pre-programmed with SoftDevice stack and UICR cannot be erase individual. Please issue mass erase before trying to write to this region");
			return ERROR_FAIL;
		};
...
static int nrf51_ll_flash_write(struct nrf51_info *chip, uint32_t offset, const uint8_t *buffer, uint32_t buffer_size)
{
	...
	for (; buffer_size > 0; buffer_size -= 4) {
		res = target_write_u32(chip->target, offset, *(uint32_t *)buffer);
		if (res != ERROR_OK)
			return res;
...
FLASH_BANK_COMMAND_HANDLER(nrf51_flash_bank_command)
{
	...
	case NRF51_UICR_BASE:
		...
		break;
	default:
		LOG_ERROR("Address 0x%08" PRIx32 " invalid bank address", bank->base);
		return ERROR_FAIL;
	}
...
COMMAND_HANDLER(nrf51_handle_mass_erase_command)
{
	...
	/* If the user declareed UICR flash in their scripts, we go
	 * and mark it as erased */
	res = get_flash_bank_by_name("nrf51.uicr", &bank);
	if (res == ERROR_OK && bank)
