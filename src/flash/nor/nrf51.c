				  ...
				  const uint8_t *buffer, uint32_t offset, uint32_t count)
{
	int res;
	struct {
		uint32_t start, end, pre, post;
	} region;
	/* region.pre and region post are not offsets, instead defines required byte count to align to page. */
	region.start = offset;
	region.end   = offset + count;
	region.pre   = region.start % chip->code_page_size;
	region.post  = chip->code_page_size - (region.end  % chip->code_page_size);

	uint32_t page_cnt = count / chip->code_page_size;
	/* we might need to have one more page to write */
	uint8_t buffer_to_flash[(page_cnt+1)*chip->code_page_size];

	/*Prepare the content between page start and region start */
	if (region.pre) {
		uint8_t page[chip->code_page_size];
		/* Retrieve the full row contents from Flash */
		res = target_read_memory(bank->target,
					 region.start - region.pre,
					 1,
					 region.pre,
					 buffer_to_flash);
		if (res != ERROR_OK)
		...
		memcpy(buffer_to_flash, page, region.pre);
	}

	/*Prepare actual program binary */
	memcpy(buffer_to_flash+region.pre, buffer, count);

	/*Prepare the content between region end and page end */
	if (region.post) {
		uint8_t page[chip->code_page_size];
		...
		memcpy(buffer_to_flash+region.pre+count, page, region.post);

	}

	nrf51_write_pages(bank, region.start-region.pre, region.end+region.post, buffer_to_flash);
