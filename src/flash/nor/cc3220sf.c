...
static int cc3220sf_mass_erase(struct flash_bank *bank)
{
	...
	value = 0;
	retval = target_write_u32(target, FMA_REGISTER_ADDR, value);
	if (ERROR_OK != retval)
		return retval;

	/* Write the MERASE bit of the FMC register */
	value = FMC_DEFAULT_VALUE | FMC_MERASE_BIT;
	retval = target_write_u32(target, FMC_REGISTER_ADDR, value);
	if (ERROR_OK != retval)
		return retval;
	...
	uint32_t offset, uint32_t count)
{
	...
	retval = target_alloc_working_area(target, ALGO_BUFFER_SIZE,
				&buffer_working_area);
	if (ERROR_OK != retval)
		return retval;
