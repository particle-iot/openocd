	...
	uint32_t offset, uint32_t count)
{
	...
	algo_alloc_size = sizeof(cc3220sf_algo);

	/* Round algo_alloc_size up to nearest 32 word (0x80 byte) boundary */
	if ((algo_alloc_size % 0x80) != 0)
		algo_alloc_size = (algo_alloc_size & ~0x7f) + 0x80;

	/* Obtain working area to use for flash helper algorithm */
	retval = target_alloc_working_area(target, algo_alloc_size,
				&algo_working_area);
	if (ERROR_OK != retval)
		return retval;
