...
static int psoc5lp_eeprom_write(struct flash_bank *bank,
	const uint8_t *buffer, uint32_t offset, uint32_t byte_count)
{
	return ERROR_FLASH_OPER_UNSUPPORTED; /* TODO implement */
}
		...
		uint32_t offset, uint32_t byte_count)
{
	...
	int i, retval;
	static const uint8_t ecc_bytes[32] = { 0 };
	...
	if (byte_count % ROW_SIZE != 0) {
		LOG_ERROR("Must write in multiples of row size, got 0x%" PRIx32,
			byte_count);
		return ERROR_FAIL;
	}

	row_size = psoc_bank->ecc_enabled ? ROW_SIZE : (ROW_SIZE + 32);
	...
	for (array_id = offset / BLOCK_SIZE; byte_count > 0; array_id++) {
		for (row = (offset / ROW_SIZE) % ROWS_PER_BLOCK;
		     row < ROWS_PER_BLOCK && byte_count >= ROW_SIZE; row++) {
			bool even_row = (row % 2 == 0);
			struct working_area *data_area = even_row ? even_row_area : odd_row_area;

			LOG_DEBUG("Writing load command for array %u row %u at 0x%08" PRIx32,
