...
struct psoc5lp_flash_bank {
	...
	const struct psoc5lp_device *device;
	bool ecc_enabled;
	/* If ecc is disabled, num_sector counts both std and ecc sectors.
	 * If ecc is enabled, num_sector indicates just the number of std sectors.
	 * However ecc sector descriptors bank->sector[num_sector..2*num_sector-1]
