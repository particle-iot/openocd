...
static int clrsetbits_le32(struct target *target, uint32_t reg,
...
static int rpc_hf_mode(struct target *target, bool man, bool wb)
{
	uint32_t val;
	int ret;
	...
	ret = clrsetbits_le32(target, rpc_base + RPC_PHYCNT,
				RPC_PHYCNT_WBUF | RPC_PHYCNT_WBUF2 |
				RPC_PHYCNT_CAL | RPC_PHYCNT_MEM(3),
				(wb ? (RPC_PHYCNT_WBUF | RPC_PHYCNT_WBUF2) : 0) |
				RPC_PHYCNT_CAL | RPC_PHYCNT_MEM(3));

	ret = clrsetbits_le32(target, rpc_base + RPC_CMNCR,
				   ...
				   const uint8_t *buffer)
{
	...
	uint32_t wdata = buffer[0] | (buffer[1] << 8);
