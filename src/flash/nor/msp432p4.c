...
#define SUPPORT_MESSAGE \
	"Your pre-production MSP432P401R silicon is not fully supported.\n" \
...
static int msp432p4_device_type(uint32_t device_id, uint32_t hardware_rev)
{
	int device_type = MSP432P4_NO_TYPE;
	...
	switch (device_id) {
		...
		case 0xA004:
		case 0xA005:
			/* Device is definitely MSP432P401R, check hardware revision */
			if (hardware_rev == 0x41 || hardware_rev == 0x42) {
				/* Rev A or B of the silicon has been deprecated */
				device_type = MSP432P401R_DEPR;
			} else if (hardware_rev >= 0x43 && hardware_rev <= 0x49) {
				/* Current and future revisions of the MSP432P401R device */
				device_type = MSP432P401R;
			} else {
				/* Unknown or unanticipated hardware revision */
				device_type = MSP432P401R_GUESS;
			}
			break;
		case 0xA010:
		case 0xA012:
		...
		default:
			if (device_id < 0xA010) {
				/* Best guess is this device is an MSP432P401R */
				device_type = MSP432P401R_GUESS;
			} else {
				/* Best guess is this device is an MSP432P4111 */
				device_type = MSP432P4111_GUESS;
			}
			break;
	}
...
static const char *msp432p4_return_text(uint32_t return_code)
{
	switch (return_code) {
		case FLASH_BUSY:
			return "FLASH_BUSY";
...
static int msp432p4_init(struct flash_bank *bank)
{
	...
	switch (msp432p4_bank->device_type) {
		case MSP432P401R:
		case MSP432P401R_DEPR:
		case MSP432P401R_GUESS:
		default:
			loader_code = msp432p401r_algo;
			loader_size = sizeof(msp432p401r_algo);
			break;
		case MSP432P4111:
	...
	if (MSP432P401R_GUESS == msp432p4_bank->device_type ||
		MSP432P4111_GUESS == msp432p4_bank->device_type) {
		/* Explicit device type check failed. Report this. */
		LOG_WARNING(
			"msp432p4.flash: Unrecognized MSP432P4 Device ID and Hardware "
			"Rev (%04X, %02X)", msp432p4_bank->device_id,
			msp432p4_bank->hardware_rev);
	} else if (MSP432P401R_DEPR == msp432p4_bank->device_type) {
		LOG_WARNING(
			"msp432p4.flash: MSP432P401R pre-production device (deprecated "
			"silicon)\n" SUPPORT_MESSAGE);
	}
...
FLASH_BANK_COMMAND_HANDLER(msp432p4_flash_bank_command)
{
	...
	msp432p4_bank->device_type = MSP432P4_NO_TYPE;
...
static int msp432p4_probe(struct flash_bank *bank)
{
	...
	msp432p4_bank->device_type = msp432p4_device_type(msp432p4_bank->device_id,
		msp432p4_bank->hardware_rev);
	...
	if (0 == bank_id) {
		...
		if (msp432p4_bank->device_type == MSP432P4111 ||
			msp432p4_bank->device_type == MSP432P4111_GUESS) {
			/* MSP432P4111 has an info size register, use that for size */
			retval = target_read_u32(target, FLASH_INFO_SIZE_REG, &size);
			if (ERROR_OK != retval)
				return retval;
		} else {
			/* All other MSP432P4 devices have fixed info region size */
...
static int msp432p4_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	switch (msp432p4_bank->device_type) {
		case MSP432P401R_DEPR:
			if (0xFFFF == msp432p4_bank->device_id) {
				/* Very early pre-production silicon currently deprecated */
				printed = snprintf(buf, buf_size,
					"MSP432P401R pre-production device (deprecated silicon)\n"
					SUPPORT_MESSAGE);
			} else {
				/* Revision A or B silicon, also deprecated */
				printed = snprintf(buf, buf_size,
					"MSP432P401R Family Rev %c (deprecated silicon)\n"
					SUPPORT_MESSAGE, (char)msp432p4_bank->hardware_rev);
			}
			break;
		case MSP432P401R:
			printed = snprintf(buf, buf_size,
				"MSP432P401R Family Rev %c\n",
				(char)msp432p4_bank->hardware_rev);
			break;
		case MSP432P4111:
			printed = snprintf(buf, buf_size,
				"MSP432P4111 Family Rev %c\n",
				(char)msp432p4_bank->hardware_rev);
			break;
		case MSP432P401R_GUESS:
		case MSP432P4111_GUESS:
