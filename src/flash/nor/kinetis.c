...
static int kinetis_ftfx_command(struct flash_bank *bank, uint32_t w0,
				uint32_t w1, uint32_t w2, uint8_t *fstat)
{
	uint8_t buffer[12];
	int result, i;
	...
	for (i = 0; i < 50; i++) {
		result =
			target_read_memory(bank->target, 0x40020000, 1, 1, fstat);
...
static int kinetis_erase(struct flash_bank *bank, int first, int last)
{
	...
	for (i = first; i <= last; i++) {
		...
		result = kinetis_ftfx_command(bank, w0, w1, w2, &fstat);
	...
	if (kinfo->flash_class == FC_PFLASH) {
		...
		if (fstat & 0x01) {
			...
			for (i = 0; i < bank->num_sectors; i++) {
				...
				if (result == ERROR_OK) {
					bank->sectors[i].is_erased = !(fstat & 0x01);
				} else {
