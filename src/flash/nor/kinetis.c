...
COMMAND_HANDLER(kinetis_check_flash_security_status)
{
	...
	retval = dap_queue_ap_write(dap, MDM_REG_CTRL, MEM_CTRL_SYS_RES_REQ);
	if (retval != ERROR_OK) {
		LOG_ERROR("MDM: failed to request reset via MDM_REG_CTRL");
		goto fail;
	}
	...
	retval = kinetis_mdm_poll_register(dap, MDM_REG_STAT,
					   ...
					   0);
	if (retval != ERROR_OK) {
		LOG_ERROR("MDM: MDM_STAT_SYSRES in MDM_REG_STAT didn't clear");
		goto fail;
	}
	...
	if (!found)
		LOG_WARNING("MDM: unkonwn ID %08" PRIX32, val);
	...
	if (val & MDM_STAT_SYSSEC) {
		...
		LOG_WARNING("**** command and restart openocd.                             ****");
		LOG_WARNING("****                                                          ****");
		LOG_WARNING("*********** ATTENTION! ATTENTION! ATTENTION! ATTENTION! **********");
	} else {
