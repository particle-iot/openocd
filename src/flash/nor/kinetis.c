...
static int kinetis_read_part_info(struct flash_bank *bank)
{
	...
	if ((unsigned)bank->bank_number < num_pflash_blocks) {
		...
		kinfo->prog_base = 0x00800000 + bank->size * (bank->bank_number - first_nvm_bank);
		...
		kinfo->protection_block = (8 / num_nvm_blocks) * (bank->bank_number - first_nvm_bank);
...
static int kinetis_blank_check(struct flash_bank *bank)
{
	...
	if (kinfo->flash_class == FC_PFLASH || kinfo->flash_class == FC_FLEX_NVM) {
		...
		result = kinetis_ftfx_command(bank, FTFx_CMD_BLOCKSTAT, kinfo->prog_base, 0, 0, 0, 0,  0, 0, 0, 0, &ftfx_fstat);
