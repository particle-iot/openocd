...
COMMAND_HANDLER(kinetis_mdm_halt)
{
	...
	if (retval != ERROR_OK) {
		...
		retval = kinetis_mdm_poll_register(dap, MDM_REG_STAT, MDM_STAT_CORE_HALTED, MDM_STAT_CORE_HALTED);
		if (retval == ERROR_OK)
			return ERROR_OK;
	}

	LOG_INFO("MDM: halt timed out, trying to recover blank device from RESET/WDOG loop");

	while (true) {
		tries++;
		retval = kinetis_mdm_write_register(dap, MDM_REG_CTRL, MDM_CTRL_CORE_HOLD_RES);
		if (retval != ERROR_OK)
		...
		if (stat & MDM_STAT_SYSRES)
			break;

		if (tries > MDM_ACCESS_TIMEOUT) {
			LOG_ERROR("MDM: reset blank timed out");
			return ERROR_FAIL;
		}
	}

	LOG_INFO("MDM: reset blank succeded after %d attempts.", tries);
...
COMMAND_HANDLER(kinetis_mdm_resume)
{
	...
	if (!dap) {
		LOG_ERROR("Cannot perform resume with a high-level adapter");
		return ERROR_FAIL;
	}
