...
static int kinetis_protect(struct flash_bank *bank, int set, int first, int last)
{
	...
	if (kinfo->flash_class == FC_PFLASH) {
		if (((last - first + 1) * kinfo->sector_size) % kinfo->protection_size != 0) {
			LOG_ERROR("Incorrect number of sectors. The total size of protected area must be a multiple of %"PRId32,
		...
		if ((fprot & 1) == 0) {
			LOG_ERROR("Flash page @ 0x00000400 is protected. Flash protection settings can't be altered. Use mass erase");
			return ERROR_FAIL;
		}
