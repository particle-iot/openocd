...
COMMAND_HANDLER(kinetis_mdm_halt)
{
	...
	if (!dap) {
		LOG_ERROR("Cannot perform mdm halt with a high-level adapter");
		return ERROR_FAIL;
	}
...
COMMAND_HANDLER(kinetis_mdm_reset)
{
	...
	if (!dap) {
		LOG_ERROR("Cannot perform mdm reset with a high-level adapter");
		return ERROR_FAIL;
	}
...
COMMAND_HANDLER(kinetis_mdm_resume)
{
	...
	if (!dap) {
		LOG_ERROR("Cannot perform mdm resume with a high-level adapter");
		return ERROR_FAIL;
	}

	retval = kinetis_mdm_write_register(dap, MDM_REG_CTRL, 0);
	if (retval != ERROR_OK) {
		LOG_ERROR("MDM: failed to resume core");
		return retval;
	}
...
COMMAND_HANDLER(kinetis_mdm_mass_erase)
{
	 ...
	 * bit in the MDM-AP control register. Write the MDM-AP register
	 * to set the Debug Request bit. This will prevent the core from
	 * attempting to boot when the reset pin is released.
	 */
	retval = kinetis_mdm_write_register(dap, MDM_REG_CTRL, MDM_CTRL_DBG_REQ);
	if (retval != ERROR_OK) {
		LOG_ERROR("MDM: failed to halt core");
