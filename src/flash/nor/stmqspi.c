 ...
 *	   2015 by Andreas Bolsch <andreas.bolsch@mni.thm.de				   *
...
#define QSPI_CLEAR_TCF() QSPI_WRITE_REG(QSPI_FCR, (1UL<<QSPI_TCF))

/* QSPI_CCR values for various commands */
#define	QSPI_CCR_READ_STATUS ((QSPI_READ_REG(QSPI_CCR) & 0xF0000000) | \
	((QSPI_READ_MODE | QSPI_1LINE_MODE | QSPI_ADDR3 | SPIFLASH_READ_STATUS) & QSPI_NO_ADDR))

#define	QSPI_CCR_WRITE_STATUS ((QSPI_READ_REG(QSPI_CCR) & 0xF0000000) | \
	((QSPI_WRITE_MODE | QSPI_1LINE_MODE | QSPI_ADDR3 | SPIFLASH_WRITE_STATUS) & QSPI_NO_ADDR))

#define QSPI_CCR_WRITE_ENABLE ((QSPI_READ_REG(QSPI_CCR) & 0xF0000000) | \
	((QSPI_WRITE_MODE | QSPI_1LINE_MODE | QSPI_ADDR3 | SPIFLASH_WRITE_ENABLE) & \
	QSPI_NO_ADDR & QSPI_NO_DATA))

#define QSPI_CCR_SECTOR_ERASE ((QSPI_READ_REG(QSPI_CCR) & 0xF0000000) | \
	((QSPI_WRITE_MODE | QSPI_1LINE_MODE | QSPI_ADDR3 | \
...
#define QSPI_CCR_PAGE_WRITE ((QSPI_READ_REG(QSPI_CCR) & 0xF0000000) | \
	((QSPI_WRITE_MODE | QSPI_1LINE_MODE | QSPI_ADDR3 | SPIFLASH_PAGE_PROGRAM)))
...
static int poll_busy(struct target *target, uint32_t io_base, int timeout)
{
	...
	if ((QSPI_READ_REG(QSPI_SR) & (1UL<<QSPI_BUSY)) == 0)
		return ERROR_OK;
...
static int poll_tcf(struct target *target, uint32_t io_base, int timeout)
{
	...
	if (QSPI_READ_REG(QSPI_SR) & (1UL<<QSPI_TCF))
		return ERROR_OK;
...
static int qspi_write_enable(struct flash_bank *bank)
{
	...
	if (((stmqspi_info->dual_mask & ((1UL<<QSPI_DUAL_FLASH) | (1UL<<QSPI_FSEL_FLASH))) != (1UL<<QSPI_FSEL_FLASH))
	...
	if (((stmqspi_info->dual_mask & ((1UL<<QSPI_DUAL_FLASH) | (1UL<<QSPI_FSEL_FLASH))) != 0)
...
COMMAND_HANDLER(stmqspi_handle_mass_erase_command)
{
	...
	dual = (stmqspi_info->dual_mask & (1UL<<QSPI_DUAL_FLASH)) ? 1 : 0;
	...
	uint32_t count, const uint8_t *buffer)
{
	 ...
	 * 3 words follow till end of code, that makes exaclty 4 words */
	...
	uint32_t flash_offset, uint32_t count)
{
	...
	uint32_t ccr_buffer[] = {
		htole32(QSPI_CCR_READ_STATUS),
		htole32(QSPI_CCR_WRITE_ENABLE),
		htole32(QSPI_CCR_PAGE_WRITE),
	};
...
static int stmqspi_probe(struct flash_bank *bank)
{
	...
	uint32_t id1 = 0, id2 = 0; /* silence uninitialized warning */
	...
	LOG_DEBUG("Valid QSPI on device %s at 0x%" PRIx32 ", QSPI_CR 0x%"
		PRIx32 ", QSPI_CCR 0x%" PRIx32, target_device->name, bank->base,
