...
#define QSPI_CCR_PAGE_PROG ((QSPI_MODE & ~QSPI_DCYC_MASK & QSPI_NO_ALTB) | \
	(QSPI_WRITE_MODE | SPIFLASH_PAGE_PROGRAM))
...
/* Poll fifo threshold flag */
/* timeout in ms */
static int poll_ftf(struct target *target, uint32_t io_base, int timeout)
{
	int retval;
	long long endtime;

	if (QSPI_READ_REG(QSPI_SR) & (1<<QSPI_FTF))
		return ERROR_OK;
	...
	LOG_ERROR("Timeout while polling FTF");
	return ERROR_FLASH_OPERATION_FAILED;
}

/* Set to memory-mapped mode, e. g. after an error */
static int set_mm_mode(struct target *target, uint32_t io_base, uint32_t saved_ccr)
{
	int retval;

	/* Reset Address register bits 0 and 1, see various errata sheets */
	retval = target_write_u32(target, io_base + QSPI_AR, 0x0);
	if (retval != ERROR_OK)
		return retval;

	/* Abort any previous operation */
	retval = target_write_u32(target, io_base + QSPI_CR,
		QSPI_READ_REG(QSPI_CR) | (1<<QSPI_ABORT));
	if (retval != ERROR_OK)
		return retval;

	/* Wait for busy to be cleared */
	retval = poll_busy(target, io_base, QSPI_PROBE_TIMEOUT);
	if (retval != ERROR_OK)
		return retval;

	/* Finally switch to memory mapped mode */
	retval = target_write_u32(target, io_base + QSPI_CCR, saved_ccr);
	return retval;
}
...
static int read_status_reg(struct flash_bank *bank, uint32_t *status)
{
	...
	retval = target_write_u32(target, io_base + QSPI_FCR, (1<<QSPI_TCF));
	if (retval != ERROR_OK)
		goto err;

	/* Read one byte per chip */
	retval = target_write_u32(target, io_base + QSPI_DLR,
		(stmqspi_info->dual_mask & (1 << QSPI_DUAL_FLASH)) ? 1 : 0);
	if (retval != ERROR_OK)
		goto err;
	...
	*status = 0;
	...
	if ((stmqspi_info->dual_mask & ((1<<QSPI_DUAL_FLASH) | (1<<QSPI_FSEL_FLASH)))
		!= (1<<QSPI_FSEL_FLASH)) {
		/* get status of flash 1 in dual or flash 1 only */
		retval = target_read_u8(target, io_base + QSPI_DR, &data);
		if (retval != ERROR_OK)
			goto err;
		*status |= data;
	}
...
COMMAND_HANDLER(stmqspi_handle_mass_erase_command)
{
	...
	if (((stmqspi_info->dual_mask & ((1<<QSPI_DUAL_FLASH) | (1<<QSPI_FSEL_FLASH)))
		!= (1<<QSPI_FSEL_FLASH)) && ((status & SPIFLASH_BSY_BIT) == 0)) {
	...
	if (((stmqspi_info->dual_mask & ((1<<QSPI_DUAL_FLASH) | (1<<QSPI_FSEL_FLASH)))
		!= (1<<QSPI_FSEL_FLASH)) && (((status >> 8) & SPIFLASH_BSY_BIT) == 0)) {
...
err:
	...
	set_mm_mode(target, io_base, stmqspi_info->saved_ccr);
...
COMMAND_HANDLER(stmqspi_handle_setparms)
{
	...
	COMMAND_PARSE_NUMBER(u8, CMD_ARGV[4], read_cmd);
	if (read_cmd != SPIFLASH_READ) {
		command_print(CMD_CTX, "stmqspi: only default 0x%02X READ cmd supported", SPIFLASH_READ);
		return ERROR_COMMAND_SYNTAX_ERROR;
	}

	COMMAND_PARSE_NUMBER(u8, CMD_ARGV[5], pprg_cmd);
	if (pprg_cmd != SPIFLASH_PAGE_PROGRAM) {
		command_print(CMD_CTX, "stmqspi: only default 0x%02X PPRG cmd supported", SPIFLASH_PAGE_PROGRAM);
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
	...
	io_base = stmqspi_info->io_base;
	fsize = (QSPI_READ_REG(QSPI_DCR)>>QSPI_FSIZE_POS) & ((1U<<QSPI_FSIZE_LEN) - 1);
	LOG_DEBUG("FSIZE = 0x%04x", fsize);
	if (bank->size != (1U<<(fsize + 1)))
		LOG_WARNING("FSIZE field in QSPI_DCR doesn't match actual capacity.");
...
COMMAND_HANDLER(stmqspi_handle_spicmd)
{
	...
	if (num_read == 0) {
		/* write */
		/* send cmd byte */
		retval = target_write_u32(target, io_base + QSPI_DLR, ((uint32_t) num_write) - 2);
		if (retval != ERROR_OK)
			goto err;
		...
		for (count = 3; count < CMD_ARGC; count++) {
			...
			if (poll_ftf(target, io_base, QSPI_CMD_TIMEOUT) != ERROR_OK)
				goto err;
			snprintf(temp, sizeof(temp), "%02x ", data);
			retval = target_write_u8(target, io_base + QSPI_DR, data); \
		...
		retval = target_write_u32(target, io_base + QSPI_DLR, ((uint32_t) num_read) - 1);
		if (retval != ERROR_OK)
			goto err;
		retval = target_write_u32(target, io_base + QSPI_CCR,
			(QSPI_MODE & ~QSPI_DCYC_MASK & QSPI_NO_ALTB & ((num_write == 1) ? QSPI_NO_ADDR : ~0U)) |
			((QSPI_READ_MODE | (((num_write - 2) & 0x3U) << QSPI_ADSIZE_POS) | cmd_byte)));
		if (retval != ERROR_OK)
			goto err;

		if (num_write > 1) {
			/* if ADMODE indicates address required, only the write to AR triggers command */
			retval = target_write_u32(target, io_base + QSPI_AR, addr);
			if (retval != ERROR_OK)
				goto err;
		}
...
static int qspi_erase_sector(struct flash_bank *bank, int sector)
{
	...
	retval = target_write_u32(target, io_base + QSPI_AR, bank->sectors[sector].offset);
	if (retval != ERROR_OK)
		goto err;
	...
	if (((stmqspi_info->dual_mask & ((1<<QSPI_DUAL_FLASH) | (1<<QSPI_FSEL_FLASH)))
		!= (1<<QSPI_FSEL_FLASH)) && ((status & SPIFLASH_BSY_BIT) == 0)) {
		LOG_ERROR("Sector erase command not accepted by flash1. Status=0x%08" PRIx32, status);
		return ERROR_FAIL;
	}

	/* Check for command in progress for flash 2 */
	if (((stmqspi_info->dual_mask & ((1<<QSPI_DUAL_FLASH) | (1<<QSPI_FSEL_FLASH)))
		!= (0<<QSPI_FSEL_FLASH)) && (((status >> 8) & SPIFLASH_BSY_BIT) == 0)) {
		LOG_ERROR("Sector erase command not accepted by flash2. Status=0x%08" PRIx32, status);
		return ERROR_FAIL;
	}
		...
		uint32_t offset, uint32_t count)
{
	...
	static const uint8_t stmqspi_write_code[] = {
		,
...
static int read_flash_id(struct flash_bank *bank, uint32_t *id1, uint32_t *id2)
{
	...
	/* Abort any previous operation */
	retval = target_write_u32(target, io_base + QSPI_CR, QSPI_READ_REG(QSPI_CR) | (1<<QSPI_ABORT));
	if (retval != ERROR_OK)
		goto err;
	...
	retval = target_write_u32(target, io_base + QSPI_DLR,
		(stmqspi_info->dual_mask & (1<<QSPI_DUAL_FLASH) ? 6 : 3) - 1);
	if (retval != ERROR_OK)
		goto err;
	...
	/* Poll transmit finished flag */
	retval = poll_tcf(target, io_base, QSPI_CMD_TIMEOUT);
	if (retval != ERROR_OK)
		goto err;
	...
	for (shift = 0; shift <= 16; shift += 8) {
		if ((stmqspi_info->dual_mask & ((1<<QSPI_DUAL_FLASH) |
			...
			retval = target_read_u8(target, io_base + QSPI_DR, &byte);
			if (retval != ERROR_OK)
				goto err;
	...
	if ((stmqspi_info->dual_mask & ((1<<QSPI_DUAL_FLASH) |
		(1<<QSPI_FSEL_FLASH))) != (1<<QSPI_FSEL_FLASH)) {
		if (!*id1)
			LOG_WARNING("No response from QSPI flash1");
	}
	if ((stmqspi_info->dual_mask & ((1<<QSPI_DUAL_FLASH) |
		(1<<QSPI_FSEL_FLASH))) != 0) {
		if (!*id2)
			LOG_WARNING("No response from QSPI flash2");
	}
...
static int stmqspi_probe(struct flash_bank *bank)
{
	...
	if (QSPI_READ_REG(OCTOSPI_MAGIC) == OCTO_MAGIC_ID) {
		LOG_ERROR("OCTOSPI not supported yet");
		return ERROR_FAIL;
	}

	/* check that QSPI is actually present at all, and if yes */
	/* save current FSEL and DFM bits in QSPI_CR, current QSPI_CCR value */
	stmqspi_info->dual_mask = QSPI_READ_REG(QSPI_CR);
	if (retval == ERROR_OK)
		stmqspi_info->saved_ccr = QSPI_READ_REG(QSPI_CCR);
	if (retval == ERROR_OK)
		retval = set_mm_mode(target, io_base, stmqspi_info->saved_ccr);
	if (retval == ERROR_OK) {
		LOG_DEBUG("QSPI at 0x%" PRIx32 ", io_base at 0x%" PRIx32 ", QSPI_CR 0x%"
			PRIx32 ", QSPI_CCR 0x%" PRIx32 ", %d-byte addr", bank->base, io_base,
			stmqspi_info->dual_mask, stmqspi_info->saved_ccr,
			((QSPI_MODE >> QSPI_ADSIZE_POS) & 0x3) + 1);
	} else {
		LOG_ERROR("No QSPI at io_base 0x%" PRIx32, io_base);
		stmqspi_info->probed = 0;
	...
	fsize = (QSPI_READ_REG(QSPI_DCR)>>QSPI_FSIZE_POS) & ((1<<QSPI_FSIZE_LEN) - 1);
	LOG_DEBUG("FSIZE = 0x%04x", fsize);
	if (bank->size != (1U<<(fsize + 1)))
		LOG_WARNING("FSIZE field in QSPI_DCR doesn't match actual capacity.");
