...
int set_sector_protection_by_blocks(struct flash_bank *bank)
{
	int i_sect, i_prot_bl = 0;
	bool warn_layout = true;

	if (bank->sectors == NULL || bank->prot_blocks == NULL
		|| bank->num_sectors == 0 || bank->num_prot_blocks == 0)
		return ERROR_FAIL;

	for (i_sect = 0; i_sect < bank->num_sectors; i_sect++) {
		struct flash_sector *sect = &bank->sectors[i_sect];

		sect->is_protected = -1;

		while (sect->offset >= bank->prot_blocks[i_prot_bl].offset + bank->prot_blocks[i_prot_bl].size
				&& i_prot_bl < bank->num_prot_blocks)
			i_prot_bl++;

		if (sect->offset + sect->size <= bank->prot_blocks[i_prot_bl].offset)
			continue;

		sect->is_protected = bank->prot_blocks[i_prot_bl].is_protected;
		if (warn_layout && sect->size > bank->prot_blocks[i_prot_bl].size) {
			LOG_WARNING("set_sector_protection_by_blocks: sector (%" PRIu32
				    ") is bigger than protection block (%" PRIu32 ")",
				    sect->size, bank->prot_blocks[i_prot_bl].size);
			warn_layout = false;
		}
	}
