 ...
 * are used in iteration, so complatibility for old flash drivers is retained.
 *
...
int set_sector_protection_by_blocks(struct flash_bank *bank)
{
	...
	for (i_sect = 0; i_sect < bank->num_sectors; i_sect++) {
		...
		while (sect->offset > bank->prot_blocks[i_prot_bl].offset + bank->prot_blocks[i_prot_bl].size - 1
