...
uwp_target_write(target, SFC_CLK_CFG, SFC_CLK_OUT_DIV_2 | SFC_CLK_OUT_2X_EN |
	 ...
	 * */
	uwp_target_write(target, REG_AON_CLK_RF_CGM_SFC_1X_CFG, CLK_SFC_1X_DIV_2);

	uwp_target_write(target, REG_AON_CLK_RF_CGM_SFC_2X_CFG, CLK_SFC_2X_XTAL_MHZ);
}
...
static void sfcdrv_int_cfg(struct target *target, uint32_t op)
{
	if (op == true)  /* for CS1 interrupt */
		uwp_target_write(target, SFC_IEN, CS1_INT);
	else  /* for CS0 interrupt */
		uwp_target_write(target, SFC_IEN, CS0_INT);
}
...
static void spiflash_disable_cache(struct target *target)
{
	...
	uwp_target_write(target, REG_ICACHE_INT_EN, RF_CMD_IRQ_DISABLE);
	uwp_target_write(target, REG_ICACHE_INT_CLR, RF_CMD_IRQ_CLR);
	uwp_target_write(target, REG_ICACHE_CMD_CFG2, RF_CMD_STR_CMD_START|RF_CMD_TYPE_INVALID_ALL);
	do {
	...
	uwp_target_write(target, REG_ICACHE_INT_CLR, RF_CMD_IRQ_CLR);
	uwp_target_write(target, REG_ICACHE_CFG0, RF_CMD_IRQ_DISABLE);

	int_timeout = 0;
	uwp_target_write(target, REG_DCACHE_INT_EN, RF_CMD_IRQ_DISABLE);
	uwp_target_write(target, REG_DCACHE_INT_CLR, RF_CMD_IRQ_CLR);
	uwp_target_write(target, REG_DCACHE_CMD_CFG2, RF_CMD_STR_CMD_START|RF_CMD_TYPE_CLEAN_INVALID_ALL);
	do {
	...
	uwp_target_write(target, REG_DCACHE_INT_CLR, RF_CMD_IRQ_CLR);
	uwp_target_write(target, REG_DCACHE_CFG0, ALL_DISABLED);	/* disable all */
}
...
static void spiflash_enter_xip(struct uwp5661_flash_bank *uwp5661_info, struct target *target, uint8_t support_4addr)
{
	...
	uwp_target_write(target, SFC_TYPE_BUF1, TYPE_BUF1_DEFAULT_VALUE);
	uwp_target_write(target, SFC_TYPE_BUF2, TYPE_BUF2_DEFAULT_VALUE);
	sfcdrv_set_cmd_cfg_reg(uwp5661_info, target, CMD_MODE_READ , BIT_MODE_1, INI_CMD_BUF_7);
	uwp_target_write(target, REG_AON_CLK_RF_CGM_MTX_CFG, CGM_MTX_DIV_2);
	/* set CPU clk to xtal MHz */
	uwp_target_write(target, REG_AON_CLK_RF_CGM_ARM_CFG, CGM_ARM_XTAL_MHZ);
	uwp_target_write(target, REG_AON_CLK_RF_CGM_MTX_CFG, CGM_MTX_DIV_1);
