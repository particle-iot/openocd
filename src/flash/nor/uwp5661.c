					...
					uint32_t cmd_buf_index, struct sfc_cmd_des_t *cmd_des)
{
	if (!cmd_des)
		return ERROR_FAIL;
	sfcdrv_set_cmd_buf(uwp5661_info, target, cmd_buf_index, cmd_des->cmd);
...
static void create_cmd(struct sfc_cmd_des_t *cmd_desc_ptr, uint32_t cmd, uint32_t byte_len,
