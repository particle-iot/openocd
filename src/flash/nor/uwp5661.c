...
uint32_t cmd_info_buf_cache[INFO_BUF_MAX] = {0};
uint32_t prev_cmd_info_buf_cache[INFO_BUF_MAX] = {0};
uint32_t cmd_buf_cache_bitmap  = 0xFFF;
uint32_t sfc_cmd_cfg_cache = 0xFFFFFFFF;
uint32_t xip_sys_clk_cfg[2] = {0x00000100, 0x00000005};
...
static void uwp5661_set_sfc_clk(struct target *target)
{
	...
	target_write_u32(target, REG_AON_CLK_RF_CGM_SFC_2X_CFG, 0x00000004);
}
...
static uint32_t SFCDRV_GetCMDBuf(struct target *target, CMD_BUF_INDEX_E index)
{
	uint32_t value = 0;
	target_read_u32(target, SFC_CMD_BUF0+index*4, &value);
	return value;
}
...
static void SFCDRV_GetReadBuf(struct target *target, uint32_t *buffer, uint32_t word_cnt)
{
	...
	for (i = 0; i < word_cnt; i++)
		buffer[i] = SFCDRV_GetCMDBuf(target, read_buf_index + i);
}
...
static void spiflash_enter_xip(struct target *target, struct uwp_flash *flash)
{
	uint32_t i = 0;
	SFC_CMD_DES_T cmd_desc[3] = { {0} };

	create_cmd(&(cmd_desc[0]), CMD_FAST_READ, BYTE_NUM_1, CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
	create_cmd(&(cmd_desc[1]), 0x0          , (flash->support_4addr == TRUE) ? BYTE_NUM_4 : BYTE_NUM_3,
...
static void spiflash_select_xip(struct target *target, struct uwp_flash *flash, uint32_t op)
{
	target_write_u32(target, SFC_INT_CLR , (1 << SHIFT_INT_CLR));
	if (op == TRUE)
		spiflash_enter_xip(target, flash);
	else
...
static void spiflash_cmd_read(struct target *target, struct uwp_flash *flash,
			uint8_t cmd, uint32_t address, uint32_t *data_in, uint32_t data_len)
{
	SFC_CMD_DES_T cmd_desc[4] = { {0} };
	BYTE_NUM_E byte_num = BYTE_NUM_1;
			...
			uint32_t timeout, uint8_t cmd, uint32_t poll_bit, uint32_t bit_value)
{
	...
	do {
		spiflash_cmd_read(target, flash, cmd, 0xFFFFFFFF, &status, 1);
		status &= 0xFF;
		if (bit_value) {
			if ((status & poll_bit))
				return ERROR_OK;
...
static int uwp5661_erase(struct flash_bank *bank, int first, int last)
{
	...
	spiflash_select_xip(target, NULL, FALSE);

	if (flash->support_4addr == TRUE) {
		ret = spiflash_4addr_enable(target, flash);
		if (ret != ERROR_OK) {
			LOG_ERROR("uwp5661 SPI 4Byte Address mode switching failed!\n");
			spiflash_reset_anyway(target, NULL);
			spiflash_select_xip(target, flash, TRUE);
			return ret;
		}
	}
			...
			uint32_t data_addr, uint8_t *data_out, uint32_t data_len)
{
	...
	SFC_CMD_DES_T cmd_desc[CMD_BUF_MAX] = { {0} };
	int ret = ERROR_OK;
	...
	for (i = 0; i < data_len;) {
		...
		if (i == 0) {
			...
			create_cmd(&(cmd_desc[buffer_cnt++]), CMD_PAGE_PROGRAM, BYTE_NUM_1,
						CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
			create_cmd(&(cmd_desc[buffer_cnt++]), dest_addr       , byte_num  ,
						CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
		}

		piece_cnt = min((CMD_BUF_MAX - buffer_cnt)*4, data_len - i);
		...
		for (j = 0; j < piece_cnt;) {
			...
			create_cmd(&(cmd_desc[buffer_cnt++]), data_tmp, byte_num, CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
		}

		spiflash_read_write(target, cmd_desc, buffer_cnt, NULL);
...
static void spiflash_data_read(struct target *target, struct uwp_flash *flash,
			uint32_t offset, uint32_t count, uint8_t *buf, uint32_t dummy_bytes)
{
	uint32_t i = 0;
	uint32_t j = 0;
	...
	uint32_t cmd_idx = 0;
	uint32_t piece_cnt = 0;
	uint32_t tmp_buf[CMD_BUF_MAX-CMD_BUF_4] = {0};
	uint8_t *data_ptr = buf;
	uint32_t dummy_cnt = dummy_bytes;
	BYTE_NUM_E byte_num = BYTE_NUM_3;
	SFC_CMD_DES_T cmd_desc[CMD_BUF_MAX] = { {0} };
	...
	for (i = 0; i < count;) {
		...
		create_cmd(&(cmd_desc[cmd_idx++]), CMD_FAST_READ, BYTE_NUM_1, CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
		...
		if (dummy_cnt > 4) {
			create_cmd(&(cmd_desc[cmd_idx++]), 0x0, BYTE_NUM_4, CMD_MODE_HIGHZ, BIT_MODE_1, SEND_MODE_0);
			dummy_cnt = dummy_cnt - 4;
		}

		if (dummy_cnt > 0) {
			byte_num = BYTE_NUM_1 + (dummy_cnt - 1);
			create_cmd(&(cmd_desc[cmd_idx++]), 0x0, byte_num  , CMD_MODE_HIGHZ, BIT_MODE_1, SEND_MODE_0);
		}

		piece_cnt = min((CMD_BUF_MAX - CMD_BUF_4)*4, count - i);
		...
		for (j = 0; j < piece_cnt; j++)
			data_ptr[j] = (tmp_buf[j/4] >> (24 - (j%4)*8)) & 0xFF;
			...
			uint32_t offset, uint32_t count)
{
	...
	spiflash_data_read(target, flash, offset, count, buffer, 1);
...
struct uwp_flash_param *spiflash_scan(struct target *target)
{
	uint32_t i = 0;
	uint32_t read_data = 0;
	uint16_t jedec = 0;
	uint16_t manufacturer_id = 0;
	SFC_CMD_DES_T cmd_desc[2] = { {0} };
