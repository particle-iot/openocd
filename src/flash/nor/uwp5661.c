...
static int sfcdrv_set_cmd_cfg_reg(struct uwp5661_flash_bank *uwp5661_info, struct target *target,
				enum cmd_mode_e cmdmode, enum bit_mode_e bitmode, enum ini_add_sel_e iniaddsel)
{
	int retval = ERROR_OK;
...
static int sfcdrv_set_cmd_buf(struct uwp5661_flash_bank *uwp5661_info, struct target *target,
				enum cmd_buf_index_e index, uint32_t value)
{
...
static int sfcdrv_set_type_inf_buf(struct uwp5661_flash_bank *uwp5661_info, struct target *target,
			enum cmd_buf_index_e index, enum bit_mode_e bitmode, enum byte_num_e bytenum, enum cmd_mode_e cmdmode,
			enum send_mode_e sendmode)
{
	uwp5661_info->cmd_info_buf_cache[INFO_BUF_0 + (index>>2)] |= ((VALID0|
												...
												(sendmode << SHIFT_BYTE_SEND_MODE0)))<<(8*(index%4));
...
static int sfcdrv_set_cmd_data(struct uwp5661_flash_bank *uwp5661_info, struct target *target,
					uint32_t cmd_buf_index, struct sfc_cmd_des_t *cmd_des)
{
	int retval = ERROR_OK;
	if (!cmd_des)
		return ERROR_FAIL;
	retval = sfcdrv_set_cmd_buf(uwp5661_info, target, cmd_buf_index, cmd_des->cmd);
	retval = sfcdrv_set_type_inf_buf(uwp5661_info, target, cmd_buf_index,
						cmd_des->bit_mode,
						cmd_des->cmd_byte_len,
						cmd_des->cmd_mode,
						cmd_des->send_mode);
	return retval;
}

static int sfcdrv_set_read_buf(struct uwp5661_flash_bank *uwp5661_info, struct target *target,
					uint32_t read_buf_index, struct sfc_cmd_des_t *cmd_des)
{
	int retval = ERROR_OK;
	if (!cmd_des)
		return ERROR_FAIL;
	retval = sfcdrv_set_type_inf_buf(uwp5661_info, target, read_buf_index,
						cmd_des->bit_mode,
						cmd_des->cmd_byte_len,
						cmd_des->cmd_mode,
						cmd_des->send_mode);
...
static int create_cmd(struct sfc_cmd_des_t *cmd_desc_ptr, uint32_t cmd, uint32_t byte_len,
			enum cmd_mode_e cmd_mode, enum bit_mode_e bit_mode, enum send_mode_e send_mode)
{
...
static int spiflash_read_write(struct uwp5661_flash_bank *uwp5661_info, struct target *target,
			struct sfc_cmd_des_t *cmd_des, uint32_t cmd_len, uint32_t *din)
{
	int retval = ERROR_OK;
	uint32_t i = 0;
	...
	for (i = 0; i < cmd_len; i++) {
		cmd_des[i].is_valid = true;
		if ((cmd_des[i].cmd_mode == CMD_MODE_WRITE) ||
			(cmd_des[i].cmd_mode == CMD_MODE_HIGHZ))
			retval = sfcdrv_set_cmd_data(uwp5661_info, target, i, &(cmd_des[i]));
			if (retval != ERROR_OK)
				return retval;
		else if (cmd_des[i].cmd_mode == CMD_MODE_READ) {
			retval = sfcdrv_set_cmd_buf(uwp5661_info, target, read_buf_index, 0);
			retval = sfcdrv_set_read_buf(uwp5661_info, target, read_buf_index, &(cmd_des[i]));
			if (retval != ERROR_OK)
				return retval;
			read_buf_index++;
			read_count++;
...
static int spiflash_disable_cache(struct target *target)
{
	int retval = ERROR_OK;
	uint32_t int_status = 0;
	uint32_t int_timeout = 0;
...
static int spiflash_enter_xip(struct uwp5661_flash_bank *uwp5661_info, struct target *target, uint8_t support_4addr)
{
	int retval = ERROR_OK;
	uint32_t i = 0;
	struct sfc_cmd_des_t cmd_desc[3];

	retval = create_cmd(&(cmd_desc[0]), CMD_FAST_READ, BYTE_NUM_1, CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
	retval = create_cmd(&(cmd_desc[1]), 0x0          , (support_4addr == true) ? BYTE_NUM_4 : BYTE_NUM_3,
															CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_1);
	retval = create_cmd(&(cmd_desc[2]), 0x0          , BYTE_NUM_1, CMD_MODE_HIGHZ, BIT_MODE_1, SEND_MODE_0);

	for (i = 0; i < 3; i++) {
		cmd_desc[i].is_valid = true;
		retval = sfcdrv_set_cmd_data(uwp5661_info, target, i, &(cmd_desc[i]));
		if (retval != ERROR_OK)
			return retval;
	}
	...
	if (retval != ERROR_OK)
		return retval;
	retval = target_write_u32(target, SFC_TYPE_BUF1, TYPE_BUF1_DEFAULT_VALUE);
	if (retval != ERROR_OK)
		return retval;
	retval = target_write_u32(target, SFC_TYPE_BUF2, TYPE_BUF2_DEFAULT_VALUE);
	if (retval != ERROR_OK)
		return retval;
	...
	retval = spiflash_disable_cache(target);
	if (retval != ERROR_OK)
		return retval;
...
static int spiflash_exit_xip(struct uwp5661_flash_bank *uwp5661_info, struct target *target)
{
	int retval = ERROR_OK;
...
static int spiflash_select_xip(struct uwp5661_flash_bank *uwp5661_info, struct target *target,
					uint8_t support_4addr, uint32_t op)
{
	...
	if (retval != ERROR_OK)
		return retval;
	if (op == true) {
		retval = spiflash_enter_xip(uwp5661_info, target, support_4addr);
		if (retval != ERROR_OK)
			return retval;
	} else {
		retval = spiflash_exit_xip(uwp5661_info, target);
		if (retval != ERROR_OK)
			return retval;
	}
...
static enum byte_num_e spi_flash_addr(uint32_t *addr, uint8_t support_4addr)
{
	uint8_t cmd[4] = {0};
...
static int spiflash_cmd_write(struct uwp5661_flash_bank *uwp5661_info, struct target *target, uint8_t cmd,
			uint32_t *data_out, uint32_t data_len, enum bit_mode_e bitmode)
{
	int retval = ERROR_OK;
	struct sfc_cmd_des_t cmd_desc[3];
	enum byte_num_e byte_num = BYTE_NUM_1;
	uint32_t cmd_idx = 0;

	retval = create_cmd(&(cmd_desc[cmd_idx++]), cmd, BYTE_NUM_1, CMD_MODE_WRITE, bitmode, SEND_MODE_0);

	if (data_len > 8)
		data_len = 8;

	if (data_len > 4) {
		retval = create_cmd(&(cmd_desc[cmd_idx]), data_out[cmd_idx-1], BYTE_NUM_4, CMD_MODE_WRITE, bitmode, SEND_MODE_0);
		cmd_idx++;
		data_len = data_len - 4;
	}
	...
	if (data_len > 0) {
		...
		retval = create_cmd(&(cmd_desc[cmd_idx]), data_out[cmd_idx-1], byte_num  , CMD_MODE_WRITE, bitmode, SEND_MODE_0);
		cmd_idx++;
	}

	retval = spiflash_read_write(uwp5661_info, target, cmd_desc, cmd_idx, NULL);

	return retval;
}

static int spiflash_cmd_read(struct uwp5661_flash_bank *uwp5661_info, struct target *target,
			uint8_t cmd, uint32_t address, uint8_t support_4addr, uint32_t *data_in, uint32_t data_len)
{
	int retval = ERROR_OK;
	struct sfc_cmd_des_t cmd_desc[4];
	enum byte_num_e byte_num = BYTE_NUM_1;
	uint32_t tmp_buf[2] = {0};
	uint32_t cmd_idx = 0;

	retval = create_cmd(&(cmd_desc[cmd_idx++]), cmd, BYTE_NUM_1, CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);

	if (address != 0xFFFFFFFF) {
		uint32_t dest_addr = address;
		byte_num = spi_flash_addr(&dest_addr, support_4addr);
		retval = create_cmd(&(cmd_desc[cmd_idx++]), dest_addr, byte_num , CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
	}
	...
	if (data_len > 4) {
		retval = create_cmd(&(cmd_desc[cmd_idx++]), 0, BYTE_NUM_4, CMD_MODE_READ, BIT_MODE_1, SEND_MODE_0);
		data_len = data_len - 4;
	}

	if (data_len > 0) {
		byte_num = BYTE_NUM_1 + (data_len - 1);
		retval = create_cmd(&(cmd_desc[cmd_idx++]), 0, byte_num  , CMD_MODE_READ, BIT_MODE_1, SEND_MODE_0);
	}

	retval = spiflash_read_write(uwp5661_info, target, cmd_desc, cmd_idx, tmp_buf);
					...
					uint8_t cmd, uint32_t poll_bit, uint32_t bit_value)
{
	...
	do {
		retval = spiflash_cmd_read(uwp5661_info, target, cmd, 0xFFFFFFFF, false, &status, 1);
		if (retval != ERROR_OK)
...
static int spiflash_write_enable(struct uwp5661_flash_bank *uwp5661_info, struct target *target)
{
	return spiflash_cmd_write(uwp5661_info, target, CMD_WRITE_ENABLE, NULL, 0, BIT_MODE_1);
}

static int spiflash_reset_anyway(struct uwp5661_flash_bank *uwp5661_info, struct target *target)
{
	int retval = ERROR_OK;
	uint32_t i = 0;
	uint32_t dummy_dat = 0;

	retval = spiflash_cmd_write(uwp5661_info, target, CMD_RSTEN, NULL, 0, BIT_MODE_4);
	if (retval != ERROR_OK)
		return retval;
	retval = spiflash_cmd_write(uwp5661_info, target, CMD_RST  , NULL, 0, BIT_MODE_4);
	if (retval != ERROR_OK)
		return retval;
	for (i = 0; i < 10; i++) {
	...
	retval = spiflash_cmd_write(uwp5661_info, target, CMD_RSTEN, NULL, 0, BIT_MODE_1);
	if (retval != ERROR_OK)
		return retval;
	retval = spiflash_cmd_write(uwp5661_info, target, CMD_RST  , NULL, 0, BIT_MODE_1);
	if (retval != ERROR_OK)
		return retval;
	for (i = 0; i < 10; i++) {
		retval = target_read_u32(target, SFC_CMD_CFG, &dummy_dat);
...
{
	int retval = ERROR_OK;
	retval = spiflash_cmd_write(uwp5661_info, target, CMD_ENTER_4ADDR, NULL, 0, BIT_MODE_1);
	if (retval != ERROR_OK)
		return retval;
...
{
	int retval = ERROR_OK;
	retval = spiflash_cmd_write(uwp5661_info, target, CMD_EXIT_4ADDR, NULL, 0, BIT_MODE_1);
	if (retval != ERROR_OK)
		return retval;
...
static int spiflash_cmd_sector_erase(struct uwp5661_flash_bank *uwp5661_info, struct target *target, uint32_t offset)
{
	...
	enum byte_num_e addr_byte_num = spi_flash_addr(&addr, uwp5661_info->flash.support_4addr);

	retval = spiflash_write_enable(uwp5661_info, target);
	if (retval != ERROR_OK)
		return retval;

	retval = spiflash_cmd_write(uwp5661_info, target, CMD_SECTOR_ERASE, &addr, addr_byte_num + 1, BIT_MODE_1);
	if (retval != ERROR_OK)
		return retval;

	retval = spiflash_cmd_poll_bit(uwp5661_info, target, SPI_FLASH_SECTOR_ERASE_TIMEOUT,
...
static int uwp5661_erase(struct flash_bank *bank, int first, int last)
{
	...
	retval = spiflash_select_xip(uwp5661_info, target, false, false);
	if (retval != ERROR_OK)
		return retval;

	if (flash->support_4addr == true) {
		retval = spiflash_4addr_enable(uwp5661_info, target);
		if (retval != ERROR_OK) {
			LOG_ERROR("uwp5661 SPI 4Byte Address mode switching ON failed!\n");
			if (spiflash_reset_anyway(uwp5661_info, target) != ERROR_OK)
				LOG_ERROR("Flash reset failed.");
			if (spiflash_select_xip(uwp5661_info, target, false, true) != ERROR_OK)
	...
	for (i = first; i <= last; i++) {
		retval = spiflash_cmd_sector_erase(uwp5661_info, target, i);
	...
	if (flash->support_4addr == true) {
		retval = spiflash_4addr_disable(uwp5661_info, target);
		if (retval != ERROR_OK) {
			LOG_ERROR("uwp5661 SPI 4Byte Address mode switching OFF failed!\n");

			retval = spiflash_reset_anyway(uwp5661_info, target);
			if (retval != ERROR_OK)
				return retval;
		}
	}

	retval = spiflash_select_xip(uwp5661_info, target, false, true);
	if (retval != ERROR_OK)
		return retval;
			...
			uint32_t data_addr, uint8_t *data_out, uint32_t data_len)
{
	...
	enum byte_num_e byte_num = BYTE_NUM_3;
	struct sfc_cmd_des_t cmd_desc[CMD_BUF_MAX];
	...
	for (i = 0; i < data_len;) {
		...
		if (i == 0) {
			...
			retval = create_cmd(&(cmd_desc[cmd_idx++]), CMD_PAGE_PROGRAM, BYTE_NUM_1,
						CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
			retval = create_cmd(&(cmd_desc[cmd_idx++]), dest_addr       , byte_num  ,
						CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
		}
		...
		for (j = 0; j < piece_cnt;) {
			...
			retval = create_cmd(&(cmd_desc[cmd_idx++]), data_tmp, byte_num, CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
		}
	...
	if (retval != ERROR_OK)
		return retval;
	retval = spiflash_cmd_poll_bit(uwp5661_info, target, SPI_FLASH_PAGE_PROG_TIMEOUT, CMD_READ_STATUS1, STATUS_WIP, 0);
			...
			uint32_t offset, uint32_t count)
{
	...
	for (actual = 0; actual < count;) {
		...
		retval = spiflash_write_page(uwp5661_info, target, (page_addr * page_size + byte_addr),
...
static int spiflash_data_read(struct target *target, uint32_t offset,
			...
			uint32_t offset, uint32_t count)
{
	...
	retval = spiflash_select_xip(uwp5661_info, target, flash->support_4addr, true);
	if (retval != ERROR_OK)
		return retval;
	...
	LOG_INFO("Flash Read");
	retval = spiflash_data_read(target, offset, count, buffer);
	if (retval != ERROR_OK)
		return retval;
...
static int uwp5661_probe(struct flash_bank *bank)
{
	...
	struct target *target = bank->target;
	struct uwp_flash *flash = &(uwp5661_info->flash);
	struct sfc_cmd_des_t cmd_desc[2];
	...
	/* scan device */
	retval = create_cmd(&(cmd_desc[0]), CMD_READ_ID, BYTE_NUM_1, CMD_MODE_WRITE, BIT_MODE_1, SEND_MODE_0);
	retval = create_cmd(&(cmd_desc[1]),           0, BYTE_NUM_3, CMD_MODE_READ , BIT_MODE_1, SEND_MODE_0);

	retval = spiflash_read_write(uwp5661_info, target, cmd_desc, 2, &read_data);
	if (retval != ERROR_OK)
		return retval;
	for (; p->name ; p++) {
