 ...
 *************************************************************************************************/
static bool timeout_expired(struct timeout_s *to)
{
 ...
 *************************************************************************************************/
static int sromalgo_prepare(struct target *tgt)
{
	int hr;
	...
	hr = target_alloc_working_area(tgt, RAM_STACK_WA_SIZE, &g_stack_area);
	if (hr != ERROR_OK)
		return hr;
 ...
 *************************************************************************************************/
static int sromalgo_release(struct target *tgt)
{
	int hr = ERROR_OK;

	/* Free Stack/Flash algorithm working area */
	if (g_stack_area) {
		hr = target_free_working_area(tgt, g_stack_area);
		g_stack_area = NULL;
	}
 ...
 *************************************************************************************************/
static int sromalgo_run(struct target *tgt)
{
	int hr;
	...
	/* mov r8, r8; mov r8, r8 */
	hr = target_write_u32(tgt, g_stack_area->address + 0, 0x46C046C0);
	if (hr != ERROR_OK)
		return hr;

	/* mov r8, r8; bkpt #0    */
	hr = target_write_u32(tgt, g_stack_area->address + 4, 0xBE0046C0);
	if (hr != ERROR_OK)
		return hr;

	hr = target_run_algorithm(tgt, 0, NULL, 1, &reg_params, g_stack_area->address,
			0, 150, &armv7m_info);
	...
	uint32_t *dataOut)
{
	...
	if (is_data_in_ram)
		hr = target_write_u32(tgt, MEM_IPC_DATA(IPC_ID), working_area);
	else
		hr = target_write_u32(tgt, MEM_IPC_DATA(IPC_ID), req_and_params);

	if (hr != ERROR_OK)
		return hr;

	/* Enable notification interrupt of IPC_INTR_STRUCT0(CM0+) for IPC_STRUCT2 */
	hr = target_write_u32(tgt, MEM_IPC_INTR_MASK(IPC_INTR_ID), 1u << (16 + IPC_ID));
	if (hr != ERROR_OK)
		return hr;

	hr = target_write_u32(tgt, MEM_IPC_NOTIFY(IPC_ID), 1);
	if (hr != ERROR_OK)
		return hr;
...
static int psoc6_probe(struct flash_bank *bank)
{
	...
	uint32_t geom;
	target_read_u32(tgt, PSOC6_SPCIF_GEOMETRY, &geom);
...
{
	struct target *tgt = bank->target;
	int hr = target_write_u32(tgt, wa->address, SROMAPI_ERASESECTOR_REQ);
	if (hr != ERROR_OK)
		return hr;
...
{
	struct target *tgt = bank->target;
	int hr = target_write_u32(tgt, wa->address, SROMAPI_ERASEROW_REQ);
	if (hr != ERROR_OK)
		return hr;
...
static int psoc6_erase(struct flash_bank *bank, int first, int last)
{
	...
	hr = target_alloc_working_area(tgt, psoc6_info->row_sz + 32, &wa);
	if (hr != ERROR_OK)
		goto exit;

	/* Number of rows in single sector */
	const uint32_t rows_in_sector = FLASH_SECTOR_SIZE / psoc6_info->row_sz;

	while (last >= first) {
		/* Erase Sector if we are on sector boundary and erase size covers whole sector */
		if ((first % rows_in_sector) == 0 && (uint32_t)(last - first + 1) >= rows_in_sector) {
	...
	bool is_sflash)
{
	...
	hr = target_alloc_working_area(tgt, count + 32, &wa);
	if (hr != ERROR_OK)
		goto exit;
	...
	uint32_t count)
{
	...
	while (addr < maxAdressSize) {
		...
		hr = psoc6_program_row(tgt, addr, page_buf, size, is_sflash);
		if (hr != ERROR_OK) {
...
COMMAND_HANDLER(psoc6_handle_mass_erase_command)
{
	...
	hr = target_alloc_working_area(tgt, 32, &wa);
	if (hr != ERROR_OK)
		goto exit;
 ...
 *************************************************************************************************/
int handle_reset_init(struct target *tgt)
{
	int hr;
	uint32_t reset_addr;
	bool is_cm0 = (tgt->coreid == 0);
...
COMMAND_HANDLER(psoc6_handle_reset_init)
{
	if (CMD_ARGC)
		return ERROR_COMMAND_SYNTAX_ERROR;

	struct target *tgt = get_current_target(CMD_CTX);
	return handle_reset_init(tgt);
}
...
static const struct command_registration psoc6_exec_command_handlers[] = {
	...
	{
		.name = "reset_init",
		.handler = psoc6_handle_reset_init,
		.mode = COMMAND_EXEC,
		.usage = NULL,
