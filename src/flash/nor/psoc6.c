...
struct row_region safe_sflash_regions[] = {
...
static int psoc6_erase(struct flash_bank *bank, int first, int last)
{
	...
	while (last >= first) {
		...
		if ((first % rows_in_sector) == 0 &&
			(uint32_t)(last - first + 1) >= rows_in_sector) {
...
COMMAND_HANDLER(psoc6_handle_mass_erase_command)
{
	...
	hr = target_alloc_working_area(target, 32, &wa);
	if (hr != ERROR_OK)
		goto exit;

	hr = target_write_u32(target, wa->address, SROMAPI_ERASEALL_REQ);
	if (hr != ERROR_OK)
		goto exit_free_wa;

	hr = call_sromapi(target, SROMAPI_ERASEALL_REQ, wa->address, &data_out);
	if (hr != ERROR_OK)
		goto exit_free_wa;
