...
#define IPC_STRUCT_DATA_TIMEOUT		250u
...
int CallSromApi(struct target *target, uint32_t callIdAndParams, uint32_t *dataOut)
{
	...
	unsigned int address = (unsigned int)(IPC_STRUC + IPC_STRUCT_DATA_OFFSET);
	...
	if (hr == ERROR_OK) {
		...
		if (isDataInRam) {
			LOG_DEBUG("CallSromApi: isDataInRam = true: address -> 0x%x, data -> 0x%x", address, SRAM_SCRATCH_ADDR);
			hr = target_write_u32(target, address, SRAM_SCRATCH_ADDR);
		} else {
			LOG_DEBUG("CallSromApi: isDataInRam = false: address -> 0x%x, data -> 0x%x", address, callIdAndParams);
			hr = target_write_u32(target, address, callIdAndParams);
		}
		if (hr == ERROR_OK) {
			...
			if (hr == ERROR_OK) {
				...
				if (hr == ERROR_OK) {
					...
					if (hr == ERROR_OK) {
						/* Poll Data byte */
						if (isDataInRam)
							hr = PollSromApiStatus(target, SRAM_SCRATCH_ADDR, IPC_STRUCT_DATA_TIMEOUT, dataOut);
						else
							hr = PollSromApiStatus(target, address, IPC_STRUCT_DATA_TIMEOUT, dataOut);
					}
				}
			}
		}
...
static int Psoc6GetSiliconId(struct target *target, uint32_t *siliconId, uint8_t *protection)
{
	...
	if (hr == ERROR_OK) {
		...
		if (hr == ERROR_OK) {
			/* Family ID High */
			familyIdHi = (dataOut0 &  MXS40_SROMAPI_SILID_FAMID_HI_MSK) >> MXS40_SROMAPI_SILID_FAMID_HI_ROR;
			/* Family ID Low */
			familyIdLo = (dataOut0 &  MXS40_SROMAPI_SILID_FAMID_LO_MSK) >> MXS40_SROMAPI_SILID_FAMID_LO_ROR;
			/* Silicon ID High */
			siliconIdHi = (dataOut1 &  MXS40_SROMAPI_SILID_SILID_HI_MSK) >> MXS40_SROMAPI_SILID_SILID_HI_ROR;
			/* Silicon ID Low */
			siliconIdLo = (dataOut1 &  MXS40_SROMAPI_SILID_SILID_LO_MSK) >> MXS40_SROMAPI_SILID_SILID_LO_ROR;
			/* Protection state */
			*protection = (dataOut1 &  MXS40_SROMAPI_SILID_PROT_MSK) >> MXS40_SROMAPI_SILID_PROT_ROR;
		} else {
			LOG_ERROR("Get Silicon ID and protection state has failed results");
		}
	}
	familyId = ((familyIdHi & 0xFF) << 8) | (familyIdLo & 0xFF);
	*siliconId = ((siliconIdHi & 0xFF) << 24) | ((siliconIdLo & 0xFF) << 16) | familyId;
	return hr;
}
...
static int EraseSector(struct target *target, int first, int last)
{
	...
	LOG_DEBUG("first-> 0x%x, last-> 0x%x", first, last);
		...
		for (int i = first; i < last; i++) {
			...
			if (hr != ERROR_OK) {
				LOG_ERROR("Sector \"%d\" from \"%d\" sectors are not erased.", i, last);
				break;
			}
...
static int WriteRow(struct target *target, int address, const uint8_t * buffer, int count)
{
	...
	if (hr == ERROR_OK) {
		...
		/* SRAM_SCRATCH + 0x04: Data location/size and Integrity check
			...
			Data size is ignored for S40 SONOS FLASH as the lowest granularity for program operation
			equals page size. */
		...
		if (hr == ERROR_OK) {
			...
			if (hr == ERROR_OK) {
				/* SRAM_SCRATCH + 0x10...n: Data word 0..n (Data provided should be proportional
				to data size provided, data to be programmed into LSB’s ) */
