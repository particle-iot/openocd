...
{
	LOG_ERROR("Erase of read-only flash alias refused. Use master flash bank '%s'", (char *)bank->driver_priv);
	return ERROR_FAIL;
}
...
static int ro_alias_write(struct flash_bank *bank, const uint8_t *buffer,
		uint32_t offset, uint32_t count)
{
	LOG_ERROR("Write to read-only flash alias refused. Use master flash bank '%s'", (char *)bank->driver_priv);
	return ERROR_FAIL;
}
