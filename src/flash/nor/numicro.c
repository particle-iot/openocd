...
static int numicro_core_reset(struct target *target)
{
	/* Reboot Mechanism from AP/LD ROM(may be not used due to cheat register) */
	int retval = ERROR_OK;
	...
	/* NUC100 Series work around */
	/* Halt the core forcely After SYSRESETREQ */
	retval = target_write_u32(target, NUMICRO_SCS_DHCSR, (DBGKEY | C_DEBUGEN | C_HALT));
	if (retval != ERROR_OK)
		return retval;

	retval = target_write_u32(target, NUMICRO_SCS_DEMCR, VC_CORERESET);
	if (retval != ERROR_OK)
		return retval;

	retval = target_write_u32(target, NUMICRO_SCS_AIRCR, (AIRCR_VECTKEY | AIRCR_SYSRESETREQ));
	if (retval != ERROR_OK)
		return retval;
...
static int numicro_restart(struct target *target, int rommode)
{
	/* Reboot Mechanism from AP/LD ROM(may be not used due to cheat register) */
	uint32_t con_stat;
	int retval = ERROR_OK;
	...
	retval = target_read_u32(target, NUMICRO_FLASH_ISPCON, &con_stat);
	if (retval != ERROR_OK)
		return retval;

	con_stat &= ~(ISPCON_BS_MASK);
	retval = target_write_u32(target, NUMICRO_FLASH_ISPCON, (con_stat | rommode));
	if (retval != ERROR_OK)
		return retval;

	numicro_core_reset(target);	/* Reboot from Specified ROM */
