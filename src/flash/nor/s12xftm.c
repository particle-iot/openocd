					 ...
					 uint16_t fccob[], uint8_t fccob_size)
{
	...
	ret = target_read_u8(target, HCS12_REG_FSTAT, &fstat);
	if (ret < 0) {
	...
	do {
		...
		if (fstat & (HCS12_REG_FSTAT_FPVIOL | HCS12_REG_FSTAT_ACCERR)) {
...
static int s12xftm_measure_cpu_speed(struct s12xftm_info *chip)
{
	...
	struct s12xftm_helper_context scratchpad = { 0 };
