...
static int fslqspi_block_erase(struct flash_bank *bank, uint32_t offset)
{
...
static int fslqspi_bulk_erase(struct flash_bank *bank)
{
...
{
	struct target *target = bank->target;
	struct fslqspi_flash_bank *info = bank->driver_priv;
	int retval = ERROR_OK;
	int sector;
	...
	if (first == 0 && last == (bank->num_sectors - 1) &&
		...
		retval = fslqspi_bulk_erase(bank);
		if (retval == ERROR_OK)
			return retval;
		else
	...
	for (sector = first; sector <= last; sector++) {
		retval = fslqspi_block_erase(bank, sector * info->dev->sectorsize);
		if (retval != ERROR_OK)
			return retval;
	}
...
static int fslqspi_flash_write(struct flash_bank *bank, const uint8_t *buffer,
...
static int fslqspi_flash_read(struct flash_bank *bank, uint8_t *buffer, uint32_t offset, uint32_t count)
{
	struct target *target = bank->target;
	struct fslqspi_flash_bank *info = bank->driver_priv;
	int retval = ERROR_OK;
...
static int fslqspi_probe(struct flash_bank *bank)
{
	...
	/* Read flash JEDEC ID */
	retval = fslqspi_read_id(bank, &id);
	if (retval != ERROR_OK)
		return retval;
...
int fslqspi_get_info(struct flash_bank *bank, char *buf, int buf_size)
{
	struct fslqspi_flash_bank *info = bank->driver_priv;
...
{
	struct fslqspi_flash_bank *info;

	if (CMD_ARGC < 7)
		return ERROR_COMMAND_SYNTAX_ERROR;

	info = malloc(sizeof(struct fslqspi_flash_bank));
	if (!info) {
		LOG_ERROR("not enough memory");
		return ERROR_FAIL;
	}
