...
static int xmc4xxx_load_bank_layout(struct flash_bank *bank)
{
	...
	LOG_DEBUG("%"PRId32" sectors", bank->num_sectors);
	...
	case 12:
		...
		break;
	default:
		LOG_ERROR("Unexpected number of sectors, %"PRId32"\n",
			  bank->num_sectors);
		return ERROR_FAIL;
	}
	...
	for (int i = 0; i < bank->num_sectors; i++) {
		...
		LOG_DEBUG("\t%"PRId32": %"PRIu32"k", i, capacity[i]);
...
static int xmc4xxx_erase(struct flash_bank *bank, int first, int last)
{
	...
	for (int i = first; i <= last; i++) {
		...
		if (res != ERROR_OK) {
			LOG_ERROR("Invalid sector %"PRId32, i);
			return res;
		}
	...
	while (count) {
		...
		if (start_pad) {
			LOG_INFO("Write does not start on a 256 byte boundary"
				 "Padding by %"PRId32" bytes", start_pad);
...
static int xmc4xxx_get_info_command(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	default:
		snprintf(buf, buf_size, "Cannot identify target as an XMC4xxx\n");
		return ERROR_FAIL;
	}
	...
	if (otp_enabled) {
		...
		for (int i = 0; i < bank->num_sectors; i++) {
			if (fb->write_prot_otp[i]) {
				snprintf(otp_str, sizeof(otp_str), "- %"PRId32"\n", i);
	...
	if (rev_str != NULL)
		...
		snprintf(buf, buf_size, "%s - Rev: unknown (0x%01"PRIx32")%s",
				 ...
				 int first, int last)
{
	...
	if (proin) {
		LOG_ERROR("Flash protection is installed for user %"PRId32
	...
	LOG_DEBUG("PROCON: %"PRIu32, procon);
