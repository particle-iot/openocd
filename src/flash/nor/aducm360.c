		...
		uint32_t count)
{
	...
	static const uint32_t   aducm360_flash_write_code[] = {
			...
			0x4282EA4F,	0x33012300,	0xD1FC4293,	0xF8842208,	0x2201201C,
...
#endif
	printf("'aducm360_write_block_sync' requested, dst:0x%08lX, count:0x%08lX(%d) bytes.\n",
	...
	while (count > 0) {
		...
		printf("Copying chunk %02d: ROM:0x%08lX-0x%08lX, bytes:%d(0x%08lX)\n", ++ix,
		...
		uint32_t count)
{
	...
	printf("'aducm360_write_block_async' requested, dst:0x%08lX, count:0x%08lX(%d) bytes.\n",
			(unsigned long)address, (unsigned long)count, count);
