...
{
	LOG_WARNING("aducm360_protect_check not implemented yet.\n");
	return ERROR_OK;
}
...
static int aducm360_page_erase(struct target *target, uint32_t padd)
{
	...
	LOG_DEBUG("performing sector erase (at 0x%08lX)... ", (unsigned long)padd);
...
{
	LOG_ERROR("aducm360_protect not implemented yet.\n");
		...
		uint32_t count)
{
	...
	while (target_alloc_working_area_try(target, target_buffer_size, &target_buffer) != ERROR_OK) {
		LOG_WARNING("couldn't allocate a buffer space of 0x%08lX bytes in the target's SRAM.\n",
	...
	while (count > 0) {
		...
		LOG_DEBUG("copying chunk %02d: ROM:0x%08lX-0x%08lX, bytes:%d(0x%08lX)\n", ++ix,
				(unsigned long)address, (unsigned long)address+(unsigned long)thisrun_count-1,
