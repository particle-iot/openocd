...
FLASH_BANK_COMMAND_HANDLER(jtagspi_flash_bank_command)
{
	...
	info->probed = 0;
	info->write_enabled = 0;

	COMMAND_PARSE_NUMBER(u32, CMD_ARGV[6], info->ir);
	COMMAND_PARSE_NUMBER(u32, CMD_ARGV[7], info->latency);
...
{
	struct jtag_tap *tap = bank->target->tap;
	struct jtagspi_flash_bank *info = bank->driver_priv;
	struct scan_field field;
	uint8_t buf[4];
	if (buf_get_u32(tap->cur_instr, 0, tap->ir_length) == info->ir)
		return;
	LOG_DEBUG("%s", __func__);
	buf_set_u32(buf, 0, tap->ir_length, info->ir);
		...
		uint32_t *addr, uint8_t *data, int len)
{
	...
	LOG_DEBUG("%s cmd=0x%02x len=%i", __func__, cmd, len);
	...
	if (lenb > 0) {
		if (data_buf == NULL) {
			LOG_ERROR("%s no memory", __func__);
			return ERROR_FAIL;
		}
		if (is_read) {
			fields[n].num_bits = info->latency;
	...
	jtag_add_dr_scan(tap, n, fields, TAP_IDLE);
	jtag_execute_queue();
	if (is_read)
...
static uint8_t jtagspi_read_status(struct flash_bank *bank)
{
	uint8_t status;
	jtagspi_cmd(bank, SPIFLASH_READ_STATUS, NULL, &status, -8);
	LOG_DEBUG("%s status=0x%08" PRIx32, __func__, status);
	return status;
}
...
static int jtagspi_wait(struct flash_bank *bank, int timeout_ms)
{
	...
	do {
		status = jtagspi_read_status(bank);
		if ((status & SPIFLASH_BSY_BIT) == 0)
			return ERROR_OK;
		alive_sleep(1);
	} while (timeval_ms() - t0 < timeout_ms);
	LOG_ERROR("%s timeout during write/erase", __func__);
	return ERROR_FAIL;
}
...
static int jtagspi_bulk_erase(struct flash_bank *bank)
{
	...
	return jtagspi_wait(bank, bank->num_sectors*JTAGSPI_MAX_TIMEOUT);
}
...
static int jtagspi_write(struct flash_bank *bank, const uint8_t *buffer, uint32_t offset, uint32_t count)
{
	...
	for (n = 0; n < count; n += info->dev->pagesize) {
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("%s page write error", __func__);
			return retval;
		}
		LOG_DEBUG("%s wrote page at 0x%08" PRIx32, __func__, offset + n);
	}
	return ERROR_OK;
}
