...
static int niietcm4_dump_uflash_page(struct flash_bank *bank, uint32_t *dump, int page_num, int mem_type)
{
	...
	else uflash_cmd = UFMC_MAGIC_KEY | UFMC_READ;
...
static int niietcm4_load_uflash_page(struct flash_bank *bank, uint32_t *dump, int page_num, int mem_type)
{
	...
	else uflash_cmd = UFMC_MAGIC_KEY | UFMC_WRITE;
...
static int niietcm4_uflash_page_erase(struct flash_bank *bank, int page_num, int mem_type)
{
	...
	else uflash_cmd = UFMC_MAGIC_KEY | UFMC_PAGE_ERASE;
...
static int niietcm4_uflash_protect(struct flash_bank *bank, int mem_type, int set, int first, int last)
{
	...
	if (mem_type == INFO_MEM_TYPE) {
		...
		else uflash_dump[INFOWORD2_ADDR] |= INFOWORD3_LOCK_IFB_UF;
		...
		for (int i = first; i <= last; i++)	{
			...
			else uflash_dump[UF_LOCK_ADDR+reg_num] |= (1<<bit_num);
		}
...
COMMAND_HANDLER(niietcm4_handle_uflash_protect_check_command)
{
	...
	if (mem_type == INFO_MEM_TYPE) {
		...
		else command_print(CMD_CTX, "All sectors of info userflash are protected!");
	}
	else {
		...
		for (i = 0; i < USERFLASH_PAGE_TOTALNUM/8; i++)
		{
			...
			{
				if(uflash_data & 0x1)
					command_print(CMD_CTX, "Userflash sector #%03d: 0x%04x (0x100) is not protected!", i*8+j, (i*8+j)*USERFLASH_PAGE_SIZE);
				else command_print(CMD_CTX, "Userflash sector #%03d: 0x%04x (0x100) is protected!", i*8+j, (i*8+j)*USERFLASH_PAGE_SIZE);
...
COMMAND_HANDLER(niietcm4_handle_uflash_protect_command)
{
	...
	if (strcmp("on", CMD_ARGV[3]) == 0) {
		command_print(CMD_CTX, "Try to enable %s userflash sectors %d through %d protection. Please wait ... ",  CMD_ARGV[0], first, last);
		set = 1;
	}
	else if (strcmp("off", CMD_ARGV[3]) == 0) {
		command_print(CMD_CTX, "Try to disable %s userflash sectors %d through %d protection. Please wait ... ",  CMD_ARGV[0], first, last);
		set = 0;
	}
	else return ERROR_COMMAND_SYNTAX_ERROR;
...
COMMAND_HANDLER(niietcm4_handle_bflash_info_remap_command)
{
	...
	else uflash_dump[INFOWORD0_ADDR] |= INFOWORD0_BOOTFROM_IFB;
...
FLASH_BANK_COMMAND_HANDLER(niietcm4_flash_bank_command)
{
	...
	COMMAND_PARSE_NUMBER(uint, CMD_ARGV[6], niietcm4_info->sys_freq);
...
static int niietcm4_erase(struct flash_bank *bank, int first, int last)
{
	...
	else flash_cmd = FMC_MAGIC_KEY | FMC_PAGE_ERASE;
...
static int niietcm4_protect(struct flash_bank *bank, int set, int first, int last)
{
	...
	if (niietcm4_info->bflash_info_remap){
		...
		for (int i = first; i <= last; i++)	{
			...
			else uflash_dump[BF_LOCK_ADDR+reg_num] |= (1<<bit_num);
		}
		// erase page 0 info userflash
		retval = niietcm4_uflash_page_erase(bank, 0, 1);
		if (retval != ERROR_OK)
		...
		uint32_t offset, uint32_t count)
{
	...
	static const uint8_t niietcm4_flash_write_code[] = {
				
	};
		...
		uint32_t offset, uint32_t count)
{
	...
	retval = niietcm4_write_block(bank, buffer, offset, count/4);
	...
	if (retval == ERROR_TARGET_RESOURCE_NOT_AVAILABLE) {
		...
		if (niietcm4_info->bflash_info_remap)
			flash_cmd = FMC_MAGIC_KEY | FMC_WRITE_IFB;
		else flash_cmd = FMC_MAGIC_KEY | FMC_WRITE;
...
static int niietcm4_probe_k1921vk01t(struct flash_bank *bank)
{
	...
	if(!service_mode) {
		...
		if (niietcm4_info->bflash_info_remap) {
			sprintf(info_bootflash_addr_str, "0x%08x base adress", bank->base);
		}
		else {
			sprintf(info_bootflash_addr_str, "not maped to global adress space");
		}
...
static int niietcm4_probe(struct flash_bank *bank)
{
	...
	/* Don`t need read because there is only one NIIET CM4 mcu at the moment  
	retval = target_write_u32(target, CHIPID_ADDR, &chipid);
	if (retval != ERROR_OK)
		return retval;
	*/
