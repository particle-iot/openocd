...
FLASH_BANK_COMMAND_HANDLER(niietcm4_flash_bank_command)
{
	...
	COMMAND_PARSE_NUMBER(uint, CMD_ARGV[6], niietcm4_info->ext_freq);
...
static int niietcm4_probe(struct flash_bank *bank)
{
	...
	LOG_INFO("\n\nNIIET Cortex M4F %s", niietcm4_info->chip_name);
	LOG_INFO("Ext. freq = %d", niietcm4_info->ext_freq);
	return ERROR_OK;
}
