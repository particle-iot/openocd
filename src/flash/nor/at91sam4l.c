...
static int sam4l_check_sector_erased(struct flash_bank *bank, uint32_t sn,
		bool *is_erased_p)
{
	struct sam4l_info *chip = (struct sam4l_info *)bank->driver_priv;
	bool is_erased = false;

	for (int i = 0; i < chip->pages_per_sector; i++) {
		int res;

		res = sam4l_check_page_erased(bank, sn * chip->pages_per_sector + i,
				&is_erased);
		if (res != ERROR_OK)
			return res;

		/* The sector is not erased if any single page within that sector is not
		 * erased. */
		if (!is_erased)
			break;
	}

	*is_erased_p = is_erased;
	return ERROR_OK;
}
...
static int sam4l_probe(struct flash_bank *bank)
{
	...
	/* Load the actual sector protection state */
	res = sam4l_protect_check(bank);
	if (res != ERROR_OK)
		return res;

	/* Load the actual sector erase state */
	for (int i = 0; i < bank->num_sectors; i++) {
		bool is_erased;

		res = sam4l_check_sector_erased(bank, i, &is_erased);
		if (res == ERROR_OK)
			bank->sectors[i].is_erased = is_erased;
		else
			LOG_WARNING("Unable to check erase state for sector %d", i);
	}
		...
		uint32_t offset, uint32_t count)
{
	int res;
	struct sam4l_info *chip = (struct sam4l_info *)bank->driver_priv;
	uint32_t f_addr = offset - (offset % chip->page_size);
	...
	while (count > 0) {
		...
		res = sam4l_flash_command(bank->target, SAM4L_FCMD_CPB, -1);
		if (res != ERROR_OK) {
			LOG_ERROR("%s: can't clear page buffer", __func__);
			return res;
		}

		for (uint16_t i = 0; i < chip->page_size; i += sizeof(uint64_t)) {
			uint64_t dw;
			uint32_t w;

			res = target_read_u32(bank->target, f_addr, &w);
			if (res != ERROR_OK) {
				LOG_ERROR("%s: %d", __func__, __LINE__);
				return res;
			}
			dw = (uint64_t)w<<32;
			res = target_read_u32(bank->target, f_addr + sizeof(uint32_t), &w);
			if (res != ERROR_OK) {
				LOG_ERROR("%s: %d", __func__, __LINE__);
				return res;
			}
			...
			res = target_write_u32(bank->target, f_addr - sizeof(uint64_t), dw);
			if (res != ERROR_OK) {
				LOG_ERROR("%s: %d", __func__, __LINE__);
				return res;
			}
			res = target_write_u32(bank->target,
					f_addr - sizeof(uint64_t) + sizeof(uint32_t), dw >> 32);
			if (res != ERROR_OK) {
				LOG_ERROR("%s: %d", __func__, __LINE__);
				return res;
			}
		...
		res = sam4l_flash_command(bank->target, SAM4L_FCMD_EP, -1);
		if (res != ERROR_OK)
			return res;

		/* Commit the page to Flash */
		res = sam4l_flash_command(bank->target, SAM4L_FCMD_WP, -1);
		if (res != ERROR_OK)
			return res;
	}
...
static int sam4l_info(struct flash_bank *bank, char *buf, int buf_size)
{
	if (bank->target->state != TARGET_HALTED) {
		LOG_ERROR("Target not halted");
		return ERROR_TARGET_NOT_HALTED;
