...
static int sam4l_flash_command(struct target *target, uint8_t cmd, int page)
{
	...
	if (err != 0) {
		if (err & (1<<2))
			LOG_ERROR("%s: Lock Error", __func__);
		if (err & (1<<3))
			LOG_ERROR("%s: Program Error", __func__);
		return ERROR_FAIL;
	}
...
static int sam4l_protect(struct flash_bank *bank, int set, int first, int last)
{
	 ...
	 * a region containing one page in that sector, we arbitrarily choose the
	 * 0th page in the sector. */
...
int sam4l_write_page(struct sam4l_info *chip, struct target *target,
		...
		uint32_t offset, uint32_t count)
{
	...
	if (offset % chip->page_size) {
		...
		if (nb > count)
			nb = count;
	} else if (count < (uint32_t)chip->page_size) {
	...
	if (nb > 0) {
		...
		pg = malloc(chip->page_size);
		if (!pg)
			return ERROR_FAIL;

		/* Retrieve the first page contents from Flash */
		res = target_read_memory(bank->target, f_addr, 4,
				chip->page_size / 4, pg);
		if (res != ERROR_OK) {
			LOG_ERROR("%s: %d", __func__, __LINE__);
			free(pg);
		...
		memcpy(pg + (offset % chip->page_size), buffer, nb);

		/* Write the page back out */
		res = sam4l_write_page(chip, bank->target, f_addr, pg);
		free(pg);
	...
	if (count >= (uint32_t)chip->page_size) {
		int np = count / chip->page_size;

		for (int i = 0; i < np; i++) {
			res = sam4l_write_page(chip, bank->target, f_addr,
			...
			f_addr += chip->page_size;
		}
	}

	/* There's an aligned partial page left */
	if (count > 0) {
		uint8_t *pg;
		...
		/* Retrieve the last page contents from Flash */
		res = target_read_memory(bank->target, f_addr, 4,
				chip->page_size / 4, pg);
		if (res != ERROR_OK) {
			LOG_ERROR("%s: %d", __func__, __LINE__);
