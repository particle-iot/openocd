		...
		uint32_t address, uint8_t *buf)
{
	...
	res = target_write_memory(target, address, sizeof(uint32_t),
			chip->page_size / sizeof(uint32_t), buf);
		...
		uint32_t offset, uint32_t count)
{
	...
	if (nb > 0) {
		...
		res = target_read_memory(bank->target, f_addr, sizeof(uint32_t),
				chip->page_size / sizeof(uint32_t), pg);
		if (res != ERROR_OK) {
			LOG_ERROR("%s: %d", __func__, __LINE__);
			free(pg);
			return res;
