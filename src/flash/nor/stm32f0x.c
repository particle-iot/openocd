...
static int stm32x_probe(struct flash_bank *bank)
{
	...
	int retval = target_read_u32(target, 0xE0042000, &device_id); // TODO Need to find the right address
	if (retval != ERROR_OK)
		return retval;
	...
	} else if ((device_id & DEV_ID_MASK) == 0x000) { //TODO Find the right DEV_ID_MASK
...
static int get_stm32x_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	} else if ((device_id & DEV_ID_MASK) == 0x000) {	//TODO Need to be fixed with the real Device_ID
