...
/*
	...
	https://download.nuvoton.com/NuvotonMOSS/DownloadService/member/DownloadGuestItemNoTermsOfUse.aspx?Qrydownloadfilename=DA00-MINI51_52_54C1.pdf&Qrydownloadfilepath=Data%20Sheet,DA00-MINI51__52__54,DA00-MINI51_52_54C1.pdf
	...
	* The TRM suggests that after the boot source has been selected, a software reset should be performed by setting bit SWRST in ISPCON. However, this doesn't seem to have any effect on the MCU I'm using. At the moment, the ARM core is reset using the IPRSTC1 register, which seems to do the trick.

	Flash access limitations
	------------------------

	APROM can only be modified when the MCU has booted off the LDROM. For write and erase operations, the microcontroller will probably need to be rebooted. Pseudocode:
...
static int mini51_write(struct flash_bank *bank, uint8_t *buffer, uint32_t offset, uint32_t count)
{
	...
	for (uint32_t i = offset; i < offset + count; i += 4) {
		...
		if (status != ERROR_OK)
			return status;
		status = target_write_u32(target, ISPDAT, *(buf++));
		if (status != ERROR_OK)
			return status;
