...
{																			\
	...
	if (retval == ERROR_OK)													\
		retval = target_write_u32(target, cmspi_info->io[index].addr		\
			+ (int8_t) ((cmspi_info->dir_off >> (index << 3)) & 0xFF),		\
			pdir);															\
}
...
#define FILL_PORT_BUFFER(spi_cmd)											\
	{																		\
		h_to_le_32((cmspi_info->inp_off << 16) |							\
			(cmspi_info->dummy << 8) | (spi_cmd)),							\
		PARAM(bit_no), PARAM(dir_off),										\
		PARAM(ncs.addr), PARAM(ncs.mask),									\
		PARAM(clk.addr), PARAM(clk.mask),									\
		PARAM(io[0].addr),													\
		PARAM(mode == MODE_SPI ? cmspi_info->io0_mask : cmspi_info->io[0].mask),	\
...
static int set_to_output(struct flash_bank *bank, bool out)
{
	...
	switch (cmspi_info->mode) {
		case MODE_SPI:
			...
			dir_off = (cmspi_info->dir_off >> (0 << 3)) & 0xFF;
			retval = target_write_u32(target,
				cmspi_info->io[0].addr + ((int8_t) dir_off),
				cmspi_info->io_def[0] ^ (out ? cmspi_info->io0_mask : 0));
			if (retval != ERROR_OK) {
		...
		case MODE_DPI:
			...
			for (int k = 0; k < 2; k++) {
				dir_off = (cmspi_info->dir_off >> (k << 3)) & 0xFF;
				if (dir_off != 0) {
					retval = target_write_u32(target,
						cmspi_info->io[k].addr + ((int8_t) dir_off),
						cmspi_info->io_def[k] ^ (out ? cmspi_info->io[k].mask : 0));
					if (retval != ERROR_OK) {
...
FLASH_BANK_COMMAND_HANDLER(cmspi_flash_bank_command)
{
	...
	COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], cmspi_info->inp_off);
	...
	do {
		...
		if (byte < 32) {
			...
			command_print(CMD_CTX, "cmspi: IO3/NHOLD, IO2/NWP, IO1/MISO/SDA, "
				"IO0/MOSI/SCL bit numbers must be in 0 ... 31");
			return ERROR_COMMAND_SYNTAX_ERROR;
		}
		COMMAND_PARSE_NUMBER(u8, CMD_ARGV[index++], byte);
		if (byte == 0) {
			command_print(CMD_CTX, "cmspi: dir offset must be non-zero");
			return ERROR_COMMAND_SYNTAX_ERROR;
		}
		cmspi_info->dir_off |= (byte << (k << 3));
	...
	if (cmspi_info->mode == MODE_I2C) {
		...
		cmspi_info->same_conf =  (cmspi_info->same_port &&
			(((cmspi_info->dir_off ^ (cmspi_info->dir_off >> 8)) & 0xFF) == 0));

		LOG_DEBUG("SDA (in/out):  0x%08" PRIx32 ", mask 0x%08" PRIx32,
			cmspi_info->io[I2C_SDA].addr, cmspi_info->io[I2C_SDA].mask);
		LOG_DEBUG("SCL (in/out):  0x%08" PRIx32 ", mask 0x%08" PRIx32,
			cmspi_info->io[I2C_SCL].addr, cmspi_info->io[I2C_SCL].mask);
		...
		do {
			LOG_DEBUG("IO%d (in/out): 0x%08" PRIx32 ", exor 0x%08" PRIx32,
				k, cmspi_info->io[k].addr, cmspi_info->io[k].mask);
		} while (k-- > 0);
	...
	LOG_DEBUG("dir_off:       0x%08" PRIx32, cmspi_info->dir_off);
...
static int cmspi_i2c_start_stop(struct flash_bank *bank, bool start)
{
	...
	retval = target_read_u32(target, cmspi_info->SPI_SCL.addr
		+ (int8_t) ((cmspi_info->dir_off >> (I2C_SCL << 3)) & 0xFF), &pdir_clk);
	if (retval != ERROR_OK)
		return retval;
	...
	if (cmspi_info->same_conf)
		...
		retval = target_read_u32(target, cmspi_info->SPI_SDA.addr
			+ (int8_t) ((cmspi_info->dir_off >> (I2C_SDA << 3)) & 0xFF), &pdir_iox);
		if (retval != ERROR_OK)
			return retval;
	}
	...
	switch (cmspi_info->mode) {
		...
		case MODE_I2C:
			...
			retval = target_read_u32(target,
				cmspi_info->SPI_SDA.addr + ((int8_t) cmspi_info->inp_off), &in_iox);
			if (retval != ERROR_OK)
				return retval;
	...
	switch (cmspi_info->mode) {
		case MODE_SPI:
			...
			for (k = 0; k < 8; k++)	{
				...
				/* read data bit */
				retval = target_read_u32(target,
					cmspi_info->io[1].addr + ((int8_t) cmspi_info->inp_off), &port_iox);
				if (retval != ERROR_OK)
					return retval;
		...
		case MODE_DPI:
			...
			for (k = 0; k < 8 / 2; k++) {
				...
				for (m = 2 - 1; m >= 0; --m) {
					/* shift in data bits, msb first */
					retval = target_read_u32(target,
						cmspi_info->io[m].addr + ((int8_t) cmspi_info->inp_off), &port_iox);
					if (retval != ERROR_OK)
						return retval;
		...
		case MODE_I2C:
			...
			for (k = 0; k < 8; k++)	{
				...
				/* read SDA */
				retval = target_read_u32(target,
					cmspi_info->SPI_SDA.addr + ((int8_t) cmspi_info->inp_off), &port_iox);
				if (retval != ERROR_OK)
					return retval;
...
static int cmspi_get_io_def(struct flash_bank *bank)
{
	...
	do {
		...
		if ((cmspi_info->io[k].addr != 0) && (cmspi_info->io[k].mask != 0)) {
			dir_reg = cmspi_info->io[k].addr
				+ ((int8_t) ((cmspi_info->dir_off >> (k << 3)) & 0xFF));
	...
	switch (cmspi_info->mode) {
		...
		case MODE_QPI:
			...
			if (cmspi_info->dev.pagesize == 0)
				cmspi_info->dev.pagesize = cmspi_info->dev.sectorsize;
...
static int get_cmspi_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	snprintf(buf, buf_size, "flash \'%s\', device id = 0x%06" PRIx32
		", flash size = %" PRIu32 "%sBytes\n(page size = %d, read = 0x%02"
		PRIx8 ", qread = 0x%02" PRIx8 ", pprog = 0x%02" PRIx8
		", mass_erase = 0x%02" PRIx8 ", sector size = %" PRIu32 "%sBytes"
		", sector_erase = 0x%02" PRIx8 ")",
		cmspi_info->dev.name, cmspi_info->dev.device_id,
		(bank->size / 4096) ? bank->size / 1024 : bank->size,
		(bank->size / 4096) ? "k" : "", cmspi_info->dev.pagesize,
		cmspi_info->dev.read_cmd, cmspi_info->dev.qread_cmd,
		cmspi_info->dev.pprog_cmd, cmspi_info->dev.chip_erase_cmd,
		(cmspi_info->dev.sectorsize / 4096) ?
			cmspi_info->dev.sectorsize / 1024 : cmspi_info->dev.sectorsize,
		(cmspi_info->dev.sectorsize / 4096) ? "k" : "",
		cmspi_info->dev.erase_cmd);
	...
	switch (cmspi_info->mode) {
		...
		case MODE_I2C:
			...
			if (index < CMD_ARGC) {
				...
				retval = cmspi_shift_out(bank, cmspi_info->i2c_addr | 0x0);
				if (retval != ERROR_OK) {
			...
			if (num_bytes > 0) {
				...
				retval = cmspi_shift_out(bank, cmspi_info->i2c_addr | 0x1);
				if (retval != ERROR_OK) {
					if (retval == ERROR_FLASH_OPERATION_FAILED)
...
struct flash_driver cmspi_flash = {
	.name = "cmspi",
	...
	.usage = "$_FLASHNAME cmspi base 0 0 0 $_TARGETNAME inp_off dir_off "
