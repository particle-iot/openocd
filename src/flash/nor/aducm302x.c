...
static int aducm302x_probe(struct flash_bank *bank)
{
	...
	obj = Jim_GetGlobalVariableStr(global_cmd_ctx->interp, "_CHIPNAME", JIM_NONE);
	if (!obj) {
		LOG_ERROR("%s: _CHIPNAME is not defined", target_name(target));
		return ERROR_FAIL;
	}
	chipname = Jim_GetString(obj, NULL);

	obj = Jim_GetGlobalVariableStr(global_cmd_ctx->interp, "_CHIPID", JIM_NONE);
	if (!obj) {
		LOG_ERROR("%s: _CHIPID is not defined", target_name(target));
		return ERROR_FAIL;
	}
	retval = Jim_GetLong(global_cmd_ctx->interp, obj, &value);
	if (retval == JIM_OK)
