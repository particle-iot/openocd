				   ...
				   uint8_t *data, int len, int *transferred)
{
	...
	/*
	 * These constants must match the worst case in the code
	 * generator below
	 */
	...
	int to_transfer = len > max_len ? max_len : len;
	int padded_len = (to_transfer + 3) & ~3;
	uint32_t *out = malloc(padded_len);

	memset(out, 0xa5, padded_len);
	...
	LOG_DEBUG("ath79_spi_bitbang_bytes(%p, %08x, %p, %d)",
		  target, io_base, data, len);

	LOG_DEBUG("max code %d => max len %d. to_transfer %d",
		  ctx.max_code, max_len, to_transfer);
	...
	if (ath79_info->spi.pre_deselect) {
		...
		/* $2 = SPI_CS_DIS */
		pracc_add(&ctx, 0, MIPS32_LUI(2, UPPER16(cs_high)));
		/* $2 = SPI_CS_DIS */
		pracc_add(&ctx, 0, MIPS32_ORI(2, 2, LOWER16(cs_high)));
		/* [$1 + WRITE] = $2 */
		pracc_add(&ctx, 0, MIPS32_SW(2, ATH79_REG_WRITE, 1));
	}
	/* t0 = CLOCK_LOW + 0-bit */
	pracc_add(&ctx, 0, MIPS32_LUI(8, UPPER16((clock_low + 0))));
	pracc_add(&ctx, 0, MIPS32_ORI(8, 8, LOWER16((clock_low + 0))));
	/* t1 = CLOCK_LOW + 1-bit */
	pracc_add(&ctx, 0, MIPS32_LUI(9, UPPER16((clock_low + 1))));
	pracc_add(&ctx, 0, MIPS32_ORI(9, 9, LOWER16((clock_low + 1))));
	/* t2 = CLOCK_HIGH + 0-bit */
	pracc_add(&ctx, 0, MIPS32_LUI(10, UPPER16((clock_high + 0))));
	pracc_add(&ctx, 0, MIPS32_ORI(10, 10, LOWER16((clock_high + 0))));
	/* t3 = CLOCK_HIGH + 1-bit */
	pracc_add(&ctx, 0, MIPS32_LUI(11, UPPER16((clock_high + 1))));
	pracc_add(&ctx, 0, MIPS32_ORI(11, 11, LOWER16((clock_high + 1))));
	...
	if (to_transfer & 3) { /* not a multiple of 4 bytes */
		/* $3 = [$1 + DATA] */
		pracc_add(&ctx, 0, MIPS32_LW(3, ATH79_REG_DATA, 1));
		/* [OUTi] = $3 */
		pracc_add(&ctx, MIPS32_PRACC_PARAM_OUT + pracc_out,
	...
	if (ath79_info->spi.post_deselect && len == to_transfer) {
		/* $2 = SPI_CS_DIS */
		pracc_add(&ctx, 0, MIPS32_LUI(2, UPPER16(cs_high)));
		/* $2 = SPI_CS_DIS */
		pracc_add(&ctx, 0, MIPS32_ORI(2, 2, LOWER16(cs_high)));
	...
	if (to_transfer & 3) { /* not a multiple of 4 bytes */
		 ...
		 * full 32 bits.
		 */
		int missed_bytes = 4 - (to_transfer & 3);

		out[pracc_words - 1] <<= 8 * missed_bytes;
	}
	...
	memcpy(data, out, to_transfer);
	*transferred = to_transfer;

exit:
	if (ctx.retval != ERROR_OK) {
		memset(data, 0x5a, to_transfer);
		*transferred = 0;
	}
	if (out)
		free(out);
...
FLASH_BANK_COMMAND_HANDLER(ath79_flash_bank_command)
{
	...
	memset(ath79_info, 0, sizeof(*ath79_info));
