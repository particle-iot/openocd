...
/* Load immediate preudo-instruction */
#define ATH79_LI(reg, val) \
	MIPS32_LUI((reg), UPPER16((val))), \
	MIPS32_ORI((reg), (reg), LOWER16((val)))
				     ...
				     int partial_xfer)
{
	...
	if (ath79_info->spi.pre_deselect) {
		...
		ath79_pracc_addn(ctx, pre_deselect, ARRAY_SIZE(preamble1));
	}
	const uint32_t preamble2[] = {
		/* t0 = CLOCK_LOW + 0-bit */
		ATH79_LI(8, clock_low + 0),
		/* t1 = CLOCK_LOW + 1-bit */
		ATH79_LI(9, clock_low + 1),
		/* t2 = CLOCK_HIGH + 0-bit */
		ATH79_LI(10, clock_high + 0),
		/* t3 = CLOCK_HIGH + 1-bit */
		ATH79_LI(11, clock_high + 1),
	};
