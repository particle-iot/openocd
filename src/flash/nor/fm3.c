                           ...
                           uint32_t offset, uint32_t count)
{
	...
	while (count > 0)
	{
		...
		// LOG_INFO("====== TARGET_WRITE_BUFFER: %X", fm3_info->write_algorithm->address);	
		...
		{
			// LOG_INFO("====== Patching addresses1: %X with %X", (fm3_info->write_algorithm->address)
			...
			// LOG_INFO("====== Patching addresses2: %X with %X", (fm3_info->write_algorithm->address)
		...
		// LOG_INFO("====== TARGET_WRITE_BUFFER: %X Count: %X", source->address, thisrun_count);
		...
		// LOG_INFO("====== RUN_ALGORITHM: %X Source r0: %X Dest r1 %X",
		//       fm3_info->write_algorithm->address, source->address, address);	
...
{
	snprintf(buf, buf_size, "Fujitsu fm3 Device does not support Chip-ID (Type \
	                         unknown)");
	return ERROR_OK;
}
