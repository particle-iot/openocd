	...
	uint32_t result_table[4])
{
	...
	retval = target_write_memory(target,
			iap_working_area->address, 4, 2, jump_gate);
	if (retval != ERROR_OK) {
		LOG_ERROR(
	...
	init_mem_param(&mem_params[0], iap_working_area->address + 8, 6 * 4,
		PARAM_OUT);
