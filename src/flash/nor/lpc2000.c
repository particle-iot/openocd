...
#define IAP_TRAMPOLINE_SIZE 0x34
...
static int lpc2000_iap_working_area_init(struct flash_bank *bank, struct working_area **iap_working_area)
{
	...
	if (target_alloc_working_area(target, IAP_TRAMPOLINE_SIZE + lpc2000_info->iap_max_stack, iap_working_area) != ERROR_OK) {
		...
		uint32_t param_table[5], uint32_t result_table[4])
{
	...
	switch (lpc2000_info->variant) {
		...
		case lpc_auto:
			...
			buf_set_u32(reg_params[3].value, 0, 32, iap_working_area->address + IAP_TRAMPOLINE_SIZE + lpc2000_info->iap_max_stack);
	...
	if (!lpc2000_info->probed) {
		if (lpc2000_info->variant == lpc_auto) {
			...
			if (status == LPC2000_CMD_SUCCESS) {
				LOG_INFO("If auto-detection fails for this part, please email openocd-devel@lists.sourceforge.net, citing part id 0x%x.\n", part_id);
			}
		}
