...
COMMAND_HANDLER(sim3x_check_security)
{
	uint32_t val;
	int ret;
	...
	if (dap == NULL) {
		...
		LOG_INFO("mass_erase can't be used by this debug interface");
		...
		if ((val && CPUID_CHECK_VALUE_MASK) != CPUID_CHECK_VALUE) {
			LOG_ERROR("Target is not ARM CortexM3 or is locked");
...
COMMAND_HANDLER(sim3x_lock)
{
	...
	if (val == LOCK_WORD_MCU_UNLOCKED) {
		...
		if (dap == NULL) { /* SIM3X_AP_ID_VALUE is not checked */
			LOG_INFO("Maybe this isn't SiM3x MCU");
		}
		return ERROR_FAIL;
	}
}
