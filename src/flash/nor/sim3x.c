...
static int sim3x_init(struct flash_bank *bank)
{
	...
	((struct sim3x_info *) bank->driver_priv)->need_init = 0;
	return ERROR_OK;
}
...
COMMAND_HANDLER(set_need_init)
{
	...
	((struct sim3x_info *) bank->driver_priv)->need_init = 1;
...
static const struct command_registration sim3x_exec_command_handlers[] = {
	{
		.name = "need_init",
		.handler = set_need_init,
		.mode = COMMAND_EXEC,
		.help = "should be called after a reset to let flash driver know that"
				"the MCU need to be initialized before writing or erasing",
		.usage = "<bank>",
	},
