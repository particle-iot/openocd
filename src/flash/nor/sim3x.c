...
{
	int			probed;
	int			flash_locked; // 1 => Locked, 0 => Unlocked by 0xFFFFFFFF, -1 => Unlocked by 0x00000000 (erase needed for lock)
...
/*
	// MCU check
	ret = target_read_u32(bank->target, DEVICEID0_DEVICEID2,  &uDeviceId.u32);
	if(ERROR_OK != ret)
		return ret;
	...
	ret = target_read_u32(bank->target, DEVICEID0_DEVICEID1,  &uDeviceId.u32);
	if(ERROR_OK != ret)
		return ret;
	...
	ret = target_read_u32(bank->target, DEVICEID0_DEVICEID0,  &uDeviceId.u32);
	if(ERROR_OK != ret)
		return ret;
	...
	do
	{
		...
		{ // If is not busy
			...
			{ // If erase is not enabled
				...
				ret = target_write_u32(target, (uint32_t)&FLASHCTRL0_CONFIG->set, FLASHCTRL0_CONFIG_ERASEEN_MASK);
				if(ERROR_OK != ret)
					return ret;
			}
			...
			ret = target_write_u32(target, FLASHCTRL0_KEY, FLASHCTRL0_KEY_INITIAL_UNLOCK);
			if(ERROR_OK != ret)
				return ret;

			// Write the single unlock value to KEY
			ret = target_write_u32(target, FLASHCTRL0_KEY, FLASHCTRL0_KEY_SINGLE_UNLOCK);
			if(ERROR_OK != ret)
				return ret;
...
static int sim3x_flash_write(struct flash_bank *bank, uint8_t * buffer,  uint32_t offset, uint32_t count)
{
	int ret;
	Sim3x_info * sim3x_info;
	struct target * target;
...
static int sim3x_flash_protect(struct flash_bank *bank, int set, int first, int last)
{
	...
	case -1: // Lock word is 0x00000000 and can not be set without erase
		...
		ret = target_read_buffer(target, LOCK_WORD - lock_word_offset, sim3x_info->flash_page_size, buffer);
		if(ERROR_OK != ret)
			break;
		...
		ret = sim3x_flash_write(bank, buffer,  LOCK_WORD - lock_word_offset, sim3x_info->flash_page_size);
		if(ERROR_OK != ret)
			break;
...
/*
	// Software Reset
	ret = target_write_u32(target, (uint32_t)&RSTSRC0_RESETEN->set, RSTSRC0_RESETEN_SWREN_MASK);
	if(ERROR_OK != ret)
		return ret;
...
*/
	...
	ret = target_write_u32(target, (uint32_t)&WDTIMER0_CONTROL->set, WDTIMER0_CONTROL_DBGMD_MASK);
	if(ERROR_OK != ret)
		return ret;

	// Enable VDD Supply Monitor and set as a reset source
	ret = target_write_u32(target, (uint32_t)&VMON0_CONTROL->set, VMON0_CONTROL_VMONEN_MASK);
	if(ERROR_OK != ret)
		return ret;

	ret = target_write_u32(target, (uint32_t)&RSTSRC0_RESETEN->set, RSTSRC0_RESETEN_VMONREN_MASK);
	if(ERROR_OK != ret)
		return ret;

	// Flash Controller Clock Enable
	ret = target_write_u32(target, (uint32_t)&CLKCTRL0_APBCLKG0->set, CLKCTRL0_APBCLKG0_FLCTRLCEN_MASK);
	if(ERROR_OK != ret)
		return ret;
...
static int sim3x_flash_protect_check(struct flash_bank *bank)
{
	...
	for (i = 0; i < bank->num_sectors; i++)
		bank->sectors[i].is_protected = ( (Sim3x_info *)bank->driver_priv )->flash_locked > 0;
...
/*
	...
	// Package
	printed = snprintf(buf, buf_size, "-%c-%s", sim3x_info->device_package[0], sim3x_info->device_package + 1);
	buf += printed;
	buf_size -= printed;
	...
	{
		printed = snprintf(buf, buf_size, " - Rev: %c", sim3x_info->device_revision + 'A');
		buf += printed;
		buf_size -= printed;
