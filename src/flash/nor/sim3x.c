...
FLASH_BANK_COMMAND_HANDLER(sim3x_flash_bank_command)
{
	...
	/* Get part number and family from target name */
	if (strlen(CMD_ARGV[5]) >= 8 &&
	   strstr(CMD_ARGV[5], "sim3") == CMD_ARGV[5]) {
		sim3x_info->part_family = CMD_ARGV[5][4];
		strncpy(tmp_str, &CMD_ARGV[5][5], 3);
		sim3x_info->part_number = atoi(tmp_str);
	} else {
		LOG_INFO("Unknown target, trying to probe chip");
	}
...
static int sim3x_read_info(struct flash_bank *bank)
{
	...
	if (ret != ERROR_OK)
		LOG_WARNING("Failed to read info from MCU, using info from target");

	ret = sim3x_parse_part_info(sim3x_info);
	if (ret != ERROR_OK) {
		LOG_WARNING("Failed to parse info from target, using info from flash bank parameters");
		...
		sim3x_info->flash_size_kb = bank->size;
	}
