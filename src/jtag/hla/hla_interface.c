...
static struct hl_interface_s interface = { {0, 0, 0, 0, 0, 0}, 0, 0 };
...
int hl_interface_open(enum hl_transports tr)
{
	...
	/* set transport mode */
	interface.param.transport = tr;

	return interface.layout->open(&interface);
}
...
int hl_interface_init_target(struct target *t)
{
	 ...
	 */
	res = interface.layout->api->idcode(interface.fd, &t->tap->idcode);
...
static int hl_interface_init(void)
{
	...
	return hl_layout_init(&interface);
}
...
COMMAND_HANDLER(hl_interface_handle_device_desc_command)
{
	...
	if (CMD_ARGC == 1) {
		interface.param.device_desc = strdup(CMD_ARGV[0]);
	} else {
		LOG_ERROR
...
COMMAND_HANDLER(hl_interface_handle_serial_command)
{
	...
	if (CMD_ARGC == 1) {
		interface.param.serial = strdup(CMD_ARGV[0]);
	} else {
		LOG_ERROR
		    ("expected exactly one argument to hl_serial <serial-number>");
	}
...
COMMAND_HANDLER(hl_interface_handle_layout_command)
{
	...
	if (interface.layout) {
		LOG_ERROR("already specified hl_layout %s",
				interface.layout->name);
		return (strcmp(interface.layout->name, CMD_ARGV[0]) != 0)
		    ? ERROR_FAIL : ERROR_OK;
	}
...
COMMAND_HANDLER(hl_interface_handle_vid_pid_command)
{
	...
	if (CMD_ARGC != 2) {
		LOG_WARNING
		    ("ignoring extra IDs in hl_vid_pid (maximum is 1 pair)");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}

	COMMAND_PARSE_NUMBER(u16, CMD_ARGV[0], interface.param.vid);
	COMMAND_PARSE_NUMBER(u16, CMD_ARGV[1], interface.param.pid);
