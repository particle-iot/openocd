...
COMMAND_HANDLER(handle_adapter_nsrst_pulse_width_command)
{
	if (CMD_ARGC > 1)
		return ERROR_COMMAND_SYNTAX_ERROR;
	if (CMD_ARGC == 1) {
	...
	command_print(CMD, "adapter_nsrst_pulse_width: %u", jtag_get_nsrst_assert_width());
	return ERROR_OK;
}
...
static const struct command_registration adapter_nsrst_command_handlers[] = {
	...
	{
		.name = "pulse_width",
		.handler = handle_adapter_nsrst_pulse_width_command,
		.mode = COMMAND_ANY,
