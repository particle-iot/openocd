 ...
 */
static void ublast_queue_byte(uint8_t byte)
{
	if (nb_buf_remaining() < 1)
		ublast_flush_buffer();
	info.buf[info.bufidx++] = byte;
	if (nb_buf_remaining() == 0)
		ublast_flush_buffer();
	DEBUG_JTAG_IO("(byte=0x%02x)", byte);
}
...
static uint8_t ublast_build_out(enum scan_type type)
{
	...
	byte |= info.tms ? TMS : 0;
	byte |= info.pin6 ? NCE : 0;
	byte |= info.pin8 ? NCS : 0;
	byte |= info.tdi ? TDI : 0;
	byte |= LED;
