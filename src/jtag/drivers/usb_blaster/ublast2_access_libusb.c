...
static int ublast2_libusb_init(struct ublast_lowlevel *low)
{
	...
	if (renumeration == false) {
		if (jtag_libusb_open(vids_renum, pids_renum, NULL, NULL, &low->libusb_dev) != ERROR_OK) {
			LOG_ERROR("Altera USB-Blaster II not found");
			return ERROR_FAIL;
		}
	} else {
		int retry = 10;
		while (jtag_libusb_open(vids_renum, pids_renum, NULL, NULL, &low->libusb_dev) != ERROR_OK && retry--) {
			usleep(1000000);
