...
/* PID/VID */
#define	OSBDM_USB_PID 0x0058
#define OSBDM_USB_VID 0x15a2
...
static int osbdm_open(struct osbdm *osbdm)
{
	...
	uint16_t pids[] = { OSBDM_USB_PID, 0 };
	uint16_t vids[] = { OSBDM_USB_VID, 0 };	
		
	if (jtag_libusb_open(vids, pids, &osbdm->devh) != ERROR_OK) { 
		return ERROR_FAIL;
	}
		
	if (jtag_libusb_set_configuration(osbdm->devh, 1) != ERROR_OK) {
		return ERROR_FAIL;
	}	
...
{	
	...
	struct sequence *next;
	if ( !(next = queue_add_tail(queue, num_states))) {
...
{			
	...
	if (len) {		
		if ( !(next = queue_add_tail(queue, len))) {
			LOG_ERROR("BUG: can't allocate bit sequence");
			return ERROR_FAIL;
		}
	...
	int count)
{
	...
	if ( !(next = queue_add_tail(queue, count))) {
		LOG_ERROR("BUG: can't allocate bit sequence");
		return ERROR_FAIL;
	}
...
{
	struct sequence *next;
	if ( !(next = queue_add_tail(queue, num_bits))) {
		LOG_ERROR("BUG: can't allocate bit sequence");
		return ERROR_FAIL;
	}	
...
{	
	...
	for (int idx = 0; idx < num_fields; idx++) {		
		struct sequence* next;
		if ( !(next = queue_add_tail(queue, fields[idx].num_bits))) {
...
{	
	...
	case JTAG_RESET:
		if (cmd->cmd.reset->trst) {
			...
			if ( (retval = osbdm_flush(osbdm, queue)) == ERROR_OK) {
...
{	
	...
	osbdm_context.buffer[osbdm_context.count++] = BDM_CMD_INIT;	
