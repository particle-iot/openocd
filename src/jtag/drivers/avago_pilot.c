...
static int avago_field_execute_in(uint8_t *buffer, unsigned int scan_size, unsigned char cmd, unsigned char ir_scan)
{
	...
	unsigned int num_bytes = DIV_ROUND_UP(scan_size, 8);
	unsigned int i = 0;
...
static int avago_khz(int khz, int *jtag_speed)
{
	...
	/* Search for exact match or the nearest (lower) frequency in the
	   table. This is why the table should be in descending order */
	for (i = 0; i < (ssize_t)(ARRAY_SIZE(g_avago_freq_map)); i++) {
		/* Speed identifier is the index in the array itself */
		if (g_avago_freq_map[i].freq <= khz) {
			*jtag_speed = i;
			return ERROR_OK;
		}
	}
...
static int avago_speed(int speed)
{
	...
	IOW(CONF_B, g_avago_freq_map[speed].regb_val);
