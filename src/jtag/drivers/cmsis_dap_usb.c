...
static uint16_t cmsis_dap_vid[MAX_USB_IDS + 1] = { 0xc251, 0xc251, 0xc251, 0x0d28, 0 };
static uint16_t cmsis_dap_pid[MAX_USB_IDS + 1] = { 0xf001, 0xf002, 0x2722, 0x0204, 0 };

#define BUFFER_SIZE       65	/* 64 bytes plus report id */
...
static int cmsis_dap_usb_open(void)
{
	...
	for (int i = 0; cmsis_dap_vid[i] || cmsis_dap_pid[i]; i++) {
...
static int cmsis_dap_usb_xfer(struct cmsis_dap *dap, int txlen)
{
	...
	int retval = hid_write(dap->dev_handle, dap->data, txlen);
	if (retval == -1) {
	...
	/* get reply */
	retval = hid_read_timeout(dap->dev_handle, dap->data, BUFFER_SIZE, USB_TIMEOUT);
...
static int cmsis_dap_cmd_DAP_SWJ_Pins(uint8_t pins, uint8_t mask, uint16_t wait, uint8_t *input)
{
	int retval;
...
static int cmsis_dap_cmd_DAP_SWJ_Clock(uint16_t swj_clock)
{
	int retval;
	uint8_t *buffer = cmsis_dap_handle->data;
...
static int cmsis_dap_swd_read_block(uint8_t cmd, uint32_t blocksize, uint32_t *dest_buf)
{
	uint8_t *buffer;
	int tfer_sz;
	int retval = ERROR_OK;
