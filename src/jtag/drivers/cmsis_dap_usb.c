...
static int cmsis_dap_usb_open(struct cmsis_dap *dap, uint16_t vids[], uint16_t pids[], char *serial)
{
	...
	for (int i = 0; i < num_devices; i++) {
		...
		if ((err = libusb_get_device_descriptor(dev, &dev_desc))) {
		...
		if ((err = libusb_open(dev, &dev_handle))) {
			LOG_ERROR("could not open device with 0x%04x:0x%04x: %s", dev_desc.idVendor, dev_desc.idProduct, libusb_strerror(err));
			continue;
		}
		...
		if (dev_desc.iSerialNumber > 0) {
			if ((err = libusb_get_string_descriptor_ascii(dev_handle, dev_desc.iSerialNumber, (uint8_t*)dev_serial, sizeof(dev_serial))) < 0) {
				LOG_ERROR("could not read serial number for device 0x%04x:0x%04x: %s", dev_desc.idVendor, dev_desc.idProduct, libusb_strerror(err));
		...
		if (dev_desc.iProduct > 0) {
			if ((err = libusb_get_string_descriptor_ascii(dev_handle, dev_desc.iProduct, (uint8_t*)product_string, sizeof(product_string))) < 0) {
				LOG_ERROR("could not read product string for device 0x%04x:0x%04x: %s", dev_desc.idVendor, dev_desc.idProduct, libusb_strerror(err));
		...
		for (int config = 0; config < dev_desc.bNumConfigurations; config++) {
			...
			if ((err = libusb_get_config_descriptor(dev, config, &config_desc))) {
				LOG_ERROR("could not get configuration descriptor %d for device 0x%04x:0x%04x: %s", config, dev_desc.idVendor, dev_desc.idProduct, libusb_strerror(err));
				continue;
			}
			...
			for (int interface = 0; interface < config_desc->bNumInterfaces; interface++) {
				...
				if (intf_desc->bInterfaceClass != LIBUSB_CLASS_VENDOR_SPEC || intf_desc->bInterfaceSubClass != 0 ||  intf_desc->bInterfaceProtocol != 0)
					continue;

				if (intf_desc->bNumEndpoints < 2)
					continue;

				if ((intf_desc->endpoint[0].bmAttributes & 3) != LIBUSB_TRANSFER_TYPE_BULK || (intf_desc->endpoint[0].bEndpointAddress & 0x80) != LIBUSB_ENDPOINT_OUT)
					continue;

				if ((intf_desc->endpoint[1].bmAttributes & 3) != LIBUSB_TRANSFER_TYPE_BULK || (intf_desc->endpoint[1].bEndpointAddress & 0x80) != LIBUSB_ENDPOINT_IN)
					continue;
				...
				if (!cmsis_dap_found) {
					...
					if ((err = libusb_get_string_descriptor_ascii(dev_handle, intf_desc->iInterface, (uint8_t*)interface_str, sizeof(interface_str))) < 0) {
						LOG_ERROR("could not read interface string for device 0x%04x:0x%04x: %s", dev_desc.idVendor, dev_desc.idProduct, libusb_strerror(err));
						continue;
				...
				LOG_INFO("Using CMSIS-DAPv2 interface with VID:PID=0x%04x:0x%04x, serial=%s", dev_desc.idVendor, dev_desc.idProduct, dev_serial);

				(void) config_num;
				/*
				if ((err = libusb_set_configuration(dev_handle, config_num))) {
				...
				if ((err = libusb_claim_interface(dev_handle, interface_num))) {
...
static int cmsis_dap_usb_read(struct cmsis_dap *dap, int timeout_ms)
{
	...
	if ((err = libusb_bulk_transfer(dap->bdata->dev_handle, dap->bdata->ep_in, dap->packet_buffer, dap->packet_size, &transferred, timeout_ms))) {
...
static int cmsis_dap_usb_write(struct cmsis_dap *dap, int txlen, int timeout_ms)
{
	...
	if ((err = libusb_bulk_transfer(dap->bdata->dev_handle, dap->bdata->ep_out, dap->packet_buffer + 1, txlen - 1, &transferred, timeout_ms))) {
		if (err == LIBUSB_ERROR_TIMEOUT) {
			return ERROR_TIMEOUT_REACHED;
		} else {
...
struct cmsis_dap_backend cmsis_dap_usb_backend = {
