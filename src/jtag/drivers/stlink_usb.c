...
static const int stlink_khz_to_speed_map[] = {
	4000,
	1800, /* default */
	1200,
	950,
...
static int stlink_usb_set_swdclk(void *handle, uint16_t clk_freq)
{
	...
	h->cmdbuf[h->cmdidx++] = 0x43;
	h_u16_to_le(h->cmdbuf+h->cmdidx, clk_freq);
	h->cmdidx += 2;
...
{
	*khz = stlink_khz_to_speed_map[speed];
	return ERROR_OK;
}
...
static int stlink_khz(int khz, int *jtag_speed)
{
	...
	for (i = 0; i < ARRAY_SIZE(stlink_khz_to_speed_map); i++) {
		if (khz == stlink_khz_to_speed_map[i]) {
			*jtag_speed = i;
			break;
		} else {
			int current_diff = khz - stlink_khz_to_speed_map[i];
