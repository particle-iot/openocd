...
static int stlink_dap_op_run(struct adiv5_dap *dap)
{
	...
	if (dap->stlink_flush_ap_write) {
		dap->stlink_flush_ap_write = false;
		stlink_dap_op_queue_dp_read(dap, DP_RDBUFF, NULL);
	}

	saved_retval = stlink_dap_get_and_clear_error();

	stlink_dap_op_queue_dp_read(dap, DP_CTRL_STAT, &ctrlstat);
	...
	if (ctrlstat & SSTICKYERR) {
		if (stlink_dap_param.transport == HL_TRANSPORT_JTAG)
			stlink_dap_op_queue_dp_write(dap, DP_CTRL_STAT,
					ctrlstat & (dap->dp_ctrl_stat | SSTICKYERR));
		else
			stlink_dap_op_queue_dp_write(dap, DP_ABORT, STKERRCLR);
		retval = stlink_dap_get_and_clear_error();
		if (retval != ERROR_OK)
			dap->do_reconnect = true;
	}
...
/** */
static int stlink_dap_system_reset(int req_srst)
{
	LOG_DEBUG("stlink_dap_system_reset(%d)", req_srst);
...
struct adapter_driver stlink_dap_adapter_driver = {
	...
	.system_reset = stlink_dap_system_reset,
