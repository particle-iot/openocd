...
static int stlink_swim_writebytes(void *handle, uint32_t addr, const uint8_t *data, uint32_t len)
{
	struct stlink_usb_handle_s *h = handle;
	int res;
	unsigned int i;
...
static int stlink_swim_readbytes(void *handle, uint32_t addr, uint8_t *data, uint32_t len)
{
	struct stlink_usb_handle_s *h = handle;
	int res;
