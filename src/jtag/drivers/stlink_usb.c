...
#define STLINK_V2_RX_EP       (1|ENDPOINT_IN)
#define STLINK_V2_TX_EP       (2|ENDPOINT_OUT)
#define STLINK_V2_TRACE_EP    (3|ENDPOINT_IN)
#define STLINK_V2_1_RX_EP     (1|ENDPOINT_IN)
...
static int stlink_usb_open(struct hl_interface_param_s *param, void **fd)
{
	...
	/* STLINK V2-1 has different EP layout than previous versions */
	switch (param->pid)
	{
		case STLINK_V2_1_PID:
			h->rx_ep = STLINK_V2_1_RX_EP;
			h->tx_ep = STLINK_V2_1_TX_EP;
			h->trace_ep = STLINK_V2_1_TX_EP;
			LOG_DEBUG("using STLINK V2-1 EPs");
			break;
		default:
			h->rx_ep = STLINK_V2_RX_EP;
			h->tx_ep = STLINK_V2_TX_EP;
			h->trace_ep = STLINK_V2_TX_EP;
			break;
	}
	/*
