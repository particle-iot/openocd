		...
		if (res == ERROR_OK) {
			...
			if (size > 0) {
				...
				if (res == ERROR_OK) {
					...
					/* Log retrieved trace output but strip out the byte
					 * designator (0x01).  Trace data is received like this:
					 * 0x01 <byte0> 0x01 <byte1> ... 0x01 <byteN> */
					for (i = 0; i < size - 1; i++) {
						if (buf[i] == 0x01)
							fputc(buf[++i], h->trace.output_f);
					}
...
static int stlink_configure_target_trace_port(void *handle)
{
	...
	if (res != ERROR_OK)
		goto out;
	/* set the data width.  The STLinkv2 only supports one byte. */
	res = stlink_usb_write_debug_reg(handle, TPI_CSPSR, 1);
	if (res != ERROR_OK)
		goto out;
