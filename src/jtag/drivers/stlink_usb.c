...
#define STLINK_RX_EP	(1|LIBUSB_ENDPOINT_IN)
#define STLINK_TX_EP	(2|LIBUSB_ENDPOINT_OUT)
...
/** */
void stlink_usb_dump(uint8_t * b, int len)
{
	while (len--)
		printf("%02X ", *b++);
	printf("\n");
		    ...
		    int rxsize)
{
	...
	if ((err =
	     jtag_libusb_bulk_write(h->fd, STLINK_TX_EP, (char *)txbuf, txsize,
				    1000)) != txsize) {
		return ERROR_FAIL;
	}
	if (rxsize && rxbuf) {
		if ((err =
		     jtag_libusb_bulk_read(h->fd, STLINK_RX_EP, (char *)rxbuf,
					   rxsize, 1000)) != rxsize) {
			return ERROR_FAIL;
		}
	}
...
int stlink_usb_version(void *handle)
{
	...
	if ((res =
	     stlink_usb_recv(handle, h->txbuf, STLINK_CMD_SIZE, h->rxbuf,
			     6)) != ERROR_OK)
		return res;
...
int stlink_usb_dfu_mode_leave(void *handle)
{
	...
	if ((res =
	     stlink_usb_recv(handle, h->txbuf, STLINK_CMD_SIZE, 0,
			     0)) != ERROR_OK)
		return res;
...
int stlink_usb_init_mode(void *handle)
{
	...
	if ((res = stlink_usb_current_mode(handle, &mode)) != ERROR_OK)
		return res;

	LOG_DEBUG("MODE: %02X", mode);

	if (mode == STLINK_DEV_DFU_MODE) {
		if ((res = stlink_usb_dfu_mode_leave(handle)) != ERROR_OK)
			return res;
	}
	...
	if (mode != STLINK_DEV_DEBUG_MODE) {
		if ((res = stlink_usb_swd_mode_enter(handle)) != ERROR_OK)
			return res;
	}
...
//      LOG_DEBUG("STATUS: %08X",h->rxbuf[0]);
			   ...
			   uint32_t * buffer)
{
	...
	if ((res =
	     stlink_usb_recv(handle, (uint8_t *) buffer, len, 0,
			     0)) != ERROR_OK) {
		return res;
	}
//      stlink_usb_dump(h->rxbuf,4);
...
int stlink_usb_open(struct stlink_interface_param_s *param, void **fd)
{
	...
	if ((h = malloc(sizeof(struct stlink_usb_handle_s))) == 0) {
	...
	libusb_set_configuration(h->fd, 1);
