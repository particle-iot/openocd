...
struct stlink_usb_handle_s {
	...
	/** */
	enum e_stlink_transports transport;
	/** */
...
/** */
enum e_stlink_mode {
	e_stlink_mode_unknown = 0,
	e_stlink_mode_dfu,
	e_stlink_mode_mass,
	e_stlink_mode_debug_jtag,
	e_stlink_mode_debug_swd,
	e_stlink_mode_debug_swim
};
...
/** */
static int stlink_usb_mode_enter(void *handle, enum e_stlink_mode type)
{
	int res;
	struct stlink_usb_handle_s *h;
...
/** */
static int stlink_usb_mode_leave(void *handle, enum e_stlink_mode type)
{
	int res;
	struct stlink_usb_handle_s *h;
...
static int stlink_usb_init_mode(void *handle)
{
	...
	switch (mode) {
		case STLINK_DEV_DFU_MODE:
			emode = e_stlink_mode_dfu;
			break;
		case STLINK_DEV_DEBUG_MODE:
			emode = e_stlink_mode_debug_swd;
			break;
		case STLINK_DEV_SWIM_MODE:
			emode = e_stlink_mode_debug_swim;
			break;
		default:
			emode = e_stlink_mode_unknown;
			break;
	}

	if (emode != e_stlink_mode_unknown) {
	...
	/* set selected mode */
	switch (h->transport) {
		case e_stlink_transport_swd:
			emode = e_stlink_mode_debug_swd;
			break;
		case e_stlink_transport_jtag:
			emode = e_stlink_mode_debug_jtag;
			break;
		case e_stlink_transport_swim:
		default:
			emode = e_stlink_mode_unknown;
			break;
	}
