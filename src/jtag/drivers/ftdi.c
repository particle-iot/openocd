...
static void ftdi_swd_swdio_en(bool enable)
{
	...
	if (oe) {
		if (oe->data_mask)
			...
			/* Sets TDI/DO pin (pin 2) to input during rx when both pins are connected to SWDIO */
			mpsse_set_data_bits_low_byte(mpsse_ctx, output & 0xff, direction & (enable ? 0xff : 0xfd));
	}
}
