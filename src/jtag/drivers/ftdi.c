...
COMMAND_HANDLER(ftdi_handle_tdo_n_tck_command)
{
	int arg;
	...
	COMMAND_PARSE_NUMBER(int, CMD_ARGV[0], arg);

	if (arg == 1)
		ftdi_jtag_mode = JTAG_MODE_ALT;
	else
		ftdi_jtag_mode = JTAG_MODE;
...
static const struct command_registration ftdi_command_handlers[] = {
	...
	{
		.name = "ftdi_tdo_n_tck",
		.handler = &ftdi_handle_tdo_n_tck_command,
		.mode = COMMAND_ANY,
