 ...
 */
struct bdm_dev {
	struct bdm bdm;				/* This is a vtable used by all of
 ...
 */
static int usbdm_get_version(struct bdm_dev *bdev)
{
	static const struct {
		uint8_t version;
		const char *name;
 ...
 */
static int usbdm_get_capabilities(struct bdm_dev *bdev)
{
	/*
	  Command:
 ...
 * @returns ERROR_OK on success, negative error code on failure
 */
static int usbdm_set_target_type(struct bdm_dev *bdev, enum usbdm_target_types type)
{
	/*
	   Command:
 ...
 * @returns ERROR_OK on success, negative error code on failure
 */
static int usbdm_control_pins(struct bdm_dev *bdev, uint16_t mask)
{
	/*
	   Command:
 ...
 */
static int usbdm_generic_read_register(struct bdm_dev *bdev,
 ...
 */
static int_least32_t usbdm_read_xx(struct bdm *bdm, uint16_t reg)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
...
static int_least32_t usbdm_read_pc(struct bdm *bdm)
{
	return usbdm_read_xx(bdm, USBDM_HCS12_REG_PC);
}

static int_least32_t usbdm_read_sp(struct bdm *bdm)
{
	return usbdm_read_xx(bdm, USBDM_HCS12_REG_SP);
}

static int_least32_t usbdm_read_d(struct bdm *bdm)
{
	return usbdm_read_xx(bdm, USBDM_HCS12_REG_D);
}

static int_least32_t usbdm_read_x(struct bdm *bdm)
{
	return usbdm_read_xx(bdm, USBDM_HCS12_REG_X);
}

static int_least32_t usbdm_read_y(struct bdm *bdm)
{
 ...
 */
static int_least16_t usbdm_read_bd_byte(struct bdm *bdm, uint32_t address)
{
	uint32_t value;
	int ret;
 ...
 * @returns ERROR_OK on success negative error code on faliure
 */
static int usbdm_generic_write_register(struct bdm_dev *bdev,
					enum usbdm_commands cmd,
					uint16_t address, uint32_t value)
{
	/*
...
static int usbdm_write_xx(struct bdm *bdm, uint16_t reg, uint16_t value)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
	return usbdm_generic_write_register(bdev, USBDM_CMD_WRITE_REG, reg, value);
}

static int usbdm_write_pc(struct bdm *bdm, uint16_t value)
{
	return usbdm_write_xx(bdm, USBDM_HCS12_REG_PC, value);
}

static int usbdm_write_sp(struct bdm *bdm, uint16_t value)
{
	return usbdm_write_xx(bdm, USBDM_HCS12_REG_SP, value);
}

static int usbdm_write_d(struct bdm *bdm, uint16_t value)
{
	return usbdm_write_xx(bdm, USBDM_HCS12_REG_D, value);
}

static int usbdm_write_x(struct bdm *bdm, uint16_t value)
{
	return usbdm_write_xx(bdm, USBDM_HCS12_REG_X, value);
}

static int usbdm_write_y(struct bdm *bdm, uint16_t value)
{
	return usbdm_write_xx(bdm, USBDM_HCS12_REG_Y, value);
}

static int usbdm_write_bd_byte(struct bdm *bdm, uint32_t address, uint8_t value)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
 ...
 * @returns ERROR_OK on success, negative error code on failure
 */
static int usbdm_read_memory(struct bdm *bdm, uint32_t address,
 ...
 * @returns ERROR_OK on success, negative error code on failure
 */
static int usbdm_write_memory(struct bdm *bdm, uint32_t address,
 ...
 * @returns ERROR_OK on success, negative error code on failure
 */
static int usbdm_target_command(struct bdm_dev *bdev, enum usbdm_commands cmd)
{
	/*
	   Command:
...
static int usbdm_connect(struct bdm_dev *bdev)
{
	return usbdm_target_command(bdev, USBDM_CMD_CONNECT);
}

static int usbdm_target_halt(struct bdm *bdm)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
	return usbdm_target_command(bdev, USBDM_CMD_TARGET_HALT);
}

static int usbdm_target_go(struct bdm *bdm)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
	return usbdm_target_command(bdev, USBDM_CMD_TARGET_GO);
}

static int usbdm_target_trace1(struct bdm *bdm)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
	return usbdm_target_command(bdev, USBDM_CMD_TARGET_STEP);
}

static int usbdm_target_assert_reset(struct bdm *bdm)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
	return usbdm_control_pins(bdev, USBDM_PIN_RESET_LOW);
}

static int usbdm_target_deassert_reset(struct bdm *bdm)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
	return usbdm_control_pins(bdev, USBDM_PIN_RESET_3STATE);
}

static int usbdm_target_assert_bknd(struct bdm *bdm)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
	return usbdm_control_pins(bdev, USBDM_PIN_BKGD_LOW);
}

static int usbdm_target_deassert_bknd(struct bdm *bdm)
{
	struct bdm_dev *bdev = (struct bdm_dev *)bdm;
	return usbdm_control_pins(bdev, USBDM_PIN_BKGD_3STATE);
}

int usbdm_interface_init_target(struct target *t)
{
	int err;
...
static void usbdm_execute_reset(struct jtag_command *cmd)
{
	...
	if (cmd->cmd.reset->srst)
		ret = usbdm_target_assert_reset((struct bdm *)dongle);
	else
		ret = usbdm_target_deassert_reset((struct bdm *)dongle);
...
static int usbdm_jim_newtap_cmd(Jim_GetOptInfo *goi)
{
	struct jtag_tap *tap;
	char *cp;
	...
	Jim_GetOpt_String(goi, &cp, NULL);
	tap->tapname = strdup(cp);
	if (!tap->tapname)
		goto free_chip;

	/* name + dot + name + null */
	const size_t x  = strlen(tap->chip) + 1 + strlen(tap->tapname) + 1;
	cp = malloc(x);
	if (!cp)
		goto free_tapname;
...
int usbdm_jim_newtap(Jim_Interp *interp, int argc, Jim_Obj * const *argv)
{
	Jim_GetOptInfo goi;
	Jim_GetOpt_Setup(&goi, interp, argc - 1, argv + 1);
	return usbdm_jim_newtap_cmd(&goi);
}
...
static const struct command_registration usbdm_transport_subcommand_handlers[] = {
	{
		.name		= "newtap",
		.mode		= COMMAND_CONFIG,
		.jim_handler	= usbdm_jim_newtap,
...
static const struct command_registration usbdm_interface_command_handlers[] = {
	{
		.name	= "usbdm",
		.mode	= COMMAND_ANY,
		.help	= "perform usbdm adapter actions",
		.usage	= "",
		.chain	= usbdm_transport_subcommand_handlers,
	},
	{
		.name = "jtag",
		.mode = COMMAND_ANY,
...
static const char *usbdm_transports[] = { "usbdm_bdm", NULL };
...
static int usbdm_bdm_transport_init(struct command_context *cmd_ctx)
{
	...
	return usbdm_interface_init_target(target);
}

static int usbdm_bdm_transport_select(struct command_context *cmd_ctx)
{
	return ERROR_OK;
}

static struct transport usbdm_bdm_transport = {
	.name	= "usbdm_bdm",
	.select = usbdm_bdm_transport_select,
	.init   = usbdm_bdm_transport_init,
};

static void usbdm_transport_constructor(void) __attribute__ ((constructor, used));
static void usbdm_transport_constructor(void)
{
	transport_register(&usbdm_bdm_transport);
}
...
{
	return get_current_transport() == &usbdm_bdm_transport;
}
