...
static int cmsis_dap_swd_run_queue(struct adiv5_dap *dap)
{
	...
	int retval = cmsis_dap_usb_xfer(cmsis_dap_handle, idx);

	idx = 2;
	uint8_t ack = buffer[idx] & 0x07;
	if (ack != SWD_ACK_OK || (buffer[idx] & 0x08)) {
		LOG_ERROR("SWD ack not OK: %d %s", buffer[idx-1],
...
static int cmsis_dap_swd_switch_seq(struct adiv5_dap *dap, enum swd_special_seq seq)
{
	...
	int retval = cmsis_dap_usb_xfer(cmsis_dap_handle, DIV_ROUND_UP(s_len, 8) + 3);
...
static int cmsis_dap_init(void)
{
	...
	if (retval != ERROR_OK)
		return ERROR_FAIL;
	retval = cmsis_dap_cmd_DAP_SWD_Configure(0x00);	/* 1 TRN, no DataPhase */
	if (retval != ERROR_OK)
		return ERROR_FAIL;
	...
	if (swd_mode)
		...
		retval = cmsis_dap_swd_switch_seq(NULL, SWD_TO_JTAG);
...
struct jtag_interface cmsis_dap_test_interface = {
	.name = "cmsis-dap-test",
