...
#else
#define DEBUG_IO(expr...) (void)0
#define DEBUG_PRINT_BUF(buf, len) (void)0
#endif

/* Constants taken from libftdi */
#define FTDI_DEVICE_OUT_REQTYPE (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE \
				 | LIBUSB_ENDPOINT_OUT)
#define FTDI_DEVICE_IN_REQTYPE (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE \
				| LIBUSB_ENDPOINT_IN)
	...
	const char *product, const char *serial)
{
	...
	libusb_detach_kernel_driver(ctx->usb_dev, ctx->interface);
	...
	if (desc.bcdDevice == 0x500)
		...
		LOG_ERROR("unsupported FTDI chip type: %04x", desc.bcdDevice);
		libusb_free_config_descriptor(config0);
		libusb_close(ctx->usb_dev);
		return false;
	}
	...
	if (ctx->max_packet_size == 0) {
		LOG_ERROR("could not determine maximum packet size");
		libusb_close(ctx->usb_dev);
		return false;
	}
	...
	const char *serial, int channel, unsigned char latency)
{
	...
	if (!open_matching_device(ctx, vid, pid, description, serial)) {
		LOG_ERROR("unable to open ftdi device with vid %04x pid %04x", vid, pid);
		ctx->usb_dev = 0;
		goto error;
	}
...
static LIBUSB_CALL void read_cb(struct libusb_transfer *transfer)
{
	...
	LOG_DEBUG("raw chunk %d, transferred %d of %d", transfer->actual_length, res->transferred,
		ctx->read_count);
...
static LIBUSB_CALL void write_cb(struct libusb_transfer *transfer)
{
	...
	LOG_DEBUG("transferred %d of %d", res->transferred, ctx->write_count);
...
{
	LOG_DEBUG("write %d%s, read %d", ctx->write_count, ctx->read_count ? "+1" : "",
			ctx->read_count);
	...
	while (!write_result.done || !read_result.done) {
		...
		if (retval < 0) {
			...
			while (!write_result.done || !read_result.done)
				if (libusb_handle_events(ctx->usb_ctx) < 0)
					break;
		}
	}
