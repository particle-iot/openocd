...
static bool open_matching_device(struct mpsse_ctx *ctx, const uint16_t *vid, const uint16_t *pid,
	const char *product, const char *serial, const char *device_path)
{
	libusb_device **list;
...
#else
		if (device_path)
			LOG_WARNING("Opening a device by path requires libusb >= 1.0.16 : Argument ignored");
#endif
		found = true;
		break;
	}
...
struct mpsse_ctx *mpsse_open(const uint16_t *vid, const uint16_t *pid, const char *description,
	const char *serial, const char *device_path, int channel)
{
