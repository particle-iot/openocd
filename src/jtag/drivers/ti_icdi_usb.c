...
#define ICDI_TIMEOUT 1000

#define ICDI_DATA_SIZE 8192
#define PACKET_START "$"
#define PACKET_END "#"
...
static int icdi_send_packet(void *handle, int len)
{
	...
	if (ch == 'x' || ch == 'X')
		...
		strncpy(buffer, h->buffer, len > 50 ? 50 : len);
...
resend:
	...
	result = libusb_bulk_transfer(h->usb_dev, ICDI_READ_ENDPOINT, (unsigned char *)h->buffer, ICDI_DATA_SIZE,
				&transferred, ICDI_TIMEOUT);
	...
	if (h->buffer[0] == '-') {
		...
		LOG_DEBUG("Unexpected Reply from ICDI");

	/* read reply from icdi */
	result = libusb_bulk_transfer(h->usb_dev, ICDI_READ_ENDPOINT, (unsigned char *)h->buffer, ICDI_DATA_SIZE,
			&h->count, ICDI_TIMEOUT);
	if (result != 0 || h->count == 0) {
		LOG_DEBUG("Error RX Data %d", result);
		return ERROR_FAIL;
	}
...
static int icdi_send_cmd(void *handle, const char *cmd)
{
	...
	int cmd_len = snprintf(h->buffer, ICDI_DATA_SIZE, PACKET_START "%s", cmd);
...
static int icdi_usb_query(void *handle)
{
	...
	h->max_packet = ICDI_DATA_SIZE;

	/* query packet buffer size */
	char *offset = strstr(h->buffer, "PacketSize");
	if (offset) {
		char *separator;
		h->max_packet = strtoul(offset + 11, &separator, 16);
	}
...
static int icdi_usb_read_mem(void *handle, uint32_t addr, uint32_t len, uint8_t *buffer)
{
	...
	/* unescape input */
	remote_unescape_input(h->buffer + 4, len, (char *)buffer, len);
...
static int icdi_usb_write_mem(void *handle, uint32_t addr, uint32_t len, const uint8_t *buffer)
{
	...
	size_t cmd_len = snprintf(h->buffer, ICDI_DATA_SIZE, PACKET_START "X%x,%x:", addr, len);

	int out_len;
	cmd_len += remote_escape_output((char *)buffer, len, h->buffer + cmd_len,
			&out_len, ICDI_DATA_SIZE - cmd_len);
...
static int icdi_usb_open(struct hl_interface_param_s *param, void **fd)
{
	...
	h = malloc(sizeof(struct icdi_usb_handle_s));
...
static int icdi_usb_close(void *fd)
{
	return ERROR_OK;
}
