...
static int icdi_send_remote_cmd(void *handle, const char *data)
{
	...
	cmd_len += hexify(h->buffer + cmd_len, data, 0, h->max_packet - cmd_len);
...
static int icdi_get_cmd_result(void *handle)
{
	...
	if (h->buffer[offset] == 'E') {
		/* get error code */
		char result;
		if (unhexify(&result, h->buffer + offset + 1, 1) != 1)
			return ERROR_FAIL;
		return result;
	}
...
static int icdi_usb_version(void *handle)
{
	...
	/* convert reply */
	if (unhexify(version, h->buffer + 2, 4) != 4) {
...
static int icdi_usb_query(void *handle)
{
	...
	if (h->max_packet != ICDI_PACKET_SIZE) {
		h->buffer = realloc(h->buffer, h->max_packet);
		if (h->buffer == 0) {
...
static int icdi_usb_read_reg(void *handle, int num, uint32_t *val)
{
	...
	/* convert result */
	if (unhexify((char *)val, h->buffer + 2, 4) != 4) {
...
static int icdi_usb_read_mem(void *handle, uint32_t addr, uint32_t len, uint8_t *buffer)
{
	...
	/* unescape input */
	int read_len = remote_unescape_input(h->buffer + 5, h->read_count - 8, (char *)buffer, len);
	if (read_len != (int)len) {
