...
int ft2232_transfer(void *device, int bits, char *mosidata, char *misodata, int nLSBfirst){
        ...
        if (bits>=8){
                ...
                for(byte=0;byte*8<bits;byte++) for(bit=0;bit<8;bit++) misodata[byte*8+bit]=buf[byte]&(1<<bit)?1:0;
        }
        ...
        for (bit=bytes*8;bit<bits;bit++) misodata[bit]=(misodata[bit]&(nLSBfirst?0x01:0x80))?1:0;
