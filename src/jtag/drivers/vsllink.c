		...
		int_least32_t hz)
{
	if (hz > 0) {
		...
		uint16_t delay = hz > 1850000 ? 0 :
						 hz > 200000 ? 1 :
						 hz > 130000 ? 2 :
						 hz > 100000 ? 3 :
						 hz > 85000 ? 4 :
						 hz > 72000 ? 5 : ((500000 / hz) - 1);

		LOG_DEBUG("SWD delay: %d", delay);
		versaloon_interface.adaptors.swd.config(0, 2, 0, delay);
		...
		enum swd_special_seq seq)
{
	...
	case LINE_RESET:
		...
		versaloon_interface.adaptors.swd.seqout(0,
				(uint8_t *)swd_seq_line_reset, swd_seq_line_reset_len);
		break;
	case JTAG_TO_SWD:
		LOG_DEBUG("JTAG-to-SWD");
		versaloon_interface.adaptors.swd.seqout(0,
				(uint8_t *)swd_seq_jtag_to_swd, swd_seq_jtag_to_swd_len);
		break;
	case SWD_TO_JTAG:
		LOG_DEBUG("SWD-to-JTAG");
		versaloon_interface.adaptors.swd.seqout(0,
				(uint8_t *)swd_seq_swd_to_jtag, swd_seq_swd_to_jtag_len);
		break;
	default:
