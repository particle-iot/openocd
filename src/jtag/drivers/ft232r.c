	...
	while (rxdone < ft232r_output_len) {
		...
		while (bytes_written < bytes_to_write) {
			LOG_DEBUG_IO("usb bulk write %d bytes",
				...
				n = jtag_libusb_bulk_write(adapter, IN_EP,
		...
		if (empty_rxfifo == 0 || txdone == ft232r_output_len) {
			...
			while (bytes_read < bytes_to_read) {
				n = jtag_libusb_bulk_read(adapter, OUT_EP,
					(char *) reply,
...
{
	if (ft232r_serial_desc == NULL) {
		LOG_WARNING("ft232r serial descriptor not specified - first "
			"matching vid:pid will be used");
	}
