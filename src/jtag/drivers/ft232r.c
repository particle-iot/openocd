...
#   define JTAG_DEBUG LOG_DEBUG
#else
#   define JTAG_DEBUG(...) /*empty*/
#endif
...
static int ft232r_send_recv(void)
{
	...
	while (rxdone < ft232r_output_len) {
		...
		while (bytes_written < bytes_to_write) {
			JTAG_DEBUG("usb bulk write %d bytes",
			...
			if (n != bytes_to_write)
				JTAG_DEBUG("usb bulk written %d bytes of %d",
		...
		if (empty_rxfifo == 0 || txdone == ft232r_output_len) {
			...
			while (bytes_read < bytes_to_read) {
				...
				if (n != bytes_to_read + 2)
					LOG_ERROR("usb bulk read %d bytes of %d",
						n, bytes_to_read - bytes_read + 2);
				else
					JTAG_DEBUG("usb bulk read %d bytes", n);

				if (n > 2) {
					/* Copy data. */
					memcpy(ft232r_output + rxdone, reply + 2, n - 2);
					bytes_read += n;
					rxdone += n - 2;
				}
			}
...
static void ft232r_write(int tck, int tms, int tdi)
{
	...
	if (ft232r_output_len >= (int) sizeof(ft232r_output)) {
...
static int ft232r_init(void)
{
	...
	if (ft232r_device_desc == NULL) {
		LOG_WARNING("no ft232r device description specified, "
					"using default 'FT232R USB UART'");
		...
		for (dev = bus->devices; dev; dev = dev->next) {
			if (dev->descriptor.idVendor == ft232r_vid &&
				...
				if (usb_get_string_simple(adapter,
					...
					strncmp(string, ft232r_device_desc,
					sizeof(string)) != 0) {
					usb_close(adapter);
					continue;
				}
	...
	LOG_ERROR("USB adapter not found: vid=%04x, pid=%04x\n",
...
COMMAND_HANDLER(ft232r_handle_device_desc_command)
{
	if (CMD_ARGC == 1)
		ft232r_device_desc = strdup(CMD_ARGV[0]);
	else
		LOG_ERROR("require exactly one argument to "
				  "ft232r_device_desc <description>");
	return ERROR_OK;
}
...
COMMAND_HANDLER(ft232r_handle_pin_command)
{
	if (CMD_ARGC == 2) {
#if 0
		uint8_t mask;
		unsigned int state;
		if (!strcmp(CMD_ARGV[0], "pin6"))
			mask = NCE;
		else if (!strcmp(CMD_ARGV[0], "pin8"))
			mask = NCS;
		else {
...
static const struct command_registration ft232r_command_handlers[] = {
	{
		.name = "ft232r_device_desc",
		.handler = ft232r_handle_device_desc_command,
		.mode = COMMAND_CONFIG,
		.help = "set the USB device description of the FT232R adapter",
	...
	},
	{
		...
		.mode = COMMAND_CONFIG,
		.help = "the vendor ID and product ID of the FT232R adapter",
		.usage = "vid pid",
	},
	{
		.name = "ft232r",
		.handler = ft232r_handle_pin_command,
		.mode = COMMAND_ANY,
...
static void syncbb_path_move(struct pathmove_command *cmd)
{
	...
	while (num_states) {
		...
		if (tap_state_transition(tap_get_state(), true) == cmd->path[state_count]) {
			tms = 1;
		} else {
...
/*	ft232r_blink(1);*/
	...
	while (cmd) {
		switch (cmd->type) {
			case JTAG_RESET:
				JTAG_DEBUG("reset trst: %i srst %i", cmd->cmd.reset->trst, cmd->cmd.reset->srst);
			...
			case JTAG_RUNTEST:
				JTAG_DEBUG("runtest %i cycles, end in %s", cmd->cmd.runtest->num_cycles,
			...
			case JTAG_TLR_RESET: /* renamed from JTAG_STATEMOVE */
				JTAG_DEBUG("statemove end in %s", tap_state_name(cmd->cmd.statemove->end_state));
			...
			case JTAG_PATHMOVE:
				JTAG_DEBUG("pathmove: %i states, end in %s", cmd->cmd.pathmove->num_states,
			...
			case JTAG_SCAN:
				JTAG_DEBUG("%s scan end in %s",  (cmd->cmd.scan->ir_scan) ? "IR" : "DR",
			...
			case JTAG_SLEEP:
				JTAG_DEBUG("sleep %" PRIi32, cmd->cmd.sleep->us);
