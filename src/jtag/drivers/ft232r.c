...
static size_t ft232r_buf_size = 4096;
static uint16_t ft232r_restore_bitmode = 0x15; /* set TXD RTS DTR as outputs before quit to restore serial port */
...
COMMAND_HANDLER(ft232r_handle_buffer_size_command)
{
	...
	ft232r_buf_size = buf_size;
	command_print(CMD_CTX,
			"FT232R buffer size: %d bytes", buf_size);
...
static const struct command_registration ft232r_command_handlers[] = {
	...
	{
		.name = "ft232r_buffer_size",
		.handler = ft232r_handle_buffer_size_command,
		.mode = COMMAND_CONFIG,
		.help = "buffer size.",
		.usage = "bytes",
	},
