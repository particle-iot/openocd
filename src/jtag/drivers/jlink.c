...
static int jlink_register(void)
{
	 ...
	 * 8 - ... reply data */
	result = jlink_usb_io(jlink_handle, 14, 8);
	if (result != ERROR_OK) {
		LOG_ERROR("J-Link register failed (%d)", result);
		return ERROR_JTAG_DEVICE_ERROR;
	}

	int extra_reply_size = buf_get_u32(&usb_in_buffer[2], 0, 16) *
		buf_get_u32(&usb_in_buffer[4], 0, 16) +
		buf_get_u32(&usb_in_buffer[6], 0, 16);

	result = jlink_usb_io(jlink_handle, 0, extra_reply_size);
	if (result != ERROR_OK) {
		LOG_ERROR("J-Link register read failed (%d)", result);
		return ERROR_JTAG_DEVICE_ERROR;
	}
