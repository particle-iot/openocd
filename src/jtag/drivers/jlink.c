...
static int jlink_swd_run_queue(struct adiv5_dap *dap)
{
	...
	for (tries = 10; tries; tries--) {
		...
		for (; i < pending_scan_results_length; i++) {
			...
			LOG_DEBUG("%d, tries = %u, first - 8 = %u, pos: %zu: %s %s %s reg %X = %08"PRIx32,
				  i, tries, pending_scan_results_buffer[i].first - 8, tap_position,
			...
			if (ack != SWD_ACK_OK) {
				...
				if (ack == SWD_ACK_WAIT) {
					...
					/* If it was RDBUFF access, we need to rollback more */
					if (!(cmd & SWD_CMD_APnDP) && (cmd & SWD_CMD_RnW) &&
					    ((cmd & SWD_CMD_A32) >> 1) == 0x0C) {
						assert(tap_position >= 48 * 2);
						tap_position -= 48 * 2;
					}
					break;
				}
