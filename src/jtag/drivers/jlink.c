...
static struct jaylink_hardware_version hwver;
...
static int select_interface(void)
{
	...
	if (!has_select_tif && iface != JAYLINK_TIF_JTAG) {
...
{
	if (CMD_ARGC != 1) {
		command_print(CMD_CTX, "Need exactly one argument for jaylink usb.");
...
{
	if (CMD_ARGC != 1) {
		command_print(CMD_CTX, "Need exactly one argument for jaylink serial.");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
...
COMMAND_HANDLER(jlink_handle_jlink_jtag_command)
{
	...
	if (!CMD_ARGC) {
		...
		command_print(CMD_CTX, "Need exactly one argument for jaylink jtag.");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
...
COMMAND_HANDLER(jlink_handle_target_power_command)
{
	...
	if (CMD_ARGC != 1) {
		command_print(CMD_CTX, "Need exactly one argument for jaylink "
			"targetpower.");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
...
static void show_config(struct command_context *ctx)
{
	...
	if (hwver.major == JLINK_HW_TYPE_JLINK_PRO) {
...
static int poll_trace(uint8_t *buf, size_t *size)
{
	...
	ret = jaylink_swo_read(devh, buf, *size);
		...
		uint32_t port_size, unsigned int *trace_freq)
{
	...
	ret = jaylink_swo_get_speed_info(devh, JAYLINK_SWO_MODE_UART, &freq, &div);

	if (ret != JAYLINK_OK) {
		LOG_ERROR("jaylink_swo_get_speed_info() failed: %s.",
			jaylink_strerror_name(ret));
		return ERROR_FAIL;
	}
...
COMMAND_HANDLER(jlink_handle_config_usb_address_command)
{
	...
	if (!CMD_ARGC) {
		...
		command_print(CMD_CTX, "Need exactly one argument for jaylink config "
			"usb.");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
...
COMMAND_HANDLER(jlink_handle_config_mac_address_command)
{
	...
	if (hwver.major != JLINK_HW_TYPE_JLINK_PRO) {
		command_print(CMD_CTX, "Network connectivity is not supported by the "
			"device.");
		return ERROR_OK;
	}
