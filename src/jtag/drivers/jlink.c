...
static void jlink_tap_init(void)
{
	...
	if (swd_mode)
		memset(tms_buffer, 0xff, sizeof(tms_buffer));
}
...
static int jlink_swd_switch_seq(struct adiv5_dap *dap, enum swd_special_seq seq)
{
	...
	jlink_tap_init();
	/* direction buffer is pre-filled with OUT */
	bit_copy(tdi_buffer, tap_length, s, 0, s_len);
	tap_length += s_len;
...
static int jlink_swd_run_queue(struct adiv5_dap *dap)
{
	...
	for (int i = 0; i < pending_scan_results_length; i++) {
		int ack = buf_get_u32(usb_in_buffer, 1 + pending_scan_results_buffer[i].first, 3);

		if (ack != SWD_ACK_OK) {
			queued_retval = ack;
			goto skip;
		} else if (pending_scan_results_buffer[i].length) {
			uint32_t data = buf_get_u32(usb_in_buffer, 1 + 3 + pending_scan_results_buffer[i].first, 32);
			int parity = buf_get_u32(usb_in_buffer, 1 + 3 + 32 + pending_scan_results_buffer[i].first, 1);
			...
			if (pending_scan_results_buffer[i].buffer)
				*(uint32_t *)(void *)pending_scan_results_buffer[i].buffer = data;
		}
	}
...
static void jlink_swd_queue_cmd(struct adiv5_dap *dap, uint8_t cmd, uint32_t *dst, uint32_t data)
{
	static uint8_t data_parity_trn[DIV_ROUND_UP(32 + 1, 8)];
	...
	if (cmd & SWD_CMD_RnW) {
		...
		pending_scan_results_buffer[pending_scan_results_length].buffer = (uint8_t *)dst;
