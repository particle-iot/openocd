...
static int nulink_usb_version(void *handle)
{
	...
	bool is_nulink_pro = ((le_to_h_u32(h->databuf + 4 * 2) & 1) ? true : false);
...
static int nulink_usb_read_reg(void *handle, int num, uint32_t *val)
{
	...
	*val = le_to_h_u32(h->databuf + 4 * 1);
	/* LOG_DEBUG("NULINK read_reg(%d): 0x%08x", num, le_to_h_u32(h->databuf + 4 * 1)); */
...
static int nulink_usb_write_reg(void *handle, int num, uint32_t val)
{
	...
	/* LOG_DEBUG("NULINK write_reg(%d): %d", num, val);	*/
			  ...
			  uint8_t *buffer)
{
	...
	if (addr % 4) {
		...
		LOG_DEBUG("nulink_usb_read_mem8: addr(0x%08x)/aligned(0x%08x)/offset(%d)", addr, alignedaddr, offset);
			   ...
			   const uint8_t *buffer)
{
	...
	if (addr % 4) {
		...
		alignedaddr = alignedaddr * 4;
		offset = addr - alignedaddr;
		LOG_DEBUG("nulink_usb_write_mem8: addr(0x%08x)/aligned(0x%08x)/offset(%d)", addr, alignedaddr, offset);
	...
	while (len) {
		...
		for (i = 0; i < count; i++) {
			...
			/* LOG_DEBUG("NULINK write_ram(0x%08x): 0x%04x", addr, u32bufferdata); */
			  ...
			  uint8_t *buffer)
{
	...
	while (len) {
		...
		for (i = 0; i < count; i++) {
			...
			buffer += 4;
			/* LOG_DEBUG("NULINK read_ram(0x%08x): 0x%08x", addr - 4, le_to_h_u32(buffer - 4)); */
		}
...
static int nulink_usb_open(struct hl_interface_param_s *param, void **fd)
{
	...
	do {
		...
		if (err == ERROR_OK) {
			...
			/* Give the device one second to settle down and reenumerate. */
			usleep(1 * 1000 * 1000);
			retry_count--;
		}
	} while (1);
...
int nulink_config_trace(void *handle, bool enabled, enum tpiu_pin_protocol pin_protocol,
