...
static enum {
	OPENJTAG_MODEL_STANDARD,
	OPENJTAG_MODEL_CY7C65215,
} openjtag_model = OPENJTAG_MODEL_STANDARD;

static const char * const openjtag_model_names[] = {
	"standard",
	"cy7c65215",
	NULL
};
...
static int openjtag_buf_write(
	uint8_t *buf, int size, uint32_t *bytes_written)
{
	switch (openjtag_model) {
	case OPENJTAG_MODEL_CY7C65215:
...
static int openjtag_execute_tap_queue(void)
{
	...
	if (openjtag_scan_result_count && usb_rx_buf_len) {
		...
		while (res_count < openjtag_scan_result_count) {
			...
			while (len > 0) {
				if (len <= 8 && openjtag_model != OPENJTAG_MODEL_CY7C65215) {
...
static void openjtag_execute_runtest(struct jtag_command *cmd)
{
	...
	if (openjtag_model != OPENJTAG_MODEL_CY7C65215 ||
...
COMMAND_HANDLER(openjtag_handle_model_command)
{
	if (CMD_ARGC == 1) {
		const char * const *name = openjtag_model_names;
		int model = 0;
		for (; *name; name++, model++) {
			if (strcasecmp(CMD_ARGV[0], *name) == 0) {
				openjtag_model = model;
				return ERROR_OK;
			}
		}
		LOG_ERROR("unknown openjtag model '%s'", CMD_ARGV[0]);
	} else {
		LOG_ERROR("require exactly one argument to "
				  "openjtag_device_desc <description>");
	}
...
static const struct command_registration openjtag_command_handlers[] = {
	...
	{
		.name = "openjtag_model",
		.handler = openjtag_handle_model_command,
		.mode = COMMAND_CONFIG,
		.help = "set the OpenJTAG model",
		.usage = "model-string",
	},
