...
{
#if BUILD_OPENJATG_FTD2XX == 1
	LOG_DEBUG("TODO: openjtag_speed() isn't implemented for libftd2xx!");
#elif BUILD_OPENJTAG_LIBFTDI == 1
	LOG_DEBUG("TODO: openjtag_speed() isn't optimally implemented!");
...
openjtag_scan_result_count = 0;
	...
	if (ftdi_usb_open(&ftdic, openjtag_vid, openjtag_pid) < 0) {
...
#endif
	jtag_get_speed(&speed);
	openjtag_speed(speed);
	...
	/* OpenJTAG speed */
	openjtag_sendcommand(0x40); /*TODO: make adapter speed adjustable*/
...
COMMAND_HANDLER(openjtag_handle_pin_command)
{
	/*
	if (CMD_ARGC == 2)
	{
...
static const struct command_registration openjtag_command_handlers[] = {
	...
	{
		.name = "openjtag_vid_pid",
		.handler = openjtag_handle_vid_pid_command,
		.mode = COMMAND_CONFIG,
		.help = "the vendor ID and product ID of the OpenJTAG",
		.usage = "vid pid",
	},
	{
		.name = "openjtag",
		.handler = openjtag_handle_pin_command,
		.mode = COMMAND_ANY,
