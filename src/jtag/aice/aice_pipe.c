...
static int aice_pipe_write(const void *buffer, int count)
{
	...
	if (!success) {
		LOG_ERROR("(WIN32) write to pipe failed, error code: 0x%08x", GetLastError());
		return -1;
	}
...
static int aice_pipe_read(void *buffer, int count)
{
	...
	if (!success || (has_read == 0)) {
		LOG_ERROR("(WIN32) read from pipe failed, error code: 0x%08x", GetLastError());
		return -1;
	}
...
static int aice_pipe_open(struct aice_port_param_s *param)
{
	...
	if (!CreatePipe(&aice_pipe_output[0], &aice_pipe_output[1], &attribute, AICE_PIPE_MAXLINE)) {
		LOG_ERROR("Create pipes failed");
		return ERROR_FAIL;
	}
	if (!CreatePipe(&aice_pipe_input[0], &aice_pipe_input[1], &attribute, AICE_PIPE_MAXLINE)) {
		LOG_ERROR("Create pipes failed");
		return ERROR_FAIL;
	}
...
static int aice_pipe_read_mem_unit(uint32_t addr, uint32_t size, uint32_t count, uint8_t *buffer)
{
	char command[AICE_PIPE_MAXLINE];
...
static int aice_pipe_write_mem_unit(uint32_t addr, uint32_t size, uint32_t count, const uint8_t *buffer)
{
	char line[AICE_PIPE_MAXLINE];
	char command[AICE_PIPE_MAXLINE];
	...
	memcpy(command + 13, buffer, size * count);  /* WRITE_MEM_UNIT|addr|size|count|data */
