...
int aice_init_target(struct target *t)
{
	...
	if (aice_num_of_target_id_codes == 0) {
		res = aice_idcode(&aice, aice_target_id_codes, &aice_num_of_target_id_codes);
		if (res != ERROR_OK) {
...
{
	if (ERROR_OK != aice_open(&aice, &(aice.param))) {
		LOG_ERROR("Cannot find AICE Interface! Please check "
				"connection and permissions.");
		return ERROR_JTAG_INIT_FAILED;
	}

	aice_set_retry_times(&aice, aice.retry_times);
	aice_set_count_to_check_dbger(&aice, aice.count_to_check_dbger);
...
{
	return aice_set_jtag_clock(&aice, speed);
}
...
COMMAND_HANDLER(aice_handle_aice_custom_srst_script_command)
{
	...
	if (CMD_ARGC > 0) {
		aice_set_custom_srst_script(&aice, CMD_ARGV[0]);
		return ERROR_OK;
	}
...
COMMAND_HANDLER(aice_handle_aice_custom_trst_script_command)
{
	...
	if (CMD_ARGC > 0) {
		aice_set_custom_trst_script(&aice, CMD_ARGV[0]);
		return ERROR_OK;
	}
...
COMMAND_HANDLER(aice_handle_aice_custom_restart_script_command)
{
	...
	if (CMD_ARGC > 0) {
		aice_set_custom_restart_script(&aice, CMD_ARGV[0]);
		return ERROR_OK;
	}
