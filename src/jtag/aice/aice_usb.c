...
static int aice_usb_write(uint8_t *out_buffer, uint32_t out_length)
{
	int32_t result;

	if (out_length > AICE_OUT_BUFFER_SIZE) {
		LOG_ERROR("aice_write illegal out_length=%" PRIu32 " (max=%" PRIu32 ")",
				out_length, (uint32_t)AICE_OUT_BUFFER_SIZE);
		return -1;
	}
	...
	DEBUG_JTAG_IO("aice_usb_write, out_length = %" PRIu32 ", result = %" PRId32,
			out_length, result);
...
{
	LOG_DEBUG("aice_usb_read_tlb, virtual address: 0x%08" PRIu32, virtual_address);
		...
		uint32_t instruction_num)
{
	...
	for (i = 0 ; i < instruction_num ; i += 3) {
		...
		if (aice_check_dbger(coreid, NDS_DBGER_DPED) != ERROR_OK) {
			...
			LOG_ERROR("<-- TARGET ERROR! Debug operations do not finish properly:"
					"0x%08" PRIu32 " 0x%08" PRIu32 " 0x%08" PRIu32 " 0x%08" PRIu32 ". -->",
					dim_instructions[0],
					dim_instructions[1],
					dim_instructions[2],
					dim_instructions[3]);
