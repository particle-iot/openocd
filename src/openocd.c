...
static int register_rtos_targets(struct command_context *cmd_ctx)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(sources); i++) {
		const char *command_start = "script ";
		size_t buffer_bytes_left = buffer_size;
		...
		buffer[0] = '\0';
		strncat(buffer, command_start, buffer_bytes_left);
		buffer_bytes_left -= strlen(command_start);
		strncat(buffer, path, buffer_bytes_left);
		buffer_bytes_left -= strlen(path);
