				    ...
				    int64_t thread_id, char **hex_reg_list)
{
	...
	if (found == 1) {
		...
		if (found == 0) {
			LOG_ERROR
			    ("current thread %lx : no target to perform access of core id %x",
...
struct threads *liste_del_task(struct threads *task_list, struct threads **t,
			       struct threads *prev)
{
	LOG_INFO("del task %ld", (*t)->threadid);
	prev->next = (*t)->next;
...
int linux_get_tasks(struct target *target, int context)
{
	...
	while (((t->base_addr != linux_os->init_task_addr) &&
		...
		if (loop > MAX_THREADS) {
			LOG_ERROR("more than %d threads !!", MAX_THREADS);
			return ERROR_FAIL;
		}
	...
	LOG_INFO("complete time %ld, thread mean %ld\n",
		 (timeval_ms() - start),
...
static int linux_task_update(struct target *target, int context)
{
	...
	LOG_INFO("update thread done %ld, mean%ld\n",
