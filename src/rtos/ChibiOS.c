...
static int ChibiOS_update_stacking(struct rtos *rtos)
{
	...
	if (is_armv7m(armv7m_target)) {
		if (armv7m_target->fp_feature == FPv4_SP) {
			/* Found ARM v7m target wich does included a FPU */
			const uint32_t cpacr_addr = 0xE000ED88;
			const uint32_t cpacr;

			retval = target_read_buffer(rtos->target, cpacr_addr, sizeof(cpacr),
										(uint8_t *)&cpacr);
			if (retval != ERROR_OK) {
			...
			if (cpacr & 0x00F00000) {
				LOG_DEBUG("The target FPU is enabled");
