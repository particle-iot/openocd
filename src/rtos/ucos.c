...
static int ucos_get_thread_reg_list(struct rtos *rtos, int64_t thread_id, char **hex_reg_list);
...
static const struct stack_register_offset rtos_ucos_Cortex_M3_stack_offsets[ARMV7M_NUM_CORE_REGS] = {
        { 0x04, 32 },		/* r0   */
        { 0x08, 32 },		/* r1   */
        { 0x0c, 32 },		/* r2   */
        { 0x10, 32 },		/* r3   */
...
static int ucos_update_threads(struct rtos *rtos)
{
    ...
    LOG_INFO("CurTask: %u TCB size: %u num threads: %u status off: %u name off:%u", OSTCBCur, tcb_size, num_threads, status_offset, name_offset);
    ...
    if (tcb_size < 0x1000) {
        ...
        LOG_INFO("Found %u active tasks, %u is current thread", active_threads, (unsigned)rtos->current_thread);
    } else {
        LOG_WARNING("ucos TCB size out of range.  Maybe the code is not loade yet??");
        rtos->current_thread = THREAD_DEFAULT_ID;
    }
    ...
    for (i=0;i<num_threads;++i) {
        ...
        if (OSTCBPrioTbl[i]){
            ...
            LOG_INFO("Task %x: id: %d: name: %s detected",OSTCBPrioTbl[i], thread_prio,  tmp_str);
            ...
            uint32_t j;
            for (j = 0; (j < UCOS_NUM_STATES) && (ucos_thread_states[j].value != thread_state); j++) {
                /*
                 * empty
                 */
            }
...
static int ucos_get_thread_reg_list(struct rtos *rtos, int64_t thread_id, char **hex_reg_list)
{
    ...
    *hex_reg_list = NULL;
    ...
    retval =  target_read_buffer(rtos->target,
                                 thread_list_head+ ((thread_id)*param->pointer_width),
                                 param->pointer_width,
                                 (uint8_t *)&tcb_ptr);

    if (retval != ERROR_OK || tcb_ptr == 0) {
        LOG_ERROR("Error reading tcb from ucos thread (err: %d id: %u ptr: %u:%u", retval, (unsigned)thread_id, thread_list_head+ ((unsigned)thread_id*param->pointer_width), tcb_ptr);
        return retval;
    }
    ...
    LOG_INFO("Query regs for thread id %u, stack: %x", (int)thread_id, stack_ptr);
...
static int ucos_create(struct target *target)
{
    ...
    while ((i < UCOS_NUM_PARAMS) &&
            (0 != strcmp(ucos_params_list[i].target_name, target->type->name))) {
        i++;
    }
