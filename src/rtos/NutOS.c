...
#define NUTOS_MAX_THREAD_NAME_STR_SIZE (9)
...
static int NutOS_update_threads(struct rtos *rtos)
{
	...
	if ((threadIdx  == 0) || (rtos->current_thread == 0)) {
		LOG_ERROR("NutOS startup");
		char tmp_str[] = "Current Execution";
		rtos->thread_details = malloc(
				sizeof(struct thread_detail) * 1);
	...
	for (i = 0; i < rtos->thread_count; i++) {
		uint8_t state;
		struct thread_detail *details = &rtos->thread_details[i];
		details->threadid = thread_link;
		details->exists = true;
		details->thread_name_str =  malloc(NUTOS_MAX_THREAD_NAME_STR_SIZE);
		if (details->thread_name_str) {
			retval = target_read_buffer(
				rtos->target, thread_link + param->thread_name_offset,
				9, (uint8_t *) details->thread_name_str);
			if (retval != ERROR_OK)
				return retval;
		} else {
		...
		retval = target_read_buffer(
			...
			1, &state);
		details->extra_info_str = malloc(sizeof(NutOS_thread_states[state]));
		if (details->extra_info_str)
			strcpy(details->extra_info_str, NutOS_thread_states[state]);
		/* Read next linked state. */
		retval = target_read_buffer(
			rtos->target, thread_link + param->thread_next_offset,
			param->pointer_width, (uint8_t *) &thread_link);
		if (retval != ERROR_OK)
