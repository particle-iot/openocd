...
#ifndef UCOS_III_MAX_NAMELEN
#define UCOS_III_MAX_NAMELEN 64
#endif
...
static int uCOS_III_find_last_thread_address(struct rtos *rtos, symbol_address_t *thread_address)
{
	...
	if (retval != ERROR_OK) {
		LOG_ERROR("uCOS-III: failed to read OSTaskDbgListPtr");
		return retval;
	}
...
static int uCOS_III_update_threads(struct rtos *rtos)
{
	...
	/* update thread offsets if needed */
	...
	/*
	 * uC/OS-III creates tasks in LIFO order; advance to the end of
	 * the list and work backwards to preserve intended order.
	 */
	...
	for (int i = 0; i < rtos->thread_count; i++) {
		...
		char thread_name_buffer[UCOS_III_MAX_NAMELEN + 1];
		...
		retval = target_read_buffer(rtos->target,
					    ...
					    (void *)thread_name_buffer);
		if (retval != ERROR_OK) {
			LOG_ERROR("uCOS-III: failed to read thread name");
			return retval;
		}
		thread_name_buffer[sizeof(thread_name_buffer) - 1] = '\0';

		thread_detail->thread_name_str = strdup(thread_name_buffer);

		/* read thread extra info */
		uint8_t thread_state = 0;
		...
		if (thread_state < ARRAY_SIZE(uCOS_III_thread_state_list))
			thread_detail->extra_info_str = strdup(uCOS_III_thread_state_list[thread_state]);
