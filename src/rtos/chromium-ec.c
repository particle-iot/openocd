...
static int chromium_ec_get_current_task_ptr(struct rtos *rtos, symbol_address_t *current_task)
{
	struct chromium_ec_params *params;

	if (!rtos || !rtos->rtos_specific_params || !rtos->symbols)
		return ERROR_FAIL;

	params = rtos->rtos_specific_params;

	return target_read_buffer(rtos->target,
				  rtos->symbols[CHROMIUM_EC_VAL_current_task].address,
				  params->ptr_size, (uint8_t *)current_task);
}
...
static int chromium_ec_get_num_tasks(struct rtos *rtos, int *num_tasks)
{
	...
	ret = target_read_buffer(rtos->target, rtos->symbols[CHROMIUM_EC_VAL_tasks_enabled].address,
			sizeof(tasks_enabled), (uint8_t *)&tasks_enabled);
	if (ret != ERROR_OK) {
...
{
	symbol_address_t current_task, thread_ptr, name_ptr;
	...
	if (rtos->current_thread == 0 || num_tasks == 0 || !start_called) {
		num_tasks++;
	...
	for (t = 0; t < CROS_EC_MAX_TASKS; t++) {
		...
		ret = target_read_buffer(rtos->target,
					 ...
					 params->ptr_size * t, params->ptr_size,
					 (uint8_t *)&name_ptr);
		...
		if (thread_ptr == current_task)
			snprintf(thread_str_buf, sizeof(thread_str_buf),
				 "State: Running, Priority: %u, Events: %0x, Runtime: %lu\n",
				 t, events, (unsigned long)runtime);
		else
			snprintf(thread_str_buf, sizeof(thread_str_buf),
				 "State: %s, Priority: %u, Events: %0x, Runtime: %lu\n",
				 tasks_ready & BIT(t) ? "Ready" : "Waiting", t,
				 events, (unsigned long)runtime);
					   ...
					   char **hex_reg_list)
{
	...
	ret = target_read_buffer(rtos->target,
			   ...
			   params->ptr_size, (uint8_t *)&stack_ptr);
	if (ret != ERROR_OK) {
