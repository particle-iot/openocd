...
static int mqx_valid_address_check(
	struct rtos *rtos, uint32_t address
);
...
static int mqx_does_scheduler_start(
	struct rtos *rtos
);
...
static int mqx_get_symbol_list_to_lookup(
	symbol_table_elem_t *symbol_list[]
);


/* local data */
static mqx_state_t mqx_states[] = {
	{ 0x0002, "READY" },
	{ 0x0003, "BLOCKED" },
...
static char *mqx_symbol_list[] = {
	"_mqx_kernel_data",
	"MQX_init_struct",
	NULL
};

static mqx_params_t mqx_params_list[] = {
...
{
	enum mqx_arch arch_type = ((mqx_params_t *)rtos->rtos_specific_params)->target_arch;
	const char * target_name = ((mqx_params_t *)rtos->rtos_specific_params)->target_name;
	...
	LOG_ERROR("MQX RTOS - unknown architecture %s", target_name);
	return ERROR_FAIL;
}
...
/*
 * Check wheteher scheduler was started
 */
static int mqx_does_scheduler_start(
	struct rtos *rtos
)
{
	...
	/* active task is system task, scheduler dit not run yet */
...
)
{
	...
	int mqx_params_list_num = (sizeof(mqx_params_list)/sizeof(mqx_params_t));
...
)
{
	...
	/* check scheduler */
	if (ERROR_OK != mqx_does_scheduler_start(rtos))
		return ERROR_FAIL;
	...
	*/
	for (
		...
		state_max = (sizeof(mqx_states)/sizeof(mqx_state_t));
...
)
{
	...
	*hex_reg_list = NULL;
	...
	return rtos_generic_stack_read(
		rtos->target, ((mqx_params_t *)rtos->rtos_specific_params)->stacking_info, stack_ptr, hex_reg_list
	);
}
