...
static int microOSplusIIIe_get_thread_reg_list(struct rtos *rtos, int64_t thread_id, char **hex_reg_list);
static int microOSplusIIIe_get_symbol_list_to_lookup(symbol_table_elem_t *symbol_list[]);
...
static const struct microOSplusIIIe_thread_state microOSplusIIIe_thread_states[] = { { 0, "Undefined" }, { 1, "Ready" },
		{ 2, "Running" }, { 3, "Suspended" }, { 4, "Terminated" }, { 5, "Destroyed" }, };

#define MICROOSPLUSIIIE_NUM_STATES (sizeof(microOSplusIIIe_thread_states)/sizeof(struct microOSplusIIIe_thread_state))
...
static const struct microOSplusIIIe_params microOSplusIIIe_params_list[] = { { "cortex_m", /* target_name */
4, /* pointer_width; */
...
#define MICROOSPLUSIIIE_NUM_PARAMS ((int)(sizeof(microOSplusIIIe_params_list)/sizeof(struct microOSplusIIIe_params)))
...
static const char * const microOSplusIIIe_symbol_list[] = { "os::rtos::scheduler::top_threads_list_",
		"os::rtos::scheduler::current_thread_", "os::rtos::scheduler::is_started_",
		NULL };

const struct rtos_type microOSplusIIIe_rtos = { .name = "µOS++ IIIe",

.detect_rtos = microOSplusIIIe_detect_rtos, .create = microOSplusIIIe_create, .update_threads =
		microOSplusIIIe_update_threads, .get_thread_reg_list = microOSplusIIIe_get_thread_reg_list,
		.get_symbol_list_to_lookup = microOSplusIIIe_get_symbol_list_to_lookup,

};

static int microOSplusIIIe_get_thread_info(struct rtos *rtos, uint32_t thread_index,
		struct thread_detail *thread_details)
{
	const struct microOSplusIIIe_params *param = (const struct microOSplusIIIe_params *) rtos->rtos_specific_params;
...
static int microOSplusIIIe_get_thread_info_recursive(struct rtos *rtos, uint32_t list_head, uint32_t list_prev,
		...
		bool count_only)
{
	...
	if (list_prev == list_head)
		return retval;
	if (!count_only) {
		microOSplusIIIe_get_thread_info(rtos, list_prev - param->thread_list_node_offset,
	...
	if (retval != ERROR_OK)
		return retval;
	retval = microOSplusIIIe_get_thread_info_recursive(rtos, children_head, children_prev, thread_list_index,
			count_only);
	if (retval != ERROR_OK)
		return retval;
	...
	if (retval != ERROR_OK)
		return retval;
	retval = microOSplusIIIe_get_thread_info_recursive(rtos, list_head, list_prev, thread_list_index, count_only);
	return retval;
}
...
static int microOSplusIIIe_update_threads(struct rtos *rtos)
{
	...
	param = (const struct microOSplusIIIe_params *) rtos->rtos_specific_params;
	...
	uint8_t is_started;
	retval = target_read_buffer(rtos->target, rtos->symbols[microOSplusIIIe_VAL_scheduler_is_started].address, 1,
			(uint8_t *) &is_started);
	if (retval != ERROR_OK)
		return retval;
	...
	uint32_t thread_list_head = rtos->symbols[microOSplusIIIe_VAL_thread_list].address;
	...
	if (first_thread == 0 || first_thread == thread_list_head) {
		...
		retval = microOSplusIIIe_get_thread_info_recursive(rtos, thread_list_head, first_thread, &thread_count, true);
		if (retval != 0)
			return retval;
		/* read the current thread id */
		uint32_t current_thread_addr;
		retval = target_read_buffer(rtos->target, rtos->symbols[microOSplusIIIe_VAL_current_thread_ptr].address, 4,
		...
		microOSplusIIIe_get_thread_info_recursive(rtos, thread_list_head, first_thread, &thread_list_index, false);
	}
	return 0;
}
...
{
	unsigned int i;
	*symbol_list = calloc(ARRAY_SIZE(microOSplusIIIe_symbol_list), sizeof(symbol_table_elem_t));
...
static int microOSplusIIIe_detect_rtos(struct target *target)
{
	if ((target->rtos->symbols != NULL) && (target->rtos->symbols[microOSplusIIIe_VAL_thread_list].address != 0)) {
		/* looks like µOS++ IIIe */
		return 1;
	}
	return 0;
...
static int microOSplusIIIe_create(struct target *target)
{
	...
	while ((i < MICROOSPLUSIIIE_NUM_PARAMS)
			&& (0 != strcmp(microOSplusIIIe_params_list[i].target_name, target->type->name))) {
		i++;
	}
	if (i >= MICROOSPLUSIIIE_NUM_PARAMS) {
		LOG_ERROR("Could not find target in microOSplusIIIe compatibility list");
		return -1;
	}

	target->rtos->rtos_specific_params = (void *) &microOSplusIIIe_params_list[i];
