...
static bool hawt_detect_rtos(struct target *target);
static int hawt_create(struct target *target);
static int hawt_update_threads(struct rtos *rtos);
static int hawt_get_thread_reg_list(struct rtos *rtos, int64_t thread_id, char **hex_reg_list);
...
static int hawt_fill_thread(struct rtos *rtos, struct target *curr, int thread_num)
{
	...
	snprintf(tmp_str, HAWT_THREAD_NAME_STR_SIZE-1, "state: %s", debug_reason_name(curr));
...
static int hawt_target_for_threadid(struct connection *connection, int64_t thread_id, struct target **p_target)
{
...
static int hawt_thread_packet(struct connection *connection, const char *packet, int packet_size)
{
	struct target *target = get_target_from_connection(connection);
	...
	if (packet[0] == 'H' && packet[1] == 'g') {
		...
		if (current_threadid > 0) {
			if (hawt_target_for_threadid(connection, current_threadid, &curr) != ERROR_OK) {
				LOG_ERROR("hawt: cannot find thread id %"PRId64, current_threadid);
