...
# Usage: run generate_arm_sysreg_table.py AArch32 or AArch64 inside
#  the unzipped Arm SysReg xml folder
...
def get_reg_info(regxml):
    ...
    ret['is_optional'] = register.attrib['is_optional']
    ret['short_name'] = register.find('reg_short_name').text
    ret['long_name'] = register.find('reg_long_name').text
    ret['reset_value'] = register.find('reg_reset_value').text
...
def print_usage():
    print("Usage: generate_arm_sysreg_table.py AArch32|AArch64")
    exit(0)
...
if __name__ == '__main__':
    if len(argv) != 2:
        print_usage()
    sectiongroup_groupname = argv[1]
    if sectiongroup_groupname == 'AArch32':
        section_type = "MCR/MRC"
    elif sectiongroup_groupname == 'AArch64':
    ...
    print("static aarch64_system_register_t aarch64_system_registers[] = {")
    for i in range(0, len(regnames)):
        ...
        for i in range(0, len(reginfo['fields'])):
            ...
            print("\t\t\t{\"%s\", %u, %u}%s" % (field[0],
                                                ...
                                                field[2],
                                                ',' if i < 63 else ''))
        for i in range(len(reginfo['fields']), 64):
            print("\t\t\t{ NULL, 0, 0 }%s" % (',' if i < 63 else ''))
        print("\t\t}")
        print("\t},")
    print("\t{ NULL }")
    print("};")
