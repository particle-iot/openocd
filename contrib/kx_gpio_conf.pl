...
foreach my $line (@conf)
{
	...
	($exor & (0x1 << ${pin})) && ($Exor .= sprintf(" 0x%02X 0x%08X", (${GPIO_PDDR}-${GPIO_PODR}) & 0xFF, $exor));

	push(@{$Port[$num]}, sprintf("P%s%02d:%s:%s", $port, $pin, $conf, $speed));
	push(@Col, $Exor);
}

my $Col = sprintf("${Sep}0x%02X ", (${GPIO_PIDR}-${GPIO_PODR}) & 0xFF);
for (my $i = 0; $i < @Col; $i++)
{
	if (($i != 0) && (($i % 2) == 0))
	{
...
for (my $i = 0; $i < @Out; $i++)
{
	...
	if (($$out{'PDDR_H'} | $$out{'PDDR_L'}) != 0)
	{
		...
		(($$out{'PODR_H'} | $$out{'PODR_L'}) != 0) &&
			($Set .= sprintf("${Form}PODR\n", $addr + ${GPIO_PODR}, $$out{'PODR_H'}, $$out{'PODR_L'}));
		(($$out{'PDDR_H'} | $$out{'PDDR_L'}) != 0) &&
			($Set .= sprintf("${Form}PDDR\n", $addr + ${GPIO_PDDR}, $$out{'PDDR_H'}, $$out{'PDDR_L'}));
	}
}
