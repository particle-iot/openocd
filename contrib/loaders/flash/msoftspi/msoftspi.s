...
/* entry point, initialize registers for SPI mode */
	...
	adr		r0, port_pin_io1			/* fetch address */
...
/* toggle direction of IOx pin
 * clobbered: r3, r4, r5, r6, r7 */
	.macro	toggle_dir
	ldr		r3, dir_off					/* dir. reg. offsets for I03 to IO0 */
...
toggle_dir_loop:
	...
	tst		r7, r7						/* if offset equals 0 */
	beq		toggle_dir_end				/* then nothing to do */
...
/* toggle direction of IO0 pin
 * clobbered: r3, r4, r5, r6, r7 */
	.macro	toggle_dir_spi
toggle_dir_spi:
	toggle_dir
	bx		lr							/* return */
	.endm
...
prepare_shift_in_spi:
	ldr		r2, port_pin_io1			/* IO1/MISO port address */
...
/* toggle direction of IO3-IO0 pin
 * clobbered: r3, r4, r5, r6, r7 */
	.macro	toggle_dir_dpi
toggle_dir_dpi:
	toggle_dir
	uxth	r3, r3						/* discard IO2 and IO3 offsets */
	lsrs	r3, r3, #8					/* shift by one byte for next pin */
	bne		toggle_dir_loop				/* again if not completed */
	bx		lr							/* return */
	.endm
...
/* get one bit from port and insert it in data byte */
	...
	ldr		r7, [r6, r3]				/* fetch port data */
	lsrs	r7, r7, r5					/* shift data bit into C */
...
/* entry point, initialize registers for I2C mode */
	...
	ldr		r1, dir_off					/* get dir offsets */
	.endm

/* internal macro */
	.macro	_i2c_set_clr	addr1 addr2 cond shift
	ldr		r2, \addr1					/* address of SDA/SCL port */
	ldr		r3,	[r2]					/* get port data */
	ldr		r5,	\addr1+4				/* get SDA/SCL bit mask */
	tst		r3, r5						/* SDA/SCL already set/cleared ? */
	\cond	1f							/* nothing to do if yes */
	eors	r3, r3, r5					/* toggle SDA/SCL bit */
	lsrs	r5, r1, #\shift				/* dir offset for SDA */
	sxtb	r5, r5						/* sign extend offset */
	ldr		r6, [r2, r5]				/* fetch port config */
	ldr		r7, \addr2+4				/* get exor mask */
	eors	r6, r6, r7					/* toggle direction */
	str		r6, [r2, r5]				/* store port config */
	str		r3, [r2]					/* store new port data */
1:
...
/* these following four macros clobber r2, r4, r5, r6, r7 */
	.macro	i2c_clr_sda
	_i2c_set_clr	port_pin_ncs port_pin_io1 beq 8
	.endm

	.macro	i2c_set_sda
	_i2c_set_clr	port_pin_ncs port_pin_io1 bne 8
	.endm

	.macro	i2c_clr_scl
	_i2c_set_clr	port_pin_clk port_pin_io0 beq 0
	.endm

	.macro	i2c_set_scl
	_i2c_set_clr	port_pin_clk port_pin_io0 bne 0
	.endm
...
addr_0_31:
	mov		r4, r12						/* get address offset */
	lsrs	r4, r4, #24					/* addr bits 31-24 */
	bl		shift_out_next_i2c			/* send addr byte */
...
shift_out_byte_i2c:
	i2c_start_cond						/* generate start condition */
shift_out_next_i2c:
...
dir_off:
	.word	0							/* offsets port output reg. to port direction reg. */
port_pin_ncs:
...
port_pin_io0:
	.word	0							/* port address (output reg) for IO0/MOSI */
	.word	0							/* exor mask for IO0/MOSI */
port_pin_io1:
	.word	0							/* port address (output reg) for IO1/MISO */
	.word	0							/* exor mask for IO1/MISO */
port_pin_io2:
